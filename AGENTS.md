...
- **단일 workspace 명령은 루트 별칭을 만들지 않고 `bun run --filter`로 직접 실행함**
- **모든 명령은 별도 안내가 없으면 저장소 루트에서 실행함**
- **`workspaces` 배열은 Tooling과 의존성 관리 계약이므로 script 최소화 과정에서 제거하지 않음**
- **명령 변경 후 영향받는 workspace 검사와 루트 `bun run check`를 실행함**
...
- 루트 script 또는 workspace 연결 변경은 `bun run check`로 전체 계약을 검증함
