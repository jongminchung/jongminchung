...
describe("validateDocuments", () => {
    ...
    test("reports duplicate and non-contiguous navigation order", () => {
        expect(() =>
            validateDocuments([
                ...createValidDocuments(),
                createDocument("ko", "handbook", {
                    id: "handbook/duplicate",
                }),
            ]),
        ).toThrow("Duplicate navigation order: ko:handbook:0");

        const gap = replaceDocuments(
            ({ metadata }) => metadata.section === "handbook",
            (document) => ({
                ...document,
                metadata: { ...document.metadata, order: 1 },
            }),
        );
        expect(() => validateDocuments(gap)).toThrow(
            "must use contiguous order values from 0",
        );
    });
    ...
    test("reports a broken internal link", () => {
        const documents = replaceDocuments(
            ({ metadata }) =>
                metadata.locale === "ko" && metadata.id === "overview",
            (document) => ({
                ...document,
