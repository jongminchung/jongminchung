...
const freshnessDays: Readonly<Record<FreshnessPolicy, number>> = {
    ...
    "upstream-api": 120,
    "imported-source": 180,
    "repository-handbook": 365,
    "evergreen-concept": 730,
};

export function freshnessPolicyFor(
    metadata: Pick<DocMetadata, "packageName" | "section" | "sourceUrl">,
): FreshnessPolicy {
    if (metadata.packageName !== undefined) return "versioned-technology";
    if (metadata.section === "handbook") return "repository-handbook";
...
export function assessFreshness(
    metadata: Pick<
        DocMetadata,
        "packageName" | "section" | "sourceUrl" | "verifiedAt"
    >,
    now: Date,
): FreshnessAssessment {
