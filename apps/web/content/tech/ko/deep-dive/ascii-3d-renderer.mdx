...
description: "좌표 변환, 투영, 래스터라이즈와 명암을 구현해 ASCII 3D 렌더러를 만듭니다."
order: 11
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/youtube.gif"
    width="80%"
    caption="영상 중 일부 캡쳐"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/perspective.png"
    caption="겨우 선 몇 개로 공간을 느낄 수 있다"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/shade.jpeg"
    caption="빛과 그림자를 통해 입체감을 표현할 수 있다"
    width="70%"
/>
...
<MaterialImage src="/materials/images/2024-03-03-ascii-3d-renderer/pixelate.png" />

좌측은 원본 이미지 우측은 Pixelate 이펙트를 입힌 이미지다. ASCII를 사용한다는 것은 단지 글자를 이용하여 우측 이미지 처럼 만드는 것과 크게 다르지 않다. 이를 이용한 작품도 존재한다.

<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/ascii-art.png"
    caption="문자를 이용하여 위키피디아 로고를 표현했다"
    width="80%"
/>
...
<MaterialImage src="/materials/images/2024-03-03-ascii-3d-renderer/shade.png" />

참고로 간격이 일정해야 하므로 반드시 고정폭 글꼴을 사용해야 한다. 빛에 기반한 명암 처리가 아니더라도 각 면의 문자를 다르게 선택하면 **입체감**을 만들 수 있다. 이제 이를 구현하기 위해 필요한 지식을 알아보자.
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/rendering-pipeline.png"
    caption="렌더링 파이프라인"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/cartesian-coordinate-system.png"
    caption="데카르트 좌표계"
    width="50%"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/low-polygon.jpeg"
    caption="적은 수의 폴리곤을 사용했기에 각져 보인다 / 버추어 파이터"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/world-transform.jpeg"
    caption="세상 어딘가에 배치하는 것이 월드 변환이다"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/transform-matrix.png"
    caption="좌표 열벡터에 4×4 변환 행렬을 곱하면 변환된 좌표를 얻을 수 있다"
    width="70%"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/perspective-projection.jpeg"
    caption="원근 투영"
    width="60%"
/>
...
<MaterialVideo
    src="/materials/images/2024-03-03-ascii-3d-renderer/rasterize.mp4"
    width="80%"
    caption="한 짤 요약"
    autoPlay
    muted
...
<figcaption style={{ marginTop: -24 }}>
    [소스 코드
    링크](https://github.com/kciter/ascii-3d-renderer.js/tree/main/src/math)
</figcaption>
...
<figcaption style={{ marginTop: -24 }}>
    [소스 코드
    링크](https://github.com/kciter/ascii-3d-renderer.js/blob/main/src/loader.ts)
</figcaption>
...
export class ASCII3DRenderer {
    ...
    constructor(_el: HTMLElement, width: number, height: number) {
        ...
        this.depthBuffer = new Array(height + 1)
            .fill(null)
            .map(() => new Array(width + 1).fill(255));
    }
...
<figcaption style={{ marginTop: -24 }}>
    [소스 코드
    링크](https://github.com/kciter/ascii-3d-renderer.js/blob/main/src/renderer.ts)
</figcaption>
...
<figcaption style={{ marginTop: -24 }}>
    [소스 코드
    링크](https://github.com/kciter/ascii-3d-renderer.js/blob/main/src/object.ts)
</figcaption>
        ...
        for (const object of this.objects) {
            for (const polygon of object.mesh) {
                ...
                let v2 = new Vector4(
                    ...
                    polygon.vertices[1].z,
                    1,
                );
                let v3 = new Vector4(
                    polygon.vertices[2].x,
                    polygon.vertices[2].y,
                    polygon.vertices[2].z,
                    1,
                );
...
<figcaption style={{ marginTop: -24 }}>
    [소스 코드
    링크](https://github.com/kciter/ascii-3d-renderer.js/blob/main/src/camera.ts)
</figcaption>
...
export class ASCII3DRenderer {
    ...
    private rasterize(v1: Vector4, v2: Vector4, v3: Vector4) {
        ...
        const p1 = new Vector2(
            ((v1.x + 1) * this.width) / 2,
            ((1 - v1.y) * this.height) / 2,
        );
        const p2 = new Vector2(
            ((v2.x + 1) * this.width) / 2,
            ((1 - v2.y) * this.height) / 2,
        );
        const p3 = new Vector2(
            ((v3.x + 1) * this.width) / 2,
            ((1 - v3.y) * this.height) / 2,
        );
...
<MaterialImage src="/materials/images/2024-03-03-ascii-3d-renderer/cube-line.gif" />
...
export class ASCII3DRenderer {
    ...
    private rasterize(v1: Vector4, v2: Vector4, v3: Vector4) {
        ...
        const maxX = Math.floor(
            Math.min(this.width, Math.max(p1.x, p2.x, p3.x)),
        );
        const maxY = Math.floor(
            Math.min(this.height, Math.max(p1.y, p2.y, p3.y)),
        );
...
<MaterialImage src="/materials/images/2024-03-03-ascii-3d-renderer/cow.gif" />
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/normal-vector.png"
    caption="법선 벡터란 이름은 쉽지 않다.."
/>
            ...
            for (let x = minX; x <= maxX; x++) {
                ...
                if (this.isPointInTriangle(p, p1, p2, p3)) {
                    ...
                    if ((v1.w + v2.w + v3.w) / 3.0 <= this.depthBuffer[y][x]) {
                        const shade =
                            this.Shade[
                                Math.round(brightness * (this.Shade.length - 1))
                            ];
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/cow-shade.gif"
    caption="최종 결과물"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/shade2.png"
    caption="조금 더 명암이 확실하다"
/>
...
<MaterialImage
    src="/materials/images/2024-03-03-ascii-3d-renderer/cow-shade2.gif"
    caption="볼만하다"
/>

## 마치며

JavaScript(TypeScript를 빌드)로 구현했기 때문에 동일한 렌더링 과정을 브라우저에서도 실행할 수 있다.
