...
- **The single-cluster RKE2, CNI, topology, and network values are declared under `inventory/`**
...
| ----------------------------- | ------------------------------------------------------- |
| Cluster versions and topology | `jamie-kr-gitops/inventory/`                            |
...
- **ClickHouse Server and Keeper use the supported `26.3.25` LTS line**
    - Merge concurrency and memory reservations are bounded for the three-replica observability workload
...
| ClickHouse S3 gateway  | 3 replicas             |                     None |
...
- **SeaweedFS uses placement `001`, retaining two volume copies on different racks for newly written S3 chunks**
...
| Dedicated private S3 gateway    | Isolate ClickHouse credentials and traffic from the public S3 route               | Adds three stateless gateway replicas and a PDB                                    |
...
- **`tests/integration/verify.sh` defines the following completion state**
    ...
    - ClickStack ingestion has two ready replicas, while the private S3 gateway has three ready replicas on distinct workers
    - SeaweedFS master and filer commands retain the `001` two-copy replication placement
...
- **Update this page's `updatedAt` and `verifiedAt` when any of the following code changes**
    - Version, topology, network, or storage inputs in `inventory/`
