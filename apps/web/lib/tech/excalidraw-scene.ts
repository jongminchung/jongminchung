...
const EXCALIDRAW_RENDER_PREFIX = "/excalidraw-assets/diagrams/";
...
/**
 * 원본 문자열에 대응하는 생성 SVG 파일 식별자를 반환함
 *
 * FNV-1a를 사용해 브라우저와 빌드 단계에서 같은 경로를 계산함
 */
export function excalidrawSceneId(source: string): string {
  let hash = 0x811c9dc5;
...
/** `excalidrawSvgSrc` 테마별 정적 SVG 경로를 반환함 */
export function excalidrawSvgSrc(
  source: string,
  theme: "light" | "dark",
): string {
  return `${EXCALIDRAW_RENDER_PREFIX}${excalidrawSceneId(source)}.${theme}.svg`;
}
...
function validateBounds(
  ...
  if (!Number.isFinite(x + y))
    fail(sourceName, `element "${id}" has invalid coordinates.`);
  if (width < 0 || height < 0 || (width === 0 && height === 0)) {
    fail(
      sourceName,
      `element "${id}" must have non-negative, non-empty bounds.`,
    );
  }
}
...
function parseFiles(
  value: unknown,
  sourceName: string,
): Readonly<Record<string, JsonRecord>> {
  ...
  return Object.freeze(
    value.map((rawElement, index) => {
      ...
      if (
        ...
        fail(
          sourceName,
          `elements[${id}].isDeleted must be a boolean when present.`,
        );
      }

      if (type === "image" && element.isDeleted !== true) {
        const fileId = requireString(
          element.fileId,
          sourceName,
          `elements[${id}].fileId`,
        );
        if (files[fileId] === undefined) {
          fail(
            sourceName,
            `image element "${id}" references missing file "${fileId}".`,
          );
        }
      }
      return element;
...
export function parseExcalidrawValue(
  ...
  if (root.type !== "excalidraw")
    fail(sourceName, 'field "type" must be "excalidraw".');
  ...
  if (elementCount === 0)
    fail(sourceName, "scene must contain at least one non-deleted element.");
...
export function parseExcalidrawFilename(filename: string): ExcalidrawAsset {
  ...
  return Object.freeze({
    filename,
    slug,
    src: `${EXCALIDRAW_ASSET_PREFIX}${filename}`,
  });
}
...
export function parseExcalidrawAssetSrc(src: string): ExcalidrawAsset {
  if (!src.startsWith(EXCALIDRAW_ASSET_PREFIX)) {
    throw new Error(
      `Excalidraw sources must use ${EXCALIDRAW_ASSET_PREFIX}: ${src}`,
    );
  }
  const filename = src.slice(EXCALIDRAW_ASSET_PREFIX.length);
  if (filename.includes("/") || filename.includes("\\")) {
    throw new Error(
      `Excalidraw sources must stay inside ${EXCALIDRAW_ASSET_PREFIX}: ${src}`,
    );
  }
  const asset = parseExcalidrawFilename(filename);
  if (asset.src !== src)
    throw new Error(`Unsupported Excalidraw source: ${src}`);
  return asset;
}
