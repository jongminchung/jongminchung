...
function requireRecord(
    ...
    if (!isRecord(value))
        fail(sourceName, `field "${field}" must be an object.`);
    return value;
}
    ...
    return Object.freeze(
        Object.fromEntries(
            Object.entries(record).map(([id, file]) => {
                if (id.length === 0)
                    fail(sourceName, "file IDs must not be empty.");
                return [
                    id,
                    requireRecord(file, sourceName, `files[${id}]`),
                ] as const;
            }),
        ),
    );
...
export function parseExcalidrawValue(
    ...
    if (elementCount === 0)
        fail(
            sourceName,
            "scene must contain at least one non-deleted element.",
        );
