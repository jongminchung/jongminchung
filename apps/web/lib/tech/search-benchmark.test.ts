...
describe("한영 검색 relevance benchmark", () => {
  it("[성공] 실제 40개 corpus가 relevance와 index 비용 예산을 충족함", async () => {
    ...
    const documents = [
      ...createSearchDocuments(snapshot.documents, snapshot.sources, "en"),
      ...createSearchDocuments(snapshot.documents, snapshot.sources, "ko"),
    ];
    ...
    expect(englishReport).toMatchObject(bilingualSearchBenchmarkBaseline.en);
