...
function analyze(x: number): FloatInfo {
    ...
    const significand = trimNumberString(
        (Number(frac) / 2 ** 52).toPrecision(17),
    );
...
export const FloatBitsDemo = () => {
    ...
    const isExact =
        info !== null && canonical !== null && canonical === info.exact;
        ...
        >
            ...
            {info && fields && (
                <>
                    ...
                    >
                        (-1)^{info.sign} × {info.significand} × 2^
                        {info.exponent2}
                    </div>
            ...
            >
                분모가 2의 거듭제곱인 소수만 정확히 저장된다. 0.5나 0.25를
                넣어보자.
            </div>
        </div>
    );
