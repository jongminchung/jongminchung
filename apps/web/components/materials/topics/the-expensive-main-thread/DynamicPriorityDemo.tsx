...
const STRINGS = {
    ...
    en: {
        header: (n: number) =>
            `Building previews for the ${n} attached photos, one at a time`,
...
function loadImage(_i: number): Promise<HTMLImageElement | null> {
...
export const DynamicPriorityDemo = ({
    locale: lang = "ko",
}: {
    locale?: Lang;
}) => {
    ...
    const start = async () => {
        ...
        imagesRef.current = await Promise.all(
            Array.from({ length: TILES }, (_, i) => loadImage(i)),
        );
        setLoading(false);
    ...
    return (
        ...
        >
            ...
            <div style={{ fontSize: 11, color: "#495057", marginBottom: 10 }}>
                {t.header(TILES)}
            </div>
