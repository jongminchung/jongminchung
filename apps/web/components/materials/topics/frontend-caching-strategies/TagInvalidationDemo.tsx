...
function eventAt(index: number): (typeof EVENTS)[number] {
    ...
    if (event === undefined)
        throw new Error(`Missing cache invalidation event ${index}.`);
    return event;
}
...
const initialCards = () =>
    Object.fromEntries(
        CARDS.map((c) => [c.id, { status: "fresh" as Status, version: 1 }]),
    ) as Record<string, { status: Status; version: number }>;

export const TagInvalidationDemo = () => {
    const [cards, setCards] =
        useState<Record<string, { status: Status; version: number }>>(
            initialCards,
        );
            ...
            setCards((prev) => {
                ...
                CARDS.filter((c) => c.tags.includes(tag)).forEach((c) => {
                    ...
                    if (previous === undefined)
                        throw new Error(`Missing cache card state ${c.id}.`);
                    next[c.id] = {
                        status,
        ...
        const fireEvent = (index: number) => {
            ...
            at(() => {
                if (index === EVENTS.length - 1) {
                    ...
                    setCards((prev) =>
                        Object.values(prev).some((s) => s.version >= 9)
                            ? initialCards()
                            : prev,
                    );
                }
        ...
        >
            ...
            >
                {CARDS.map((card) => {
                    ...
                    if (state === undefined)
                        throw new Error(`Missing cache card state ${card.id}.`);
            ...
            >
                저장할 때 의존하는 데이터를 태그로 기록해두면, 변경은 태그를
                조준하는 것으로 끝난다
            </div>
        </div>
    );
