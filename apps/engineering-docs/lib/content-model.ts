...
export const sectionLandingSections = [
    "handbook",
    "packages",
    "deep-dive",
] as const;
...
function validateStringArray(value: unknown, key: string): readonly string[] {
    ...
    if (normalized.some((item) => item.length === 0)) {
        throw new Error(
            `Metadata field "${key}" must not contain empty strings.`,
        );
    }
...
export function parseDocMetadata(
    ...
    const unknownFields = Object.keys(value).filter(
        (key) => !metadataFieldNames.has(key),
    );
    ...
    if (!isOneOf(locale, locales))
        throw new Error(`${source}: unsupported locale "${locale}".`);
    if (!isOneOf(section, sections))
        throw new Error(`${source}: unsupported section "${section}".`);
    if (!isOneOf(status, documentStatuses)) {
        throw new Error(`${source}: unsupported status "${status}".`);
    }
    if (!DOCUMENT_ID_PATTERN.test(id)) {
        throw new Error(
            `${source}: metadata field "id" must be a lowercase path.`,
        );
    }
    if (
        ...
        throw new Error(
            `${source}: document ID "${id}" does not belong to section "${section}".`,
        );
    }
    if (typeof order !== "number" || !Number.isInteger(order) || order < 0) {
        throw new Error(
            `${source}: metadata field "order" must be a non-negative integer.`,
        );
    }
    if (!isIsoDate(updatedAt)) {
        throw new Error(
            `${source}: metadata field "updatedAt" must be an ISO date.`,
        );
    }
    if (verifiedAt !== undefined && !isIsoDate(verifiedAt)) {
        throw new Error(
            `${source}: metadata field "verifiedAt" must be an ISO date.`,
        );
    }
    if (verifiedAt !== undefined && verifiedAt < updatedAt) {
        throw new Error(
            `${source}: metadata field "verifiedAt" must not precede "updatedAt".`,
        );
    }
    ...
    try {
        ...
        throw new Error(
            `${source}: metadata field "sourceUrl" must be an absolute URL.`,
        );
    }
    if (
        ...
        throw new Error(
            `${source}: metadata field "sourceUrl" must be a credential-free HTTPS URL.`,
        );
    }
