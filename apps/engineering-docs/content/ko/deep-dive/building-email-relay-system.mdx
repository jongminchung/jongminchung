...
description: "SMTP 세션, 주소 토큰과 회신 파서를 연결해 이메일 릴레이 시스템을 설계합니다."
order: 14
...
export function decodeEncodedWords(value: string): string {
  return value.replace(/=\?([^?]+)\?([BQbq])\?([^?]*)\?=/g, (_, charset, enc, payload) => {
    const bytes =
      enc.toUpperCase() === "B"
...
export function parseMail(raw: string): ParsedMail {
  ...
  if (contentType.startsWith("multipart/")) {
    ...
    for (const part of parts) {
      ...
      if (partType.startsWith("text/plain") || (partType.startsWith("multipart/") && parsed.text)) {
...
export function signReplyToken(ctx: ReplyContext): string {
  ...
  const sig = crypto.createHmac("sha256", SECRET).update(payload).digest("hex").slice(0, 16); // 로컬 파트 64자 제한 안에 들어가도록 잘라 쓴다
  return `${payload}.${sig}`;
}

export function verifyReplyToken(token: string, now: number): ReplyContext | null {
  ...
  return { ticketId: Number(ticketId), userId: Number(userId), exp: Number(exp) };
}
...
export function composeNotification(
  ...
  const headers = [
    ...
    ...(ticket.lastMessageId
      ? [`In-Reply-To: ${ticket.lastMessageId}`, `References: ${ticket.lastMessageId}`]
      : []),
...
export function handleInbound(mail: InboundMail, now: number): string {
  ...
  if (messageId && processedMessageIds.has(messageId)) return "skip: 이미 처리한 메일";
