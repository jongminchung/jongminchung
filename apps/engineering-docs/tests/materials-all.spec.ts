...
const manifest = JSON.parse(
  await readFile(resolve(process.cwd(), "generated/materials-manifest.json"), "utf8"),
...
test.describe("all material demos", () => {
  ...
  for (const topic of topics) {
    test(`${topic} loads every registered demo without runtime errors`, async ({ page }) => {
      ...
      for (const entry of entries) {
        ...
        await expect(demo).toHaveAttribute("data-material-active", "true");
        await expect.poll(() => demo.locator(":scope > div > *").count()).toBeGreaterThan(0);
      }
...
test("continuous material animation sustains the frame budget with low CLS", async ({ page }) => {
  await page.addInitScript(() => {
    const measured = window as Window & { __materialLayoutShift: number };
    ...
    new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        if (!shift.hadRecentInput) measured.__materialLayoutShift += shift.value;
      }
  ...
  expect(
    await page.evaluate(
      () => (window as Window & { readonly __materialLayoutShift: number }).__materialLayoutShift,
    ),
  ).toBeLessThan(0.1);
});
