...
async function resolveModuleFile(topicRoot: string, specifier: string): Promise<string> {
  const base = resolve(topicRoot, specifier);
  for (const extension of [".tsx", ".ts"]) {
    const candidate = `${base}${extension}`;
    if (await exists(candidate)) return candidate;
  }
  throw new Error(`Cannot resolve material module ${specifier} from ${topicRoot}.`);
}
...
function rendererFor(topic: string, fileName: string, source: string): MaterialExport["renderer"] {
  ...
  if (source.includes("<SvgCanvas") || source.includes("<svg")) return "svg-motion";
  return "dom-motion";
}

function minHeightFor(topic: string, renderer: MaterialExport["renderer"]): number {
  if (topic === "building-nes-emulator") return 520;
  if (topic === "building-llm" || topic === "it-is-the-boundary-stupid") return 400;
  if (renderer === "dom-motion") return 240;
  return 320;
}

async function readTopicExports(topic: string): Promise<readonly MaterialExport[]> {
  ...
  const declarationPattern = /export\s+(\*|\{[^}]+\})\s+from\s+['"]([^'"]+)['"]/gu;
  ...
  for (const declaration of indexSource.matchAll(declarationPattern)) {
    ...
    if (fileName === undefined) throw new Error(`Invalid material specifier: ${specifier}`);

    const items =
      clause === "*"
        ? namedExports(fileSource).map((name) => ({ exportName: name, localName: name }))
        : clause
            .slice(1, -1)
            .split(",")
            .map((item) => item.trim())
            .filter(Boolean)
            .map((item) => {
              const alias = /^(default|[A-Za-z_$][\w$]*)\s+as\s+([A-Za-z_$][\w$]*)$/u.exec(item);
              return alias === null
                ? { exportName: item, localName: item }
                : { exportName: alias[2] ?? item, localName: alias[1] ?? item };
            });
...
async function validateRenderingPolicy(): Promise<void> {
  ...
  for (const filePath of await listSourceFiles(topicsRoot)) {
    ...
    if (hasCanvas && !isCanvasException(relativePath)) {
      violations.push(`${relativePath}: native Canvas is outside the allowlist`);
    }
    if (/\b(?:requestAnimationFrame|cancelAnimationFrame)\s*\(/u.test(source)) {
      violations.push(`${relativePath}: direct animation frame scheduling is forbidden`);
    }
    if (/@keyframes|\banimation(?:Name)?\s*:/u.test(source)) {
      violations.push(`${relativePath}: CSS keyframe animation must use Motion`);
    }
    if (/https?:\/\/(?:picsum\.photos|images\.unsplash\.com)/u.test(source)) {
      violations.push(`${relativePath}: runtime sample images must be local`);
    }
  }
...
function createManifest(exports: readonly MaterialExport[]): string {
  const manifest = exports.map(({ exportName, fileName, id, minHeight, renderer, topic }) => ({
    id,
    topic,
    name: exportName,
    fileName,
...
if (topics.length !== expectedTopicCount) {
  throw new Error(`Expected ${expectedTopicCount} material topics, found ${topics.length}.`);
}
...
if (exports.length !== expectedDemoCount) {
  throw new Error(`Expected ${expectedDemoCount} public material demos, found ${exports.length}.`);
}
...
console.log(`Validated ${topics.length} material topics and ${exports.length} public demos.`);
