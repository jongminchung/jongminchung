...
export function RouteTransitionProvider({
    ...
    const clearTimers = useCallback((): void => {
        if (progressDelayRef.current !== null)
    ...
    const finishViewTransition = useCallback(
        async (transition: ViewTransition): Promise<void> => {
            try {
            ...
            const performNavigation = (): void => {
                startTransition(() => {
                    if (options.replace === true)
                        router.replace(targetHref, { scroll: options.scroll });
        ...
        <DocsNavigationContext value={value}>
            {isProgressVisible ? (
                <div
                    ...
                    data-docs-navigation-progress="true"
                >
                    <div
...
export function useDocsNavigation(): DocsNavigationValue {
    ...
    if (value === null) {
        throw new Error(
            "useDocsNavigation must be rendered inside RouteTransitionProvider.",
        );
    }
    return value;
}
