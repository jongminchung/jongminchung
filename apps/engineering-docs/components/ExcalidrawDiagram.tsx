...
export function ExcalidrawDiagram(
        ...
        const loadScene = async (): Promise<void> => {
            try {
                ...
                if (source !== undefined) sceneSource = source;
                    ...
                    sceneSource = await loadRemoteSource(
                        src,
                        controller.signal,
                    );
                ...
                const scene = parseExcalidrawSource(
                    sceneSource,
                    src ?? "inline Excalidraw scene",
                );
                if (!controller.signal.aborted)
                    setSceneState({ kind: "ready", scene });
            } catch (error: unknown) {
                if (!controller.signal.aborted) {
                    setSceneState({
                        kind: "error",
    ...
    const error =
        rendererError ??
        (sceneState.kind === "error" ? sceneState.message : null);
    const ready =
    ...
    return (
        ...
        >
            ...
            <div className={styles.canvas}>
                ...
                {sceneState.kind === "ready" &&
                sceneState.scene.textContent.length > 0 ? (
                    <span
