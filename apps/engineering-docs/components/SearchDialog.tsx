...
function parseSearchDocument(value: unknown): SearchDocument {
    if (!isRecord(value))
        throw new Error("Search index item must be an object.");
    const {
        id,
        locale,
        section,
...
export function SearchDialog({
    ...
    useEffect(() => {
        if (open && indexState.kind === "idle") void loadIndex();
    }, [indexState.kind, loadIndex, open]);
