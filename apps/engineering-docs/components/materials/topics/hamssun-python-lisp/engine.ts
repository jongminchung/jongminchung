...
export type Value = number | boolean | string | Value[] | Procedure | BuiltinFn | null;
...
export class Env {
  ...
  constructor(params: string[] = [], args: Value[] = [], outer: Env | null = null) {
...
const asNum = (v: Value): number => {
  if (typeof v !== "number") throw new TypeError(`숫자가 아니다: ${toLispString(v)}`);
  return v;
};
const asList = (v: Value): Value[] => {
  if (!Array.isArray(v)) throw new TypeError(`리스트가 아니다: ${toLispString(v)}`);
  return v;
};
...
const isTruthy = (v: Value): boolean =>
  v !== false && v !== null && v !== 0 && !(Array.isArray(v) && v.length === 0);
...
export function standardEnv(): Env {
  ...
  const b: Record<string, BuiltinFn | number> = {
    ...
    car: (xs) => {
      const l = asList(xs);
      if (l.length === 0) throw new TypeError("빈 리스트에는 car를 쓸 수 없다");
      return l[0];
    },
    ...
    filter: (f, xs) => asList(xs).filter((x) => isTruthy(callProc(asFn(f), [x]))),
    pi: Math.PI,
  };
...
export function evaluate(x: Expr, env: Env, depth = 0): Value {
  if (depth > MAX_DEPTH) {
    throw new RangeError("재귀가 너무 깊다 (함수썬의 재귀 깊이는 숙주 언어에 묶여 있다)");
  }
  if (typeof x === "string") {
    // 심볼 → 환경에서 찾는다
    return env.get(x);
  ...
  if (head === "if") {
    ...
    const branch = isTruthy(evaluate(test, env, depth + 1)) ? conseq : (alt ?? false);
  ...
  if (head === "set!") {
    const [, name, expr] = x;
    env.find(name as string).set(name as string, evaluate(expr, env, depth + 1));
    return null;
  }
  if (head === "lambda") {
    const [, params, body] = x;
  ...
  if (proc instanceof Procedure) {
    return evaluate(proc.body, new Env(proc.params, args, proc.env), depth + 1);
  }
  return proc(...args);
}
...
export function toLispString(value: Value | Expr): string {
  ...
  if (Array.isArray(value)) return "(" + value.map(toLispString).join(" ") + ")";
  if (value instanceof Procedure) return `#<함수 (${value.params.join(" ")})>`;
