...
import { tokenize, parse, type Expr } from "./engine";
...
const CHIP_STYLE: Record<ChipKind, { color: string; bg: string; border: string }> = {
...
const PRESETS = ["(+ 1 (* 2 3))", "(define square (lambda (x) (* x x)))", "(+ 1 (* 2 3)"];
...
const AtomChip = ({ value }: { value: number | boolean | string }) => {
  ...
  const label = value === true ? "#t" : value === false ? "#f" : String(value);
  return (
    <span
      style={{
        display: "inline-block",
...
const TreeNode = ({ expr, depth }: { expr: Expr; depth: number }) => {
  ...
  return (
    ...
    >
      {expr.length === 0 ? (
        <span style={{ fontFamily: MONO, fontSize: 12, color: "#adb5bd" }}>()</span>
      ) : (
        expr.map((child, i) => <TreeNode key={i} expr={child} depth={depth + 1} />)
      )}
    </span>
  );
};
...
export const SexprDemo = () => {
  ...
  const parsed = useMemo<{ expr: Expr; error: null } | { expr: null; error: string }>(() => {
    try {
      // parse는 토큰 배열을 소모(shift)하므로 복사본을 넘긴다
      const expr = parse([...tokens]);
      return { expr, error: null };
    } catch (e) {
      return { expr: null, error: e instanceof Error ? e.message : String(e) };
    }
  }, [tokens]);
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 18 }}>
        ...
        >
          {tokens.length === 0 ? (
            <span style={{ fontSize: 12, color: "#adb5bd" }}>토큰이 없다</span>
          ) : (
