...
import { standardEnv, readAll, evaluate, toLispString, Env } from "./engine";
  ...
  const runSource = (source: string) => {
    ...
    try {
      ...
      for (const expr of exprs) {
        ...
        if (value !== null) {
          newLines.push({ kind: "result", text: toLispString(value) });
        }
      }
    } catch (e) {
    ...
    >
      ...
      >
        {lines.map((line, i) => (
          <div
            key={i}
            style={{ color: LINE_COLOR[line.kind], whiteSpace: "pre-wrap", wordBreak: "break-all" }}
          >
            {line.kind === "echo" ? (
              <>
                <span style={{ color: "#6c7086" }}>{"함수썬> "}</span>
                {line.text}
              </>
            ) : (
              line.text
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: "#adb5bd", textAlign: "center" }}>
        define한 이름은 초기화 전까지 계속 살아 있으니 자기만의 함수를 하나 정의해서 굴려보자.
      </div>
    </div>
  );
};
