...
type Step =
  ...
  | { kind: "resolve"; path: string; expr: Expr; value: TraceValue; depth: number }
  | { kind: "skip"; path: string; expr: Expr; reason: string; depth: number };

const asNum = (v: TraceValue): number => {
  if (typeof v !== "number") throw new TypeError(`숫자가 아니다: ${fmtValue(v)}`);
  return v;
};

const BUILTINS: Record<string, (...xs: TraceValue[]) => TraceValue> = {
  "+": (...xs) => xs.map(asNum).reduce((a, c) => a + c),
  "-": (...xs) => (xs.length === 1 ? -asNum(xs[0]) : xs.map(asNum).reduce((a, c) => a - c)),
  "*": (...xs) => xs.map(asNum).reduce((a, c) => a * c),
  "/": (...xs) => xs.map(asNum).reduce((a, c) => a / c),
  ">": (a, b) => asNum(a) > asNum(b),
  "<": (a, b) => asNum(a) < asNum(b),
...
function traceEval(root: Expr): { steps: Step[]; result: TraceValue } {
  ...
  function ev(x: Expr, path: string, depth: number): TraceValue {
    ...
    if (head === "if") {
      ...
      const value = x.length > 3 ? ev(x[3], `${path}.3`, depth + 1) : false;
      steps.push({ kind: "resolve", path, expr: x, value, depth });
      return value;
    }
    ...
    const args = x.slice(1).map((arg, i) => ev(arg, `${path}.${i + 1}`, depth + 1));
    const value = fn(...args);
    steps.push({ kind: "resolve", path, expr: x, value, depth });
    return value;
  }
...
const renderExpr = (expr: Expr, path: string, view: TreeView): React.ReactNode => {
  // 이미 값이 된 부분식은 초록 칩으로 치환한다
  if (view.resolved.has(path)) {
    const justNow = view.current?.kind === "resolve" && view.current.path === path;
    return (
      <span
        style={{
          color: "#2f9e44",
  ...
  if (view.skipped.has(path)) {
    return (
      <span style={{ color: "#adb5bd", textDecoration: "line-through" }}>{exprToStr(expr)}</span>
    );
  }

  const entering = view.current?.kind === "enter" && view.current.path === path;
...
const PRESETS = ["(* (+ 1 2) (- 7 3))", "(+ 1 (* 2 (max 3 4)))", "(if (> 10 5) (+ 1 1) (/ 1 0))"];

const stepLog = (step: Step): string => {
  const indent = "  ".repeat(step.depth);
  if (step.kind === "enter") return `${indent}${exprToStr(step.expr)} 평가 시작`;
  if (step.kind === "resolve") return `${indent}${exprToStr(step.expr)} → ${fmtValue(step.value)}`;
...
export const EvalTraceDemo = () => {
  ...
  const view = useMemo<TreeView>(() => {
    ...
    return { resolved, skipped, current: applied.length > 0 ? applied[applied.length - 1] : null };
    // eslint 없이도 안전: applied는 trace와 stepIndex에서만 파생된다
  }, [trace, stepIndex]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    ...
    >
        ...
        {PRESETS.map((preset, i) => (
          <button
            ...
            style={{
              ...
              background: presetIndex === i ? "#e7f5ff" : "#f8f9fa",
              border: presetIndex === i ? "1px solid #a5d8ff" : "1px solid #dee2e6",
              borderRadius: 100,
              cursor: "pointer",
            }}
          >
      ...
      >
        ...
        <span style={{ marginLeft: "auto", fontSize: 12, fontFamily: MONO, color: "#868e96" }}>
          스텝 {stepIndex}/{total}
        </span>
      </div>
      ...
      >
        {applied.length === 0 ? (
          <div style={{ fontFamily: MONO, fontSize: 12, color: "#adb5bd" }}>
            [다음 ▶]을 눌러 평가를 시작하자
          </div>
        ) : (
          applied.map((step, i) => (
                ...
                color:
                  i === applied.length - 1
                    ...
                    : step.kind === "skip"
                      ? "#adb5bd"
                      : "#868e96",
                fontWeight: i === applied.length - 1 ? 700 : 400,
              }}
            >
              {stepLog(step)}
            </div>
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: "#adb5bd", textAlign: "center" }}>
        세 번째 프리셋에서 if가 (/ 1 0)을 평가하지 않고 건너뛰는 순간을 눈으로 확인해보자.
      </div>
    </div>
  );
};
