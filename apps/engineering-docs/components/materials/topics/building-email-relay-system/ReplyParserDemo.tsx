  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (t < d.classifyStart) {
        phase = 0;
        prog = Math.min(n, Math.floor(Math.max(0, t) / (REVEAL_MS / n)) + 1);
      } else if (t < d.sweepStart) {
        phase = 1;
        prog = Math.min(n, Math.floor(Math.max(0, t - d.classifyStart) / CLASSIFY_PER_LINE) + 1);
      } else if (t < d.resultStart) {
        phase = 2;
        prog = Math.min(
          d.sweepCount,
          Math.floor(Math.max(0, t - d.sweepStart) / SWEEP_PER_LINE) + 1,
        );
      } else {
        phase = 3;
        prog = d.sweepCount;
    ...
    >
      ...
      <div className="rpd-legend">
        {LEGEND.map((l) => (
          <span key={l.label} className="rpd-legend-item">
            <span className="rpd-legend-swatch" style={{ background: l.color }} />
            {l.label}
          </span>
        ))}
        <span className="rpd-legend-item" style={{ textDecoration: "line-through" }}>
          취소선 = 걷어냄
        </span>
      </div>
      ...
      <div className="rpd-lines">
        {d.lines.map((l, i) => {
          ...
          const struck = l.hidden && (phase === 3 || (phase === 2 && i >= n - prog));
          ...
          return (
            <div
              ...
              data-struck={struck}
              data-scan={scanning}
              style={{ color: classified ? KIND_COLOR[l.kind] : "#495057" }}
            >
              {l.text === "" ? " " : l.text}
            </div>
          );
        ...
        {phase === 3 &&
          (d.reply === "" ? (
            <motion.div
              ...
              style={{ background: "#fff5f5", border: "1px solid #fa5252" }}
            >
              <div className="rpd-result-label" style={{ color: "#c92a2a" }}>
                코멘트로 저장될 내용
              </div>
              <div
                className="rpd-result-body"
                style={{ color: "#c92a2a", fontFamily: FONT, fontSize: 12 }}
              >
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.35, ease: "easeOut" }}
              style={{ background: "#ebfbee", border: "1px solid #40c057" }}
            >
              <div className="rpd-result-label" style={{ color: "#2b8a3e" }}>
                코멘트로 저장될 내용
              </div>
