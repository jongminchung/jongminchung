...
const STEPS: Step[] = [
    ...
    },
    {
        lines: [
            [
                "C",
                "RCPT TO:<reply+42.7.1784209600.a58f8f3c9a6a265e@relay.example.com>",
            ],
            ["S", "250 OK"],
        ],
...
STEPS.forEach((step, si) => {
    ...
    cursor += isLast
        ? span + END_HOLD
        : Math.max(MIN_STEP_DUR, span + STEP_TAIL);
});
const CYCLE = cursor;
...
const Chip = ({
    ...
    <div
        className={
            "smtpd-chip" +
            (dataMode ? " smtpd-chip-data" : filled ? " smtpd-chip-filled" : "")
        }
    >
...
export const SmtpSessionDemo = () => {
    ...
    useEffect(() => {
        ...
        const tick = (now: number) => {
            ...
            setView((v) =>
                v.step === step && v.lineCount === lineCount
                    ? v
                    : { step, lineCount },
            );
            ...
            <div className="smtpd-progress">
                ...
                <div className="smtpd-progress-track">
                    <div
                        ...
                        style={{
                            width: `${((view.step + 1) / STEPS.length) * 100}%`,
                        }}
                    />
                </div>
            </div>
