...
function drawScene(ctx: SvgDrawingContext, w: number, t: number, render: boolean): number {
  const s = w / BASE_W;
  ...
  // 애니메이션 상태 계산
  const downP = t <= DOWN_START ? 0 : Math.min((t - DOWN_START) / (DOWN_END - DOWN_START), 1);
  const upP = t <= UP_START ? 0 : Math.min((t - UP_START) / (UP_END - UP_START), 1);
  const blueVisible = t >= DOWN_START && t <= DOWN_END;
  const yellowVisible = t >= UP_START && t <= UP_END;
  const leftFlash = t >= DOWN_END ? Math.max(0, 1 - (t - DOWN_END) / FLASH_DUR) : 0;
  const parentFlash = t >= UP_END ? Math.max(0, 1 - (t - UP_END) / FLASH_DUR) : 0;
  ...
  ctx.stroke();
  drawArrowHead(ctx, leftEdgeA, leftEdgeB, 0.94, Math.max(7 * s, 6), "#adb5bd");
  drawArrowHead(ctx, rightEdgeA, rightEdgeB, 0.94, Math.max(7 * s, 6), "#adb5bd");
  ...
  const lfs = fitFontSize(ctx, leftLabel, labelFs, leftMid.x - PAD - 22, "600");
  ...
  const rfs = fitFontSize(ctx, rightLabel, labelFs, w - rightMid.x - PAD - 22, "600");
  ...
  // 자식 박스 2개
  const children: { label: string; cx: number; flash: number; flashColor: string }[] = [
    { label: "CartBadge", cx: leftCx, flash: leftFlash, flashColor: "#228be6" },
    { label: "AddToCartButton", cx: rightCx, flash: 0, flashColor: "#228be6" },
  ];
  ...
  if (blueVisible) {
    drawDot(ctx, lerp(leftEdgeA, leftEdgeB, easeInOutCubic(downP)), dotR, "#228be6");
  }
  if (yellowVisible) {
    drawDot(ctx, lerp(rightEdgeA, rightEdgeB, easeInOutCubic(upP)), dotR, "#fab005");
  }
...
export const DataFlowDiagram = ({ caption }: Props) => {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      if (w !== sizedW || h !== sizedH || canvas.width !== Math.round(w * dpr)) {
