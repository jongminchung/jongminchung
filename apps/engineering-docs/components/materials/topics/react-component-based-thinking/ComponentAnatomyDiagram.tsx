...
const BRANCHES: BranchSpec[] = [
  ...
  { title: "데이터", color: "#40c057", bg: "#d3f9d8", chips: ["props", "상태", "파생 값"] },
  {
    title: "로직",
    color: "#845ef7",
    bg: "#f3f0ff",
...
function drawBranchBox(
  ...
  if (render) {
    ...
    for (const p of pos) {
      drawBox(ctx, x + pad + p.x, chipTop + p.y, p.w, chipH, 5, "#fff", branch.color, 1.2);
      const fs = fitFontSize(ctx, p.label, chipFs, p.w - 10, "600");
      ctx.font = `600 ${fs}px ${FONT}`;
      ctx.fillStyle = "#495057";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(p.label, x + pad + p.x + p.w / 2, chipTop + p.y + chipH / 2 + 0.5);
    }
  }
  return boxH;
}
...
function drawDesktop(ctx: SvgDrawingContext, w: number, render: boolean): number {
  ...
  const heights = BRANCHES.map((b, i) =>
    drawBranchBox(ctx, PAD + i * (colW + colGap), colY, colW, b, s, "stack", false),
  );
  ...
  if (render) {
    ...
    BRANCHES.forEach((_, i) => {
      ...
      ctx.bezierCurveTo(fromX, fromY + linkH * 0.55, toX, colY - linkH * 0.55, toX, colY);
      ctx.stroke();
    });
    ...
    BRANCHES.forEach((b, i) => {
      drawBranchBox(ctx, PAD + i * (colW + colGap), colY, colW, b, s, "stack", true);
    });
  }
