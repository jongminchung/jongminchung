...
const OUTSIDE_BOXES: OutsideBoxSpec[] = [
  ...
  { title: "쿼리 캐시", sub: "서버 상태의 사본", fill: "#d3f9d8", stroke: "#40c057" },
  { title: "순수 함수", sub: "도메인 로직", fill: "#fff5f5", stroke: "#fa5252" },
];
...
function drawTreeNode(ctx: SvgDrawingContext, node: TreeNode) {
  drawBox(ctx, node.rect.x, node.rect.y, node.rect.w, node.rect.h, 6, "#e7f5ff", "#228be6", 1.3);
  ctx.font = `600 ${node.fs}px ${FONT}`;
  ctx.fillStyle = "#495057";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(node.label, node.rect.x + node.rect.w / 2, node.rect.y + node.rect.h / 2 + 0.5);
}
...
function dashedLine(ctx: SvgDrawingContext, x1: number, y1: number, x2: number, y2: number) {
  ctx.beginPath();
  ctx.moveTo(x1, y1);
  ctx.lineTo(x2, y2);
  ctx.stroke();
...
function drawDesktop(ctx: SvgDrawingContext, w: number, render: boolean): number {
  const s = w / BASE_W;
  ...
  // 바깥 박스 3개
  OUTSIDE_BOXES.forEach((box, i) => drawOutsideBox(ctx, box, boxRects[i], boxTitleFs, boxSubFs));
...
function drawMobile(ctx: SvgDrawingContext, w: number, render: boolean): number {
  const s = w / BASE_W;
  const pw = w - PAD * 2;
  const gap1 = 20;
  const gap2 = 20;
  ...
  OUTSIDE_BOXES.forEach((box, i) => {
    const rect: Rect = { x: PAD, y: boxesTop + i * (boxH + boxGap), w: pw, h: boxH };
