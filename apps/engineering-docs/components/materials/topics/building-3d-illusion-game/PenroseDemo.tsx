...
import { CUBE_FACES, drawCube, FONT, PALETTE, type ProjectFn } from "./draw";
import {
    ISO_PITCH,
    ISO_YAW,
    orthoProject,
...
export const PenroseDemo = () => {
    ...
    const viewRef = useRef<{ tile: number; cx: number; cy: number } | null>(
        null,
    );
        ...
        const render = () => {
            try {
                ...
                if (anim) {
                    const t = Math.min(
                        1,
                        Math.max(0, now - anim.start) / anim.dur,
                    );
                    const e = easeOutCubic(t);
                    yawRef.current =
                        anim.fromYaw + (anim.toYaw - anim.fromYaw) * e;
                    pitchRef.current =
                        anim.fromPitch + (anim.toPitch - anim.fromPitch) * e;
                ...
                const proj = (v: Vec3) =>
                    usePersp
                        ? perspectiveProject(v, yaw, pitch, CAM_DIST)
                        : orthoProject(v, yaw, pitch);
                ...
                ctx.fillText(
                    closed ? "펜로즈 삼각형 완성!" : "각도를 맞춰보세요",
                    w - 12,
                    10,
                );
                if (usePersp) {
                    const near = perspectiveProject(
                        vec3(N, N, N),
                        yaw,
                        pitch,
                        CAM_DIST,
                    ).scale;
                    const far = perspectiveProject(
                        vec3(0, 0, 0),
                        yaw,
                        pitch,
                        CAM_DIST,
                    ...
                    ctx.fillText(
                        `가까운 팔이 약 ${(near / far).toFixed(1)}배 두껍게 보임`,
                        12,
                        10 + fs + 6,
                    );
                }
                ...
                ctx.fillStyle = PALETTE.subtext;
                ctx.fillText(
                    `yaw ${yawDeg.toFixed(1)}° · pitch ${(pitch / DEG).toFixed(1)}°`,
                    12,
                    h - 8,
                );
            } catch {
    ...
    const goRandom = () => {
        const sign =
            seededMaterialRandom("building-3d-illusion-game/PenroseDemo") < 0.5
                ? -1
                : 1;
        const toYaw =
            yawRef.current +
        ...
        let toPitch =
            (12 +
                seededMaterialRandom("building-3d-illusion-game/PenroseDemo") *
                    60) *
            DEG;
