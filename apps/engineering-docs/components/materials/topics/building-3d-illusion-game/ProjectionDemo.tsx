      ...
      for (let dx = 0; dx <= 1; dx++) {
        for (let dy = 0; dy <= 1; dy++) {
          for (let dz = 0; dz <= 1; dz++) {
            ...
            const p =
              kind === "persp"
                ? perspectiveProject(v, yaw, ISO_PITCH, CAM_DIST)
...
export const ProjectionDemo = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const panelH = stacked ? Math.min(250, Math.max(190, Math.round(w * 0.62))) : 250;
      ...
      const panel = (px: number, py: number, label: string, kind: "persp" | "ortho") => {
        // 패널 배경
        ctx.beginPath();
        ...
        const tile = Math.min(sceneW / (spanX + 0.3), sceneH / (spanY + 0.3));
        const originX = px + panelW / 2 - ((b.minX + b.maxX) / 2) * tile;
        const originY = py + labelH + sceneH / 2 - ((b.minY + b.maxY) / 2) * tile;

        const project: ProjectFn =
          kind === "ortho"
            ? makeOrthoCam(yaw, ISO_PITCH, tile, originX, originY)
            : (v: Vec3) => {
                const p = perspectiveProject(v, yaw, ISO_PITCH, CAM_DIST);
        ...
        ctx.fillText(`가까운 칸 ${near}px · 먼 칸 ${far}px`, px + 12, py + panelH - 10);
      };

      panel(0, 0, "원근 투영", "persp");
      panel(stacked ? 0 : panelW + gap, stacked ? panelH + gap : 0, "평행 투영", "ortho");
    };
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, fontSize: 11, color: "#adb5bd", textAlign: "center" }}>
        드래그해서 카메라를 돌려보세요 — 직교 투영에서는 아무리 돌려도 크기가 변하지 않는다
      </div>
    </div>
  );
};
