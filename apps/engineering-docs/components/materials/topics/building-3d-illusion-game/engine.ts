...
// 4방향 한 칸 이동 (수평)
const DIRS: Vec3[] = [vec3(1, 0, 0), vec3(-1, 0, 0), vec3(0, 0, 1), vec3(0, 0, -1)];
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    for (const d of DIRS) {
      ...
      const target = orthoProject(
        vec3(nodes[i].pos.x + d.x, nodes[i].pos.y + d.y, nodes[i].pos.z + d.z),
        yaw,
        pitch,
      );
      ...
      for (let j = 0; j < nodes.length; j++) {
        ...
        if (Math.hypot(dx, dy) < epsilon) {
          ...
          if (found < 0 || projected[j].depth < projected[found].depth) found = j;
        }
      }
...
export function buildWorldGraph(nodes: WalkNode[]): Map<string, string[]> {
  ...
  for (const n of nodes) {
    const edges: string[] = [];
    for (const d of DIRS) {
      const neighbor = keyOf(vec3(n.pos.x + d.x, n.pos.y + d.y, n.pos.z + d.z));
...
export function findPath(graph: Map<string, string[]>, from: string, to: string): string[] | null {
...
export function paintersOrder(positions: Vec3[], yaw: number, pitch: number): number[] {
  const depths = positions.map(
    (p) => orthoProject(vec3(p.x + 0.5, p.y + 0.5, p.z + 0.5), yaw, pitch).depth,
  );
...
export function blockCovers(block: Vec3, point: Vec3, yaw: number, pitch: number): boolean {
  const p = orthoProject(point, yaw, pitch);
  const center = orthoProject(vec3(block.x + 0.5, block.y + 0.5, block.z + 0.5), yaw, pitch);
  if (center.depth >= p.depth - 0.05) return false; // 점 뒤에 있는 블록은 가릴 수 없다
  for (const face of UNIT_FACES) {
    const quad = face.map(([dx, dy, dz]) =>
      orthoProject(vec3(block.x + dx, block.y + dy, block.z + dz), yaw, pitch),
    );
...
export function pointHidden(point: Vec3, blocks: Vec3[], yaw: number, pitch: number): boolean {
...
// 정확히 ISO 각도라면 0이 나온다.
export function penroseGapOnScreen(n: number, yaw: number, pitch: number): number {
