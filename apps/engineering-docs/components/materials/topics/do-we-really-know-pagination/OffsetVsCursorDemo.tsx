...
export const OffsetVsCursorDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // --- 오프셋 레인 ---
            const offsetRead = Math.min(
                TARGET_START,
                Math.floor(elapsed / SCAN_MS),
            );
            const offsetScanDone = offsetRead >= TARGET_START;
            const offsetReturned = offsetScanDone
                ? Math.min(
                      PAGE,
                      Math.floor((elapsed - TARGET_START * SCAN_MS) / SCAN_MS) +
                          1,
                  )
                : 0;
            ...
            const hopIdx = Math.min(
                hops.length - 1,
                Math.floor(elapsed / CURSOR_HOP_MS),
            );
    ...
    return (
        ...
        >
            ...
            >
                같은 5건을 반환하지만 오프셋은 앞의 40건을 읽어서 버리고, 커서는
                시작 지점으로 바로 이동한다
            </div>
        </div>
    );
