...
export const ScatterGatherDemo = () => {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      ctx.fillText(
        deep ? "from: 990, size: 10 — 100번째 페이지" : "search_after — 같은 지점을 커서로",
        w / 2,
        20,
      );
      ...
      const returned = doneGather ? 10 : 0;
      ...
      for (let i = 0; i < SHARDS; i++) {
        ...
        if (t > 600 && !doneGather) {
          ...
          const fromY = shardY;
          const toX = coordX + coordW / 2 + (i - (SHARDS - 1) / 2) * 20;
          const toY = coordY + coordH;
          const x = fromX + (toX - fromX) * p;
          const y = fromY + (toY - fromY) * p;
          // 문서량을 뭉치 크기로 표현
          const size = deep ? Math.max(10, 16 * s) : Math.max(4, 6 * s);
          ctx.beginPath();
          ctx.roundRect(x - size / 2, y - size / 2, size, size * 1.2, 2);
