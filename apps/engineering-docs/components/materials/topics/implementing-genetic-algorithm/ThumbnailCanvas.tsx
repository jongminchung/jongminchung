...
function drawTrack(ctx: SvgDrawingContext, track: ReturnType<typeof buildTrack>, view: View) {
  const { center } = track;
...
function draw(ctx: SvgDrawingContext) {
  ...
  for (let i = 0; i < 260 && shown < 22; i++) {
    ...
    shown++;
    drawTrace(ctx, trace, view, "#adb5bd", Math.max(2, view.scale * 3), 0.32);
    // 궤적 끝(죽은 지점)에 흐린 자동차
    const last = trace[trace.length - 1];
    const prev = trace[trace.length - 2];
    const heading = Math.atan2(last[1] - prev[1], last[0] - prev[0]);
    drawCar(ctx, view, last[0], last[1], heading, "#ced4da", view.scale * 9, 0.5);
  }

  // --- 챔피언: 트랙을 완주하는 매끄러운 궤적 (파랑, 강조) ---
  const champTrace = runTrace(track, CHAMPION, 1800);
  // 글로우 언더레이
  drawTrace(ctx, champTrace, view, "#a5cff5", Math.max(10, view.scale * 15), 0.5);
  ...
  if (champTrace.length >= 2) {
    ...
    const heading = Math.atan2(head[1] - beforeHead[1], head[0] - beforeHead[0]);
    // 강조용 흰 테두리 원
    ctx.beginPath();
    ctx.arc(view.wx(head[0]), view.wy(head[1]), view.scale * 20, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(34,139,230,0.16)";
    ctx.fill();
    drawCar(ctx, view, head[0], head[1], heading, "#1971c2", view.scale * 14, 1);
  }
}
