...
function buildCycle(cycleIndex: number, uniform: boolean): CycleData {
    ...
    const pick = Array.from({ length: GENES }, () =>
        rnd() < 0.5 ? "A" : "B",
    ) as ("A" | "B")[];
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            data.parentA.forEach((v, i) => {
                drawCell(ctx, cellX(i), yA, cellSize, geneColor(v, "A"), {
                    alpha: showP,
                });
            });
            ...
            data.parentB.forEach((v, i) => {
                drawCell(ctx, cellX(i), yB, cellSize, geneColor(v, "B"), {
                    alpha: showP,
                });
            });
            ...
            // 각 칸이 어느 부모에서 오는지
            const fromA = (i: number) =>
                isUniform ? data.pick[i] === "A" : i < data.cutPoint;
            ...
            data.parentA.forEach((_, i) => {
                ...
                if (isMut && e >= mutStart && e < mutStart + 900) {
                    const pulse =
                        0.5 +
                        0.5 * Math.sin(((e - mutStart) / 900) * Math.PI * 3);
                    stroke = "#fa5252";
                    lineWidth = 1 + pulse * 2;
                } else if (mutDone) {
                    stroke = "#fa5252";
                ...
                drawCell(ctx, cellX(i), y, cellSize, fill, {
                    alpha: p,
                    stroke,
                    lineWidth,
                });
