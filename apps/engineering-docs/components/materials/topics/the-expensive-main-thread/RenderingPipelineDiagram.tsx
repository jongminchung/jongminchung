...
function drawVertical(ctx: SvgDrawingContext, w: number, t: Strings): number {
    ...
    const tagW = Math.max(
        ctx.measureText(t.mainTag).width,
        ctx.measureText(t.compositorTag).width,
    );
    const tagGap = 12;
    const boxW = w * 0.62;
    ...
    t.stages.forEach(([title, sub], i) => {
        ...
        ctx.textBaseline = "middle";
        ctx.fillText(
            main ? t.mainTag : t.compositorTag,
            boxX + boxW + tagGap,
            y + boxH / 2,
        );
...
export const RenderingPipelineDiagram = ({
    caption,
    locale: lang = "ko",
}: Props) => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const probeH = isMobile ? 420 : 150 * (w / 620);
            canvas.width = w * dpr;
