...
const STRINGS = {
  ...
  en: {
    ...
    render: "Rendering",
    renderShort: "Render",
    waitNote: "Input and rendering wait in the queue until the task finishes",
    rowYield: "With yielding",
    chunks: ["1–20", "21–40", "41–60"],
    scheduleNote: "setTimeout schedules the remaining work as the next task",
...
function drawLegend(ctx: SvgDrawingContext, x: number, y: number, fs: number, s: number, t: T) {
...
function drawHorizontal(ctx: SvgDrawingContext, w: number, t: T): number {
  ...
  drawBox(ctx, left + longW + 4 * s + inW + 4 * s, t1, renW, boxH, C.render, t.render, boxFs);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (i < chunks.length - 1) {
      ...
      drawBox(ctx, x + gapW * 0.03, t2, gInW, boxH, C.input, t.input, smallFs);
      drawBox(
        ctx,
...
export const YieldTimelineDiagram = ({ caption, locale: lang = "ko" }: Props) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      let probeH = isMobile ? 620 : 230 * (w / 620);
      canvas.width = w * dpr;
