...
export const FrameBudgetChunkingDemo = ({
    ...
    useEffect(() => {
        ...
        const animate = (t: number) => {
            ...
            if (fpsRef.current)
                fpsRef.current.textContent = String(frames.length);
            ...
            const dt = lastTRef.current
                ? Math.min(t - lastTRef.current, 100)
                : 16.7;
            lastTRef.current = t;
            ...
            if (active) {
                if (modeRef.current === "once") {
                    ...
                    for (let i = 0; i < ps.length; i++)
                        steerParticle(i, ps, energy, mouse.x, mouse.y, t);
                } else {
                    // 5ms씩: 예산 안에서 계산하고, 나머지는 다음 프레임으로 미룬다
                    let count = 0;
                    while (
                        performance.now() - t0 < BUDGET_MS &&
                        count < ps.length
                    ) {
                        steerParticle(
                            cursorRef.current,
            ...
            if (costRef.current)
                costRef.current.textContent = `${cost.toFixed(1)}ms`;
            ...
            for (const p of ps) {
                ...
                if (!active) {
                    ...
                    p.vx =
                        p.vx * 0.9 +
                        Math.sin(p.phase + t * 0.0012) * IDLE_DRIFT * 0.1;
                    p.vy =
                        p.vy * 0.9 +
                        Math.cos(p.phase * 1.3 + t * 0.0009) * IDLE_DRIFT * 0.1;
                }
                if (p.x < 0) p.x += w;
                else if (p.x > w) p.x -= w;
                if (p.y < 0) p.y += h;
