...
const STRINGS = {
    ko: {
        ...
        blockTwo: "2초 막기",
        blockingCaption:
            "메인 스레드를 붙잡는 중 — JS 애니메이션과 입력이 멈춘다",
        idleCaption:
            "버튼을 누르면 JS 애니메이션과 입력은 멈추지만 CSS 애니메이션은 계속 돈다",
    },
    en: {
        ...
        blockingCaption:
            "Holding the main thread — the JS animation and typing freeze",
        idleCaption:
...
export const LongTaskBlockingDemo = ({
    locale: lang = "ko",
}: {
    locale?: Lang;
}) => {
    ...
    useEffect(() => {
        ...
        const animate = (t: number) => {
            ...
            lastRef.current = t;
            if (jsBoxRef.current)
                jsBoxRef.current.style.transform = `rotate(${angleRef.current}deg)`;
        ...
        >
            ...
            >
                <motion.div
                    ref={boxRef}
                    animate={{
                        rotate: animated && visible && !reducedMotion ? 360 : 0,
                    }}
                    transition={{
                        duration: 1.5,
                        ease: "linear",
