...
export const TransformVsLayoutDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
    ...
    >
      <div style={{ background: "#f8f9fa", borderRadius: 8, padding: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#2f9e44", marginBottom: 6 }}>
          ...
          <span style={{ fontWeight: 400, color: "#868e96" }}>· {t.compositor}</span>
        </div>
        <div style={trackStyle}>
          <motion.div
            animate={{ x: visible && !reducedMotion ? "calc(100cqw - 34px)" : 0 }}
            transition={{
              duration: 1.8,
              ease: "easeInOut",
              repeat: Infinity,
        ...
        <div style={{ fontSize: 12, fontWeight: 600, color: "#e8590c", margin: "16px 0 6px" }}>
          left <span style={{ fontWeight: 400, color: "#868e96" }}>· {t.mainThread}</span>
        </div>
        <div style={trackStyle}>
          <motion.div
            animate={{ left: visible && !reducedMotion ? "calc(100% - 34px)" : 0 }}
            transition={{
              duration: 1.8,
              ease: "easeInOut",
              repeat: Infinity,
