...
const STRINGS = {
  ko: {
    ...
    mainItems: ["JS 실행", "DOM", "스타일 계산", "레이아웃", "페인트", "이벤트 처리"],
    mainThread: "메인 스레드",
    directControl: "직접 제어",
    compositor: "컴포지터 스레드",
  ...
  en: {
    mainItems: ["JS execution", "DOM", "Style calculation", "Layout", "Paint", "Event handling"],
...
function drawHorizontal(ctx: SvgDrawingContext, w: number, t: T): number {
  ...
  drawChips(ctx, t.mainItems, mainX + 18 * s, mainX + mainW - 18 * s, mainY + 40 * s, chipFs);
...
function drawVertical(ctx: SvgDrawingContext, w: number, t: T): number {
  ...
  drawBadge(ctx, mainX + mainW - 46, mainY + 20, t.directControl, "#fff", "#228be6", badgeFs);
  drawChips(ctx, t.mainItems, mainX + 14, mainX + mainW - 14, mainY + 38, chipFs);
  ...
  for (const [title, role, badge, bc, bg] of subs) {
    ...
    drawBadge(ctx, mainX + mainW - 44, y + subH / 2, badge, bc, bg, badgeFs);
    y += subH + subGap;
  }
  return y;
}
...
export const BrowserThreadMapDiagram = ({ caption, locale: lang = "ko" }: Props) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      canvas.width = w * dpr;
      let probeH = isMobile ? 480 : 260 * (w / 620);
