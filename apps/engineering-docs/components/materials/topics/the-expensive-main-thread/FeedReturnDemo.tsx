...
const STRINGS = {
    ko: {
        ...
        header: (count: string) =>
            `게시물 ${count}개가 쌓인 피드 — 알림 탭에 다녀와 보자`,
        modeIo: "보일 때만 렌더",
        modeOff: "그대로 렌더",
...
export const FeedReturnDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
    ...
    const buildInitialDom = useEffectEvent(() => {
        buildFeed();
        buildNotifs();
    });
    ...
    const tabBtn = (t: "feed" | "notif", label: string) => (
        <button
            ...
            style={{
                ...
                border: "none",
                borderBottom:
                    tab === t ? "2px solid #228be6" : "2px solid transparent",
                background: "none",
    ...
    return (
        ...
        >
            ...
            >
                {mode === "off"
                    ? t.footerOff(POSTS.toLocaleString())
                    : t.footerIo}
            </div>
        </div>
    );
