...
type CarveRequest = { buf: ArrayBuffer; stride: number; w: number; h: number; seams: number };
...
addEventListener("message", (e: MessageEvent<CarveRequest>) => {
  ...
  for (let s = 1; s <= seams; s++) {
    ...
    if (s % FRAME_EVERY === 0 && s < seams) {
      ...
      postMessage({ type: "frame", buf: frame.buffer, w, done: s }, { transfer: [frame.buffer] });
    }
  }

  const frame = packFrame(px, stride, w, h);
  postMessage({ type: "done", buf: frame.buffer, w, done: seams }, { transfer: [frame.buffer] });
});
