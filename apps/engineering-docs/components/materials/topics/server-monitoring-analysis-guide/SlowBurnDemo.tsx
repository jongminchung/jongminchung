...
const STEPS: Step[] = [
    {
        caption:
            "① 4주 동안 알람은 한 번도 울리지 않았다 — 어제와 비교하면 늘 '정상'이었으니까",
        color: "#868e96",
    },
    {
        caption:
            "② 4주 전 같은 요일과 겹쳐보면 — 같은 트래픽에 응답만 72% 느려졌다",
        color: "#fa5252",
    },
    {
        caption:
...
function p99At(t: number): number {
    ...
    const texture =
        25 * wave(t, 28, 0.15) + 7 * wave(t, 56, -0.4) + 4 * wave(t, 92, 0.5);
            ...
            if (p3 > 0) {
                ...
                if (markA > 0) {
                    ...
                    const ang = Math.atan2(
                        toY(trendAt(1)) - toY(trendAt(0.9)),
                        toX(1) - toX(0.9),
                    );
                    ctx.beginPath();
                    ctx.moveTo(exitX, exitY);
                    ctx.lineTo(
                        exitX - 9 * Math.cos(ang - 0.4),
                        exitY - 9 * Math.sin(ang - 0.4),
                    );
                    ctx.lineTo(
                        exitX - 9 * Math.cos(ang + 0.4),
                        exitY - 9 * Math.sin(ang + 0.4),
