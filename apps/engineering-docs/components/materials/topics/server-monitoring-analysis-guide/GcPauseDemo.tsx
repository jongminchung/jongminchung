...
function heapAt(t: number): number {
    ...
    return (
        HEAP_BASE +
        (HEAP_TOP - HEAP_BASE) * phase +
        0.012 * Math.sin(x * Math.PI * 2 * 13)
    );
}
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            if (progress < 1) {
                ...
                const p99Y =
                    t1Y + trackH * (1 - clamp(p99Now, 0, P99_MAX) / P99_MAX);
                ...
                const drawDot = (
                    ...
                    const nearRight =
                        cx > plotX + plotW - ctx.measureText(label).width - 12;
                ...
                drawDot(
                    ...
                    t2Y,
                    "#228be6",
                    `힙 ${Math.round(heapNow * 100)}%`,
                );
            }

            // --- 하단 단계 캡션 ---
            let caption =
                "① P99에만 주기적으로 바늘 스파이크 — 트래픽·CPU·에러는 전부 평온하다";
            if (e >= T_ALIGN) {
                caption =
