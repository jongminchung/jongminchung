    ...
    useEffect(() => {
        ...
        const queueIndex = (i: number, e: number) => {
            ...
            for (let j = 0; j < REQS.length; j++) {
                if (j === i) continue;
                const r = REQS[j]!;
                if (r.t >= REQS[i]!.t || e < r.t) continue;
        ...
        const render = (now: number) => {
            ...
            const doneCount = REQS.filter(
                (r) => e > r.p + (r.io ? IO_LIFE : NORMAL_LIFE),
            ).length;
            ...
            const spin = (t: number) => {
                ...
                return (
                    T_BLOCK * BASE_SPEED + (tt - T_UNBLOCK) * BASE_SPEED * 1.4
                );
            };
            ...
            REQS.forEach((r, i) => {
                ...
                const slotY =
                    cy +
                    queueIndex(i, e) * slotGap +
                    Math.sin(e / 300 + i * 1.7) * 1.2;
                ...
                if (e < r.p) {
                    ...
                    r.io &&
                    e < r.p + D_TO_LOOP + D_RING + D_IO_DWELL + D_RING
                ) {
                    // 콜백으로 루프에 복귀
                    const q = easeOut(
                        (e - r.p - D_TO_LOOP - D_RING - D_IO_DWELL) / D_RING,
                ...
                if (r.io) {
                    ...
                    if (seg >= 0 && seg < D_RING + D_IO_DWELL) {
                        ...
                        seg >= D_RING + D_IO_DWELL &&
                        seg < D_RING + D_IO_DWELL + D_RING
                    ) {
                        ctx.textAlign = "left";
            ...
            if (e >= T_UNBLOCK) {
                caption =
                    "④ 작업이 끝난 뒤에야 밀린 큐가 한꺼번에 처리된다 — 루프 지연이 곧 응답 지연이다";
            } else if (e >= T_BLOCK + 800) {
                caption =
                    "③ 스레드 풀 서버라면 스레드 1개가 느려질 뿐이지만, Node는 프로세스 전체가 멈춘다";
            } else if (e >= T_CPU_IN) {
                caption =
                    "② 무거운 CPU 작업이 들어오면 루프가 그 자리에서 멈춘다";
            }
