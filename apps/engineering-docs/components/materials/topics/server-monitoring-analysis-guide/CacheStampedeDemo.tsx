    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            for (let i = 0; i < DOT_N; i++) {
                ...
                const ny = flowY;
                const na =
                    clamp(ph / 0.05, 0, 1) * clamp((0.98 - ph) / 0.06, 0, 1);
            ...
            if (progress >= 0.74) {
                caption =
                    "④ 한 명만 다시 계산하게 하라 — 잠금, 조기 갱신, TTL 지터가 해법이다";
                capColor = "#2f9e44";
            } else if (progress >= 0.46) {
                caption =
                    "③ 전원이 같은 쿼리를 들고 DB로 몰린다 — DB QPS 100 → 4,800";
                capColor = "#fa5252";
            } else if (progress >= EXPIRE_T) {
                caption =
                    "② 인기 키의 TTL이 만료되는 순간, 그 키를 찾던 동시 요청 전부가 미스";
                capColor = "#f08c00";
            }
