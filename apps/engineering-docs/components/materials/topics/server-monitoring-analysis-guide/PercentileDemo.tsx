...
export const PercentileDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            const captionH = 38;
            const h =
                pad + (isMobile ? PANEL_H * 2 + vGap : PANEL_H) + captionH;
            ...
            // --- 하단 단계 설명 ---
            let caption =
                "① 요청 40건이 쌓여 두 서버의 응답 시간 분포가 만들어진다";
            if (e >= T_P50) {
                caption =
                    "③ 평균은 같지만 P99는 6배 차이(130ms vs 850ms) — 평균은 꼬리를 숨긴다";
            } else if (e >= T_MEAN) {
                caption =
                    "② 평균 응답 시간은 둘 다 100ms — 평균만 보면 두 서버는 똑같다";
            }
            ...
            ctx.textBaseline = "alphabetic";
            if (
                ctx.measureText(caption).width > w - 20 &&
                caption.includes(" — ")
            ) {
