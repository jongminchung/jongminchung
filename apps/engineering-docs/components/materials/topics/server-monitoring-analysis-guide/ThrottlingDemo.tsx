...
function p99At(u: number): number {
    const wob =
        3 * Math.sin(u * Math.PI * 3.1) + 2 * Math.sin(u * Math.PI * 7.3);
    const levels = [62, 140, 195, 235, 262]; // 스로틀 주기를 지날 때마다의 기준선
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            if (throttleA > 0) {
                ...
                const groupCx =
                    cellX(LOW_PERIODS) +
                    (cellW * (N_PERIODS - LOW_PERIODS) +
                        cellGap * (N_PERIODS - LOW_PERIODS - 1)) /
                        2;
            ...
            ctx.fillStyle = "#868e96";
            const lowCx =
                plotX + (cellW * LOW_PERIODS + cellGap * (LOW_PERIODS - 1)) / 2;
            ...
            ctx.fillText(
                "(ms)",
                plotX + plotW - ctx.measureText("(ms)").width,
                trackY - 7,
            );
            ...
            // 곡선 (a와 시간 동기화)
            const toY = (v: number) =>
                trackY + trackH * (1 - clamp(v, 0, P99_MAX) / P99_MAX);
            ...
            const nearRight =
                cx > plotX + plotW - ctx.measureText(valLabel).width - 12;
