...
export const TimeoutMismatchDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            ctx.fillText(
                `${simT.toFixed(1)}s`,
                pad + ctx.measureText(clockPrefix).width,
                14,
            );
            ctx.textAlign = "right";
            ...
            if (timedOut) {
                ...
                ctx.fillStyle =
                    e < T_FWD ? "#adb5bd" : remain <= 1 ? "#fa5252" : "#1c7ed6";
                ctx.fillText(
                    `${remain.toFixed(1)}s`,
                    gwX + gwW / 2,
                    boxTop + boxH * 0.66,
            ...
            // --- 하단 단계 설명 ---
            let caption =
                "① 게이트웨이는 3초까지만 기다린다 — 백엔드의 쿼리는 5초짜리";
            if (e >= T_DONE) {
                caption =
                    "③ 두 대시보드가 서로 모순이면 고장이 아니라 타임아웃 계층부터 의심하라";
            } else if (e >= T_TIMEOUT) {
                caption =
                    "② 사용자는 이미 에러를 받았는데, 백엔드는 계속 일한다";
            }
