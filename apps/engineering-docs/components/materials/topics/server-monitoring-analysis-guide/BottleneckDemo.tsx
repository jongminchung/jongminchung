...
export const BottleneckDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // --- 현재 유입/처리량 ---
            const inflow =
                IN_LOW + (IN_HIGH - IN_LOW) * easeOut((e - T_SURGE) / T_RAMP);
            ...
            const qFill =
                e >= T_QUEUE
                    ? Math.min(MAX_Q_DOTS, Math.floor((e - T_QUEUE) / 240))
                    : 0;
            const qWait =
                e >= T_QUEUE
                    ? Math.floor((e - T_QUEUE) * (IN_HIGH - CAP_APP) * 0.001)
                    : 0;
            ...
            STAGES.forEach((stage, i) => {
                ...
                ctx.fillText(
                    `용량 ${stage.cap.toLocaleString()} req/s`,
                    x + bw / 2,
                    capY,
                );
            ...
            // --- 하단 단계 설명 ---
            let caption =
                "① 트래픽이 가장 좁은 단(앱 300)보다 작으면|모든 게 평화롭다";
            if (e >= T_DIAG) {
                caption =
                    "③ 병목 앞은 줄을 서고 병목 뒤는 한가하다|— 어디가 좁은지는 큐가 알려준다";
            } else if (e >= T_SURGE) {
                caption =
                    "② 트래픽 600인데 처리량은 300|— 초과분은 전부 앱 앞의 줄이 된다";
            }
