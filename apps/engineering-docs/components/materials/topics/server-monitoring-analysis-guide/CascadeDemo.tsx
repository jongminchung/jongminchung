...
const DOTS: Dot[] = (() => {
    ...
    for (let t = 200; t < DIE[2]!; t += 300) {
        ...
        dots.push({
            t0: t,
            srv: alive[k % alive.length]!,
            leg1,
            leg2: t + leg1 >= T_SLOW ? 1800 : 600,
        });
        k += 1;
    }
    return dots;
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            for (let n = 3; n >= 3 - deadCount; n -= 1) {
                ...
                chain.push([
                    `${n}`,
                    current ? (n === 3 ? "#2f9e44" : "#fa5252") : "#adb5bd",
                    current,
                ]);
            }
            ...
            ctx.fillText(
                "정상 서버",
                lbX + lbW / 2,
                lbY + lbH + Math.max(11, 13 * s),
            );
            drawSegs(
                ctx,
                chain,
                lbX + lbW / 2,
                lbY + lbH + Math.max(24, 28 * s),
                smallFs,
            );
            ...
            if (e >= T_VERDICT) {
                ...
                caption =
                    "③ 빠진 서버의 몫이 남은 서버를 더 빨리 쓰러뜨린다 — 도미노";
            } else if (e >= T_SLOW) {
                caption =
                    "② DB가 느려지자 스레드가 마르고, 서버 1이 헬스체크에 응답하지 못한다";
            }
