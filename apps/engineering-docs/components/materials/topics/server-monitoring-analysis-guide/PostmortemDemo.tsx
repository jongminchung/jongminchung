...
const STEPS: Step[] = [
    {
        caption: "① 복구가 끝났다 — 이제 그래프를 되감아 타임라인을 재구성한다",
        color: "#868e96",
    },
    {
        caption: "② 네 개의 시각을 찍는다 — 첫 흔적, 알람, 대응 시작, 복구",
        color: "#495057",
    },
    {
        caption:
...
export const PostmortemDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            const toY = (v: number) =>
                trackY + trackH * (1 - clamp(v, 0, Y_MAX) / Y_MAX);
            ...
            const traceCurve = () => {
                ...
                ctx.lineTo(
                    plotX + endPx,
                    toY(p99At((endPx / plotW) * TOTAL_MIN)),
                );
            };
            ...
            if (p1 < 1) {
                ...
                const nearRight =
                    cx > plotX + plotW - ctx.measureText(dotLabel).width - 12;
            ...
            const subA =
                clamp01((e - MARKERS[0]!.at - 150) / 400) *
                (1 - clamp01((e - 6800) / 400));
            if (subA > 0.01) {
            ...
            // --- 하단 캡션: 단계별 판독 ---
            const stepStart =
                stepIdx === 0 ? 0 : stepIdx === 1 ? P1_END : P2_END;
