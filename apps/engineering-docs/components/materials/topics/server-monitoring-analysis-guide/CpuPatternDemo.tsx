...
function cpuAt(scenario: number, t: number): number {
    if (scenario === 0) {
        ...
        return clamp(
            50 + 11 * wave(t, 1.6, 0.5) + 4 * wave(t, 4.3, 1.7),
            35,
            65,
        );
    }
    if (scenario === 1) {
        // 20~30%에서 낮고 평평하다
        return clamp(
            25 + 3 * wave(t, 2.8, 0.3) + 1.5 * wave(t, 6.5, 1.1),
            20,
            30,
        );
    }
...
function p99At(scenario: number, t: number, cpu: number): number {
    ...
    if (scenario === 1) {
        ...
        return clamp(
            80 + 820 * jump + 45 * wave(t, 7, 0.4) * jump,
            50,
            P99_MAX,
        );
    }
...
function wrapReading(
    ...
    const chunks =
        sep >= 0 ? [text.slice(0, sep), `— ${text.slice(sep + 3)}`] : [text];
