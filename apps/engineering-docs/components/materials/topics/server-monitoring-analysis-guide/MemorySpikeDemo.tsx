...
function heapAt(x: number) {
    ...
    if (x < XS1_END)
        return BASE + (V1 - BASE) * easeOut((x - XS1) / (XS1_END - XS1));
    if (x < XS2) return V1 + 0.008 * Math.sin((x - XS1_END) * 180);
    if (x < XS2_END)
        return V1 + (V2 - V1) * easeOut((x - XS2) / (XS2_END - XS2));
    return (
        V2 + ((x - XS2_END) / (1 - XS2_END)) * 0.04 + 0.006 * Math.sin(x * 220)
    );
}
...
const LOGS = [
    ...
    {
        t: XS1,
        text: "GET /api/orders/export?range=all 200 · 8,412ms",
        culprit: true,
    },
...
export const MemorySpikeDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            ctx.font = `700 ${sfs}px ${FONT}`;
            const nearRight =
                cx > plotX + plotW - ctx.measureText(vLabel).width - 12;
            ...
            LOGS.forEach((log, i) => {
                ...
                const body =
                    isMobile && log.culprit
                        ? "GET /api/orders/export?range=all …"
                        : log.text;
