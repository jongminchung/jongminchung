...
export const DeployDemo = () => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            // --- 하단 단계 설명 ---
            let caption =
                "① 배포 전: 두 시나리오 모두 안정 — 배포 마커는 그래프에 반드시 남겨라";
            if (p >= U_ROLLBACK) {
                caption =
                    "④ B는 롤백 후 빠르게 회복한다 — 판단 기준은 수치가 아니라 방향이다";
            } else if (p >= U_DIVERGE) {
                caption =
                    "③ 갈림길: A는 우하향(회복), B는 고착·악화 — 방향이 나쁘면 고민하지 말고 롤백";
            } else if (p >= U_DEPLOY) {
                caption =
            ...
            ctx.textBaseline = "alphabetic";
            if (
                ctx.measureText(caption).width > w - 8 &&
                caption.includes(" — ")
            ) {
