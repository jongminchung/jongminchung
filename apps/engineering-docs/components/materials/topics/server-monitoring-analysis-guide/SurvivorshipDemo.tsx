    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            const drawDot = (
                ...
                const nearRight =
                    cursorX > plotX + plotW - ctx.measureText(label).width - 12;
            ...
            const errLabel =
                errNow < 1 ? `${errNow.toFixed(1)}%` : `${Math.round(errNow)}%`;
            ...
            for (let i = 0; i < DOT_N; i++) {
                ...
                let dy = baseY;
                let alpha =
                    clamp(ph / 0.05, 0, 1) * clamp((0.98 - ph) / 0.05, 0, 1);
            ...
            ctx.textAlign = "center";
            ctx.textBaseline = "alphabetic";
            wrapLines(ctx, caption, w - pad * 2, capLines).forEach(
                (line, k) => {
