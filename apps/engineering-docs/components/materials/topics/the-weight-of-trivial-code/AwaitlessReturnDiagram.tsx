...
function drawPanel(args: PanelArgs): number {
    ...
    ctx.fillText(
        isCorrect ? "return await db.findUser(id)" : "return db.findUser(id)",
        x,
        cy,
    );
    cy += titleFs + 4;
    ...
    if (phase > 0.15) {
        ...
        // 실수: caller까지 (i=0의 중앙)
        const endY = isCorrect
            ? stackItems[1]!.y + boxH / 2
            : stackItems[0]!.y + boxH / 2;
