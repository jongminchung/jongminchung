...
function drawScene(args: SceneArgs): number {
  ...
  for (let i = 0; i < ROWS.length; i++) {
    ...
    drawRoundRect(ctx, rightX, resultBoxY, valueBoxW, resultBoxH, 5, resultBg, resultColor, 1.2);
    ctx.fillStyle = resultColor;
    ctx.font = `700 ${rowFs}px ${MONO}`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(resultText, rightX + valueBoxW / 2, resultBoxY + resultBoxH / 2);
    ...
    if (t > T_COMPARE) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(badgeText, badgeX + badgeW / 2, resultBoxY + resultBoxH / 2);
  ...
  if (t > T_HIGHLIGHT) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "top";
    ctx.fillText('"admin이 아닌" 행을 찾았는데 NULL 행이 빠졌다', w / 2, cy + msgPadV);
