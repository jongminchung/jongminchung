...
const STRINGS = {
  ko: {
    ...
    trackDetails: "상세",
    spring: "스프링",
    caption: "카드를 클릭하세요 — 세 속성이 각자의 곡선으로 독립 변화한다",
  },
  en: {
    planName: "Pro plan",
    planPrice: "$29/mo",
    ...
    spring: "Spring",
    caption: "Click the card — three properties change independently, each on its own curve",
  },
} as const;

export const PropertySplitDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
  ...
  const animate = useCallback(() => {
    ...
    // 크기: 스프링
    {
      const force = -STIFFNESS * (s.scale.current - s.scale.target) - DAMPING * s.scale.velocity;
    ...
    // 테두리: 150ms ease-out
    {
      ...
      s.border.current = s.border.from + (s.border.to - s.border.from) * easeOut(t);
    ...
    // 상세 내용: 400ms ease-out
    {
      ...
      s.content.current = s.content.from + (s.content.to - s.content.from) * easeOut(t);
      if (t < 1) settled = false;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#868e96" }}>{t.planPrice}</div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        >
          ...
          <span style={{ fontSize: 10, color: "#adb5bd", width: 100, flexShrink: 0 }}>
            150ms, ease-out
          </span>
        </div>
        <div
