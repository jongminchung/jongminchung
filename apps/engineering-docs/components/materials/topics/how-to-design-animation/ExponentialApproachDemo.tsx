...
export const ExponentialApproachDemo = ({
    ...
    useEffect(() => {
        ...
        container.addEventListener("touchmove", handlePointer, {
            passive: true,
        });

        const animate = () => {
            posRef.current.x +=
                (mouseRef.current.x - posRef.current.x) * factor;
            posRef.current.y +=
                (mouseRef.current.y - posRef.current.y) * factor;

            if (followerRef.current) {
                followerRef.current.style.transform = `translate(${posRef.current.x - 14}px, ${posRef.current.y - 14}px)`;
            }
            if (targetRef.current) {
                targetRef.current.style.transform = `translate(${mouseRef.current.x - 10}px, ${mouseRef.current.y - 10}px)`;
            }
