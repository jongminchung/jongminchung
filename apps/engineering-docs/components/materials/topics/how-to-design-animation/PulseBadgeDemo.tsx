...
const STRINGS = {
  ko: { caption: "톱니파 — 값이 0→1로 단방향 진행 후 리셋을 반복한다" },
  en: { caption: "Sawtooth wave — the value runs one way from 0 to 1, then resets" },
} as const;

export const PulseBadgeDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
  const t = STRINGS[lang];
  const ringRef = useRef<HTMLDivElement>(null);
  const graphCanvasRef = useRef<SvgCanvasHandle>(null);
