...
const STRINGS = {
  ko: {
    ...
    naive: "고려하지 않은 경우",
    smart: "고려한 경우",
    caption: "나타나는 도중에 빠르게 토글해보세요",
  },
  en: {
    hide: "Hide",
    show: "Show",
    naive: "Without handling",
    smart: "With handling",
    caption: "Try toggling quickly mid-animation",
  },
} as const;
...
const NaivePanel = ({ show }: { show: boolean }) => {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      if (t < 1) animRef.current = scheduleMaterialFrame(animate);
    };
...
export const BidirectionalCompareDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
