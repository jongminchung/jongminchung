...
const STRINGS = {
  ko: {
    ballCaption: "← 이 공의 위치가 커브를 따라 변합니다 →",
    playing: "재생 중...",
    play: "▶ 재생",
  },
  en: {
    ballCaption: "← The ball moves along the curve →",
    playing: "Playing...",
    play: "▶ Play",
  },
} as const;

export const EasingVisualizer = ({ locale: lang = "ko" }: { locale?: Lang } = {}) => {
  ...
  const drawCurve = useCallback(
    (ctx: SvgDrawingContext, width: number, height: number, currentProgress: number) => {
      const padding = 40;
