  ...
  return (t: number): number => {
    ...
    for (let i = 0; i < 8; i++) {
      const currentX =
        ...
        3 * p1x * guessT;
      const slope =
        (3 * (1 - 3 * p2x + 3 * p1x) * guessT + 2 * (3 * p2x - 6 * p1x)) * guessT + 3 * p1x;
      if (Math.abs(slope) < 1e-6) break;
      guessT -= (currentX - t) / slope;
    }
    return (
      ((1 - 3 * p2y + 3 * p1y) * guessT + (3 * p2y - 6 * p1y)) * guessT * guessT + 3 * p1y * guessT
    );
  };
}
...
const STRINGS = {
  ko: {
    toast: "저장되었습니다",
    caption: "같은 토스트가 이징에 따라 어떻게 다른지 비교해보세요",
  },
  en: {
    toast: "Saved",
    caption: "Compare how the same toast feels with each easing",
  },
} as const;

export const EasingPracticeDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
  ...
  useEffect(() => {
    const easingFns = EASINGS.map((e) => cubicBezier(e.p1[0], e.p1[1], e.p2[0], e.p2[1]));
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: "#adb5bd", textAlign: "center" }}>{t.caption}</div>
    </div>
  );
};
