...
const STATE_COLORS = ["#228be6", "#fab005", "#fa5252", "#868e96"];
...
const STRINGS = {
  ko: {
    states: ["발사", "폭발", "확산", "소멸"],
    transitions: ["속도 = 0", "즉시", "시간 경과"],
  },
  en: {
    states: ["Launch", "Explode", "Spread", "Fade"],
    transitions: ["velocity = 0", "instantly", "time elapsed"],
  },
} as const;

export const FireworkStateDiagram = ({ locale: lang = "ko" }: { locale?: Lang } = {}) => {
  const canvasRef = useRef<SvgCanvasHandle>(null);

  useEffect(() => {
    const t = STRINGS[lang];
    const STATES = t.states.map((label, i) => ({ label, color: STATE_COLORS[i] }));
