...
const STRINGS = {
    ko: {
        notifTitle: "새 메시지",
        notifBody: "안녕하세요! 확인해주세요.",
        stageBg: "배경",
        stageCard: "카드",
        stageContent: "내용",
        caption: "세 조각이 시간축 위에 순서대로 배치된다",
    },
    en: {
        notifTitle: "New message",
        notifBody: "Hi! Please check this out.",
        stageBg: "Background",
        stageCard: "Card",
        stageContent: "Content",
        caption: "Three pieces placed in order along the time axis",
    },
} as const;

export const PipelineDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
    ...
    useEffect(() => {
        ...
        const animate = (now: number) => {
            ...
            const inExit =
                elapsed >= ENTER + HOLD && elapsed < ENTER + HOLD + EXIT;
            ...
            if (inEnter) {
                ...
                if (stage1Ref.current)
                    stage1Ref.current.style.opacity =
                        active >= 1 ? "1" : "0.25";
                if (stage2Ref.current)
                    stage2Ref.current.style.opacity =
                        active >= 2 ? "1" : "0.25";
                if (stage3Ref.current)
                    stage3Ref.current.style.opacity =
                        active >= 3 ? "1" : "0.25";
            } else {
                if (indicatorRef.current)
