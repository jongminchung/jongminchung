...
const STRINGS = {
    ko: {
        label: "총 방문자 수",
        caption: "목표값을 바꿔보세요 — 숫자가 부드럽게 새 목표를 쫓아간다",
    },
    en: {
        label: "Total visitors",
        caption: "Change the target — the number smoothly chases the new goal",
    },
} as const;

export const CounterDemo = ({ locale: lang = "ko" }: { locale?: Lang }) => {
    ...
    useEffect(() => {
        const animate = () => {
            ...
            if (displayRef.current) {
                displayRef.current.textContent = Math.round(
                    currentRef.current,
                ).toLocaleString();
            }
