...
function useSelectContext(part: string) {
    ...
    if (!context) {
        throw new Error(
            `<Select.${part}>는 <Select.Root> 안에서만 사용할 수 있다`,
        );
    }
    return context;
}
...
export interface SelectListboxProps extends React.SelectHTMLAttributes<HTMLSelectElement> {}

const Listbox = ({ children, ...props }: SelectListboxProps) => {
    const { open, listboxId, options, select, value } =
        useSelectContext("Listbox");
    return (
        <select
            id={listboxId}
            hidden={!open}
            size={Math.max(1, options.length)}
            value={value ?? ""}
            data-state={open ? "open" : "closed"}
            {...props}
