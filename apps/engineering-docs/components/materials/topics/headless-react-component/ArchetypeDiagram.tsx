...
function drawArrow(
    ...
    ctx.moveTo(x1, y1);
    ctx.lineTo(
        x2 - Math.cos(angle) * (head - 2),
        y2 - Math.sin(angle) * (head - 2),
    );
    ctx.stroke();
...
export const ArchetypeDiagram = ({ caption }: Props) => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const chipsRowW =
                chipWs.reduce((a, b) => a + b, 0) +
                chipGap * (CHIPS.length - 1);
            const chipsHorizontal =
                !isMobile && chipsRowW <= archW - boxPadH * 2;

            const chipsBlockH = chipsHorizontal
                ? chipH
                : CHIPS.length * chipH + (CHIPS.length - 1) * 6;
            const archH =
                boxPadV + titleFs + titleChipGap + chipsBlockH + boxPadV;
            const archTop = pad;
            ...
            if (isMobile) {
                ...
                totalH =
                    svcTop +
                    SERVICES.length * svcH +
                    (SERVICES.length - 1) * mobileSvcGap +
                    pad;
            } else {
