...
type SampleItem = (typeof sampleData)[number];
type SampleKey = keyof SampleItem;
type SampleValue = SampleItem[SampleKey];

interface TableParams {
    readonly page: number;
    readonly pageSize: number;
    readonly sortBy: SampleKey | "";
    readonly sortDirection: "ASC" | "DESC";
    readonly filter: SampleKey | "";
    readonly filterValue: string;
}

const sampleKeys: ReadonlySet<string> = new Set([
    "id",
    "name",
    "email",
    "joinDate",
...
function isSampleKey(value: string): value is SampleKey {
    return sampleKeys.has(value);
}

// 예제 API 호출 함수
const fetchTableData = async (
    params: TableParams,
): Promise<TableViewModel<SampleValue>> => {
    // 실제로는 API 호출
...
export function TableExample() {
    ...
    const [viewModel, setViewModel] =
        useState<TableViewModel<SampleValue> | null>(null);
    ...
    const handleSortChange = (sortInfo: SortInfo) => {
        if (!isSampleKey(sortInfo.columnId)) return;
        setParams({
            ...params,
    ...
    // 행 클릭 처리
    const handleRowClick = (row: TableRow<SampleValue>) => {
        // 행 클릭 시 ID 찾기
        const idCell = row.cells.find((cell) => cell.columnId === "id");
        if (idCell) {
    ...
    const handleFilterChange = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const filterColumnValue = formData.get("filterColumn");
        const filterColumn =
            typeof filterColumnValue === "string" &&
            isSampleKey(filterColumnValue)
                ? filterColumnValue
                : "";
        const filterValueEntry = formData.get("filterValue");
        const filterValue =
            typeof filterValueEntry === "string" ? filterValueEntry : "";
