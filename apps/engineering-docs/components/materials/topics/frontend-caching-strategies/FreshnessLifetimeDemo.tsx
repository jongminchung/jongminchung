...
export const FreshnessLifetimeDemo = () => {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      ctx.fillRect(toX(SWR_END), barY, toX(TIMELINE_END) - toX(SWR_END), barH);
      ctx.restore();
      ...
      ctx.fillText(isMobile ? "SWR 창" : "낡음 — SWR 창", (toX(MAX_AGE) + toX(SWR_END)) / 2, midY);
      ...
      ctx.textAlign = "right";
      ctx.textBaseline = "top";
      const speedLabel = isFF ? `${SPEED_FF}배속 ≫` : `${SPEED_NORMAL}배속`;
      ...
      if (age === null) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("캐시 없음 — 첫 요청이 들어온다", w / 2, topH / 2 + 4);
      } else {
        ...
        const label = isFF ? `나이 ${Math.floor(age)}초 ≫≫` : `나이 ${Math.floor(age)}초`;
        const lw = ctx.measureText(label).width;
        const lx = Math.min(Math.max(px, pad + lw / 2), w - pad - speedW - 10 - lw / 2);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: "#228be6", fontWeight: 600 }}>max-age=60</span>,{" "}
        <span style={{ color: "#e8590c", fontWeight: 600 }}>stale-while-revalidate=300</span>
      </div>
      ...
      <div style={{ marginTop: 12, minHeight: 4 * 29 }}>
        {logs.length === 0 ? (
          <div style={{ fontSize: 12, color: "#adb5bd", padding: "6px 8px" }}>
            아직 요청이 없다…
          </div>
        ) : (
          logs.map((log) => (
            ...
            >
              ...
              <span style={{ color: log.sub ? "#868e96" : "#495057", flex: "1 1 auto" }}>
                {log.text}
              </span>
              <span style={{ flexShrink: 0, color: "#adb5bd", fontSize: 11 }}>{log.ageText}</span>
            </div>
          ))
        )}
      </div>
