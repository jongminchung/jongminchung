...
function stepInfoAt(index: number): (typeof STEP_INFO)[number] {
    ...
    if (info === undefined)
        throw new Error(`Missing stale-time step ${index}.`);
    return info;
}

// 스텝 시작 시점에 카운터가 오른 뒤의 값
function countersAt(step: number) {
    const firstVisits = STEP_INFO.slice(0, step + 1).filter(
        (s) => s.firstVisit,
    ).length;
    return {
                ...
                >
                    ...
                    >
                        {showBadge && (
                            <motion.div
                                animate={{
                                    opacity: reducedMotion ? 1 : [1, 0.35, 1],
                                }}
                                transition={{
                                    duration: 0.5,
                                    ease: "easeInOut",
