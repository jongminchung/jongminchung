...
export const StampedeDemo = () => {
  ...
  useEffect(() => {
    ...
    const makeLane = (mode: Mode, label: string, counterColor: string): Lane => ({
      mode,
      label,
      counterColor,
      dots: [],
        ...
        if (d.phase === "in" && d.t >= IN_DUR) {
          ...
          if (lane.cacheState === "fresh") {
            ...
            if (lane.cacheState === "expired") {
              ...
              d.phase = "wait";
              d.t = 0;
              d.qi = lane.dots.filter((o) => o.phase === "wait").length;
              d.color = "#adb5bd";
            }
          } else {
            ...
            if (lane.cacheState === "expired") {
              lane.cacheState = "fetching";
              lane.cacheT = 0;
              lane.dots.push({ phase: "toOrigin", t: 0, jy: 0, color: "#845ef7", ghost: true });
            }
            d.phase = "out";
            d.t = 0;
    ...
    const render = (now: number) => {
      ...
      lanes.forEach((lane, li) => {
        ...
        const numFs = Math.max(15, 19 * sc) * pulseScale * (highlight ? 1.12 : 1);
        ...
        if (highlight) {
          ...
          ctx.beginPath();
          ctx.roundRect(originR - pillW, headerY - pillH / 2, pillW, pillH, pillH / 2);
          ctx.fillStyle = lane.counterColor === "#fa5252" ? "#ffe3e3" : "#d3f9d8";
          ctx.fill();
        }
        ctx.textAlign = "right";
        ctx.font = `800 ${numFs}px ${FONT}`;
        ...
        ctx.fillText(labStr, originR - numW - 3 - (highlight ? 6 : 0), headerY);
        ctx.textAlign = "left";
        ...
        ctx.fillText("요청", spawnX, boxTop + bh + Math.max(9, 11 * sc));

        // --- 캐시 박스 ---
        const staleDuringFetch = lane.mode === "swr" && lane.cacheState !== "fresh";
        let cacheFill = "#fff";
        ...
        if (!mobile) {
          ...
          if (lane.cacheState === "fresh") {
            ...
            ctx.fillText("낡음(stale)", cacheCX, boxTop + bh * 0.55);
          } else {
        ...
        if (lane.cacheState === "fresh") {
          frac =
            lane.cacheT < GAUGE_FILL
              ...
              : 1 - (lane.cacheT - GAUGE_FILL) / (TTL - GAUGE_FILL);
        } else if (staleDuringFetch) {
        ...
        if (frac > 0.005) {
          ...
          ctx.fillStyle = staleDuringFetch ? "#fab005" : frac < 0.25 ? "#fab005" : "#40c057";
          ctx.fill();
        }
        ...
        ctx.strokeStyle = lane.originFlash > 0.05 ? "#fa5252" : "#adb5bd";
        ctx.lineWidth = lane.originFlash > 0.05 ? 2 : 1.5;
        ctx.stroke();

        ctx.font = `700 ${fs}px ${FONT}`;
        ctx.fillStyle = "#495057";
        ctx.fillText("원본", (originL + originR) / 2, boxTop + bh * 0.36);
        // 서버 느낌의 가로줄
        ...
        for (const d of lane.dots) {
          ...
          if (d.phase === "in") {
            ...
            const p = easeInOut(d.t / ORIGIN_DUR);
            x = originL - dotR * 2 - (originL - dotR * 2 - cacheR - dotR) * p;
            y = laneY + d.jy * jAmp * 0.6;
          } else if (d.phase === "wait") {
            const qi = d.qi ?? 0;
            const col = qi % 3;
            const row = Math.floor(qi / 3);
            x = cacheL - Math.max(12, 16 * sc) - col * (dotR * 2 + 4);
