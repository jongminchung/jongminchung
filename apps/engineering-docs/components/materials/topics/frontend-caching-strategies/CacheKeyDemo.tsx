  ...
  for (let i = 0; i < REQUEST_COUNT; i++) {
    ...
    if (rand() < 0.45) {
      if (rand() < 0.5) {
        params.push({
          key: "utm_source",
          value: UTM_SOURCES[Math.floor(rand() * UTM_SOURCES.length)],
          tracking: true,
        });
      } else {
        let token = "";
        while (token.length < 10) token += Math.floor(rand() * 36 ** 6).toString(36);
        params.push({ key: "fbclid", value: token.slice(0, 10), tracking: true });
      }
    }
    // 파라미터 순서 셔플
    ...
    const rawKey = "/products?" + params.map((p) => `${p.key}=${p.value}`).join("&");
    const kept = params
      .filter((p) => !p.tracking)
      .slice()
      .sort((a, b) => a.key.localeCompare(b.key));
    const normKey = "/products?" + kept.map((p) => `${p.key}=${p.value}`).join("&");
    const label = `c${category}·${sort}`;
    const tracked = params.some((p) => p.tracking);

    const rawHit = rawCache.has(rawKey);
    if (rawHit) rawHits += 1;
    else rawCache.set(rawKey, { label: tracked ? `${label}+추적` : label, tracked });
...
const Panel = ({
  ...
  const rateColor = goodRate ? "#2f9e44" : entries.length > 6 ? "#fa5252" : "#495057";
  return (
    <div
      style={{
    ...
    >
      ...
      >
        <span style={{ fontSize: narrow ? 12 : 13, fontWeight: 700, color: "#495057" }}>
          {title}
        </span>
        {hit !== null && <span style={badgeStyle(hit)}>{hit ? "HIT" : "MISS"}</span>}
      </div>

      <div
        style={{
          background: final ? (goodRate ? "#e7f5ff" : "#fff5f5") : "#f8f9fa",
      ...
      >
        ...
        <span style={{ fontSize: final ? 15 : 12, color: "#adb5bd", margin: "0 6px" }}>·</span>
        <span
          style={{
            fontSize: final ? (narrow ? 20 : 24) : narrow ? 14 : 15,
            fontWeight: 700,
      ...
      >
        {entries.length === 0 ? (
          <span style={{ fontSize: 11, color: "#adb5bd" }}>아직 캐시된 항목이 없다</span>
        ) : (
          <>
            {shown.map((e, i) => (
              <span
                ...
                style={{
                  ...
                  background: e.tracked ? "#f8f9fa" : "#e7f5ff",
                  border: e.tracked ? "1px solid #dee2e6" : "1px solid #a5d8ff",
                }}
              >
                {e.label}
              </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={{ fontSize: 11, color: "#868e96" }}>들어오는 요청</span>
          <span style={{ fontSize: 11, color: "#adb5bd", fontVariantNumeric: "tabular-nums" }}>
              ...
              {frame.params.map((p, i) => (
                <React.Fragment key={p.key}>
                  ...
                  <span
                    style={{
                      color: "#495057",
                      background: p.tracking ? "#fff9db" : "transparent",
                      borderRadius: 3,
                  ...
                  <React.Fragment key={p.key}>
                    {i > 0 && (
                      <span
                        style={{
                          color: "#adb5bd",
                          textDecoration: p.tracking ? "line-through" : "none",
                    ...
                    <span
                      style={{
                        textDecoration: p.tracking ? "line-through" : "none",
                        opacity: p.tracking ? 0.35 : 1,
                        color: p.tracking ? "#868e96" : "#1971c2",
                      }}
                    >
                      {p.key}={p.value}
                    </span>
      ...
      <div style={{ fontSize: 11, color: "#adb5bd", textAlign: "center" }}>
        실제로 서로 다른 페이지는 6개뿐이다 — 왼쪽의 나머지 항목은 전부 파편이다
      </div>
    </div>
  );
};
