...
const DATA = [
  { name: "맨몸", desc: "(스킬 없이 과제만)", value: 0, color: COLORS.none, best: false },
  { name: "산문", desc: "(한 문단 지시)", value: 24, color: COLORS.prose, best: false },
  { name: "스킬 v1", desc: "", value: 24, color: COLORS.skill, best: false },
  { name: "스킬 v2", desc: "(사다리 3번 명확화)", value: 25, color: COLORS.skill, best: true },
];
...
function drawHorizontal(ctx: SvgDrawingContext, w: number): number {
  ...
  DATA.forEach((d) => {
    ...
    if (d.value === 0) {
      ...
      drawValueLabel(ctx, trackX + trackW + 8 * s, cy, d, valueFs, smallFs);
    }
