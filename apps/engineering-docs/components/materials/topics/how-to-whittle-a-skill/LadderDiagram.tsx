...
const ROWS: Array<{ question: string; type: string; kind: RowKind }> = [
    {
        question: "사용자가 겪던 잘못된 동작이 사라지는가?",
        type: "fix",
        kind: "decided",
    },
    {
        question: "사용자에게 보이는 동작이 새로 생기는가?",
        type: "feat",
        kind: "normal",
    },
    {
        question: "측정 가능한 성능 개선이 목적인가?",
        type: "perf",
        kind: "normal",
    },
    {
        question: "동작은 같고 코드 구조만 바뀌는가?",
        type: "refactor",
        kind: "shadowed",
    },
...
function draw(ctx: SvgDrawingContext, w: number): number {
    ...
    const tagY = y + boxPadY + titleLines.length * lineH + 6 * s + tagH / 2;
    if (stackTags) {
        tags.forEach((tag, i) => {
    ...
    for (let i = 0; i < ROWS.length; i++) {
        ...
        if (row.kind === "decided") {
            extraW =
                measurePill(ctx, "여기서 결정!", smallFs, s, false) + 6 * s;
        ...
        const availQ = Math.max(
            rowW - rowPadX * 2 - badgeW - extraW - 8 * s,
            60 * s,
        );
