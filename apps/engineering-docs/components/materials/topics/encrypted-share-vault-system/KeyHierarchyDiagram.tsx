...
function draw(ctx: SvgDrawingContext, w: number): number {
  ...
  drawRoundRect(ctx, mpX, y, pairBoxW, boxH, 6 * s, COLORS.passwordBg, COLORS.password);
  ...
  drawRoundRect(ctx, skX, y, pairBoxW, boxH, 6 * s, COLORS.secretKeyBg, COLORS.secretKey);
  ...
  // ── PBKDF2 ──
  drawRoundRect(ctx, cx - kdfW / 2, kdfY, kdfW, procH, 4 * s, COLORS.kdfBg, COLORS.kdf);
  ...
  // ── AUK ──
  drawRoundRect(ctx, cx - boxW / 2, aukY, boxW, boxH, 6 * s, COLORS.aukBg, COLORS.auk);
  ...
  // ── 암호화 ──
  drawRoundRect(ctx, cx - boxW / 2, enc1Y, boxW, procH, 4 * s, COLORS.encryptBg, COLORS.encrypt);
  ...
  // ── Key Pair ──
  drawRoundRect(ctx, cx - boxW / 2, kpY, boxW, boxH, 6 * s, COLORS.keyPairBg, COLORS.keyPair);
  ...
  ctx.fillStyle = COLORS.textLight;
  ctx.font = `${smallFs}px ${FONT}`;
  ctx.fillText("공개 키: 서버 저장 / 개인 키: AUK로 암호화", cx, kpY + boxH + descGap);
  ...
  // ── 캡슐화 ──
  drawRoundRect(ctx, cx - boxW / 2, enc2Y, boxW, procH, 4 * s, COLORS.encryptBg, COLORS.encrypt);
  ctx.fillStyle = COLORS.encrypt;
  ctx.font = `${labelFs}px ${FONT}`;
  ctx.textAlign = "center";
  ...
  // ── Vault Key ──
  drawRoundRect(ctx, cx - boxW / 2, vkY, boxW, boxH, 6 * s, COLORS.vaultKeyBg, COLORS.vaultKey);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    drawRoundRect(ctx, ix, itemY, itemW, boxH, 6 * s, COLORS.itemBg, COLORS.item);
    ctx.fillStyle = COLORS.item;
    ctx.font = `600 ${labelFs}px ${FONT}`;
    ctx.textAlign = "center";
    ctx.fillText(["비밀 #1", "비밀 #2", "비밀 #3"][i], ix + itemW / 2, itemY + boxH / 2);
  }
