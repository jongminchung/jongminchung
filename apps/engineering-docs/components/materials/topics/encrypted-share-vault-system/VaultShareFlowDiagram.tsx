...
function draw(ctx: SvgDrawingContext, w: number): number {
  ...
  drawRoundRect(ctx, bobCx - actorW / 2, actorY, actorW, actorH, 6 * s, COLORS.bobBg, COLORS.bob);
  ...
  ctx.font = `${smallFs}px ${FONT}`;
  ctx.textAlign = "left";
  ctx.fillText("금고 생성 + Vault Key 생성", aliceCx + loopW + 8 * s, y + 5 * s);
  ...
  ctx.font = `${smallFs}px ${FONT}`;
  ctx.textAlign = "center";
  ctx.fillText("E(Bob PK, VK) 저장 (공유)", (aliceCx + serverCx) / 2, y - 10 * s);
  ...
  ctx.font = `${smallFs}px ${FONT}`;
  ctx.textAlign = "right";
  ctx.fillText("D(Bob SK, 암호문) → VK 획득", bobCx - loopW - 8 * s, y + 5 * s);
