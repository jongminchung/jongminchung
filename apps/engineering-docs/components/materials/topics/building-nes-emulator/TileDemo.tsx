...
const TileDemo = () => {
    ...
    const pixels = useMemo(
        () => (tileBytes ? decode_tile(tileBytes) : null),
        [tileBytes],
    );
    ...
    const bitRows = (plane: 0 | 1) =>
        Array.from({ length: 8 }, (_, row) => {
            ...
            return (
                <div
                    key={row}
                    style={{ display: "flex", gap: 1, lineHeight: "16px" }}
                >
        ...
        >
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                ...
                <div style={{ flex: "1 1 300px", minWidth: 280 }}>
                    ...
                    />
                    <div
                        style={{ fontSize: 11, color: "#adb5bd", marginTop: 6 }}
                    >
                        타일 데이터에는 색이 없다 — 0~3 번호뿐이고, 위 팔레트를
                        바꾸면 같은 타일이 다른 색으로 칠해진다. 색 0번은
                        투명(배경색)이라 체커보드로 표시했다.
                    </div>
                </div>
            </div>
        </div>
