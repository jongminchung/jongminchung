...
const PROGRAM = [
    0xa9, 0x50, 0x69, 0x50, 0x85, 0x00, 0xa2, 0x03, 0xe8, 0x8d, 0x00, 0x02,
    0xca, 0xd0, 0xfa, 0x00,
];
...
const CpuStepDemo = () => {
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            while (addr < LOAD_ADDR + PROGRAM.length) {
                ...
                if (sizeStr === undefined || text === undefined) {
                    throw new Error(
                        `Unexpected disassembly output at ${addr.toString(16)}`,
                    );
                }
    ...
    const regRow = (
        ...
        >
            <span style={{ color: "#868e96", fontSize: 12, fontWeight: 600 }}>
                {label}
            </span>
            <span
                data-testid={testid}
                style={{ fontFamily: MONO, fontSize: 13, color: "#212529" }}
            >
                {value}
            </span>
        </div>
