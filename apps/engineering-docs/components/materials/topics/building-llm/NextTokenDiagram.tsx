...
function drawScene(ctx: SvgDrawingContext, w: number): number {
  ...
  ctx.fillText("다음 글자 후보", candidateStartX + candidateW / 2, candidateStartY - 14 * s);
  ...
  for (let i = 0; i < CANDIDATES.length; i++) {
    ...
    if (barW > 0) {
      ...
      ctx.quadraticCurveTo(barX + barW, cy - barH / 2, barX + barW, cy - barH / 2 + br);
      ctx.lineTo(barX + barW, cy + barH / 2 - br);
      ctx.quadraticCurveTo(barX + barW, cy + barH / 2, barX + barW - br, cy + barH / 2);
    ...
    if (prob === 0) {
      ...
      ctx.fillText(`${(prob * 100).toFixed(0)}%`, barX + barW + 6 * s, cy);
    }
  }
