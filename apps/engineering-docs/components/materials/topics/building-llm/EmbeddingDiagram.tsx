...
function drawScene(ctx: SvgDrawingContext, w: number): number {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < vec.length; j++) {
      ...
      ctx.fillStyle = val >= 0 ? "rgba(34,139,230,0.6)" : "rgba(250,82,82,0.6)";
      ctx.beginPath();
      ...
      if (8 * s >= 7) {
        ctx.fillText(val.toFixed(1), bx + barW / 2, y + boxSize + 3 * s);
      }
    }
  }
  ...
  /* ══════════════════════════════════════
     ...
     ══════════════════════════════════════ */
  const plotPad = 16 * s;
  const plotSize = Math.min(halfW - plotPad * 2, contentBottom - topY - 10 * s);
  ...
  /* scatter dots */
  const dots = [
    { char: "가", cx: plotX + plotSize * 0.3, cy: plotY + plotSize * 0.32, color: "#228be6" },
    { char: "나", cx: plotX + plotSize * 0.38, cy: plotY + plotSize * 0.42, color: "#40c057" },
    { char: "다", cx: plotX + plotSize * 0.78, cy: plotY + plotSize * 0.75, color: "#fa5252" },
  ];
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "top";
  ctx.fillText("비슷한 글자는 가까이", plotX + plotSize / 2, plotY + plotSize + 8 * s);
