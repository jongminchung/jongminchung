...
function drawScene(ctx: SvgDrawingContext, w: number): number {
    ...
    for (let i = 0; i < posLabels.length; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
            rCx + innerR * Math.cos(a),
            rotCenterY - innerR * Math.sin(a),
        );
        ctx.lineTo(
            rCx + outerR * Math.cos(a),
            rotCenterY - outerR * Math.sin(a),
        );
        ctx.stroke();
        ...
        ctx.fillText(
            posLabels[i]!,
            rCx + labelR * Math.cos(a),
            rotCenterY - labelR * Math.sin(a),
        );
    }
    ...
    ctx.textBaseline = "top";
    ctx.fillText(
        "위치에 따라 Q·K 벡터를 회전시킨다",
        rCx,
        rotCenterY + rotR + 18 * s,
    );
