...
export const TemperatureSamplingDemo = ({ caption }: { caption?: string }) => {
  ...
  const { tokens, maxProb } = useMemo(() => {
    ...
    const maskedLogits = scaledLogits.map((l) => (l < kthLogit ? -Infinity : l));
    ...
    const maxProb = Math.max(...tokens.filter((t) => t.active).map((t) => t.prob));
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            <label style={{ fontSize: 14, color: "#495057", fontWeight: 600, minWidth: 100 }}>
                ...
                >
                  <div
                    style={{
                      width: `${barWidth}%`,
                      height: "100%",
                      background: active ? "#228be6" : "#adb5bd",
                      borderRadius: 4,
        ...
        >
          Temperature가 낮을수록 확률 분포가 뾰족해지고, 높을수록 균일해집니다. Top-k는 상위 k개의
          토큰만 샘플링 후보로 남깁니다.
        </div>
      </div>
