...
function drawHorizontal(ctx: SvgDrawingContext, w: number, p: number): number {
    const s = w / 640;
    const h = 220 * s;
    ...
    if (resPhase > 0) {
        ...
        const tokenIndex = Math.min(
            Math.floor(tokenProgress * tokens.length),
            tokens.length,
        );
        if (tokenIndex > 0) {
...
function drawVertical(ctx: SvgDrawingContext, w: number, p: number): number {
    const s = w / 360;
...
export const LlmApiDiagram = ({ caption }: Props) => {
    ...
    useEffect(() => {
        ...
        const animate = (time: number) => {
            ...
            const h = isMobile
                ? drawVertical(tempCtx, w, p)
                : drawHorizontal(tempCtx, w, p);
