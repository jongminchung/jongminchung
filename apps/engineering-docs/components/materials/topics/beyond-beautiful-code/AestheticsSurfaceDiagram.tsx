...
function chipWidth(ctx: SvgDrawingContext, text: string, fs: number, padX: number): number {
  ctx.font = `${fs}px ${MONO}`;
...
function drawChip(
  ...
  drawRoundRect(ctx, x, midY - h / 2, w, h, Math.min(5, h / 2), "#fff", "#dee2e6", 1);
  ctx.fillStyle = "#495057";
  ctx.font = `${fs}px ${MONO}`;
  ctx.textAlign = "left";
...
function drawHorizontal(ctx: SvgDrawingContext, w: number): number {
  ...
  drawRoundRect(ctx, bandX, bandY, bandW, bandH, 10 * s, "#e7f5ff", "#228be6", 1.5);
  ...
  for (let i = DESKTOP_CHIPS.length - 1; i >= 0; i--) {
    ...
    drawChip(ctx, chipRight - cw, bandMidY, DESKTOP_CHIPS[i], chipFs, chipPad, chipH);
...
function drawVertical(ctx: SvgDrawingContext, w: number): number {
  ...
  const bandW = w - 28;
  const bandH = 54;
  drawRoundRect(ctx, bandX, bandY, bandW, bandH, 8, "#e7f5ff", "#228be6", 1.5);
  ...
  const totalChipW = widths.reduce((a, b) => a + b, 0) + chipGap * (MOBILE_CHIPS.length - 1);
