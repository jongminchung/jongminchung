...
function pillWidth(ctx: SvgDrawingContext, lines: string[], fs: number, padX: number): number {
  ctx.font = `${fs}px ${FONT}`;
  let textW = 0;
...
function drawHorizontal(ctx: SvgDrawingContext, w: number): number {
  ...
  const pillPadX = 12 * s;
  drawPill(ctx, (top.x + left.x) / 2, (top.y + left.y) / 2, [LABEL_COG_EXEC], pillFs, pillPadX);
  drawPill(ctx, (top.x + right.x) / 2, (top.y + right.y) / 2, [LABEL_COG_STRUCT], pillFs, pillPadX);
  drawPill(
    ctx,
    (left.x + right.x) / 2,
    (left.y + right.y) / 2,
  ...
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("같은 코드", (top.x + left.x + right.x) / 3, (top.y + left.y + right.y) / 3);
