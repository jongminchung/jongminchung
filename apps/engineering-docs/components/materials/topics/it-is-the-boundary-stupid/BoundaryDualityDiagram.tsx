...
function mapXY(px: number, py: number, src: Rect, dst: Rect): [number, number] {
  return [dst.x + ((px - src.x) / src.w) * dst.w, dst.y + ((py - src.y) / src.h) * dst.h];
}

function drawNode(ctx: SvgDrawingContext, x: number, y: number, r: number, color: string) {
  ctx.beginPath();
...
function drawMonolith(ctx: SvgDrawingContext, rect: Rect, nodeR: number) {
  // Background
  ctx.beginPath();
