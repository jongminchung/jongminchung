...
import { scheduleMaterialFrame } from "#components/materials/runtime/scheduler";
...
function tokenize(
    ...
    while (i < line.length) {
        ...
        if (isDigit(ch)) {
            ...
            tokens.push({
                kind: "number",
                value: parseFloat(line.slice(start, i).replace(/,/g, "")),
            });
...
class Parser {
    ...
    parse(): Node {
        ...
        if (this.peek().kind !== "eof")
            throw new Error("수식을 끝까지 이해하지 못했다");
        return node;
    }
    ...
    private parseExpression(minBp: number): Node {
        ...
        while (true) {
            ...
            if (
                ...
                if (unit.kind !== "unit")
                    throw new Error("in 뒤에는 단위가 와야 한다");
    ...
    private parsePrefix(): Node {
        ...
        if (token.kind === "number") {
            ...
            if (unitTok.kind === "unit") {
                ...
                return {
                    type: "num",
                    q: { value: token.value * u.factor, dim: u.dim },
                };
            }
    ...
    switch (node.type) {
        ...
        case "convert": {
            ...
            if (!sameDim(q.dim, unit.dim)) {
                throw new Error(
                    `${dimName(q.dim)}를 ${node.unit}로 바꿀 수 없다`,
                );
            }
            ...
            switch (node.op) {
                ...
                case "-": {
                    if (!sameDim(left.dim, right.dim)) {
                        throw new Error(
                            `${dimName(left.dim)}와 ${withTopic(dimName(right.dim))} 더할 수 없다`,
                        );
                    }
                    return {
                        value:
...
function formatQuantity(q: Quantity, displayUnit?: string): string {
    ...
    if (sameDim(q.dim, UNITS.m!.dim)) {
        return abs >= 1000
            ? `${formatNumber(q.value / 1000)} km`
            : `${formatNumber(q.value)} m`;
    }
    ...
    if (sameDim(q.dim, UNITS.kg!.dim)) {
        return abs >= 1
            ? `${formatNumber(q.value)} kg`
            : `${formatNumber(q.value * 1000)} g`;
    }
    ...
    for (const line of lines) {
        ...
        try {
            ...
            if (!Number.isFinite(value.value))
                throw new Error("계산 결과가 정의되지 않는다");

            if (target) variables.set(target, value);
            variables.set("ans", value); // 직전 결과
            outputs.push({
                spans,
                result: formatQuantity(value, ctx.displayUnit),
            });
        } catch (e) {
...
export const NaturalCalcDemo = () => {
    ...
    return (
        ...
        >
            ...
            >
                자유롭게 고쳐 써보자. 회색으로 표시된 단어는 엔진이 무시한
                부분이다.
            </div>
        </div>
    );
