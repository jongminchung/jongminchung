...
class PrattParser {
    ...
    parse(): AstNode {
        ...
        if (token.type !== "eof") {
            throw new Error(
                `수식이 끝났는데 '${token.value}'가 남아 있다 (위치 ${token.pos})`,
            );
        }
        return node;
    }
    ...
    private parsePrefix(): AstNode {
        ...
        if (token.type === "op" && token.value === "-") {
            ...
            return {
                type: "unary",
                op: "-",
                operand: this.parseExpression(25),
            };
        }
...
function evaluate(node: AstNode, env: Environment): number {
    switch (node.type) {
        ...
        case "ident": {
            ...
            if (value === undefined)
                throw new Error(`모르는 이름: ${node.name}`);
            return value;
        }
...
function layoutTree(root: AstNode): {
    root: Placed;
    totalLeaves: number;
    maxDepth: number;
} {
...
function nodeStyle(node: AstNode): {
    fill: string;
    stroke: string;
    text: string;
} {
...
export const AstDemo = () => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const unitX = Math.min(
                (w - padX * 2) / totalLeaves,
                isMobile ? 72 : 96,
            );
