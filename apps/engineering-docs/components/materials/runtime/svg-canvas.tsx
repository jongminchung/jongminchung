...
export class SvgDrawingContext {
    ...
    ellipse(
        ...
        if (delta >= Math.PI * 2 - 0.0001) {
            const middleAngle =
                startAngle + (counterclockwise ? -Math.PI : Math.PI);
    ...
    private appendText(
        ...
        if (
            maxWidth !== undefined &&
            textWidth(value, this.state.font) > maxWidth
        ) {
    ...
    private applyPaint(
        ...
        const fillValue = fill
            ? this.resolveStyle(this.state.fillStyle)
            : "none";
        const strokeValue = stroke
            ? this.resolveStyle(this.state.strokeStyle)
            : "none";
    ...
    private resolveStyle(style: SvgStyle): string {
        ...
        if (!this.definitions.has(style.id)) {
            ...
            const gradient = createElement(
                style.kind === "linear" ? "linearGradient" : "radialGradient",
            );
...
export const SvgCanvas = forwardRef<SvgCanvasHandle, SvgCanvasProps>(
    function SvgCanvas({ role = "presentation", ...props }, forwardedRef) {
        ...
        useImperativeHandle(forwardedRef, () => {
            ...
            if (svg === null)
                throw new Error("SvgCanvas mounted without an SVG element.");
            return {
                get width() {
                ...
                set width(value: number) {
                    ...
                    svg.setAttribute(
                        "viewBox",
                        `0 0 ${value} ${Number(svg.getAttribute("height") ?? 150)}`,
                    );
                ...
                set height(value: number) {
                    ...
                    svg.setAttribute(
                        "viewBox",
                        `0 0 ${Number(svg.getAttribute("width") ?? 300)} ${value}`,
                    );
                    contextRef.current?.clearRect();
                },
                style: svg.style,
