...
function simulate(): Req[] {
  const freeAt = new Array<number>(THREADS).fill(0);
  const reqs: Req[] = [];
  for (let a = ARRIVE_START; a <= ARRIVE_END; a += ARRIVE_INTERVAL) {
    let slot = 0;
    for (let i = 1; i < THREADS; i += 1) {
      const candidate = freeAt[i];
      const selected = freeAt[slot];
      if (candidate !== undefined && selected !== undefined && candidate < selected) slot = i;
    }
    const availableAt = freeAt[slot];
    if (availableAt === undefined) continue;
    const start = Math.max(a, availableAt);
    let end: number;
    if (start < SLOW_START) {
...
function colHeader(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, fs: number) {
...
export const ThreadPoolDemo = () => {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const queueW = Math.max(44, w - pad * 2 - inflowW - poolW - dbW - arrowW * 2);
      ...
      const segs: Array<[string, string, boolean]> = [
        ["활성 스레드 ", "#868e96", false],
        [`${active.length}/${THREADS}`, active.length >= THREADS ? "#fa5252" : "#228be6", true],
        ["  ·  대기 큐 ", "#868e96", false],
        [`${waiting.length}`, waiting.length > 0 ? "#fa5252" : "#228be6", true],
      ...
      const slotGap = 4;
      const visible = Math.max(2, Math.min(8, Math.floor((queueW - 6) / (slotSize + slotGap))));
      const slotX = (i: number) => queueX + queueW - (i + 1) * (slotSize + slotGap) + slotGap;
      ...
      waiting.forEach((r, i) => {
        ...
        ctx.beginPath();
        ctx.arc(slotX(i) + slotSize / 2, midY, slotSize * 0.32 * pop, 0, Math.PI * 2);
      ...
      for (const r of REQS) {
        ...
        ctx.beginPath();
        ctx.arc(pad + 4 + (destX - 6 - pad) * p, midY, dotR, 0, Math.PI * 2);
      ...
      for (let slot = 0; slot < THREADS; slot += 1) {
        ...
        if (occ) {
          ...
          let flashA = 0;
          for (const r of REQS) {
            if (r.slot === slot && e >= r.end && e - r.end < FLASH) {
      ...
      ctx.fillText(slow ? "2,000ms" : "20ms", dbX + dbW / 2, midY + dbFs * 0.6);

      // --- 하단 단계 설명 ---
      let caption = "① 정상: DB가 20ms에 응답해 스레드 8개가 여유 있게 순환한다";
      if (e >= RECOVER) {
        caption = "④ DB 회복: 밀린 대기 큐가 빠르게 소화되고 응답 시간이 정상으로 돌아온다";
      } else if (e >= EXHAUST_T) {
        caption = "③ 스레드 8개에 처리 중 요청이 8개를 넘는 순간, 누군가는 반드시 기다린다";
      } else if (e >= SLOW_START) {
        caption = "② DB에 슬로 쿼리 발생: 반납이 느려져 슬롯 8칸이 순식간에 가득 찬다";
      }
