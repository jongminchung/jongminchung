...
const DIE = [5000, 7000, 8600] as const; // 서버 1·2·3이 차례로 헬스체크에서 제외되는 시각
...
const DOTS: Dot[] = (() => {
  ...
  for (let t = 200; t < DIE[2]; t += 300) {
    const alive: number[] = [];
    for (const [i, dieAt] of DIE.entries()) {
      if (t < dieAt) alive.push(i);
    }
    if (alive.length === 0) break;
    const leg1 = 500;
    const srv = alive[k % alive.length];
    if (srv === undefined) break;
    dots.push({ t0: t, srv, leg1, leg2: t + leg1 >= T_SLOW ? 1800 : 600 });
    k += 1;
  }
  return dots;
})();
...
// 키프레임 선형 보간
function interp(e: number, keys: readonly (readonly [number, number])[]): number {
  ...
  for (let i = 1; i < keys.length; i += 1) {
    ...
    if (e < current[0]) {
      const [t0, v0] = previous;
      const [t1, v1] = current;
      return v0 + ((v1 - v0) * (e - t0)) / (t1 - t0);
    }
  }
  return keys.at(-1)?.[1] ?? first[1];
}

// LB가 각 서버에 보내는 트래픽 비율 (제외되면 null)
function share(e: number, i: number): number | null {
  const dieAt = DIE[i];
  if (dieAt === undefined || e >= dieAt) return null;
  if (e < DIE[0]) return 33;
  if (e < DIE[1]) return 50;
  return 100;
}

function drawXMark(ctx: CanvasRenderingContext2D, x: number, y: number, r: number) {
  ctx.beginPath();
...
function drawSegs(ctx: CanvasRenderingContext2D, segs: Seg[], cx: number, y: number, fs: number) {
    ...
    const render = (now: number) => {
      ...
      for (let i = 0; i < 3; i += 1) {
        ...
        if (dead) {
          ...
          // 제외 직후 잠깐 "헬스체크 실패" 라벨
          if (e < dieAt + FAIL_LABEL_DUR) {
            ctx.globalAlpha = easeOut((e - dieAt) / 250);
      ...
      for (let i = 0; i < 3; i += 1) {
        const dieAt = DIE[i];
        const gaugeKeys = GAUGE[i];
        if (dieAt === undefined || gaugeKeys === undefined) continue;
        const by = srvY(i);
        const dead = e >= dieAt;
        let g = interp(e, gaugeKeys);
