...
const releaseSigningIdentity = process.env.GIT_CLIENT_CODESIGN_IDENTITY?.trim() || null;
const notarizationKeychainProfile = process.env.GIT_CLIENT_NOTARY_KEYCHAIN_PROFILE?.trim() || null;
const notarizationKeychain = process.env.GIT_CLIENT_NOTARY_KEYCHAIN?.trim() || null;
const packageRoot = dirname(fileURLToPath(import.meta.url));
const nodePtySource = realpathSync(join(packageRoot, "node_modules/node-pty"));
const nodePtyResource = join(tmpdir(), "git-client-electron-native-resources/node-pty");

if (releaseMode !== null && releaseMode !== "production" && releaseMode !== "local-ad-hoc") {
...
if (
  ...
  throw new Error(`GIT_CLIENT_RELEASE_VERSION must be stable SemVer: ${releaseVersion}`);
}
if (releaseMode !== null && releaseVersion === null) {
  throw new Error("Electron release packaging requires GIT_CLIENT_RELEASE_VERSION");
}
if (releaseMode === "production") {
  if (releaseSigningIdentity === null) {
    throw new Error("Production Electron packaging requires GIT_CLIENT_CODESIGN_IDENTITY");
  }
  if (notarizationKeychainProfile === null) {
    throw new Error("Production Electron packaging requires GIT_CLIENT_NOTARY_KEYCHAIN_PROFILE");
  }
}
...
const config: ForgeConfig = {
  packagerConfig: {
    ...
    ...(releaseSigningIdentity === null ? {} : { osxSign: { identity: releaseSigningIdentity } }),
    ...(notarizationKeychainProfile === null
      ? {}
      : {
          osxNotarize: {
            keychainProfile: notarizationKeychainProfile,
            ...(notarizationKeychain === null ? {} : { keychain: notarizationKeychain }),
          },
        }),
  },
  hooks: {
    async generateAssets() {
      ...
      await Promise.all([
        cp(join(nodePtySource, "package.json"), join(nodePtyResource, "package.json")),
      ...
      await chmod(join(nodePtyResource, "prebuilds/darwin-arm64/spawn-helper"), 0o755);
    },
