...
const RepositoryWorkspaceStoreContext = createContext<RepositoryWorkspaceStoreApi | null>(null);
...
export function RepositoryWorkspaceStoreProvider({
  ...
  if (storeRef.current === null) storeRef.current = createRepositoryWorkspaceStore(options);
...
export function useRepositoryWorkspaceStore<T>(
  ...
  if (store === null) {
    throw new Error("useRepositoryWorkspaceStore must be used inside its provider");
  }
...
export function useRepositoryWorkspaceStoreApi(): RepositoryWorkspaceStoreApi {
  const store = useContext(RepositoryWorkspaceStoreContext);
  if (store === null) {
    throw new Error("useRepositoryWorkspaceStoreApi must be used inside its provider");
  }
  return store;
}
