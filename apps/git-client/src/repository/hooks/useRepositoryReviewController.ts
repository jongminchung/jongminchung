...
import type { ActionAvailability, Commit, FileChange, RepositoryView } from "../../domain/types";
import type { GitSessionController } from "../../git-session/useGitSessionController";
import { isElectronRuntime } from "../../platform/electron";
import { openExternalUrl, selectPatchExportPath } from "../../platform/electronActions";
...
export function useRepositoryReviewController({
  ...
  useEffect(() => {
    setHistorySelectedPath((current) => {
      if (current && commitFiles.some((file) => file.path === current)) return current;
  ...
  useEffect(() => {
    ...
    if (!primaryCommit || !file || !historyParentRevision || file.binary || file.submodule) {
  ...
  }, [changeSelection, diffPreferences, loadWorkingDiff, setChangeDiff, workingEntries]);
  ...
  useEffect(() => {
    ...
    void loadRevisionDiff(from.oid, to.oid, nativeDiffOptions(diffPreferences)).then(
      (patch) => {
        if (active) {
          setRevisionComparison({
            from: from.oid,
  ...
  }, [diffPreferences, loadRevisionDiff, selectedCommits, setRevisionComparison]);
  ...
  const runAction = useCallback(
    async (action: keyof ActionAvailability): Promise<void> => {
      ...
      if (action === "copyRevision") {
        ...
        const patch = await createPatchText(selectedCommits.map((commit) => commit.oid));
