...
import { openExternalUrl } from "../../platform/electronActions";
import type { GitOperation, RemoteInfo } from "../../shared/contracts/model";
...
function remoteBrowserUrl(remote: string): string | null {
  try {
    ...
    if (match?.[1] && match[2]) return `https://${match[1]}/${match[2].replace(/\.git$/u, "")}`;
  }
  return null;
}

interface RepositoryHostingCoordinatorOptions {
  readonly executeOperation: (operation: GitOperation, throwOnError?: boolean) => Promise<void>;
...
export function useRepositoryHostingCoordinator({
  ...
  const bindSharedProject = useCallback(
    async (binding: ShareProjectBinding): Promise<void> => {
      const existingRemote = remotes.find((remote) => remote.name === binding.remoteName);
      if (!existingRemote) {
        await executeOperation(
          { kind: "remoteAdd", name: binding.remoteName, url: binding.remoteUrl },
          true,
        );
      } else if (
        ...
        throw new Error(`Remote '${binding.remoteName}' now points to a different repository.`);
      }
      if (binding.initialCommit && !repository.snapshot.hasCommits) {
        ...
        if (excludedStagedPaths.length > 0) {
          await executeOperation({ kind: "removeCached", paths: excludedStagedPaths }, true);
        }
        await executeOperation({ kind: "stage", paths: [...binding.initialCommit.paths] }, true);
        await executeOperation(
          {
            kind: "commitAdvanced",
      ...
      if (repository.snapshot.hasCommits || binding.initialCommit !== null) {
        ...
        if (!branch) throw new Error("Check out a local branch before pushing the shared project.");
        await executeOperation(
          {
            kind: "push",
            destination: {
      ...
      onToast(createdEmptyRepository ? title : `Project shared on ${service}`);
