...
export function useRepositoryContentLoader({
    ...
    void runRepositoryTask(
      () =>
        Promise.all([
          ...
          file.binary
            ? loadFilePreview(beforeSource, file.oldPath ?? file.path)
            : Promise.resolve(null),
          file.binary ? loadFilePreview(afterSource, file.path) : Promise.resolve(null),
        ]),
  ...
  useEffect(() => {
    ...
    void runRepositoryTask(
      () =>
        Promise.all([
          ...
          entry.file.binary
            ? loadFilePreview(before, entry.file.oldPath ?? entry.file.path)
            : Promise.resolve(null),
          entry.file.binary ? loadFilePreview(after, entry.file.path) : Promise.resolve(null),
        ]),
      ([beforeContent, afterContent, beforePreview, afterPreview]) => {
    ...
    return () => {
      if (changeGeneration.current === generation) changeGeneration.current += 1;
    };
  }, [
    changeSelection,
    loadFile,
