...
export function useRepositoryVcsController({
  ...
  const openVcsFileTab = useCallback(
    ...
    [openInspector, repository.snapshot.headOid, setRepositoryViewMode, vcsFilePath],
  );
  ...
  }, [setBookmarksOpen, setChangeSelection, setProjectOpen, setRepositoryViewMode, vcsFileEntry]);
  ...
  const compareVcsFile = useCallback(
    async (selection: "revision" | "ref"): Promise<void> => {
      ...
      const options: DiffOptions = {
        ...
        contextLines: diffPreferences.contextLines === "full" ? null : diffPreferences.contextLines,
      };
      try {
        const patch = await loadRevisionDiff(revision, null, options, [vcsFilePath]);
  ...
  const createPatchFromLocalChanges = useCallback(async (): Promise<void> => {
    ...
    if (patch.trim() === "") throw new Error("There are no tracked local changes to export.");
    if (
  ...
  const applyPatchFromClipboard = useCallback(async (): Promise<void> => {
    ...
    if (patch.trim() === "") throw new Error("The clipboard does not contain a patch.");
    const accepted = await dialog.confirm({
      title: "Apply Patch",
