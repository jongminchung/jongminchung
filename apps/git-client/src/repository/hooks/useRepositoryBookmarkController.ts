...
export function useRepositoryBookmarkController({
  ...
  const openLineBookmark = useCallback(
    ...
    [openInspector, repository.snapshot.headOid, scratchFiles, setRepositoryViewMode],
  );
  ...
  const chooseBookmarkMnemonic = useCallback(
    ...
    [bookmarks, setBookmarks, dialog, setBookmarkGroupTarget, setBookmarkMnemonicTarget],
  );
