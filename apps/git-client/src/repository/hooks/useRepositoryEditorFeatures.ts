...
import { nextScratchName, type ScratchFile, type ScratchLanguage } from "../../domain/scratchFiles";
import type { RepositoryView } from "../../domain/types";
import type { GitSessionController } from "../../git-session/useGitSessionController";
import { exportHtmlFiles } from "../../platform/electronActions";
...
export function useRepositoryEditorFeatures({
  ...
  const exportToHtml = useCallback(
    async (
      ...
      if (scope === "selection") {
        ...
        if (scratch) {
          ...
          const content = await sessionReadFile(inspector.source, inspector.path);
    ...
    [editorStatus?.selectedText, inspector, projectFiles, scratchFiles, sessionReadFile],
  );
    ...
    async (
      ...
      try {
        for (const change of pending) {
          await sessionWriteWorkingTreeFile(change.path, change.after, "Replace in Files");
          written.push(change);
        }
      } catch (reason) {
  ...
  const runCodeInspection = useCallback(
    ...
    [codeAnalysisPaths, inspector?.scratchId, scratchFiles, sessionReadFile, setInspectionResults],
  );
  const runCodeCleanup = useCallback(
    async (scope: CodeAnalysisScope): Promise<void> => {
      ...
      try {
        for (const change of pending) {
          await sessionWriteWorkingTreeFile(change.path, change.after, "Code Cleanup");
          written.push(change);
        }
      } catch (reason) {
        const rollbackFailures: string[] = [];
        for (const change of [...written].reverse()) {
          try {
            await sessionWriteWorkingTreeFile(change.path, change.before, "Rollback Code Cleanup");
          } catch {
            rollbackFailures.push(change.path);
          }
        }
