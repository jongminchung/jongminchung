import { storedAppearancePreference, type AppearancePreference } from "../../domain/appearance";
...
export async function persistProductSettings(settings: ProductSettings): Promise<void> {
  await writeElectronSettings({ [PRODUCT_SETTINGS_KEY]: settings });
}

export async function hydrateToolWindowLayouts(): Promise<readonly NamedToolWindowLayout[]> {
  return parseNamedToolWindowLayouts(await readElectronSetting(TOOL_WINDOW_LAYOUT_KEY));
}
...
export async function persistSavedMacros(macros: readonly SavedMacro[]): Promise<void> {
...
export async function persistProjectDefaults(snapshot: ProjectDefaultsSnapshot): Promise<void> {
