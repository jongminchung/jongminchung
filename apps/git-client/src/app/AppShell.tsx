...
export function AppShell() {
  ...
  const repositoryPaletteItems = useMemo<readonly PaletteItem[]>(
    ...
    [confirmDiscardEditors, openRepositories, sessionActivateTab, sessionActiveTab],
  );
  ...
  return (
    ...
    >
      ...
      {welcomeVisible ? (
        ...
        <WorkspaceTitlebar
          ...
          showRepositoryActions={
            sessionActiveTab.kind === "repository" && repositoryChromeMode === "editor"
          }
        />
      )}
      {sessionError && (
        ...
        >
          <span className="flex w-full items-center gap-2">
            ...
            {sessionActivity?.status === "failed" && sessionActivity.canRetry && (
              <Button
      ...
      <AppWorkspaceRouter openRepositoryFromPicker={openRepositoryFromPicker} session={session} />
      <AppOverlays openRepositoryToolSafely={openRepositoryToolSafely} session={session} />
      {dialog.node}
    </div>
  );
}
