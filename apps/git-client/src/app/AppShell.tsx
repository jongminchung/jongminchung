...
export function AppShell() {
  ...
  const overlay = useAppStore(
    useShallow((state) => ({
      ...
      setRunConfigurationTemplatesOpen: state.setRunConfigurationTemplatesOpen,
  ...
  return (
    <div
      className={`appShell appShell [position:relative] [display:grid] [height:100%] [min-width:800px] [grid-template-rows:35px_32px_minmax(0,_1fr)_20px] [overflow:hidden] [background:var(--background)] data-[window-mode=welcome]:grid-rows-[30px_29px_minmax(0,1fr)_20px]! [&_button]:rounded-sm [&_button:not(:disabled):hover]:[background-color:color-mix(in_oklch,_var(--foreground)_7%,_transparent)] [&_input]:rounded-sm [&_select]:rounded-sm [&_textarea]:rounded-sm [html[data-presentation-mode=true]_&]:grid-rows-[30px_29px_minmax(0,1fr)_0]! [html[data-presentation-mode=true]_&[data-window-mode=welcome]]:grid-rows-[30px_29px_minmax(0,1fr)_0]! [html[data-status-bar-visible=false]_&]:grid-rows-[30px_29px_minmax(0,1fr)_0]! [html[data-status-bar-visible=false]_&[data-window-mode=welcome]]:grid-rows-[30px_29px_minmax(0,1fr)_0]!`}
    ...
    >
      ...
      {pendingTrustPath && (
        <TrustProjectDialog
          ...
          parentName={
            pendingTrustPath.split("/").filter(Boolean).at(-2) ?? "project"
          }
          projectName={
            pendingTrustPath.split("/").filter(Boolean).at(-1) ?? "project"
          }
        />
      )}
      {welcomeVisible ? (
