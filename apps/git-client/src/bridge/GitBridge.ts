...
export interface GitBridge {
  ...
  initializeRepository(path: string, bare: boolean): Promise<RepositorySnapshot>;
  cloneRepository(url: string, path: string, depth: number | null): Promise<RepositorySnapshot>;
  refreshRepository(repositoryId: string): Promise<RepositorySnapshot>;
  execute(request: GitRequest, onEvent: (event: GitEvent) => void): Promise<RequestId>;
  cancel(requestId: RequestId): Promise<void>;
  createShelf(repositoryId: string, message: string, paths: readonly string[]): Promise<ShelfEntry>;
  ...
  watchRepository(repositoryId: string, onEvent: (event: FileWatchEvent) => void): Promise<void>;
  ...
  listRecoveryEntries(repositoryId: string): Promise<readonly RecoveryEntry[]>;
  restoreRecoveryEntry(repositoryId: string, entryId: string): Promise<RecoveryRestoreResult>;
...
export class TauriGitBridge implements GitBridge {
  readonly #watchChannels = new Map<string, Channel<FileWatchEvent>>();
  ...
  writeConflictResult(
    ...
    return invoke("write_conflict_result", { repositoryId, path, result, stage });
  }
  ...
  executeSynchronizedBranchOperation(
    ...
    return invoke("execute_synchronized_branch_operation", { repositoryIds, operation });
  }
