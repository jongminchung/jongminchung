...
import type {
    ...
    GitLocalHistoryEntry as ElectronLocalHistoryEntry,
...
import { GitLocalHistoryEntrySchema } from "../shared/contracts/git-utility";
...
export interface ElectronGitApi {
    ...
    listChangelists(
        repositoryId: string,
    ): Promise<readonly ElectronChangelist[]>;
    saveChangelist(
        repositoryId: string,
        id: string | null,
        name: string,
    ...
    captureLocalHistory?(
        repositoryId: string,
        label: string | null,
    ): Promise<ElectronLocalHistoryEntry>;
    listLocalHistory?(
        repositoryId: string,
        path: string | null,
    ): Promise<readonly ElectronLocalHistoryEntry[]>;
    ...
    listConflicts(
        repositoryId: string,
    ): Promise<readonly ElectronConflictFile[]>;
    readConflict(
        repositoryId: string,
        path: string,
...
function asPatchExportResult(
    result: ElectronPatchExportResult,
): PatchExportResult {
    return { ...result };
}
...
function asChangelistCommitResult(
...
function asRecoveryRestoreResult(
...
function asMultiRootOutcome(
    outcome: ElectronMultiRootOutcome,
): MultiRootOutcome {
    return { ...outcome };
}
...
export class ElectronGitBridge implements GitBridge {
    ...
    writeWorkingTreeFile(
        ...
        const write = this.#api.writeWorkingTreeFile;
        if (write === undefined)
            return Promise.reject(new Error("File editing is unavailable"));
        return write(repositoryId, path, content);
    }
    ...
    async listLocalHistory(
        ...
        if (this.#api.listLocalHistory === undefined)
            throw new Error("Electron Local History API is unavailable");
        const entries = await this.#api.listLocalHistory(repositoryId, path);
        return entries.map((entry) =>
            GitLocalHistoryEntrySchema.parse(entry),
        );
    }
    ...
    async readLocalHistoryDiff(
        ...
        if (this.#api.readLocalHistoryDiff === undefined)
            throw new Error("Electron Local History API is unavailable");
        return this.#api.readLocalHistoryDiff(repositoryId, entryId, path);
    }
    ...
    async restoreLocalHistory(
        ...
        if (this.#api.restoreLocalHistory === undefined)
            throw new Error("Electron Local History API is unavailable");
        await this.#api.restoreLocalHistory(repositoryId, entryId, path);
    }

    async labelLocalHistory(
        repositoryId: string,
        entryId: string,
        label: string,
    ): Promise<ElectronLocalHistoryEntry> {
        if (this.#api.labelLocalHistory === undefined)
            throw new Error("Electron Local History API is unavailable");
        return GitLocalHistoryEntrySchema.parse(
            await this.#api.labelLocalHistory(repositoryId, entryId, label),
        );
    }
