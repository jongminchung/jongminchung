...
function relativeModule(sourcePath: string, specifier: string): string | null {
  ...
  throw new Error(`Could not resolve renderer import ${specifier} from ${sourcePath}`);
}

function moduleSpecifiers(source: string): readonly string[] {
  return [...source.matchAll(STATIC_IMPORT), ...source.matchAll(DYNAMIC_IMPORT)].flatMap((match) =>
    match[1] === undefined ? [] : [match[1]],
  );
}

function rendererGraph(entryPaths: readonly string[]): ReadonlyMap<string, string> {
  ...
  while (pending.length > 0) {
    ...
    for (const specifier of moduleSpecifiers(source)) {
      ...
      if (dependency !== null && dependency.startsWith(APP_ROOT)) pending.push(dependency);
    }
  }
  return sources;
}
...
describe("hosting renderer and preload boundaries", () => {
  it("does not reach Electron's main-only hosting barrel or node built-ins", () => {
    ...
    expect([...sources.keys()]).not.toContain(join(APP_ROOT, "electron/hosting/index.ts"));
  });
});
