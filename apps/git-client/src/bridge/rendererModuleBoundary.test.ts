...
const NODE_MODULES = new Set(builtinModules.flatMap((name) => [name, name.replace(/^node:/u, "")]));
...
describe("production renderer module boundary", () => {
  it("does not import Electron or Node built-ins", () => {
    const violations = productionSources(SOURCE_ROOT).flatMap((path) =>
      moduleSpecifiers(readFileSync(path, "utf8")).flatMap((specifier) => {
        const root = specifier.replace(/^node:/u, "").split("/")[0] ?? specifier;
  ...
  it("keeps Zustand slice internals behind their store composer", () => {
    const violations = productionSources(SOURCE_ROOT).flatMap((path) => {
      ...
      return moduleSpecifiers(readFileSync(path, "utf8")).flatMap((specifier) => {
        if (!specifier.includes("/state/slices/")) return [];
        const ownsStoreComposition =
          sourcePath.includes("/state/") || sourcePath.endsWith("gitSessionStore.ts");
