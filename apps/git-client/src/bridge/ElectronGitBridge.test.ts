...
class FakeGitApi implements ElectronGitApi {
  ...
  async executeQuery(
    ...
    listener({
      ...
      stream: "stdout",
      data:
        request.kind === "operation" ? "" : (this.outputs[request.kind] ?? ""),
    });
    listener({
      kind: "output",
      requestId: request.requestId,
...
describe("ElectronGitBridge", () => {
  ...
  it("[성공] 사전 로드가 리스너와 Promise를 통해 터미널을 치료할 때 한 번 처리됨", async () => {
    ...
    const id = await bridge.execute({ kind: "refs", repositoryId }, (event) =>
      events.push(event),
    );
  ...
  it("[실패] 로그 의미를 요청하고 비작업을 전송함", async () => {
    ...
    await expect(bridge.execute(operation, () => undefined)).resolves.toMatch(
      /^[0-9a-f-]{36}$/u,
    );
    await Promise.resolve();
    expect(api.queries.at(-1)).toMatchObject({
