...
class FakeGitApi implements ElectronGitApi {
    ...
    readonly cloned: Array<
        Readonly<{ url: string; path: string; options: CloneOptions }>
    > = [];
    readonly fileReads: Array<
        Readonly<{ source: FileSource; path: string; preview: boolean }>
    > = [];
    readonly unwatched: string[] = [];
    readonly watchers = new Map<
        string,
        (event: RepositoryChangedEvent) => void
    >();
    readonly outputs: Readonly<
        Partial<
            Record<Exclude<GitExecutionRequest["kind"], "operation">, string>
        >
    >;
    inspectionCount = 0;
    ...
    constructor(
        outputs: Readonly<
            Partial<
                Record<
                    Exclude<GitExecutionRequest["kind"], "operation">,
                    string
                >
            >
        > = {},
...
class AliasingGitApi extends FakeGitApi {
    ...
    override async listChangelists(
        _id: string,
    ): Promise<readonly Changelist[]> {
        return [this.changelist];
    }

    override async listRecoveryEntries(
        _id: string,
...
describe("ElectronGitBridge", () => {
    ...
    it("opens with the complete utility-inspected repository snapshot", async () => {
        ...
        await expect(
            bridge.openRepository(repository.path),
    ...
    it("initializes and stores a status-backed repository snapshot", async () => {
        ...
        await expect(
            bridge.initializeRepository("/tmp/new repository", false, (event) =>
                events.push(event),
            ),
        ).resolves.toMatchObject({
            id: repositoryId,
        ...
        expect(api.initialized).toEqual([
            { path: "/tmp/new repository", bare: false },
        ]);
        expect(events.map(({ kind }) => kind)).toEqual(["started"]);
        await expect(
            bridge.refreshRepository(repositoryId),
        ).resolves.toMatchObject({
            currentBranch: "main",
        });
    });
    ...
    it("settles once when preload delivers the terminal through both listener and Promise", async () => {
        ...
        expect(events.filter((event) => event.kind === "started")).toHaveLength(
            1,
        );
        expect(
    ...
    it("delegates branch comparison, pre-commit inspection, and commit signatures", async () => {
        ...
        await expect(
            bridge.compareBranches(repositoryId, "feature", "main"),
        ).resolves.toEqual({
            ahead: 1,
            behind: 0,
            leftOnly: [headOid],
        ...
        await expect(
            bridge.preCommitCheck(repositoryId),
        ).resolves.toMatchObject({
            branch: "main",
            protectedBranch: true,
            hooks: ["pre-commit"],
        ...
        await expect(
            bridge.loadCommitSignature(repositoryId, "HEAD"),
        ).resolves.toMatchObject({
            status: "N",
            fingerprint: null,
        });
    ...
    it("delegates repository configuration, refs, submodules, remotes, and worktrees", async () => {
        ...
        await expect(
            bridge.listMergedBranches(repositoryId, "HEAD"),
    ...
    it("forwards repository invalidations and tears down the watcher before closing", async () => {
        ...
        await bridge.watchRepository(repositoryId, (event) =>
            events.push(event),
        );
        ...
        expect(events).toEqual([
            { repositoryId, invalidations: ["status", "history"] },
        ]);
    ...
    it("transports non-operation requests without narrowing log semantics", async () => {
        ...
        await expect(
            bridge.execute(commitDetails, () => undefined),
        ...
        await expect(
            bridge.execute(operation, () => undefined),
        ).resolves.toMatch(/^[0-9a-f-]{36}$/u);
        await Promise.resolve();
        expect(api.queries.at(-1)).toMatchObject({
            kind: "operation",
        ...
        ).resolves.toMatchObject({ branch: "main", headOid });
    });
    ...
    it("synthesizes started before failed when the query transport rejects", async () => {
        ...
        const returnedRequestId = await bridge.execute(
    ...
    it("closes the utility repository when the session unwatches it", async () => {
        ...
        expect(api.closed).toEqual([repositoryId]);
        await expect(bridge.refreshRepository(repositoryId)).rejects.toThrow(
