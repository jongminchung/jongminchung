    ...
    return failedPaths.reduce(
        (projects, path, index) =>
            updateRecentProjects(projects, {
                ...
                branch:
                    projects.find((project) => project.path === path)?.branch ??
                    null,
...
export async function loadWorkspaceStartupState(
    ...
    const [storedPaths, storedActivePath, storedProjects, legacyRepositories] =
        await Promise.all([
    ...
    return {
        activeRepositoryPath:
            typeof storedActivePath === "string" && storedActivePath.length > 0
                ? storedActivePath
                : null,
        ...
        recentProjects:
            recentProjects.length > 0
                ? recentProjects
                : parseRecentProjects(legacyRepositories),
    };
}
