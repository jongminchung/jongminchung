...
export function parseToolWindowLayout(value: unknown): ToolWindowLayout {
    ...
    const tab = BOTTOM_PANEL_TABS.find(
        (candidate) => candidate === value.bottomPanelTab,
    );
    return {
        bookmarksOpen:
...
export function parseNamedToolWindowLayout(
    value: unknown,
...
export function parseNamedToolWindowLayouts(
    ...
    const layouts = value
        ...
        .filter(
            (layout, index, all) =>
                all.findIndex((candidate) => candidate.id === layout.id) ===
                index,
        );
