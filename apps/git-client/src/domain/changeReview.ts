...
export function changeEntries(status: StatusModel): readonly ChangeEntry[] {
  return status.changes.flatMap((file) => {
    ...
    if (file.staged) {
      entries.push({
        selection: { path: file.path, layer: "index" },
        file,
      });
    }
    if (file.worktree) {
      entries.push({
        selection: { path: file.path, layer: "worktree" },
        file,
      });
    }
    return entries;
...
export function uniqueChangePaths(
  ...
  return [
    ...new Set(
      entries
        .filter((entry) => entry.selection.layer === layer)
...
export function reconcileChangeSelection(
  ...
  const exact = entries.find((entry) =>
    hasSameChangeSelection(entry.selection, previous),
  );
  if (exact) return exact.selection;

  const otherLayer = entries.find(
    (entry) => entry.selection.path === previous.path,
  );
  ...
  if (previousPathIndex >= 0)
    return entries[previousPathIndex]?.selection ?? null;
...
export function revisionDiffEntries(
  patch: string,
  ...
  return starts.flatMap((start, index) => {
    ...
    const submodule =
      /(?:old mode 160000|new mode 160000|index [0-9a-f]+\.\.[0-9a-f]+ 160000)/.test(
        filePatch,
      );
    return [
      {
...
export function parseDiffPreferences(value: unknown): DiffPreferences {
  ...
  const viewMode: DiffViewMode =
    value.viewMode === "split" || value.viewMode === "unified"
      ? value.viewMode
      : "auto";
  const whitespace: DiffWhitespace =
    value.whitespace === "ignoreAll" ? "ignoreAll" : "show";
  const contextLines: DiffContextLines =
    value.contextLines === 5 ||
    value.contextLines === 10 ||
    value.contextLines === "full"
      ? value.contextLines
      : 3;
  return {
    viewMode,
...
export function parseCommitDraft(value: unknown): CommitDraft {
  ...
  return {
    ...
    changelistId:
      typeof value.changelistId === "string" ? value.changelistId : null,
  };
}
