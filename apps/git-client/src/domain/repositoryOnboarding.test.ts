...
describe("repository onboarding predicates", () => {
  it("[성공] local-only와 unpublished branch의 다음 command를 계산함", () => {
    ...
    expect(tasks.find((task) => task.id === "configure-remote")).toMatchObject({
      commandId: "repository.manageRemotes",
      complete: false,
    });
    expect(
      tasks.find((task) => task.id === "configure-upstream")?.disabledReason,
    ).toContain("Configure a remote");
    expect(tasks.find((task) => task.id === "publish-commits")).toMatchObject({
      commandId: "repository.push",
      complete: false,
    });
    expect(tasks.find((task) => task.id === "connect-hosting")).toMatchObject({
  ...
  it("[성공] safe mode, offline, detached HEAD와 conflict 사유를 구분함", () => {
    ...
    expect(
      offline.find((task) => task.id === "publish-commits")?.disabledReason,
    ).toContain("Reconnect");
    expect(
