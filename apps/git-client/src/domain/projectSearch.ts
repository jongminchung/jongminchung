...
export function parseProjectTextMatches(
    output: string,
): readonly ProjectTextMatch[] {
    ...
    while (cursor < output.length && results.length < RESULT_LIMIT) {
        ...
        if (path.length > 0 && fields !== null) {
            ...
            if (line > 0 && column > 0)
                results.push({ path, line, column, content });
        }
        cursor = end + 1;
    }
    return results;
...
export function projectSearchResults(
    ...
    const expressions =
        mode === "class" ? CLASS_DECLARATIONS : SYMBOL_DECLARATIONS;
    return matches
        .flatMap((match): readonly ProjectSearchResult[] => {
            const symbol = declaration(match.content, expressions);
            if (!symbol || !nameMatches(symbol.name, query, matchCase))
                return [];
            return [
                {
                    ...match,
