...
import {
    REBASED_ACTION_BINDINGS,
    REBASED_FEATURE_GROUPS,
    rebasedActionIds,
} from "./rebasedParity";
...
describe("Rebased generated action bindings", () => {
    it("maps every candidate command to one source-oracle obligation", () => {
        ...
        expect(new Set(REBASED_ACTION_BINDINGS.map((entry) => entry.commandId)).size).toBe(
            243,
        );
    });
    ...
    it("keeps every binding complete and machine-checkable", () => {
        ...
        expect(
            new Set(Object.values(REBASED_FEATURE_GROUPS).flat()),
        ).toEqual(new Set(rebasedActionIds()));
    });

    it("does not claim source-wide parity from the candidate subset", () => {
        expect(REBASED_ACTION_BINDINGS).toHaveLength(243);
        expect(
            REBASED_ACTION_BINDINGS.some(
                (binding) => binding.sourceId === "toolwindow:Terminal",
            ),
        ).toBe(true);
    });
});
