...
function importSpecifiers(contents: string): readonly string[] {
    return Array.from(
        contents.matchAll(
            /(?:\bfrom\s+|\bimport\s*(?:\(\s*)?|@import\s+)["']([^"']+)["']/g,
        ),
        (match) => match[1] ?? "",
    );
}
...
function isAppRuntimePackage(packageName: string): boolean {
    ...
    const isUiPackage =
        ...
        packageName.startsWith("@jongminchung/ui/");
    ...
    return (
        ...
        appRuntimes.some(
            (runtime) =>
                packageName === runtime ||
                packageName.startsWith(`${runtime}/`),
        )
    );
}
...
describe("workspace shadcn design system boundary", () => {
    test("routes every app to the shared primitive package", () => {
        const baseTsconfig = readJson(
            join(workspaceRoot, "tsconfig.base.json"),
        );
        const baseCompilerOptions = baseTsconfig.compilerOptions as Record<
            string,
            unknown
        >;
        const sharedConfig = readJson(join(uiRoot, "components.json"));
        const sharedAliases = sharedConfig.aliases as Record<string, string>;
        const sharedTailwind = sharedConfig.tailwind as Record<
            string,
            string | boolean
        >;
        ...
        for (const app of apps) {
            ...
            const packageImports = packageJson.imports as Record<
                string,
                string
            >;
            ...
            const tailwind = config.tailwind as Record<
                string,
                string | boolean
            >;
            ...
            if (app === "git-client")
                expect(compilerOptions.customConditions).toEqual(["source"]);
        ...
        for (const file of sourceFiles(join(uiRoot, "src"))) {
            for (const specifier of importSpecifiers(
                ...
                if (specifier.startsWith(".")) {
                    ...
                    expect(target.startsWith(`${appsRoot}${sep}`), file).toBe(
                        false,
                    );
                }
            }
    ...
    test("separates shared Tailwind input from app-owned themes", () => {
        const sharedStyles = readFileSync(
            join(uiRoot, "src", "styles", "globals.css"),
            "utf8",
        );
        const defaultTheme = readFileSync(
            join(uiRoot, "src", "styles", "theme.css"),
            "utf8",
        );
        const appStyles = [
            join(
        ...
        for (const path of appStyles) {
            ...
            expect(stylesheet, path).toContain(
    ...
    test("keeps product theme overrides local and satisfies the semantic token contract", () => {
        const tokenContract = readFileSync(
            join(uiRoot, "src", "styles", "tokens.css"),
            "utf8",
        );
        const defaultTheme = readFileSync(
            join(uiRoot, "src", "styles", "theme.css"),
        ...
        expect(tokenContract).toContain(
        ...
        for (const token of requiredTokens)
            expect(defaultTheme).toContain(`--${token}:`);
    ...
    test("exposes the official Button variants and composes loading explicitly", () => {
        const button = readFileSync(
            join(uiRoot, "src", "components", "button.tsx"),
            "utf8",
        );
        const spinner = readFileSync(
            join(uiRoot, "src", "components", "spinner.tsx"),
            "utf8",
        );
    ...
    test("publishes the shadcn primitives used by dense Git Client surfaces", () => {
        for (const component of [
            ...
            expect(
                existsSync(
                    join(uiRoot, "src", "components", `${component}.tsx`),
                ),
                component,
            ).toBe(true);
        }
        expect(
            existsSync(join(uiRoot, "src", "components", "native-select.tsx")),
        ).toBe(false);
    });

    test("keeps product behavior in app-local compositions", () => {
        const dialog = readFileSync(
            join(sourceRoot, "components", "ProductDialog.tsx"),
            "utf8",
        );
        const form = readFileSync(
            join(sourceRoot, "components", "ProductFormControls.tsx"),
            "utf8",
        );
        const select = readFileSync(
            join(sourceRoot, "components", "ProductSelect.tsx"),
            "utf8",
        );
        ...
        const overlays = readFileSync(
            join(sourceRoot, "components", "ProductOverlays.tsx"),
            "utf8",
        );
        ...
        expect(
            sourceFiles(sourceRoot).filter((file) =>
                importSpecifiers(readFileSync(file, "utf8")).includes(
                    "@jongminchung/ui/components/select",
                ),
            ),
        ...
        expect(collections).toContain(
            'from "@jongminchung/ui/components/badge"',
        );
        expect(collections).toContain(
            'from "@jongminchung/ui/components/empty"',
        );
        expect(collections).toContain(
            'from "@jongminchung/ui/components/spinner"',
        );
    ...
    test("keeps component styling out of a central Tailwind registry", () => {
        expect(existsSync(join(sourceRoot, "styles", "tailwind.ts"))).toBe(
            false,
        );
        ...
        expect(
            applicationSources.filter((path) => path.endsWith(".module.css")),
        ).toEqual([]);

        for (const path of applicationSources.filter((path) =>
            /\.tsx?$/.test(path),
        )) {
            const source = readFileSync(path, "utf8");
            expect(source, path).not.toMatch(
                /from\s+["'][^"']*styles\/tailwind["']/,
            );
            expect(source, path).not.toMatch(
                /from\s+["'][^"']*\.module\.css["']/,
            );
        ...
        expect(indexCss).not.toMatch(
            /\.(?:appShell|titlebar|toolStripe|workbench|local-history-)/,
        );
