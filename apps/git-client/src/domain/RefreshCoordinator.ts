...
export class RefreshCoordinator {
  ...
  invalidate(repositoryId: string, invalidations: readonly RepositoryInvalidation[]): void {
    ...
    for (const invalidation of invalidations) state.pending.add(invalidation);
    this.schedule(repositoryId, state);
  }

  defer(repositoryId: string, invalidations: readonly RepositoryInvalidation[]): void {
    if (invalidations.length === 0) return;
    const pending = this.#deferred.get(repositoryId) ?? new Set<RepositoryInvalidation>();
  ...
  private schedule(repositoryId: string, state: CoordinationState): void {
    if (state.scheduled || state.running || state.pending.size === 0) return;
  ...
  private start(repositoryId: string, state: CoordinationState): void {
    ...
    const invalidations = INVALIDATION_ORDER.filter((scope) => state.pending.delete(scope));
