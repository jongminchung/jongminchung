...
export function parseStatusV2(output: string): StatusModel {
    ...
    for (let index = 0; index < records.length; index += 1) {
        ...
        if (record.startsWith("# branch.oid "))
            branchOid = record.slice(13).trim();
            ...
            stashCount = Number(record.slice(8)) || 0;
            ...
            changes.push({
                ...
                oldPath,
                status:
                    STATUS_MAP[xy.charAt(0)] ??
                    STATUS_MAP[xy.charAt(1)] ??
                    "modified",
                staged: xy[0] !== ".",
                worktree: xy[1] !== ".",
...
export function assembleHunkPatch(
    fileHeader: string,
...
export function assembleSelectedLinePatch(
    ...
    if (selected.length === 0)
        throw new Error("Select at least one changed line");
    const groups: DiffLinePosition[][] = [];
    for (const line of selected) {
        const current = groups.at(-1);
        if (current && line.index === (current.at(-1)?.index ?? -2) + 1)
            current.push(line);
    ...
    const hunks = groups.flatMap((group) => {
        ...
        const oldStart =
            deletions[0]?.oldLine ?? Math.max(0, first.oldLine - 1);
        const newStart =
            additions[0]?.newLine ?? Math.max(0, first.newLine - 1);
        const lines = group.flatMap((line) =>
            line.noNewlineMarker
                ? [line.text, line.noNewlineMarker]
                : [line.text],
        );
        return [
            {
