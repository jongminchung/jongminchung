...
export class RepositoryAccessPolicy {
    ...
    safePaths(paths: readonly string[]): readonly string[] {
        const openSafePaths = new Set(
            [...this.#modesByRepository.entries()].flatMap(
                ([repositoryId, mode]) => {
        ...
        return [...new Set(paths)].filter(
            (path) =>
                this.modeForPath(path) === "safe" || openSafePaths.has(path),
        );
    }
    ...
    activeMode(): RepositoryAccessMode {
        return this.#activeRepositoryId === null
            ? "trusted"
            : this.mode(this.#activeRepositoryId);
    }
    ...
    assert(
        repositoryId: string,
        capability: Exclude<RepositoryCapability, "query">,
    ): void {
        if (!this.allows(repositoryId, capability))
...
export function assertGitRequestAllowed(
    policy: RepositoryAccessPolicy,
    request: GitRequest,
): void {
    if (request.kind === "operation")
        policy.assert(request.repositoryId, "gitMutation");
}
