...
export const DEFAULT_STATUS_BAR_WIDGETS: Readonly<
    Record<StatusBarWidget, boolean>
> = {
    statusText: true,
    fileSystemSync: true,
    aggregator: true,
...
export function isProductKeymapPreset(
    value: unknown,
): value is ProductKeymapPreset {
    return (
        value === "macOS" ||
        value === "Emacs" ||
...
export function isEditorColorScheme(
    value: unknown,
): value is EditorColorScheme {
    return (
        value === "themeDefault" ||
        value === "light" ||
        value === "dark" ||
...
export function parseProductSettings(value: unknown): ProductSettings {
    ...
    const keymapOverrides: Readonly<Record<string, string | null>> = isRecord(
    ...
    return {
        compactMode:
            typeof value.compactMode === "boolean" ? value.compactMode : false,
        ideFontSize:
        ...
        zoom,
        presentationMode:
            typeof value.presentationMode === "boolean"
                ? value.presentationMode
                : false,
        distractionFreeMode:
            typeof value.distractionFreeMode === "boolean"
                ? value.distractionFreeMode
                : false,
        zenMode: typeof value.zenMode === "boolean" ? value.zenMode : false,
        presentationAssistant:
            typeof value.presentationAssistant === "boolean"
                ? value.presentationAssistant
                : false,
        powerSaveMode:
            typeof value.powerSaveMode === "boolean"
                ? value.powerSaveMode
                : false,
        toolbarVisible:
        ...
        adjustRedGreenVision:
            typeof value.adjustRedGreenVision === "boolean"
                ...
                : false,
        showNotifications:
            typeof value.showNotifications === "boolean"
        ...
        keymapOverrides,
        bidiTextDirection:
            value.bidiTextDirection === "ltr" ||
            value.bidiTextDirection === "rtl"
        ...
        editorColorScheme: isEditorColorScheme(value.editorColorScheme)
            ...
            : "themeDefault",
        language: "English",
        region: isProductRegion(value.region)
            ? value.region
            : "asiaExceptChinaMainland",
    };
}
