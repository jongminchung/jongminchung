...
export const CODE_INSPECTIONS: readonly CodeInspectionDefinition[] = [
    ...
    },
    {
        ...
        description:
            "Reports spaces or tabs after the final visible character.",
        severity: "warning",
        cleanup: true,
    },
    {
...
export function inspectText(
    path: string,
    text: string,
    enabled: ReadonlySet<CodeInspectionId> = new Set(
        CODE_INSPECTIONS.map(({ id }) => id),
    ),
): readonly CodeIssue[] {
    ...
    for (const [index, content] of text.split("\n").entries()) {
        const line = index + 1;
        if (
            enabled.has("conflictMarkers") &&
            /^(?:<{7}|={7}|>{7})(?:\s|$)/u.test(content)
        ) {
            issues.push(
                issue(
                    "conflictMarkers",
...
export function parseOfflineInspectionXml(
    ...
    for (const match of content.matchAll(
        ...
        const description =
            ...
            xmlTag(block, "problem_class") ||
            "Inspection problem";
        issues.push(
            issue("offline", "warning", path, line, 1, description, ""),
        );
    }
    return issues;
}
