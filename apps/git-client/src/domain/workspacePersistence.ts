...
export function isManagementSection(value: unknown): value is ManagementSection {
  return (
    value === "roots" ||
    value === "refs" ||
    value === "remotes" ||
...
function clampPaneWidth(value: unknown, fallback: number, minimum: number, maximum: number): number {
...
export function migrateRepositoryUiState(value: unknown): RepositoryUiState {
  ...
  return {
    ...
    historyReviewWidth: clampPaneWidth(value.historyReviewWidth, 760, 640, 1000),
    ...
    logTabIds:
      ...
      value.logTabIds.some((tabId) => typeof tabId === "string")
        ? [...new Set(value.logTabIds.filter((tabId): tabId is string => typeof tabId === "string"))]
        : ["log-1"],
    activeLogTabId:
...
export function migrateWorkspacePreferences(value: unknown): WorkspacePreferences {
  ...
  return {
    ...
    hostingAccounts: Array.isArray(value.hostingAccounts)
      ...
      : [],
    managementSection: isManagementSection(value.managementSection)
