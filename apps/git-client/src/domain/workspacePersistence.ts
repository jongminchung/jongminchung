...
function isFetchTagMode(
    value: unknown,
): value is WorkspacePreferences["fetchTagMode"] {
    return (
        value === "auto" ||
        value === "sync" ||
        value === "always" ||
...
export function migrateRepositoryUiState(value: unknown): RepositoryUiState {
    ...
    return {
        selectedOids: Array.isArray(value.selectedOids)
            ? value.selectedOids.filter(
                  (oid): oid is string => typeof oid === "string",
              )
            : [],
        selectedRef:
            typeof value.selectedRef === "string" ? value.selectedRef : null,
        bottomCollapsed:
        ...
        historyReviewWidth:
            typeof value.historyReviewWidth === "number" &&
            value.historyReviewWidth >= 640
                ? 210
        ...
        bookmarksOpen:
            typeof value.bookmarksOpen === "boolean"
                ? value.bookmarksOpen
                : false,
        logOpen: typeof value.logOpen === "boolean" ? value.logOpen : true,
        logTabIds:
            Array.isArray(value.logTabIds) &&
            value.logTabIds.some((tabId) => typeof tabId === "string")
                ? [
                      ...new Set(
                          value.logTabIds.filter(
                ...
                : ["log-1"],
        activeLogTabId:
            typeof value.activeLogTabId === "string"
                ? value.activeLogTabId
                : "log-1",
    };
}

function isHostingAccountMetadata(
    value: unknown,
): value is HostingAccountMetadata {
...
export function migrateWorkspacePreferences(
    ...
    return {
        schemaVersion: WORKSPACE_SCHEMA_VERSION,
        autoFetchMinutes:
            typeof value.autoFetchMinutes === "number" &&
            value.autoFetchMinutes > 0
                ? value.autoFetchMinutes
                : null,
        fetchTagMode: isFetchTagMode(value.fetchTagMode)
            ? value.fetchTagMode
            : "auto",
        recurseSubmodules:
            typeof value.recurseSubmodules === "boolean"
...
export function workspacePaths(
    ...
    return sessions.map((session) =>
        session.kind === "repository"
            ? session.repository.snapshot.path
            : session.path,
    );
}
...
export function restoredWorkspaceTab(
    ...
    if (typeof activePath !== "string") {
        const firstRepository = sessions.find(
            (session) => session.kind === "repository",
        );
    ...
    if (selected?.kind === "error")
        return { kind: "error", sessionId: selected.id };
    const firstRepository = sessions.find(
        (session) => session.kind === "repository",
    );
    return firstRepository?.kind === "repository"
...
export function workspaceTabAfterClose(
    ...
    const closingIsActive =
        (activeTab.kind === "repository" &&
            activeTab.repositoryId === closingSessionId) ||
        (activeTab.kind === "error" &&
            activeTab.sessionId === closingSessionId);
    ...
    if (replacement?.kind === "error")
        return { kind: "error", sessionId: replacement.id };
    return { kind: "welcome" };
}
