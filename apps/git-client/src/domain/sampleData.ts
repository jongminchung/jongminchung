...
const QA_REFERENCE_EPOCH_SECONDS = Math.floor(
    Date.parse("2025-01-15T12:00:00.000Z") / 1000,
);
...
export const sampleCommits: readonly Commit[] = Array.from(
    { length: 140 },
    (_, index) => {
...
function fixtureSourceLabel(source: FileSource): string {
    if (source.kind === "revision")
        return `revision ${source.revision.slice(0, 8)}`;
