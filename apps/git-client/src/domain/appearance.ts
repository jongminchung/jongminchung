...
export function storedAppearancePreference(
  value: unknown,
  ...
  if (
    ...
    typeof value.syncWithOs === "boolean"
  ) {
...
export function synchronizeAppearancePreference(
  ...
  if (!preference.syncWithOs || preference.theme === systemTheme)
    return preference;
  ...
  load(): AppearancePreference {
    try {
      return parseStoredPreference(
        this.#storage.getItem(APPEARANCE_STORAGE_KEY),
      );
    } catch {
      return DEFAULT_APPEARANCE_PREFERENCE;
    }
