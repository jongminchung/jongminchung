...
describe("작업과정을 거치는 과정", () => {
  ...
  it("[실패] 값을 신뢰하지 않고 반환된 기본 설정을 마이그레이션함", () => {
    ...
    expect(
      "forceWithLease" in migrateWorkspacePreferences({ forceWithLease: true }),
    ).toBe(false);
  });
  ...
  it("[성공] 대피 장소와 경계를 사용하여 선반 패널 상태를 마이그레이션함", () => {
    ...
    expect(
      migrateRepositoryUiState({ bottomPanelHeight: 999 }).bottomPanelHeight,
  ...
  it("[성공] v3 검토 상태를 마이그레이션하고 초안 및 창의 경량화를 확인함", () => {
    ...
    expect(
      migrateRepositoryUiState({ historyReviewWidth: 760 }).historyReviewWidth,
    ).toBe(253);
  });
