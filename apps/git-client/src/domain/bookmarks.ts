...
export function parseProjectBookmarks(
    ...
    const groups = value.groups.flatMap(
        (candidate): readonly BookmarkGroup[] => {
            ...
            const bookmarks = candidate.bookmarks.flatMap(
                (bookmark): readonly LineBookmark[] => {
...
export function allLineBookmarks(
    state: ProjectBookmarks,
): readonly LineBookmark[] {
...
export function toggleLineBookmark(
    ...
    const group =
        state.groups.find((candidate) => candidate.isDefault) ??
        state.groups[0];
    return group
...
export function moveBookmark(
    ...
    return {
        ...state,
        groups: state.groups.map((group) => {
            const index = group.bookmarks.findIndex(
                (bookmark) => bookmark.id === bookmarkId,
            );
...
export function createBookmarkGroup(
    ...
    if (!normalized || state.groups.some((group) => group.name === normalized))
        return state;
...
export function renameBookmarkGroup(
    ...
    if (
        !normalized ||
        state.groups.some(
            (group) => group.id !== groupId && group.name === normalized,
        )
    )
        return state;
    return {
