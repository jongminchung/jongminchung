...
export function bookmarkAt(
  ...
  return (
    allLineBookmarks(state).find(
      (bookmark) =>
        bookmark.path === location.path && bookmark.line === location.line,
    ) ?? null
  );
}
...
export function toggleLineBookmark(
  ...
  const group =
    state.groups.find((candidate) => candidate.isDefault) ?? state.groups[0];
  return group
...
export function relativeBookmark(
  ...
  const currentIndex = current
    ? bookmarks.findIndex(
        (bookmark) =>
          bookmark.path === current.path && bookmark.line === current.line,
      )
    : -1;
  const index =
    currentIndex < 0
