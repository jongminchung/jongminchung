...
export function requiresPushConfirmation(
    ...
    return (
        ...
        (preview.protectedBranch ||
            (preview.fastForward === false && !knownRewrite))
    );
}
...
export function createPushOperation(
    ...
    if (choice === "normal" && !canNormalPush(preview)) {
        throw new Error(
            "Normal push is unavailable for a non-fast-forward destination",
        );
    }
    if (choice === "forceWithLease" && !canForceWithLease(preview)) {
        throw new Error(
            "Force push requires an exact reviewed remote object ID",
        );
    }
    return {
        kind: "push",
        ...
        mode:
            choice === "normal"
                ...
                : {
                      kind: "forceWithLease",
                      expectedRemoteOid: preview.expectedLeaseOid ?? "",
                  },
    };
}
