...
import type { GitRequest, RequestId } from "../generated";
...
describe("Git Console", () => {
  ...
  it("records the ordered lifecycle and redacts credentials", () => {
    let entries = recordGitConsoleEvent([], request, {
      kind: "started",
      requestId,
      displayCommand: "git status --porcelain=v2",
      startedAtMs: 10,
    }, 10);
    entries = recordGitConsoleEvent(entries, request, {
      kind: "output",
      requestId,
      sequence: 0,
      stream: "stderr",
  ...
  it("records failures and cancellations as terminal states", () => {
    const failed = recordGitConsoleEvent([], request, {
      kind: "failed",
      requestId,
    ...
    const cancelled = recordGitConsoleEvent([], request, {
      kind: "cancelled",
      requestId,
      durationMs: 10,
    }, 30);
