...
describe("Git operation policies", () => {
  it("covers every operation kind with an explicit policy", () => {
    expect(Object.keys(gitOperationPolicies).sort()).toEqual([...operationKinds].sort());
  });
  ...
  it("keeps index, stash, management, push, and default invalidations distinct", () => {
    ...
    expect(gitOperationPolicies.stashApply.invalidations).toEqual(["status", "history", "stash"]);
    expect(gitOperationPolicies.remoteAdd.invalidations).toEqual([
      "status",
      "history",
      "management",
    ]);
    expect(gitOperationPolicies.push.invalidations).toEqual(["status", "history"]);
    expect(gitOperationPolicies.checkout.invalidations).toEqual(["status", "history", "operation"]);
  });
  ...
  it("centralizes recovery recording and retry behavior", () => {
    ...
    expect(operationKinds.filter((kind) => gitOperationPolicies[kind].retryable)).toEqual([
      "fetch",
    ]);
  });
});
