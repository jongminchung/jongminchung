...
function isCommandId(value: unknown): value is CommandId {
    return (
        typeof value === "string" &&
        /^[a-z]+(?:[A-Z][a-z]+|\.[a-z][A-Za-z]+)+$/.test(value)
    );
}
...
function parseManifestEntry(value: unknown): CommandManifestEntry | null {
    ...
    if (
        ...
        return null;
    if (
        !includesString(categories, value.category) ||
        !includesString(menus, value.menu)
    )
        return null;
    if (
    ...
    if (
        ...
        return null;
    if (
        value.allowInCodeEditor !== undefined &&
        typeof value.allowInCodeEditor !== "boolean"
    )
        return null;
    return {
        id: value.id,
...
export function parseCommandManifest(value: unknown): CommandManifest {
    ...
    const validated = commands.filter(
        (command): command is CommandManifestEntry => command !== null,
    );
...
export function commandDefinition(
    ...
    const manifest = COMMAND_MANIFEST.commands.find(
...
export function keyboardShortcutFromAccelerator(
    ...
    const modifiers = new Set(
        parts.slice(0, -1).map((part) => part.toLocaleLowerCase()),
    );
    if (
        [...modifiers].some(
            (part) =>
...
export function isCommandSearchElement(target: EventTarget | null): boolean {
    return (
        ...
        target.closest("[data-command-search]") !== null
    );
}
...
export function isTerminalElement(target: EventTarget | null): boolean {
    return (
        target instanceof HTMLElement &&
        target.closest('[data-command-scope="terminal"]') !== null
    );
}
...
export function canHandleShortcut(
    ...
    if (
        ...
        !(
            ...
            (command.id === "view.findNext" ||
                command.id === "view.findPrevious")
        )
    )
        return false;
    return true;
...
function normalizedSearchText(item: PaletteItem): string {
    return [item.label, item.detail, item.category, ...item.keywords]
        .join(" ")
        .toLocaleLowerCase();
}
...
export function selectDismissLayer<T extends DismissLayer>(
    ...
    if (layer === null || (inputOwnsEscape && layer.priority < 100))
        return null;
    return layer;
}
...
export class CommandRegistry {
    ...
    unregister(owner: string): void {
        for (const [id, definitions] of this.#definitions) {
            ...
            else if (remaining.length !== definitions.length)
                this.#definitions.set(id, remaining);
        }
    }
