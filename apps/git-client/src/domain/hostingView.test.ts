...
import { adjacentHostingChangeRequest, filterHostingChangeRequests } from "./hostingView";
...
describe("hosting change request view", () => {
  it("applies the Rebased-style state scope and text search together", () => {
    expect(filterHostingChangeRequests(ITEMS, "octocat", "open")).toEqual([ITEMS[0]]);
    expect(filterHostingChangeRequests(ITEMS, "13", "draft")).toEqual([ITEMS[1]]);
    expect(filterHostingChangeRequests(ITEMS, "legacy", "all")).toEqual([ITEMS[2]]);
  });
