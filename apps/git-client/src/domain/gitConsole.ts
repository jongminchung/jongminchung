...
export function describeGitRequest(request: GitRequest): string {
  switch (request.kind) {
    case "status":
      return "git status --porcelain=v2 --branch -z";
    case "refs":
      return "git for-each-ref";
    case "files":
      return "git ls-files --cached --others --exclude-standard -z";
    case "searchText":
      return "git grep [search] --untracked --exclude-standard";
    case "log":
      return `git log --max-count=${request.limit} --skip=${request.skip}`;
    case "commitDetails":
      return `git show --format= ${request.revision.slice(0, 12)}`;
    case "diff":
      return `git diff${request.staged ? " --cached" : ""}${request.paths.length > 0 ? ` -- ${request.paths.length} path(s)` : ""}`;
    case "tree":
      return `git ls-tree ${request.revision.slice(0, 12)}`;
    case "fileHistory":
      return `git log -- ${request.path}`;
    case "blame":
      return `git blame ${request.revision?.slice(0, 12) ?? "HEAD"} -- ${request.path}`;
    case "stashList":
      return "git stash list";
    case "stashShow":
      return `git stash show ${request.stash}`;
    case "configList":
      return "git config --list --show-origin --show-scope";
    case "submoduleStatus":
      return "git submodule status --recursive";
    case "signature":
      return `git show --show-signature ${request.revision.slice(0, 12)}`;
    case "checkIgnored":
      return `git check-ignore -- ${request.paths.length} path(s)`;
    case "mergedBranches":
      return `git branch --merged ${request.target}`;
    case "pushPreview":
      return `git rev-list ${request.localRevision} --not ${request.remote}/${request.remoteRef}`;
    case "historyRewritePreview":
      return `git log ${request.fromRevision.slice(0, 12)}..HEAD`;
    case "operation":
      return `git ${request.operation.kind}  # ${operationActivityLabel(request.operation)}`;
  }
}
        ...
        : event.kind === "completed"
          ...
          : event.kind === "failed"
            ? {
                ...
                completedAt: now,
                output: appendOutput(base.output, `${base.output ? "\n" : ""}${event.message}`),
              }
