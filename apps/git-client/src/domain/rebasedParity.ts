...
function parseBinding(value: unknown): RebasedActionBinding | null {
    ...
    return {
        ...
        checkedWhen:
            typeof value.checkedWhen === "string" ? value.checkedWhen : null,
    };
}
...
function parseBindings(value: unknown): readonly RebasedActionBinding[] {
    ...
    return bindings.filter(
        (binding): binding is RebasedActionBinding => binding !== null,
    );
}
...
for (const binding of bindings) {
    if (!sourceIds.has(binding.sourceId)) {
        throw new Error(
            `Rebased action binding is absent from the source oracle: ${binding.sourceId}`,
        );
    }
}
...
function sourceIdsForGroup(
    group: RebasedFeatureGroup,
): readonly RebasedSourceId[] {
    return bindings
...
export const REBASED_FEATURE_GROUPS = Object.freeze({
    ...
    Record<RebasedFeatureGroup, readonly RebasedSourceId[]>
>;
