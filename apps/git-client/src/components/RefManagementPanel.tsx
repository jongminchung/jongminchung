        ...
        <div className={tw.refManagement} aria-busy={busy}>
            <div className={tw.managementToolbar}>
                ...
                <button
                    ...
                    onClick={() =>
                        currentBranch &&
                        void onLoadMergedBranches(currentBranch).then(
                            (branches) =>
                                setMergedBranches(
                                    branches.filter(
                                        (branch) =>
            ...
            <section className={tw.refForms}>
                ...
                >
                    ...
                    <input
                        ...
                        value={tagMessage}
                    />
                    <button disabled={!tagName.trim() || busy}>
                        Create tag
                    </button>
                </form>
            </section>
            <section className={tw.mergeOptions}>
                ...
                <label>
                    <input
                        checked={squashMerge}
                        onChange={(event) =>
                            setSquashMerge(event.target.checked)
                        }
                        type="checkbox"
                    />{" "}
            ...
            {comparison && (
                <section className={tw.branchComparison}>
                    ...
                    <span>
                        {comparison.ahead} only on current · {comparison.behind}{" "}
                        only on selected
                    </span>
                    <details>
            ...
            {mergedBranches.length > 0 && (
                <section className={tw.mergedBranches}>
                    ...
                    {mergedBranches.map((branch) => (
                        <span key={branch}>
                            <code>{branch}</code>
                            <button onClick={() => void deleteMerged(branch)}>
                                Delete
                            </button>
                        </span>
                    ))}
