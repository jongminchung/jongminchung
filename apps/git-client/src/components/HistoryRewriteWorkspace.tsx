...
const ACTIONS: readonly RebasePlanAction[] = [
    "pick",
    "reword",
    "edit",
    "squash",
...
export function HistoryRewriteWorkspace({
    ...
    useEffect(() => {
        if (preview !== null) firstActionRef.current?.focus();
    }, [preview]);
    ...
    const dropOn = (targetOid: string): void => {
        if (!draggedOid || draggedOid === targetOid || preview?.hasMerges)
            return;
        setEntries((current) =>
            moveHistoryPlanEntry(current, draggedOid, targetOid),
        );
        setDraggedOid(null);
    };
    ...
    const execute = async (): Promise<void> => {
        ...
        if (preview.protectedBranch) {
            const accepted = await confirm(
                protectedRewriteConfirmation(preview),
            );
            if (!accepted) return;
        }
        setRunning(true);
                ...
                >
                    ...
                    {loading ? (
                        <Spinner
                            ...
                            label="Inspecting branch history…"
                        />
                    ) : completed && preview ? (
                        <div className="m-auto grid max-w-xl gap-4 rounded-lg border border-border bg-card p-6 text-center">
                            ...
                            <p className="m-0 text-muted-foreground">
                                {preview.branch} was rewritten locally. A
                                Recovery entry was recorded before the
                                operation.
                            </p>
                            <code className="rounded-lg bg-muted p-2 text-xs">
                                {preview.headOid.slice(0, 10)} →{" "}
                            ...
                            {preview.publishedCommitCount > 0 && (
                                <p className="m-0 rounded-lg border border-warning bg-warning/10 p-3">
                                    {preview.publishedCommitCount} published
                                    commit(s) changed. Review the destination
                                    before pushing.
                                </p>
                            )}
                        ...
                        <div className="grid min-h-0 grid-cols-[minmax(0,1fr)_320px] max-[900px]:grid-cols-1 max-[900px]:grid-rows-[minmax(0,1fr)_auto]">
                                ...
                                <Table className="border-collapse text-left text-xs">
                                    <TableHeader className="sticky top-0 z-10 bg-card text-muted-foreground">
                                        <TableRow>
                                            ...
                                            <TableHead className="h-8 w-28 p-2">
                                                Action
                                            </TableHead>
                                            <TableHead className="h-8 p-2">
                                                Commit
                                            </TableHead>
                                            <TableHead className="h-8 w-52 p-2">
                                                Impact
                                            </TableHead>
                                        </TableRow>
                                    </TableHeader>
                            ...
                            <aside className="grid content-start gap-4 overflow-auto bg-card p-4">
                                <section className="grid gap-2">
                                    ...
                                    <span>
                                        {preview.descendantCount} commit(s)
                                        rewritten
                                    </span>
                                    <span>
                                        {preview.publishedCommitCount} published
                                    ...
                                    <span>
                                        {preview.dependentRefs.length} dependent
                                        local ref(s)
                                    </span>
                                    <span>
                                        {preview.root
                                ...
                                <section className="grid gap-2">
                                    ...
                                    {preview.dependentRefs.length > 0 && (
                                        <label>
                                            <Checkbox
                                                checked={updateRefs}
                                                onCheckedChange={setUpdateRefs}
                                            />{" "}
                                            Update dependent refs
                                        </label>
                                    )}
                                    <label>
                                        <Checkbox checked disabled /> Preserve
                                        merge topology
                                    </label>
                                </section>
                                {preview.dependentRefs.length > 0 && (
                                    <section className="grid gap-1">
                                        ...
                                        {preview.dependentRefs.map(
                                            (reference) => (
                                                <code key={reference.name}>
                                                    {reference.name} ·{" "}
                                                    {reference.oid.slice(0, 8)}
                                                </code>
                                            ),
                                        )}
                                    </section>
                                ...
                                <small className="text-muted-foreground">
                                    Conflicts and edit stops continue in Changes
                                    / Recovery with Continue, Skip, or Abort.
                                </small>
                            </aside>
                        </div>
                    ) : (
                    ...
                    <DialogFooter alignment="start">
                        ...
                        {!completed && (
                            <Button
                                type="submit"
                                disabled={
                                    !preview || Boolean(validation) || running
                                }
                                className={cn("h-7 px-2.5")}
                                variant="destructive"
                                size="sm"
