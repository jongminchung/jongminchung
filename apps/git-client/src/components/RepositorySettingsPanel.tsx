...
function isRepositorySettingsTab(
    value: unknown,
): value is RepositorySettingsTab {
...
export function RepositorySettingsPanel({
    ...
    const removeConfig = async (entry: GitConfig): Promise<void> => {
        const accepted = await dialog.confirm({
            ...
            description:
                "Removes the repository-local value. A global or system value may still apply.",
            impact: entry.value,
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <Table
                        aria-label="Git config values"
                        className="min-w-[660px] table-fixed text-xs"
                    >
                        ...
                        <TableBody>
                            {filteredConfig.map((entry) => (
                                <TableRow
                                    key={`${entry.scope}-${entry.origin}-${entry.key}`}
                                >
