...
const UPDATES = [
  ...
  },
  {
    ...
    description:
      "Accounts, pull or merge requests, reviews, discussions, and project sharing.",
...
export function WhatsNewDialog({ onClose }: { readonly onClose: () => void }) {
  return (
    ...
    >
      <section
        className={`whatsNewDialog whatsNewDialog [display:grid] [grid-template-rows:auto_minmax(300px,_1fr)_auto] [&>_footer]:[display:flex] [&>_footer]:[justify-content:flex-end] [&>_footer]:[padding:8px_10px] [&>_footer]:[border-top:1px_solid_var(--border)] [&>_main]:[min-height:0] [&>_main]:[overflow:auto] [&>_main>_div[role=list]]:[padding:5px_9px_12px] [&>_main>_p]:[margin:14px_16px_8px] [&>_main>_p]:[color:var(--muted-foreground)]`}
      >
        <DialogHeader
          hasDivider
          onOpenChange={(open) => !open && onClose()}
