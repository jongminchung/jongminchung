...
export function ShareInitialCommitDialog({
  ...
  return (
    ...
    >
      <section
        className={`shareInitialCommitDialog shareInitialCommitDialog [display:grid] [height:min(640px,_calc(100vh_-_70px))] [grid-template-rows:auto_32px_minmax(220px,_1fr)_auto_auto] [&>_[data-slot=text-area]]:[padding:10px_12px] [&>_[data-slot=text-area]]:[border-top:1px_solid_var(--border)] [&>_footer]:[display:flex] [&>_footer]:[justify-content:flex-end] [&>_footer]:[gap:8px] [&>_footer]:[padding:8px_10px] [&>_footer]:[border-top:1px_solid_var(--border)] [&>_header]:[display:flex] [&>_header]:[align-items:center] [&>_header]:[justify-content:space-between] [&>_header]:[padding:0_10px_0_13px] [&>_header]:[color:var(--muted-foreground)] [&>_header]:[border-bottom:1px_solid_var(--border)] [&>_main]:[min-height:0] [&>_main]:[overflow:auto] [&>_main>_div[role=list]]:[padding:5px_7px] [&>_main>_p]:[margin:18px] [&>_main>_p]:[color:var(--muted-foreground)]`}
      >
        <DialogHeader
          hasDivider
          onOpenChange={(open) => !open && onCancel()}
