...
export function usePaletteItems(items: readonly PaletteItem[]): void {
    ...
    const { registerPaletteItems, unregisterPaletteItems } =
        useCommandContext();
...
export function useDismissLayer(layer: DismissLayer): void {
    ...
    const { registerDismissLayer, unregisterDismissLayer } =
        useCommandContext();
    useEffect(() => {
...
> {
    const { execute, openPalette, openPaletteFor, closePalette, announce } =
        useCommandContext();
...
export function CommandProvider({
    ...
    if (registryRef.current === null)
        registryRef.current = CommandRegistry.create();
    ...
    const registerPaletteItems = useCallback(
        (owner: string, items: readonly PaletteItem[]): void => {
    ...
    const unregisterPaletteItems = useCallback((owner: string): void => {
        if (paletteItems.current.delete(owner))
            setRevision((current) => current + 1);
    }, []);
    const registerDismissLayer = useCallback(
        (owner: string, layer: DismissLayer): void => {
            dismissLayers.current.set(owner, {
                ...layer,
                order: dismissOrder.current,
            });
        ...
        (
            ...
            if (isTerminalElement(target) && !definition.allowInTerminal)
                return commandDisabled(
                    "This shortcut is reserved for the focused terminal.",
                );
            if (isCodeEditorElement(target) && !definition.allowInCodeEditor)
                return commandDisabled(
                    "This shortcut is reserved for the focused code editor.",
                );
            if (
                ...
                !(
                    ...
                    (definition.id === "view.findNext" ||
                        definition.id === "view.findPrevious")
                )
            )
                return commandDisabled(
                    "This shortcut is reserved for the focused editor.",
                );
    ...
    const execute = useCallback(
        async (id: CommandId): Promise<void> => {
            ...
            const availability = contextualAvailability(
                definition,
                document.activeElement,
            );
            ...
            if (definition.mutation) {
                ...
                if (
                    runningMutations.current.has(id) ||
                    now - (lastMutationAt.current.get(id) ?? 0) < 350
                ) {
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent): void => {
            ...
            if (event.key === "Escape") {
                ...
                const layer = selectDismissLayer(
                    [...dismissLayers.current.values()],
                    inputOwnsEscape,
                );
                if (!layer) return;
    ...
    useEffect(() => {
        const states: readonly NativeMenuItemState[] =
    ...
    const commandItems: readonly PaletteItem[] = COMMAND_MANIFEST.commands.map(
        (entry) => {
            const definition = registry.find(entry.id);
            return {
                id: entry.id,
                ...
                availability: definition
                    ? contextualAvailability(definition, focusTarget)
                    : commandDisabled(
                          "This command is not available in the current view.",
                      ),
                execute: () => execute(entry.id),
                shortcut:
                    displayAccelerator(
                        resolvedAccelerator(entry, keymapOverrides),
                    ) || undefined,
            };
        },
    );
    const allPaletteItems = [
        ...commandItems,
        ...[...paletteItems.current.values()].flat(),
    ];
