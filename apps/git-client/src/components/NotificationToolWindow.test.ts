...
describe("NotificationToolWindow", () => {
  it("renders operation history and clear/close actions", () => {
    const markup = renderToStaticMarkup(createElement(NotificationToolWindow, {
      notifications: [{ id: "fetch-1", title: "Fetching", message: "Completed", kind: "success", createdAt: 10 }],
      onClear: vi.fn(),
      onClose: vi.fn(),
    }));
