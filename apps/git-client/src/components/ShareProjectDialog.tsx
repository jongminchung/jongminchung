...
export function ShareProjectDialog({
    ...
    const [accountInformationLoading, setAccountInformationLoading] =
        useState(false);
    const [accountInformationError, setAccountInformationError] = useState<
        string | null
    >(null);
    const [canCreatePrivate, setCanCreatePrivate] = useState(true);
    const [ownedRepositoryNames, setOwnedRepositoryNames] = useState<
        readonly string[]
    >([]);
    const [repositoryCheckLoading, setRepositoryCheckLoading] = useState(false);
    const [repositoryCheckError, setRepositoryCheckError] = useState<
        string | null
    >(null);
    ...
    const [initialCommit, setInitialCommit] =
        useState<InitialCommitSelection | null>();
    const [initialCommitDialogOpen, setInitialCommitDialogOpen] =
        useState(false);
        ...
        const loadNamespaces = async (): Promise<void> => {
            ...
            try {
                ...
                if (response.kind !== "namespaces") {
                    throw new Error(
                        `${service} did not return project namespaces.`,
                    );
                }
                setNamespaces(response.items);
                setSelectedNamespaceKey((current) =>
                    response.items.some(
                        (item) => namespaceKey(item) === current,
                    )
                        ? current
                        : response.items[0]
                ...
                setSelectedNamespaceKey("");
                setNamespaceError(
                    `Cannot load namespaces for account: ${message(reason)}`,
                );
            } finally {
    ...
    const selectedNamespace =
        namespaces.find(
            (namespace) => namespaceKey(namespace) === selectedNamespaceKey,
        ) ?? null;
        ...
        const loadAccountInformation = async (): Promise<void> => {
            ...
            try {
                ...
                if (response.kind !== "shareRepositories") {
                    throw new Error(
                        "GitHub did not return repository information.",
                    );
                }
                ...
                setOwnedRepositoryNames([]);
                setAccountInformationError(
                    `Cannot load information for this account: ${message(reason)}`,
                );
            } finally {
    ...
    useEffect(() => {
        if (provider === "gitHub") {
            setRemoteRepositoryExists(
                ownedRepositoryNames.includes(repositoryName),
            );
        }
            ...
            const checkRepository = async (): Promise<void> => {
                ...
                try {
                    ...
                    if (response.kind !== "repositoryAvailability") {
                        throw new Error(
                            "GitLab did not return repository availability.",
                        );
                    }
                    ...
                    if (!active) return;
                    setRemoteRepositoryExists(false);
                    setRepositoryCheckError(
                        `Cannot check repository availability: ${message(reason)}`,
                    );
                } finally {
    ...
    const validation = (): string | null => {
        ...
        if (provider === "gitLab" && selectedNamespace === null)
        ...
        if (remoteRepositoryExists)
            return "Repository with selected name already exists.";
    ...
    const completeLocalSetup = async (
        ...
        try {
            ...
            setError(
                `The remote repository was created, but local setup failed: ${message(reason)}`,
            );
        } finally {
            setBusy(false);
        }
        ...
        try {
            ...
            if (repository === null) {
                const response = await bridge.execute(accountId, {
                    ...
                    namespaceId:
                        provider === "gitLab"
                            ? (selectedNamespace?.id ?? null)
                            : null,
                });
                if (response.kind !== "repository") {
                    throw new Error(
                        `${service} did not return the created repository.`,
                    );
                }
                repository = response;
                setCreated(response);
                ...
                <main>
                    ...
                    >
                        ...
                        <CheckboxInput
                            ...
                            isDisabled={
                                busy || created !== null || !canCreatePrivate
                            }
                            label="Private"
                            onChange={setPrivate}
                            value={isPrivate}
                    ...
                    {created && (
                        <p role="status">
                            Remote repository created at{" "}
                            <code>{created.project}</code>. Retrying will only
