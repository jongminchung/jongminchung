...
export function BookmarksToolWindow({
    ...
    const onRowKeyDown = (
        ...
        if (event.key === "Enter") onOpenBookmark(bookmark);
            ...
            const next =
                bookmarks[
                    Math.min(bookmarks.length - 1, Math.max(0, index + offset))
                ];
            if (next) {
                setSelectedBookmarkId(next.id);
                document
                    .querySelector<HTMLElement>(
                        `[data-bookmark-id="${CSS.escape(next.id)}"]`,
                    )
                    ?.focus();
            }
        } else return;
        ...
        >
            <Button
                ...
                onClick={() => {
                    ...
                    if (
                        state.view.autoscrollToSource ||
                        state.view.openInPreviewTab
                    ) {
            ...
            >
                <span>
                    {bookmark.mnemonic ?? (
                        <Icon name="bookmarkFilled" size={11} />
                    )}
                </span>
                <strong>
                    {bookmark.description || bookmark.path.split("/").at(-1)}
                </strong>
                <small>
    ...
    const renderGroupBookmarks = (group: BookmarkGroup) => {
        if (!state.view.groupLineBookmarks)
            return group.bookmarks.map(renderBookmark);
        ...
        >
                    ...
                    <TooltipTrigger
                        render={
                            <Button
                                ...
                                onClick={() =>
                                    selectedBookmark &&
                                    void editBookmark(selectedBookmark)
                                }
                                type="button"
                                className={cn(
                                    "gap-1.5 text-xs min-h-[26px] min-w-[26px] p-1 text-muted-foreground",
                                    ...
                                    <TooltipTrigger
                                        render={
                                            <Toggle
                                                aria-label={
                                                    group.isDefault
                                                        ? "Unmark List as Default"
                                                        : "Mark List as Default"
                                                }
                                                onPressedChange={() =>
                                                    onSetDefaultGroup(group.id)
                                                }
            ...
            {creatingGroup && (
                <BookmarkGroupCreateDialog
                    existingNames={
                        new Set(state.groups.map((group) => group.name))
                    }
