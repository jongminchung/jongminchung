...
export function BookmarksToolWindow({
    ...
    const name = await dialog.input({
      ...
      validate: (value) => {
        ...
        return state.groups.some(
          (candidate) =>
            candidate.id !== group.id && candidate.name === normalized,
        )
  ...
  const onRowKeyDown = (
    ...
    if (event.key === "Enter") onOpenBookmark(bookmark);
      ...
      const next =
        bookmarks[Math.min(bookmarks.length - 1, Math.max(0, index + offset))];
      if (next) {
  ...
  const renderBookmark = (bookmark: LineBookmark) => (
    ...
    >
      ...
      >
        <span>
          {bookmark.mnemonic ?? <Icon name="bookmarkFilled" size={11} />}
        </span>
        <strong>
          ...
          <TooltipTrigger
            render={
              <Button
                ...
                onClick={() =>
                  selectedBookmark && void editBookmark(selectedBookmark)
                }
                type="button"
                className={cn(
                  "gap-1.5 text-xs min-h-[26px] min-w-[26px] p-1 text-muted-foreground",
          ...
          <TooltipTrigger
            render={
              <Button
                ...
                onClick={() =>
                  setCollapsedGroups(
                    new Set(state.groups.map((group) => group.id)),
                  )
                }
                type="button"
                className={cn(
