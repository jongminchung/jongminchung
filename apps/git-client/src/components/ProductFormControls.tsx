...
export function TextInput({
    ...
    useEffect(() => {
        if (hasAutoFocus) inputRef.current?.focus();
    }, [hasAutoFocus]);
    return (
        <FieldShell
        ...
        >
            ...
            >
                {startIcon ?? (
                    <Search
                        aria-hidden
                        className="hidden size-3.5 text-muted-foreground"
                    />
                )}
                <Input
    ...
    return (
        ...
        >
            <Checkbox
                {...props}
                aria-describedby={
                    [descriptionId, statusId].filter(Boolean).join(" ") ||
                    undefined
                }
                aria-invalid={status?.type === "error" || undefined}
                checked={value === true}
            ...
            >
                ...
                {status?.message ? (
                    <FieldError id={statusId}>{status.message}</FieldError>
                ) : null}
            </FieldLabel>
        </Field>
    );
...
export function Selector({
    ...
    useEffect(() => {
        if (hasAutoFocus) triggerRef.current?.focus();
    }, [hasAutoFocus]);
    ...
    return (
        ...
        >
            ...
            >
                <SelectTrigger
                    aria-describedby={
                        [ariaDescribedBy, statusId].filter(Boolean).join(" ") ||
                        undefined
                    }
                    aria-invalid={status?.type === "error" || undefined}
                    className={cn(
