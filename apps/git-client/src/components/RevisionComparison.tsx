...
export function RevisionComparison({
  ...
  const selected =
    entries.find((entry) => entry.file.path === selectedPath) ??
    entries[0] ??
    null;
    ...
    void Promise.all([
      ...
      ([before, after]) => {
        if (generation.current === current)
          setContent({ before, after, loading: false });
      },
      () => {
        if (generation.current === current)
          setContent({ before: null, after: null, loading: false });
  ...
  const move = (offset: number): void => {
    const next =
      entries[
        Math.min(entries.length - 1, Math.max(0, selectedIndex + offset))
      ];
    ...
    >
      ...
      {loading ? (
        <Spinner
          ...
          label="Loading revision comparison…"
        />
      ) : error ? (
        <Notice
        ...
        <div data-revision-content>
          <nav aria-label="Compared files">
            {entries.map((entry) => (
              <Button
                aria-current={
                  selected?.file.path === entry.file.path ? "true" : undefined
                }
                key={entry.file.path}
