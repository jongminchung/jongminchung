  ...
  return listenWorkbenchEvent(
    ...
    ({ action }, control) => {
      ...
      if (action === "selectionScope") {
        ...
        const query = new SearchQuery({
          search:
            current.search || view.state.sliceDoc(selection.from, selection.to),
...
function joinSelectedLines(view: EditorView): boolean {
  ...
  const replacement = view.state
    .sliceDoc(first.from, to)
    .replace(/\s*\n\s*/gu, " ");
  view.dispatch({
  ...
  return replaceSelectedLines(view, (lines) =>
    lines.map((line) => {
      ...
      const next =
        target === "spaces"
          ? " ".repeat(width)
          : "\t".repeat(Math.floor(width / tabSize)) +
            " ".repeat(width % tabSize);
...
function navigateMethod(view: EditorView, direction: -1 | 1): boolean {
  ...
  for (let number = 1; number <= view.state.doc.lines; number += 1) {
    if (declaration.test(view.state.doc.line(number).text))
      matches.push(number);
  }
  const target =
    direction > 0
      ? (matches.find((line) => line > currentLine) ?? matches[0])
      : ([...matches].reverse().find((line) => line < currentLine) ??
        matches.at(-1));
