...
export function useChangeSelection({
    ...
    const runFileAction = async (): Promise<void> => {
        ...
        await onOperation({
            kind:
                selectedEntry.selection.layer === "index" ? "unstage" : "stage",
                ...
                setSelectedKeys(
                    new Set(
                        filteredEntries
                            .slice(start, end + 1)
                            .map((candidate) =>
                                selectionKey(candidate.selection),
                            ),
                    ),
                );
    ...
    useEffect(() => {
        ...
        setSelectedKeys((current) =>
            current.size > 1 ? current : new Set([key]),
        );
    ...
    const collapseSelection = useCallback(
        () =>
            setSelectedKeys(
                new Set(selection ? [selectionKey(selection)] : []),
            ),
        [selection],
    );
