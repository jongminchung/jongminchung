...
export function useChangeSelection({
  ...
  const filteredEntries = useMemo(
    () =>
      entries.filter(
        (entry) =>
          !normalizedQuery ||
          entry.file.path.toLocaleLowerCase().includes(normalizedQuery),
      ),
  ...
  const selectedEntry =
    entries.find(
      (entry) =>
        selection && hasSameChangeSelection(entry.selection, selection),
    ) ?? null;
  ...
  const selectEntry = (event: ReactMouseEvent, entry: ChangeEntry): void => {
    ...
    if (event.shiftKey) {
      const anchor =
        selectionAnchor ?? (selection ? selectionKey(selection) : key);
  ...
  const handleNavigatorKeyboard = (
    ...
    if (
      ...
      event.preventDefault();
      setSelectedKeys(
        new Set(filteredEntries.map((entry) => selectionKey(entry.selection))),
      );
      return;
    }
    if (event.key === "Home" || event.key === "End") {
      event.preventDefault();
      const next =
        event.key === "Home" ? filteredEntries[0] : filteredEntries.at(-1);
      if (next) {
        const key = selectionKey(next.selection);
        onSelectionChange(next.selection);
        setSelectedKeys(new Set([key]));
    ...
    const accepted = await dialog.confirm({
      title:
        paths.length === 1
          ...
          : `Discard ${paths.length} files?`,
      description:
        "Restore each selected working-tree path to its indexed version.",
  ...
  useEffect(() => {
    return listenWorkbenchEvent("git-client:find", ({ direction }) => {
      ...
      const nextIndex =
        (Math.max(0, selectedIndex) + direction + filteredEntries.length) %
