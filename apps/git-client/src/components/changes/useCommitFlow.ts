...
export function useCommitFlow({
    ...
    const selectedChangelist =
        draft.changelistId === null
            ? null
            : (changelists.find((item) => item.id === draft.changelistId) ??
              null);
    const stagedFiles = status.changes.filter((file) => file.staged);
    const hasCommitAllChanges =
        draft.commitAll &&
        status.changes.some(
            (file) => file.worktree && file.status !== "untracked",
        );
    ...
    const deleteSelectedChangelist = async (): Promise<void> => {
        ...
        try {
            const accepted = await dialog.confirm({
                ...
                description:
                    "Files remain unchanged and return to the default group.",
        ...
        try {
            ...
            const warnings = [
                ...
                check.protectedBranch
                    ...
                    : "",
                check.crlfPaths.length > 0
                    ? `CRLF: ${check.crlfPaths.join(", ")}`
                    : "",
