...
export async function loadHostingAccounts(): Promise<readonly HostingAccount[]> {
...
export async function persistHostingAccounts(accounts: readonly HostingAccount[]): Promise<void> {
  await writeSetting(HOSTING_ACCOUNTS_KEY, accountJson(accounts));
}

export function viewedFilesKey(accountId: string, project: string, number: number): string {
...
export async function loadViewedFiles(
  ...
  return new Set(
    Array.isArray(stored) ? stored.filter((path): path is string => typeof path === "string") : [],
  );
}
...
export async function persistViewedFiles(
  ...
  await writeSetting(viewedFilesKey(accountId, project, number), [...paths].sort());
}
