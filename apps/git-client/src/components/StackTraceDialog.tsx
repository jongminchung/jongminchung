...
export function StackTraceDialog({
  ...
  return (
    ...
    >
      <section
        className={`stackTraceDialog stackTraceDialog [display:grid] [height:min(680px,_calc(100vh_-_80px))] [grid-template-rows:auto_minmax(0,_1fr)_auto] [&>_footer]:[display:flex] [&>_footer]:[justify-content:flex-end] [&>_footer]:[padding:8px_10px] [&>_footer]:[border-top:1px_solid_var(--border)] [&>_main]:[display:grid] [&>_main]:[min-height:0] [&>_main]:[grid-template-rows:auto_minmax(0,_1fr)] [&>_main]:[gap:10px] [&>_main]:[padding:12px] [&>_main_>_div[role=listbox]]:[min-height:0] [&>_main_>_div[role=listbox]]:[overflow:auto]`}
      >
        <DialogHeader
          hasDivider
          onOpenChange={(open) => !open && onClose()}
