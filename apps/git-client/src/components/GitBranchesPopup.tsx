...
import type { BranchComparison, GitOperation, RemoteInfo } from "../generated";
...
export function GitBranchesPopup({
  ...
  const rows = useMemo<readonly BranchPopupRow[]>(
    () =>
      refs
        .filter(
          (ref) =>
            !normalizedQuery || ref.shortName.toLowerCase().includes(normalizedQuery),
        )
        .sort((left, right) => {
  ...
  const renameActive = async (): Promise<void> => {
    ...
    await run({ kind: "renameBranch", oldName: activeRef.shortName, newName: name });
  };
  ...
  const deleteActive = async (): Promise<void> => {
    ...
    const accepted = await dialog.confirm({
      ...
      description: activeRef.kind === "remote"
  ...
  return (
    ...
    >
      <header>
        ...
        <button aria-label="Branches Settings" onClick={onOpenManagement} title="Settings">
      ...
      <div className={tw.gitBranchesList} id="git-branches-list" role="listbox">
        {(["local", "remote", "tag"] as const).map((kind) => {
          ...
          return (
            <section aria-label={kind === "local" ? "Local" : kind === "remote" ? "Remote" : "Tags"} key={kind}>
              ...
              {group.map(({ row, index }) => (
                ...
                >
                  <Icon name={row.ref.kind === "remote" ? "remote" : row.ref.kind === "tag" ? "tag" : "branch"} size={14} />
                  <span>{row.label}</span>
                  {row.ref.favorite && <Icon className={tw.favorite} name="star" size={12} />}
                  {(row.ref.current || row.ref.shortName === currentBranch) && <small>Current</small>}
                </button>
              ))}
            </section>
          );
      ...
      {error && <p className={tw.gitBranchesError} role="alert">{error}</p>}
      {activeRef && (
        <div className={tw.gitBranchSelectedActions} aria-label={`Actions for ${activeRef.shortName}`}>
          <button disabled={activeRef.current || busy} onClick={() => void checkoutActive()}>Checkout</button>
          <button disabled={busy} onClick={() => void createBranch()}>New Branch from…</button>
          <button disabled={!onCompare || !currentBranch || activeRef.current || busy} onClick={() => void compareActive()}>Compare</button>
          {activeRef.kind === "local" && <button disabled={busy} onClick={() => void renameActive()}>Rename…</button>}
          {activeRef.kind === "local" && <button disabled={busy} onClick={() => void setUpstream()}>Set Upstream…</button>}
          <button disabled={busy || !onOperation} onClick={() => void createTag()}>New Tag…</button>
          {activeRef.kind === "tag" && <button disabled={busy || remotes.length === 0} onClick={() => void pushActiveTag()}>Push Tag</button>}
          <button disabled={busy || !onOperation} onClick={() => void addWorktree()}>New Worktree…</button>
          <button disabled={activeRef.current || busy || !onOperation} onClick={() => void deleteActive()}>Delete…</button>
        </div>
      )}
      {comparison && activeRef && (
        <div className={tw.gitBranchComparison} role="status">
          <strong>{currentBranch} ↔ {activeRef.shortName}</strong>
          <span>{comparison.ahead} ahead · {comparison.behind} behind</span>
        </div>
      )}
      <footer>
