...
export function HostingRequestList({
  ...
  const navigate = (event: KeyboardEvent<HTMLElement>): void => {
    ...
    const next = adjacentHostingChangeRequest(visibleItems, selectedNumber, direction);
  ...
  return (
    <section
      ...
      onKeyDown={navigate}
      // oxlint-disable-next-line jsx-a11y/no-noninteractive-tabindex -- The request navigator owns keyboard selection.
      tabIndex={0}
    >
      <div
      ...
      >
        ...
        <Select
          onValueChange={(value) => value && setScope(value as HostingListScope)}
          value={scope}
        >
          <SelectTrigger
      ...
      {items.length === 0 && <EmptyState title="Load pull or merge requests for this project." />}
