...
describe("Hosting OAuth 설정", () => {
  it("[성공] cloud provider는 패키지 client ID를 사용할 수 있음", () => {
    expect(requiresExplicitOAuthClientId("gitHub", "https://github.com")).toBe(
      false,
    );
    expect(requiresExplicitOAuthClientId("gitLab", "https://gitlab.com/")).toBe(
      false,
    );
  });
  ...
  it("[경계] self-hosted 또는 잘못된 URL은 client ID를 요구함", () => {
    expect(
      requiresExplicitOAuthClientId("gitLab", "https://gitlab.example.com"),
    ).toBe(true);
