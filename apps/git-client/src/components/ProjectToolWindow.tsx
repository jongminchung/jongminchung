    ...
    return [...entries].sort((left, right) => {
        ...
        if (sortKey === "type") {
            ...
            const rightExtension =
                right.kind === "tree"
                    ? ""
                    : (right.name.split(".").at(-1) ?? "");
            const typeOrder = leftExtension.localeCompare(
                rightExtension,
                undefined,
                {
                    numeric: true,
    ...
    useEffect(() => {
        ...
        for (const path of ordered) {
            const parentPath = path.includes("/")
                ...
                : "";
            if (path === "" || children.has(parentPath))
    ...
    const revealActiveFile = useCallback(async (): Promise<void> => {
        if (!activePath) {
            ...
            window.requestAnimationFrame(() =>
                document
                    .querySelector<HTMLElement>('[data-project-path=""]')
                    ?.focus(),
            );
            return;
        }
                        ...
                        <TooltipTrigger
                            render={
                                <Button
                                    ...
                                    aria-selected="false"
                                    onClick={() =>
                                        setScratchesExpanded((value) => !value)
                                    }
                                    onDoubleClick={(event) => {
