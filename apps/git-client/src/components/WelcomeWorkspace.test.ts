...
describe("WelcomeWorkspace", () => {
  it("matches the Rebased welcome project actions and category structure", () => {
    ...
    expect(markup).toContain('style="grid-template-columns:224px minmax(0, 1fr)"');
    ...
    expect(markup).toContain("Open existing project from disk or version control.");
    expect(markup).not.toContain("Open an existing project from disk or version control.");
    ...
    const labels = ["Projects", "Customize", "New Project", "Open", "Clone Repository"];
    let previous = -1;
