...
export function useGitBranchActions({
  ...
  const setUpstream = async (): Promise<void> => {
    ...
    const upstream = await dialog.input({
      ...
      initialValue: activeRef.upstream?.replace(/^refs\/remotes\//u, "") ?? "origin/",
      placeholder: "origin/main",
      confirmLabel: "Set Upstream",
    });
    if (!upstream) return;
    await run({ kind: "setUpstream", branch: activeRef.shortName, upstream });
  };
  ...
  const pushActiveTag = async (): Promise<void> => {
    ...
    await run({ kind: "pushTag", remote: remotes[0].name, name: activeRef.shortName });
  };
