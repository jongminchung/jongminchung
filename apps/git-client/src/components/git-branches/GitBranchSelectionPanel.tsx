...
function BranchAction({
  ...
  return (
    <Button
      className={cn("gap-1.5 min-h-[25px] px-1.5 text-xs text-muted-foreground")}
      disabled={disabled}
      onClick={() => void onRun()}
      size="default"
      type="button"
    ...
    <>
      {detailsOpen && (
        ...
        >
          ...
          <BranchAction
            disabled={!canCompare || !currentBranch || activeRef.current || busy}
            onRun={onCompare}
          >
            Compare
          </BranchAction>
          <BranchAction
            disabled={!currentBranch || activeRef.current || busy || !canOperate}
            onRun={onMerge}
          >
          ...
          {activeRef.kind === "tag" && (
            <BranchAction disabled={busy || remoteCount === 0} onRun={onPushTag}>
              Push Tag
            </BranchAction>
          )}
          ...
          <BranchAction disabled={activeRef.current || busy || !canOperate} onRun={onDelete}>
            Delete…
          </BranchAction>
        </div>
      )}
