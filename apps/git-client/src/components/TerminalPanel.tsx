...
export function TerminalPanel({
  ...
  const [menuRequest, setMenuRequest] = useState<TerminalMenuRequest | null>(null);
  ...
  useEffect(() => {
    ...
    const openInitialTerminal = async (): Promise<void> => {
      try {
        ...
        if (cancelled || terminalService.sessions(repositoryId).length > 0) return;
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        ...
        if (!cancelled) announce(error instanceof Error ? error.message : String(error));
      }
    };
    void load();
    return () => {
  ...
  const createLaunchTarget = useCallback(
    async (target: TerminalShellDescriptor | TerminalAgentDescriptor): Promise<void> => {
      if (fixture || !isElectronRuntime()) return;
      const launchTarget =
        target.kind === "shell"
    ...
    async (key: string): Promise<void> => {
      ...
      window.requestAnimationFrame(() => {
        if (nextKey !== null) xterm.current?.focus();
          ...
          root.current?.querySelector<HTMLButtonElement>('button[aria-label="New Tab"]')?.focus();
      });
    },
    [activeKey, sessions],
  );
  ...
  useEffect(() => {
    const navigateTab = (event: Event): void => {
      ...
      const keys = terminalService.sessions(repositoryId).map((session) => session.key);
      if (keys.length < 2) return;
      const currentIndex = Math.max(0, keys.indexOf(activeKeyRef.current ?? ""));
    ...
    return () => {
      window.removeEventListener("git-client:terminal-tab-navigate", navigateTab);
      window.removeEventListener("git-client:terminal-tab-close", closeActiveTab);
    };
  }, [repositoryId]);
  ...
  const restoreMenuFocus = useCallback((): void => {
    ...
    else if (menuRequest?.origin === "toolbar") optionsButton.current?.focus();
  ...
  return (
    ...
    >
      ...
      {(menuRequest?.kind === "shells" || menuRequest?.kind === "agents") && (
        <TerminalLaunchTargetMenu
          items={menuRequest.items}
          label={menuRequest.kind === "shells" ? "New Predefined Session" : "AI Agents"}
          onClose={closeMenu}
          onRestoreFocus={restoreMenuFocus}
