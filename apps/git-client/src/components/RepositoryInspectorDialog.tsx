...
export function RepositoryInspectorDialog({
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      ...
      try {
        if (tab === "tree")
          setTree(await loadTree(revision, treePath || undefined));
        ...
        } else if (tab === "history" && path)
          setHistory(await loadFileHistory(path));
        else if (path) {
          setBlame(
            await loadBlame(
              path,
              source.kind === "workingTree" ? undefined : revision,
            ),
          );
        }
      } catch (reason) {
        ...
        <nav className="flex min-w-0 items-center gap-2 border-b border-border bg-muted px-3 py-1.5">
          ...
          {tab === "tree" ? (
            <>
              <Button
                onClick={() =>
                  setTreePath(treePath.split("/").slice(0, -1).join("/"))
                }
                type="button"
                disabled={!treePath}
              ...
              {tree.map((entry) => (
                <Button
                  ...
                  onDoubleClick={() => {
                    if (entry.kind === "tree") {
                      setTreePath(
                        [treePath, entry.path].filter(Boolean).join("/"),
                      );
                    } else {
                ...
                >
                  <Icon
                    name={entry.kind === "tree" ? "folder" : "file"}
                    size={14}
                  />
            ...
            (!content ? (
              ...
              <figure className="imageViewer">
                ...
                <figcaption>
                  ...
                  <span>
                    {preview.preview.mimeType} ·{" "}
                    {preview.preview.sizeBytes.toLocaleString()} bytes
                  </span>
                  {source.kind === "workingTree" && (
                    <Button
                      disabled={readOnly}
                      onClick={() =>
                        void openWorkingTreeFile(preview.preview.path)
                      }
                      type="button"
                      className={cn("h-7 px-2.5")}
                      variant="outline"
            ...
            <div className="historyList">
              {history.map((commit) => (
                <article key={commit.oid}>
                  ...
                  <time>
                    {new Date(commit.authoredAt * 1000).toLocaleString()}
                  </time>
                </article>
              ))}
            </div>
