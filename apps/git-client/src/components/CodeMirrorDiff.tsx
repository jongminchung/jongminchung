...
class LineActionMarker extends GutterMarker {
    ...
    override eq(other: GutterMarker): boolean {
        return (
            other instanceof LineActionMarker &&
...
function lineActionGutter(
    ...
    for (const action of actions.filter(
        (action) => side === "unified" || action.side === side,
    )) {
        const lineNumber =
            side === "unified" ? action.unifiedLineNumber : action.lineNumber;
...
export default function CodeMirrorDiff({
        ...
        const render = async (): Promise<void> => {
            ...
            if (viewMode === "split") {
                ...
                handle.current = {
                    ...
                    secondary: merge.a,
                    destroy: () => {
                        merge.a.scrollDOM.removeEventListener(
                            "scroll",
                            syncFromA,
                        );
                        merge.b.scrollDOM.removeEventListener(
                            "scroll",
                            syncFromB,
                        );
                        merge.destroy();
                ...
                onStatisticsChange({
                    ...
                    matches:
                        countMatches(before, searchQuery) +
    ...
    useEffect(() => {
        ...
        const current = handle.current;
        if (!current) return;
        const beforeMatches = current.secondary
            ? matchOffsets(before, searchQuery)
            : [];
    ...
    useEffect(() => {
        ...
        if (!view) return;
        (differenceNavigation.direction === 1
            ? goToNextChunk
