...
const XtermSurface = forwardRef<XtermSurfaceHandle, XtermSurfaceProps>(
  function XtermSurface({ sessionKey, onAction, onContextMenu }, forwardedRef) {
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        execute: (
          action: TerminalSurfaceActionId,
          ...
          return TerminalActionExecutor.of(
            terminal,
            browserClipboard(),
          ).execute(action);
        },
    ...
    useEffect(() => {
      ...
      const terminal = new Terminal({
        ...
        cursorBlink: true,
        fontFamily:
          '"JetBrains Mono Rebased", "JetBrains Mono", Menlo, monospace',
        fontSize: 13,
        lineHeight: 1,
        scrollback: 10_000,
      ...
      const unsubscribe = terminalService.subscribeEvents(
        sessionKey,
        renderEvent,
      );
      const input = terminal.onData(
        (data) => void terminalService.write(sessionKey, data),
      );
