...
function contentDescription(
    content: FileContent | null | undefined,
): string | null {
    if (!content || content.kind === "text" || content.kind === "missing")
        return null;
...
export function DiffViewer({
    ...
    const contentUnavailable =
        contentDescription(afterContent) ?? contentDescription(beforeContent);
    ...
    const hasImagePreview =
        imageFrom(beforePreview) !== null || imageFrom(afterPreview) !== null;
    const matchCount = statistics.matches;
    const updateStatistics = useCallback(
        (next: {
            readonly differences: number;
            readonly matches: number;
        }): void => setStatistics(next),
        [],
    );
    ...
    return (
        ...
        >
                    ...
                    content={
                        <div className="grid gap-2 p-1">
                            ...
                            <label>
                                <Checkbox
                                    checked={
                                        preferences.whitespace === "ignoreAll"
                                    }
                ...
                {mode !== "readOnly" && (
                    <>
                        <Select
                            ...
                            onValueChange={(value) =>
                                value !== null && setHunkIndex(Number(value))
                            }
                            value={String(
                                Math.min(
                                    hunkIndex,
                                    Math.max(0, document.hunks.length - 1),
                                ),
                            )}
                        >
                            <SelectTrigger
                        ...
                        <Button
                            ...
                            onClick={() =>
                                void applyHunk(true, mode === "unstage")
                            }
                            type="button"
                            className={cn("h-7 px-2.5")}
                            variant="outline"
            ...
            <div
                aria-label={
                    file ? `Diff content for ${file.path}` : "Diff content"
                }
            ...
            >
                {loading ? (
                    ...
                    >
                        ...
                        <div>
                            <section>
                                <small>Before</small>
                                <code>
                                    {submoduleDiff.beforeOid ?? "Not present"}
                                </code>
                                {submoduleDiff.beforeSubject && (
                                    <span>{submoduleDiff.beforeSubject}</span>
                                )}
                            </section>
                            ...
                            <section>
                                <small>After</small>
                                <code>
                                    {submoduleDiff.afterOid ?? "Not present"}
                                </code>
                                {submoduleDiff.afterSubject && (
                                    <span>{submoduleDiff.afterSubject}</span>
                                )}
                            </section>
                        </div>
                        {submoduleDiff.ahead !== null &&
                        submoduleDiff.behind !== null ? (
                            <p>
                                {submoduleDiff.ahead} ahead ·{" "}
                                {submoduleDiff.behind} behind
                            </p>
                        ) : (
                            <p>
                                Commit relationship is unavailable because one
                                or both objects are not present locally.
                            </p>
                        )}
                    </div>
                ) : unsupportedReason ? (
                    ...
                    >
                        <Icon
                            name={file.submodule ? "worktree" : "warning"}
                            size={24}
                        />
                    ...
                    >
                        <CodeMirrorDiff
                            ...
                            synchronizedScroll={
                                split && preferences.synchronizedScroll
                            }
                    ...
                    >
                        ...
                        <p>
                            {contentUnavailable ??
                                "The before/after file content could not be loaded."}
                        </p>
                        {onOpenExternally && (
                            <Button
                                onClick={() => void onOpenExternally()}
