...
export function useCommitLogState({
  ...
  const queryPattern = useMemo<RegExp | null>(() => {
    ...
    try {
      return new RegExp(normalizedQuery, filterState.matchCase ? "" : "i");
    } catch {
      return null;
    }
        ...
        (commit) =>
          (normalizedQuery.length === 0 ||
            (filterState.regex
              ...
              : (filterState.matchCase ? commit.subject : commit.subject.toLowerCase()).includes(
                  filterState.matchCase ? normalizedQuery : normalizedQuery.toLowerCase(),
                ) ||
                (filterState.matchCase ? commit.oid : commit.oid.toLowerCase()).startsWith(
                  filterState.matchCase ? normalizedQuery : normalizedQuery.toLowerCase(),
                ))) &&
          (filterState.author === "all" || commit.author === filterState.author),
      ),
    [
      commits,
      filterState.author,
  ...
  const setFilter = useCallback(
    <Key extends keyof CommitLogFilterState>(key: Key, value: CommitLogFilterState[Key]): void => {
      setFilterState((current) => ({ ...current, [key]: value }));
    },
    [],
  );
  const setViewOption = useCallback(
    <Key extends keyof CommitLogViewOptions>(key: Key, value: CommitLogViewOptions[Key]): void => {
      setViewOptions((current) => ({ ...current, [key]: value }));
    },
    [],
  );
  const resetFilters = useCallback((): void => setFilterState(INITIAL_FILTERS), []);
