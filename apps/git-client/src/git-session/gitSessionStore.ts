...
export function createGitSessionStore({
  ...
  const refreshCoordinator =
    ...
    RefreshCoordinator.of(
      (repositoryId, invalidations) => coordinatorCallbacks.refresh(repositoryId, invalidations),
      (repositoryId, error) => coordinatorCallbacks.handleError(repositoryId, error),
    );
  const initialRepositoryId =
    initialWorkspace.activeTab.kind === "repository"
      ? initialWorkspace.activeTab.repositoryId
      : null;
  const runtime = injectedRuntime ?? createGitSessionRuntime(initialRepositoryId);
