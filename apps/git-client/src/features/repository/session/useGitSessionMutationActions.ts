...
import type { GitSessionMutationPort } from "../../../application/git-session/ports/GitSessionBackend";
import type {
  RepositorySession,
  WorkspaceState,
...
export function useGitSessionMutationActions({
  ...
  const createShelf = useCallback(
    async (message: string, paths: readonly string[]): Promise<void> => {
      const snapshot = activeSnapshot();
      const shelf = await mutationPort.createShelf(async () => {
        repositoryAccessPolicy.assert(snapshot.id, "gitMutation");
        return gitBridge.createShelf(snapshot.id, message, paths);
      });
      ...
      setState((current) =>
        updateRepositorySession(current, snapshot.id, (session) => ({
          ...
          shelves: [
            shelf,
            ...session.shelves.filter((item) => item.id !== shelf.id),
          ],
        })),
      );
    ...
    [activeSnapshot, mutationPort, refreshCoordinator, gitBridge, setState],
  );

  const applyShelf = useCallback(
    async (shelfId: string, dropAfterApply: boolean): Promise<void> => {
      const snapshot = activeSnapshot();
      const applied = await mutationPort.applyShelf(async () => {
        repositoryAccessPolicy.assert(snapshot.id, "gitMutation");
  ...
  const abortOperation = useCallback(
    async (operation: AbortableOperation): Promise<void> => {
      ...
      const recoveryOperation =
        operation === "cherryPick" ? "cherry-pick" : operation;
      const recoveryEntries = await mutationPort.recoveryEntries(() =>
        gitBridge.listRecoveryEntries(snapshot.id),
      );
  ...
  const saveConflictResult = useCallback(
    async (path: string, result: string, stage: boolean): Promise<void> => {
      await mutateAndRefresh(
        (repositoryId) =>
          gitBridge.writeConflictResult(repositoryId, path, result, stage),
        ["status", "operation"],
      );
    },
    ...
    async (
      ...
      if (activeSession) {
        ...
        refreshCoordinator.invalidate(
          repositoryId,
          operationPolicyFor(operation).invalidations,
        );
