...
export function useGitSessionController() {
    ...
    const gitConsoleEntries = useGitSessionStore(
        (store) => store.consoleEntries,
    );
    ...
    useEffect(() => {
        const load = async (): Promise<void> => {
            const initialWorkspace =
                await backend.repository.initialWorkspace();
    ...
    const loadHistoryRewritePreview = useCallback(
        async (fromRevision: string): Promise<HistoryRewritePreview> => {
            ...
            return backend.history.loadHistoryRewritePreview(
                ...
                () =>
                    gitBridge.loadHistoryRewritePreview(
                        snapshot.id,
                        fromRevision,
                    ),
            );
    ...
    const capabilities = {
        ...
        repository: {
            ...
            remotes:
                (activeSession ?? managementSession)?.remotes ?? EMPTY_ARRAY,
            worktrees:
