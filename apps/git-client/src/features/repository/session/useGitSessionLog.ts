    ...
    const loadLog = useCallback(
        (
            ...
            historyPort.loadLog(async () => {
                ...
                const generation =
                    (runtime.logGenerations.get(snapshot.id) ?? 0) + 1;
                runtime.logGenerations.set(snapshot.id, generation);
                const previousRequest = runtime.activeLogRequests.get(
                    snapshot.id,
                );
                if (previousRequest)
                    await cancelRequests(gitBridge, [previousRequest]);
                setState((current) =>
                    updateRepositorySession(
                        current,
                        snapshot.id,
                ...
                try {
                    const output = await runRequest(
                        createLogRequest(snapshot.id, selection, skip),
                        {
                            ...
                            onStarted: (requestId) => {
                                if (
                                    runtime.logGenerations.get(snapshot.id) ===
                                    generation
                                ) {
                                    runtime.activeLogRequests.set(
                                        snapshot.id,
                                        requestId,
                                    );
                                } else {
