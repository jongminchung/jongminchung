...
export function createRepositoryWorkspaceFeatureModel({
  ...
  const hasEditorTabs = toolWindows.logOpen || editor.inspectorTabs.length > 0;
  ...
  return {
    ...
    vcs: {
      ...
      sessionCreateLocalHistoryPatch: session.queries.createLocalHistoryPatch,
      ...
      sessionReadLocalHistoryActivity: session.queries.readLocalHistoryActivity,
    ...
    overlays: {
      ...
      sessionRemotes: session.repository.remotes,
      sessionResolveBinaryConflict: session.mutations.resolveBinaryConflict,
