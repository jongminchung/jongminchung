  ...
  const chooseBookmarkMnemonic = useCallback(
    async (
      ...
      const conflict = allLineBookmarks(bookmarks).find(
        (bookmark) =>
          bookmark.id !== target.bookmarkId && bookmark.mnemonic === mnemonic,
      );
      if (conflict) {
        const accepted = await dialog.confirm({
          ...
          impact:
            "The existing bookmark will remain, but its mnemonic will be removed.",
      ...
      setBookmarks((current) => {
        ...
        const bookmarkId =
          bookmarkAt(withBookmark, target.location)?.id ?? target.bookmarkId;
