...
describe("RepositoryWatchSession", () => {
  it("retries after an initial subscription failure", async () => {
    ...
    await expect(session.ensure("repository-1", subscribe)).rejects.toThrow("watch unavailable");
    expect(session.isWatched("repository-1")).toBe(false);

    await expect(session.ensure("repository-1", subscribe)).resolves.toBeUndefined();
