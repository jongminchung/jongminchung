...
export async function selectPatchExportPath(defaultPath: string): Promise<string | null> {
...
export async function setElectronFullScreen(value: boolean): Promise<void> {
  const api = electronApi();
  if (api !== null) {
...
export function loadCommandLineLauncherInfo(): Promise<CommandLineLauncherInfo> {
  const api = electronApi();
  if (api === null) {
    return Promise.reject(new Error("Command-line launcher information requires Electron."));
  }
  return api.runtime.getCommandLineLauncherInfo();
}

export async function getElectronFullScreen(): Promise<boolean> {
  const api = electronApi();
  if (api !== null) return api.window.getFullScreen();
  return document.fullscreenElement !== null;
}

export async function relaunchElectronApp(invalidateCaches: boolean): Promise<void> {
  const api = electronApi();
  if (api !== null) {
...
export function readDiagnosticConfiguration(kind: DiagnosticConfigurationKind): Promise<string> {
...
export function listLeftoverDirectories(): Promise<readonly DiagnosticLeftoverDirectory[]> {
  return requireDiagnosticsApi().listLeftoverDirectories();
}

export function deleteLeftoverDirectories(ids: readonly string[]): Promise<readonly string[]> {
...
export async function exportPatchText(request: PatchTextExportRequest): Promise<boolean> {
