...
const RevisionSchema = z
  ...
  .min(1)
  .max(512)
  .refine(isSafeRevision, "Revision must be a non-option value without unsafe characters");
...
const SearchTextSchema = z
  ...
  .min(1)
  .max(1_024)
  .refine((value) => !value.includes("\0"), "Search text must not contain null bytes");
...
export const GitDomainRequestSchema = z
  .discriminatedUnion("kind", [...queryVariants(domainIdentity), operationVariant(domainIdentity)])
  .readonly();
