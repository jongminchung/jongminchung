...
describe("desktop stream contract", () => {
  it("accepts the current handshake and rejects version skew", () => {
    expect(DesktopStreamConnectSchema.parse({ version: DESKTOP_STREAM_PROTOCOL_VERSION })).toEqual({
      version: 1,
    });
  ...
  it("accepts a typed Git barrier and rejects extra fields", () => {
    ...
    expect(() => DesktopStreamEnvelopeSchema.parse({ ...barrier, channel: "legacy" })).toThrow();
  });
});
