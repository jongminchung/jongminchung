...
import { GitRepositoryServiceRequestSchema, GitRepositoryServiceResultSchema } from "./git-utility";
...
describe("Git repository service contracts", () => {
  ...
  it("rejects unknown fields, unsafe revisions, null bytes, and invalid result objects", () => {
    ...
    for (const request of invalidRequests) {
      expect(GitRepositoryServiceRequestSchema.safeParse(request).success).toBe(false);
    }
    expect(
  ...
  it("defines all 17 bounded special service request and result variants", () => {
    ...
    for (const request of requests) {
      expect(GitRepositoryServiceRequestSchema.safeParse(request).success).toBe(true);
    }
    for (const result of results) {
      expect(GitRepositoryServiceResultSchema.safeParse(result).success).toBe(true);
    }
  });
        ...
        steps: [
          {
            ...
            operations: [{ kind: "checkout", target: "main", force: false }],
          },
        ],
      },
    ];
