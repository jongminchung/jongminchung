...
describe("GitDomainQueryRequestSchema", () => {
  it.each([
    ...
    expect(GitQueryRequestSchema.parse(transportRequest)).toEqual(transportRequest);
    expect(
  ...
  it.each(VALID_GIT_OPERATIONS)(
    ...
    (operation) => {
      ...
      expect(GitDomainRequestSchema.parse(domainRequest)).toEqual(domainRequest);
      expect(GitExecutionRequestSchema.parse(transportRequest)).toEqual(transportRequest);
      expect(
        MainToGitUtilityMessageSchema.parse({
          kind: "query",
          correlationId,
  ...
  it("rejects structural abuse before command construction", () => {
    ...
    for (const request of invalid) {
      expect(GitDomainQueryRequestSchema.safeParse(request).success).toBe(false);
    }
  });
});
