...
export async function terminateSmokeChild(
  ...
  child.kill("SIGKILL");
  if (await waitForChildClose(child, forceTerminationGraceMs)) return "SIGKILL";
...
export function detectSmokeLogFailure(output: any) {
  ...
  return (
    FAILURE_PATTERNS.find(({ pattern }: any) => pattern.test(output))?.label ??
    null
  );
}
  ...
  return new Promise((resolveHandshake: any, rejectHandshake: any) => {
    ...
    socket.addEventListener("message", (event: any) => {
      ...
      if (
        ...
        finish(
          new Error("Renderer/preload API handshake evaluation failed"),
          false,
        );
        return;
      }
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      for (const target of ordered) {
        if (await evaluateHandshake(target.webSocketDebuggerUrl, signal))
          return;
      }
    } catch (error) {
...
export async function smokeElectronPackage(inputPath: any) {
  ...
  return new Promise((resolveResult: any, rejectResult: any) => {
    ...
    const maybeStop = () => {
      ...
      if (isSmokeStartupReady(output, handshakeComplete)) stopAndValidate(true);
    };
    ...
    timeout = setTimeout(() => {
      ...
      handshakeController.abort();
      void terminateSmokeChild(child).then(
        (cleanup: any) => finish(createSmokeTimeoutFailure(output, cleanup)),
        (error: any) =>
          finish(
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === scriptPath) {
  ...
  const requestedPath =
    process.argv[2] === undefined ? defaultAppPath : resolve(process.argv[2]);
