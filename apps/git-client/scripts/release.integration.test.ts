...
describe("Electron release build contract", () => {
    ...
    it("uses deterministic production and visibly ad-hoc ARM64 artifact names", () => {
        ...
        expect(
            createReleaseArtifactNames("1.2.3", RELEASE_MODES.localAdHoc),
        ).toEqual({
            checksum: "Git-Client_1.2.3_macos_arm64_adhoc.dmg.sha256",
            dmg: "Git-Client_1.2.3_macos_arm64_adhoc.dmg",
            provenance:
                "Git-Client_1.2.3_macos_arm64_adhoc.dmg.provenance.json",
        });
    });
    ...
    it("requires a clean main branch exactly matching fetched origin/main", async () => {
        ...
        await expect(
            verifyReleaseSource("/workspace", { runCommand }),
        ).resolves.toEqual({
            ...
            async (_command: string, arguments_: readonly string[]) => {
                ...
                if (key === "status --porcelain=v1 --untracked-files=all") {
                    return {
                        code: 0,
                        stderr: "",
                        stdout: " M apps/git-client/src/main.tsx\n",
                    };
                }
                return { code: 0, stderr: "", stdout: "" };
            },
    ...
    it("updates a stale origin/main tracking ref before comparing release source", async () => {
        ...
        try {
            ...
            const staleTrackingSha = git(checkout, [
                "rev-parse",
            ...
            ).toBe(staleTrackingSha);

            await expect(verifyReleaseSource(checkout)).rejects.toThrow(
                "must exactly match origin/main",
            );
            expect(
                git(checkout, ["rev-parse", "refs/remotes/origin/main"]),
            ).not.toBe(staleTrackingSha);
        } finally {
    ...
    it("accepts only the requested app version and a single arm64 executable", () => {
        ...
        expect(() =>
            assertReleaseBundleMetadata("1.2.4", "arm64", "1.2.3"),
        ).toThrow("version mismatch");
        expect(() =>
            assertReleaseBundleMetadata("1.2.3", "x86_64 arm64", "1.2.3"),
        ).toThrow("only arm64");
    });
    ...
    it("requires Developer ID and notarization configuration for production", () => {
        expect(
            ...
            keychainProfile:
                productionEnvironment.GIT_CLIENT_NOTARY_KEYCHAIN_PROFILE,
            mode: RELEASE_MODES.production,
        });
        expect(() =>
            resolveReleaseSecurity(RELEASE_MODES.production, {}),
        ).toThrow("GIT_CLIENT_CODESIGN_IDENTITY");
        expect(() =>
            resolveReleaseSecurity(RELEASE_MODES.production, {
                GIT_CLIENT_CODESIGN_IDENTITY:
                    productionEnvironment.GIT_CLIENT_CODESIGN_IDENTITY,
            }),
    ...
    it("parses production by default and requires an explicit local ad-hoc flag", () => {
        ...
        expect(() => findReleaseArguments(["--unknown", "1.2.3"])).toThrow(
            "Unknown release argument",
        );
        expect(() =>
            findReleaseArguments(["--local-ad-hoc", "--local-ad-hoc", "1.2.3"]),
        ).toThrow("Duplicate");
    });
    ...
    it("stages a DMG with a streaming SHA-256 manifest", async () => {
        ...
        try {
            ...
            expect(artifacts).toMatchObject({
                checksum: join(
                    output,
                    "Git-Client_1.2.3_macos_arm64.dmg.sha256",
                ),
                dmg: join(output, "Git-Client_1.2.3_macos_arm64.dmg"),
                mode: RELEASE_MODES.production,
                provenance: join(
                    output,
                    "Git-Client_1.2.3_macos_arm64.dmg.provenance.json",
                ),
                sourceSha: SOURCE_SHA,
            });
    ...
    it("discovers exactly one nested Forge app and rejects intermediate DMGs", async () => {
        ...
        try {
            ...
            await expect(
                discoverForgeOutputs(join(appRoot, "out")),
            ).resolves.toEqual({
                app: join(
                    appRoot,
                    "out",
            ...
            await expect(
                discoverForgeOutputs(join(appRoot, "out")),
            ).rejects.toThrow("one Electron app and no DMG");
    ...
    it("verifies the configured Developer ID identity from mocked keychain output", async () => {
        ...
        await expect(
            assertDeveloperIdIdentityAvailable(identity, runCommand),
    ...
    it("runs package, strict codesign, Gatekeeper, and notarization-ticket validation", async () => {
        ...
        const runCommand = vi.fn(
            async (command: string, arguments_: readonly string[]) => {
                ...
                if (command.endsWith("plutil"))
                    return { code: 0, stderr: "", stdout: "1.2.3\n" };
        ...
        const verifyPackage = vi
            .fn()
            .mockResolvedValue({ electronVersion: "43.3.0" });

        try {
            await mkdir(join(appPath, "Contents", "MacOS"), {
                recursive: true,
            });
            ...
            expect(
                calls.some((call) =>
                    call.includes("codesign --verify --deep --strict"),
                ),
            ).toBe(true);
            expect(
                calls.some((call) =>
                    call.includes("spctl --assess --type execute"),
                ),
            ).toBe(true);
            expect(
                calls.some((call) => call.includes("xcrun stapler validate")),
            ).toBe(true);
        } finally {
            await rm(directory, { force: true, recursive: true });
        }
    ...
    it("executes the production Electron pipeline in order with mocked subprocesses", async () => {
        ...
        try {
            ...
            expect(
                calls.map(({ command, arguments: arguments_ }) => [
                    command,
                    arguments_,
                ]),
            ).toEqual([
            ...
            expect(artifacts.dmg).toBe(
                join(
                    appRoot,
                    "release-artifacts",
                    "Git-Client_1.2.3_macos_arm64.dmg",
                ),
            );
