...
describe("전자 릴리스 빌드 계약", () => {
  ...
  it("[성공] 릴리스를 비교하기 전에 오래된 원본/기본 추적 참조를 업데이트함", async () => {
    ...
    try {
      ...
      expect(git(checkout, ["rev-parse", "refs/remotes/origin/main"])).not.toBe(
        staleTrackingSha,
      );
    } finally {
  ...
  it("[성공] 제작을 위한 개발자 ID 및 공증 구성이 필요함", () => {
    expect(
      resolveReleaseSecurity(RELEASE_MODES.production, productionEnvironment),
    ).toEqual({
  ...
  it("[성공]패키지, 협공 설계, 메달키퍼 및 공증 입증 검증 실행함", async () => {
    ...
    try {
      ...
      expect(
        calls.some((call) => call.includes("spctl --assess --type execute")),
      ).toBe(true);
      expect(
