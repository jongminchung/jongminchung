...
const FAILURE_PATTERNS = Object.freeze([
  ...
  Object.freeze({
    ...
    pattern: /\b(?:cannot find module|module not found|(?:err_)?module_not_found)\b/iu,
  }),
  Object.freeze({
...
function appendLog(current, chunk) {
  ...
  return next.length <= MAX_LOG_CHARACTERS ? next : next.slice(-MAX_LOG_CHARACTERS);
}

export function detectSmokeLogFailure(output) {
  if (typeof output !== "string") throw new TypeError("Smoke output must be a string");
  return FAILURE_PATTERNS.find(({ pattern }) => pattern.test(output))?.label ?? null;
}
...
export function validateSmokeOutcome({
  ...
  if (!output.includes(READY_SENTINEL)) {
    throw new Error(`Packaged app exited before its renderer loaded\n${output}`);
  }
...
function abortError() {
  return new DOMException("Renderer/preload handshake was cancelled", "AbortError");
}
...
async function cdpTargets(port, signal) {
  ...
  if (!Array.isArray(value)) throw new Error("CDP target discovery returned an invalid response");
  return value.flatMap((candidate) => {
    if (typeof candidate !== "object" || candidate === null) return [];
    const type = Reflect.get(candidate, "type");
    const url = Reflect.get(candidate, "url");
    const webSocketDebuggerUrl = Reflect.get(candidate, "webSocketDebuggerUrl");
    if (type !== "page" || typeof url !== "string" || typeof webSocketDebuggerUrl !== "string") {
      return [];
    }
  ...
  return new Promise((resolveHandshake, rejectHandshake) => {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (message.error !== undefined || message.result?.exceptionDetails !== undefined) {
        finish(new Error("Renderer/preload API handshake evaluation failed"), false);
        return;
      }
      finish(null, message.result?.result?.value === true);
    });
    socket.addEventListener("error", () => finish(new Error("CDP WebSocket failed"), false), {
      once: true,
    });
    socket.addEventListener(
      "close",
      () => finish(new Error("CDP WebSocket closed before the handshake"), false),
      { once: true },
    );
  });
}
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      for (const target of ordered) {
        if (await evaluateHandshake(target.webSocketDebuggerUrl, signal)) return;
      }
    } catch (error) {
...
export async function smokeElectronPackage(inputPath) {
  if (process.platform !== "darwin") {
    throw new Error("Packaged Electron smoke testing currently supports macOS only");
  }
  if (typeof inputPath !== "string" || !isAbsolute(inputPath) || !inputPath.endsWith(".app")) {
    throw new Error("Packaged Electron smoke testing requires an absolute .app path");
  }
  ...
  return new Promise((resolveResult, rejectResult) => {
    ...
    const handshake = probeRendererPreloadApi(remoteDebuggingPort, handshakeController.signal)
      .then(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      child.kill("SIGTERM");
      finish(
        new Error(`Packaged app did not finish its startup smoke test within ${TIMEOUT_MS}ms`),
      );
    }, TIMEOUT_MS);

    const capture = (chunk) => {
      const text = chunk.toString("utf8");
      observedFailure ??= detectSmokeLogFailure(`${output.slice(-4_096)}${text}`);
