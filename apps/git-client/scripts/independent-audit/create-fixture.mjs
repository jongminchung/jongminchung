...
export async function createAuditFixture(rootPath) {
  ...
  await writeFile(join(seedPath, "README.md"), "# Rebased 1.1.11 audit fixture\n", "utf8");
  await mkdir(join(seedPath, "src"));
  await writeFile(join(seedPath, "src", "index.ts"), 'export const version = "1.0.0";\n', "utf8");
  await runGit(seedPath, ["add", "."]);
  await commit(seedPath, "fixture: initial state", "2020-01-02T03:04:05Z");

  await writeFile(join(seedPath, "src", "shared.ts"), "export const shared = true;\n", "utf8");
  ...
  await writeFile(join(seedPath, "src", "topic.ts"), "export const topic = true;\n", "utf8");
  ...
  await runGit(seedPath, ["push", "--set-upstream", "origin", "main", "feature/topic"]);
  await runGit(seedPath, ["push", "origin", "v1.0.0"]);

  for (const repositoryPath of [rebasedPath, gitClientPath]) {
    await runGit(root, ["clone", "--origin", "origin", remotePath, repositoryPath]);
...
async function configureRepository(repositoryPath) {
  ...
  for (const [key, value] of entries) await runGit(repositoryPath, ["config", key, value]);
}

async function commit(repositoryPath, message, date) {
  await runGit(repositoryPath, ["commit", "--no-gpg-sign", "-m", message], {
    env: { ...IDENTITY_ENV, GIT_AUTHOR_DATE: date, GIT_COMMITTER_DATE: date },
  });
}
...
async function seedWorkingState(repositoryPath) {
  ...
  await writeFile(join(repositoryPath, "notes.txt"), "untracked audit note\n", "utf8");
}

function parseRootArgument(args) {
  if (args.length !== 2 || args[0] !== "--root" || !isAbsolute(args[1])) {
    throw new Error("Usage: create-fixture.mjs --root /absolute/empty/path");
  }
  return args[1];
}
...
if (isEntryPoint) {
  try {
    const result = await createAuditFixture(parseRootArgument(process.argv.slice(2)));
