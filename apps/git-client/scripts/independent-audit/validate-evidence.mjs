...
async function listEvidenceFiles(root, directory = root) {
    ...
    for (const entry of entries) {
        ...
        if (stat.isSymbolicLink())
            throw new Error(
                `Evidence must not contain symlinks: ${relativePath}`,
            );
        if (stat.isDirectory()) {
...
function parseManifest(contents) {
    ...
    for (const line of lines) {
        ...
        if (path === manifestName)
            throw new Error("SHA256SUMS must not hash itself");
        if (entries.has(path))
            throw new Error(`Duplicate evidence manifest entry: ${path}`);
...
function assertMetadata(metadata, manifestPaths) {
    ...
    if (metadata?.environment?.architecture !== "arm64") {
        throw new Error(
            "Evidence metadata must identify the arm64 environment",
        );
    }
    ...
    for (const path of screenshotPaths) {
        if (!path.startsWith("screenshots/") || !manifestPaths.has(path)) {
            throw new Error(
                `Metadata references an invalid screenshot: ${path}`,
            );
        }
    }
...
export async function validateEvidenceBundle(root = checkedInEvidenceRoot) {
    ...
    const metadata = JSON.parse(
        await readFile(resolve(root, "evidence.json"), "utf8"),
    );
    assertMetadata(metadata, new Set(manifest.keys()));

    return {
        files: files.length,
        applicationVersion: metadata.application.version,
    };
}
