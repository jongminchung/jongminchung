...
export async function captureGitState(repositoryPath) {
  ...
  const [headOid, symbolicHead, refs, porcelainV2, cachedDiff, workingDiff, stash, remotes] =
    await Promise.all([
      gitText(repository, ["rev-parse", "HEAD"]),
      runGit(repository, ["symbolic-ref", "--quiet", "HEAD"], { acceptedExitCodes: [0, 1] }),
      gitText(repository, ["for-each-ref", "--format=%(objectname)%00%(refname)"]),
      gitText(repository, ["status", "--porcelain=v2", "--branch", "-z"]),
      gitText(repository, ["diff", "--binary", "--no-ext-diff", "--no-renames", "--cached"]),
      gitText(repository, ["diff", "--binary", "--no-ext-diff", "--no-renames"]),
  ...
  const remoteRefs = await Promise.all(
    remoteNames.map(async (remote) =>
      Object.freeze({
        remote,
        refs: parsePairs(await gitText(repository, ["ls-remote", "--refs", remote]), "\t"),
      }),
    ),
  );
  ...
  return Object.freeze({
    ...
    head: Object.freeze({
      oid: headOid.trim(),
      ref: symbolicHead.exitCode === 0 ? symbolicHead.stdout.trim() : null,
    }),
    ...
    stash: lines(stash).map((line) => {
      ...
      if (!oid || !selector || subject === undefined) {
        throw new Error(`Could not parse stash entry: ${JSON.stringify(line)}`);
      }
...
export function compareGitStates(left, right) {
  ...
  const differences = sections.filter(
    (section) => JSON.stringify(left[section]) !== JSON.stringify(right[section]),
  );
  return Object.freeze({
    ...
    lines(value)
      .map((line) => {
        ...
        if (separatorIndex < 1 || separatorIndex === line.length - 1) {
          throw new Error(`Could not parse Git ref: ${JSON.stringify(line)}`);
        }
        return Object.freeze({
...
function parseCliArguments(args) {
  ...
  if (args[0] === "compare" && args.length === 3 && isAbsolute(args[1]) && isAbsolute(args[2])) {
    return Object.freeze({ command: "compare", left: args[1], right: args[2] });
  }
  throw new Error(
...
if (isEntryPoint) {
  try {
    ...
    if (input.command === "snapshot") {
      process.stdout.write(`${JSON.stringify(await captureGitState(input.repository), null, 2)}\n`);
    } else {
