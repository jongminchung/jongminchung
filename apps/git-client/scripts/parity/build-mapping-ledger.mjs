...
const OBSERVED_RUNTIME_MAPPINGS = Object.freeze([
  {
    ...
    evidence: [
      {
        accessibility: "runtime/workbench/log/golden-1.1.8/reference.ax.json",
        fingerprint: {
  ...
  },
  {
    ...
    evidence: [
      {
        accessibility: "runtime/terminal/local/golden-1.1.8/reference-empty.ax.json",
        fingerprint: {
          accessibleName: "Local Tool Window",
          role: "AXGroup",
          surface: "terminal.local.empty",
        },
        screenshot: "runtime/terminal/local/golden-1.1.8/reference-empty.png",
      },
      {
        accessibility: "runtime/terminal/local/golden-1.1.8/reference-pwd.ax.json",
        fingerprint: {
          accessibleName: "Local Tool Window",
          role: "AXGroup",
        ...
        screenshot: "runtime/terminal/local/golden-1.1.8/reference-pwd.png",
      },
    ],
    mappingBasis:
...
function sourceLocations(value, locations = new Map()) {
  ...
  if (typeof value.path === "string" && Number.isInteger(value.line) && value.line > 0) {
    const location = {
...
function addObligation(
  obligations,
  { stableId, kind, sourceFile, record, value, details, occurrenceWeight = 1 },
) {
  ...
  if (current.kind !== kind) {
    throw new Error(`Stable ID ${stableId} is shared by ${current.kind} and ${kind}`);
  }
...
function addRecordList(obligations, records, { kind, sourceFile, recordName, details }) {
    ...
    for (const action of keymap.actions) {
      ...
      for (const shortcut of action.shortcuts) {
        addObligation(obligations, {
          ...
          sourceFile: "source/keymaps.json",
          stableId: shortcutIdentity(keymap.name, action.id, shortcut),
    ...
    addObligation(obligations, {
      details: {
        ...
        shortcut: Object.fromEntries(
          Object.entries(shortcut).filter(([key]) => !["actionId", "source"].includes(key)),
        ),
      },
      kind: "shortcut",
...
function hasOnlyInternalDeclarations(obligation) {
  return (
    ...
    obligation.declarations.every((declaration) => declaration.details.internal === true)
  );
}
...
export function buildMappingLedgers({ baseline, runtimeObservations, source }) {
  ...
  for (const observation of observations) {
    for (const sourceId of observation.sourceIds) {
      if (observedBySource.has(sourceId)) {
        throw new Error(`Source obligation has multiple runtime observations: ${sourceId}`);
      }
      observedBySource.set(sourceId, observation);
    }
  }
  for (const sourceId of observedBySource.keys()) {
    if (!obligations.some((obligation) => obligation.stableId === sourceId)) {
      throw new Error(`Observed source obligation is missing: ${sourceId}`);
    }
  }
  ...
  const sourceEntries = obligations.map((obligation) => {
    ...
    return {
      classification: mapping ? "observed" : internal ? "internal/test-only" : "unresolved",
      ...
      sourceFiles: [
        ...new Set(obligation.declarations.map((declaration) => declaration.sourceFile)),
      ].sort(compareText),
  ...
  const observedSourceCount = sourceEntries.filter((entry) => entry.status === "observed").length;
  ...
  const mappedRuntimeCount = runtimeEntries.filter((entry) => entry.status === "mapped").length;
  ...
  const runtimeToSource = {
    ...
    summary: {
      ...
      percentMapped: percentage(mappedRuntimeCount, runtimeEntries.length),
...
export function loadMappingInputs(parityRoot = DEFAULT_PARITY_ROOT) {
  ...
  const runtimeObservations = readJson(join(parityRoot, "manifest", "runtime-observations.json"));
  if (runtimeObservations.schemaVersion !== 1 || !Array.isArray(runtimeObservations.observations)) {
  ...
  for (const mapping of runtimeObservations.observations) {
    if (!Array.isArray(mapping.sourceIds) || mapping.sourceIds.length === 0) {
      throw new Error(`Runtime observation has no source IDs: ${String(mapping.runtimeId)}`);
    }
    for (const evidence of mapping.evidence) {
      for (const evidencePath of [evidence.accessibility, evidence.screenshot]) {
        if (!existsSync(join(parityRoot, evidencePath))) {
          throw new Error(`Missing runtime evidence: ${evidencePath}`);
        }
      }
      const accessibility = readFileSync(join(parityRoot, evidence.accessibility), "utf8");
...
if (pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  const { sourceToRuntime, runtimeToSource } = generateMappingLedgers(parityRoot);
