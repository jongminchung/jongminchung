...
const PRODUCT_LAYOUT_FILES = Object.freeze({
  rebasedProperties: "build/src/org/jetbrains/intellij/build/RebasedProperties.kt",
  baseIdeaProperties:
  ...
  platformLangPlugin: "platform/platform-resources/src/META-INF/PlatformLangPlugin.xml",
  communityCustomization: "community-resources/resources/META-INF/community-customization.xml",
  applicationInfo: "community-resources/resources/idea/RebasedApplicationInfo.xml",
});
...
const ICON_CLASS_FILES = Object.freeze({
  ...
  CollaborationToolsIcons: "platform/collaboration-tools/gen/icons/CollaborationToolsIcons.java",
  DvcsImplIcons: "platform/dvcs-impl/shared/src/icons/DvcsImplIcons.java",
  GitIcons: "plugins/git4idea/backend/gen/com/intellij/vcs/git/icons/GitIcons.java",
  GithubIcons: "plugins/github/github-core/gen/org/jetbrains/plugins/github/GithubIcons.java",
  GitlabIcons: "plugins/gitlab/gitlab-core/gen/com/intellij/vcs/gitlab/icons/GitlabIcons.java",
  PlatformVcsImplIcons:
    "platform/vcs-impl/gen/com/intellij/platform/vcs/impl/icons/PlatformVcsImplIcons.java",
  TerminalIcons: "plugins/terminal/src/com/intellij/terminal/icons/TerminalIcons.java",
...
export function scanXml(source, sourcePath = "<memory>") {
  ...
  while (offset < sanitized.length) {
    ...
    if (closing >= sanitized.length) {
      throw new Error(`Unterminated XML tag in ${sourcePath}:${lineNumberAt(starts, opening)}`);
    }
...
function nearestAncestor(elements, index, names) {
  return ancestors(elements, index).find((parent) => names.has(elements[parent].name)) ?? null;
}
...
export function extractPersistentList(source, variableName) {
  ...
  const declaration = cleaned.search(new RegExp(`\\bval\\s+${variableName}\\b`));
  ...
  if (call < 0 || opening < 0) throw new Error(`Missing persistentListOf for ${variableName}`);
  const closing = findMatchingParen(cleaned, opening);
  return [...cleaned.slice(opening + 1, closing).matchAll(/"([^"\\]*(?:\\.[^"\\]*)*)"/g)].map(
...
class GitTagReader {
  ...
  verify() {
    ...
    if (build !== REBASED_BASELINE.build) {
      throw new Error(`Rebased build is ${build}; expected ${REBASED_BASELINE.build}`);
    }
  ...
  read(path) {
    ...
    if (!entry || entry.type !== "blob") throw new Error(`Missing Rebased blob ${path}`);
    const content = run("git", ["-C", this.repository, "cat-file", "blob", entry.oid]);
...
function moduleIndex(reader) {
  const index = new Map();
  for (const path of reader.paths.filter((candidate) => candidate.endsWith(".iml"))) {
...
function rootsForModule(reader, imlPath) {
  ...
  const resources = [];
  const sources = [];
  for (const element of elements.filter((candidate) => candidate.name === "sourceFolder")) {
    const url = element.attrs.url ?? "";
    const prefix = "file://$MODULE_DIR$/";
    if (!url.startsWith(prefix)) continue;
    const root = posix.normalize(posix.join(moduleDirectory, url.slice(prefix.length)));
...
function descriptorResourceBundle(source) {
  ...
  const match = /<resource-bundle>\s*([^<]+?)\s*<\/resource-bundle>/.exec(cleaned);
  return match?.[1] ?? null;
}
...
function contentModuleRegistrations(elements) {
  return elements
    .filter((element, index) => {
      ...
      return nearestAncestor(elements, index, new Set(["content"])) !== null;
    })
    .map((element) => ({
...
function moduleSetNames(elements) {
  return elements
    .filter((element) => element.name === "module" && (element.attrs.name || element.attrs.value))
...
function resolveResource(reader, roots, resourceName, currentPath = null) {
  ...
  const relativeToCurrent =
    currentRoot && currentPath
      ...
      : null;
  if (relativeToCurrent && reader.has(relativeToCurrent)) candidates.add(relativeToCurrent);
  if (currentRoot) {
...
function yamlNameRegistration(reader, path, name) {
  ...
  const match = new RegExp(`^\\s*-\\s*name:\\s*${escapedName}\\s*(?:#.*)?$`, "m").exec(
    reader.read(path),
  );
  if (!match) return null;
  return reader.source(path, lineNumberAt(lineStarts(reader.read(path)), match.index));
}
...
function classifyNonImlModule(reader, name, imlPaths, registrations) {
  ...
  const layoutEvidence = pluginContentPathsForRegistrations(reader, registrations)
...
function buildProductClosure(reader) {
  ...
  const bundledPluginNames = [...new Set([...defaultPlugins, ...rebasedPlugins])].sort(compareText);
  ...
  const generatedElements = scanXml(reader.read(generatedPath), generatedPath);
  ...
  for (const name of bundledPluginNames) {
    ...
    if (imlPaths.length !== 1) {
      throw new Error(`Bundled plugin module ${name} maps to ${imlPaths.length} IML files`);
    }
    const roots = rootsForModule(reader, imlPaths[0]);
    const descriptor = mainPluginDescriptor(reader, name, imlPaths[0], roots.resources);
  ...
  for (const [name, registrations] of [...activeNames].sort(([left], [right]) =>
    ...
    if (imlPaths.length !== 1) {
      const nonImlModule = classifyNonImlModule(
        ...
        name,
        imlPaths,
        stableSort(registrations, (entry) => `${entry.kind}:${entry.plugin ?? ""}`),
      );
    ...
    const roots = rootsCache.get(name) ?? rootsForModule(reader, imlPaths[0]);
    ...
    const moduleDescriptors = roots.resources
      ...
      .filter((path) => reader.has(path));
    for (const descriptor of moduleDescriptors) descriptorPaths.add(descriptor);
    activeModules.push({
      name,
      iml: imlPaths[0],
      ...
      registrations: stableSort(registrations, (entry) => `${entry.kind}:${entry.plugin ?? ""}`),
    });
  }

  const resourceRoots = [...allResourceRoots]
    .filter((root) => reader.paths.some((path) => path.startsWith(`${root}/`)))
    .sort(compareText);
  const includeEdges = [];
  ...
  while (pending.length > 0) {
    ...
    const elements = scanXml(reader.read(descriptor), descriptor);
    for (const element of elements.filter((candidate) => candidate.name === "xi:include")) {
      const href = element.attrs.href;
      if (!href) continue;
      const matches = resolveResource(reader, resourceRoots, href, descriptor);
      const edge = {
        from: descriptor,
        line: element.line,
        href,
  ...
  return {
    ...
    unresolvedModules: stableSort(unresolvedModules, (module) => module.name),
    resourceRoots,
    ...
    unresolvedIncludes: stableSort(unresolvedIncludes, (edge) => `${edge.from ?? ""}:${edge.href}`),
...
function javaProperties(source) {
  ...
  for (const physical of source.split(/\r?\n/)) {
    current += physical;
    let slashCount = 0;
    for (let index = current.length - 1; current[index] === "\\"; index -= 1) slashCount += 1;
    if (slashCount % 2 === 1) {
...
function resolveBundle(reader, closure, bundleName, descriptor) {
  ...
  const matches = resolveResource(reader, closure.resourceRoots, resource, descriptor);
  if (matches.length !== 1) return { name: bundleName, paths: matches, values: null };
  return {
    name: bundleName,
    paths: matches,
...
function declarationId(element, descriptor) {
  return element.attrs.id ?? element.attrs.class ?? `anonymous@${descriptor}:${element.line}`;
}
...
function extractActionSystem(reader, closure) {
  ...
  for (const descriptor of closure.descriptorPaths) {
    ...
    const bundleFor = (name) => {
      ...
      if (!bundleCache.has(name)) {
        bundleCache.set(name, resolveBundle(reader, closure, name, descriptor));
      }
    ...
    for (let index = 0; index < elements.length; index += 1) {
      ...
      const actionsAncestor = nearestAncestor(elements, index, new Set(["actions"]));
      if ((element.name === "action" || element.name === "group") && actionsAncestor !== null) {
        const id = declarationId(element, descriptor);
        const parentDeclaration = nearestDeclaration(elements, index);
        const actionsBundle = elements[actionsAncestor].attrs["resource-bundle"] ?? null;
        const bundleName = element.attrs.bundle ?? actionsBundle ?? descriptorBundle;
        ...
        const record = {
          ...
          text: element.attrs.text ?? bundle?.values?.[labelKey] ?? null,
          description: element.attrs.description ?? bundle?.values?.[descriptionKey] ?? null,
          labelKey,
          bundle: bundle
            ? {
                name: bundle.name,
          ...
          parentId:
            parentDeclaration === null
              ? null
              : declarationId(elements[parentDeclaration], descriptor),
      ...
      if (element.name === "add-to-group" && actionsAncestor !== null) {
        ...
        addToGroups.push({
          ...
          relativeToAction: element.attrs["relative-to-action"] ?? null,
          source: reader.source(descriptor, element.line),
        });
      }
    }

    const records = [...actions, ...groups].filter((record) => record.source.path === descriptor);
    ...
    for (let index = 0; index < elements.length; index += 1) {
      const element = elements[index];
      if (element.name === "keyboard-shortcut" || element.name === "mouse-shortcut") {
  ...
  return {
    actions: stableSort(
      actions,
      (record) => `${record.stableId}:${record.source.path}:${record.source.line}`,
    ),
    groups: stableSort(
      groups,
      (record) => `${record.stableId}:${record.source.path}:${record.source.line}`,
...
function keymapActions(elements, sourcePath) {
  ...
  for (let index = 0; index < elements.length; index += 1) {
    ...
    for (let child = 0; child < elements.length; child += 1) {
      ...
      if (
        !["keyboard-shortcut", "keyboard-gesture-shortcut", "mouse-shortcut"].includes(
          candidate.name,
        )
      ) {
        continue;
    ...
    result.push({
      ...
      shortcuts: stableSort(
        shortcuts,
        (shortcut) => `${shortcut.type}:${shortcut["first-keystroke"] ?? shortcut.keystroke ?? ""}`,
      ),
...
function extractKeymaps(reader, closure, parsedDescriptors, actions) {
  ...
  for (const { descriptor, elements } of parsedDescriptors) {
    for (const element of elements.filter((candidate) => candidate.name === "bundledKeymap")) {
      const file = element.attrs.file;
      const matches = file
        ? resolveResource(reader, closure.resourceRoots, `keymaps/${file}`, descriptor)
        : [];
      registrations.push({
        file,
        paths: matches,
  ...
  const keymaps = [];
  for (const registration of stableSort(registrations, (entry) => entry.file ?? "")) {
  ...
  const unique = [...new Map(keymaps.map((keymap) => [keymap.name, keymap])).values()];
  const byName = new Map(unique.map((keymap) => [keymap.name, keymap]));
  const resolved = new Map();
  const resolveEffective = (name, visiting = new Set()) => {
    if (resolved.has(name)) return resolved.get(name);
    if (visiting.has(name)) throw new Error(`Cyclic keymap inheritance at ${name}`);
    const keymap = byName.get(name);
    if (!keymap) return null;
    const next = new Set(visiting).add(name);
    const parent = keymap.parent ? resolveEffective(keymap.parent, next) : null;
  ...
  return {
    ...
    descriptorShortcuts: stableSort(
      descriptorShortcuts,
      (entry) => `${entry.keymap ?? ""}:${entry.actionId}:${entry["first-keystroke"] ?? ""}`,
    ),
  };
}
...
function extractExtensions(reader, parsedDescriptors) {
  ...
  for (const { descriptor, elements } of parsedDescriptors) {
    for (const element of elements) {
      if (
        ["toolWindow", "toolWindowExtractorMode", "toolWindowInitializer"].includes(element.name)
      ) {
        toolWindows.push({
      ...
      if (["applicationConfigurable", "projectConfigurable"].includes(element.name)) {
        const id =
          element.attrs.id ??
        ...
        configurables.push({
          ...
          scope: element.name === "applicationConfigurable" ? "application" : "project",
          attributes: element.attrs,
          source: reader.source(descriptor, element.line),
        });
      }
  ...
  return {
    toolWindows: stableSort(
      toolWindows,
      (entry) => `${entry.stableId}:${entry.source.path}:${entry.source.line}`,
    ),
...
function classPathCandidates(reader, className) {
  ...
  const suffixes = [`/${packageName}/${base}.kt`, `/${packageName}/${base}.java`];
  const basenameCandidates = reader.paths.filter(
    (path) => path.endsWith(`/${base}.kt`) || path.endsWith(`/${base}.java`),
  );
...
function inspectDynamicSource(reader, path) {
  ...
  return {
    ...
    createsActions: /\b(create|build|get)[A-Z]\w*Actions?\s*\(/.test(source),
...
function extractDynamicProviders(reader, closure, actionSystem) {
  ...
  for (const group of actionSystem.groups.filter((entry) => entry.className)) {
  ...
  for (const { descriptor, elements } of actionSystem.parsedDescriptors) {
    for (const element of elements) {
      for (const [attribute, value] of Object.entries(element.attrs)) {
        if (
          !["class", "implementation", "implementationClass", "provider", "factoryClass"].includes(
            attribute,
          )
        ) {
          continue;
        }
        if (!/(ActionGroup|ActionProvider|ActionsProvider|Popup)/.test(value)) continue;
        register(value, {
  ...
  for (const path of reader.paths.filter(fileLooksDynamic)) {
    ...
    const className = inspection.packageName ? `${inspection.packageName}.${base}` : base;
    if (!inspection.getChildren && !inspection.createsActions && !/Provider|Popup/.test(base))
      continue;
    register(className, {
    ...
    .map(([className, entries]) => {
      ...
      return {
        ...
        registrations: stableSort(
          entries,
          (entry) => `${entry.kind}:${entry.source.path}:${entry.source.line}`,
        ),
        sourceCandidates: sourcePaths.map((path) => inspectDynamicSource(reader, path)),
      };
    })
...
function leafCount(value) {
  ...
  return Object.values(value).reduce((total, child) => total + leafCount(child), 0);
}
...
function extractThemes(reader, closure, parsedDescriptors) {
  ...
  const themes = [];
  for (const provider of stableSort(providers, (entry) => entry.attributes.id ?? "")) {
  ...
  return {
    ...
    unresolvedProviders: providers.filter((provider) => provider.paths.length !== 1),
    colorSchemes: stableSort(colorSchemes, (scheme) => scheme.attributes.id ?? ""),
  };
}
...
function parseGeneratedIconClass(source, rootName) {
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    const field = /public static final[^\n]*Icon\s+(\w+)\s*=\s*load\(([^;]+)\);/.exec(line);
    if (field) {
      const paths = [...field[2].matchAll(/"([^"]+\.(?:svg|png))"/g)].map((match) => match[1]);
      result.set([rootName, ...stack.map((entry) => entry.name), field[1]].join("."), {
...
function extractIcons(reader, closure, parsedDescriptors, themes) {
  ...
  for (const [className, path] of Object.entries(ICON_CLASS_FILES)) {
    ...
    for (const [key, value] of parseGeneratedIconClass(reader.read(path), className)) {
      generated.set(key, {
        ...value,
  ...
  const roots = [...new Set([...closure.resourceRoots, ...EXTRA_RESOURCE_ROOTS])].sort(compareText);
  ...
  const assets = [...occurrencesByReference.entries()].map(([reference, occurrences]) => {
    const generatedEntry = generated.get(reference);
    const resourceNames = generatedEntry
      ? [generatedEntry.expUiPath, generatedEntry.fallbackPath].filter(Boolean)
      : /\.(svg|png)$/.test(reference)
        ? [reference]
        : [];
    const resolved = new Map();
    for (const resourceName of resourceNames) {
      for (const path of resolveResource(reader, roots, resourceName)) {
        const asset = {
          resourceName,
          path,
          blob: reader.source(path).blob,
    ...
    const sortedOccurrences = stableSort(
      occurrences,
      (occurrence) => `${occurrence.kind}:${occurrence.source.path}:${occurrence.source.line ?? 0}`,
    );
    return {
      stableId: `icon:${reference}`,
      reference,
      reusable: sortedOccurrences.every((occurrence) => occurrence.reusable ?? true),
      generated: generatedEntry ?? null,
      assets: stableSort([...resolved.values()], (asset) => `${asset.resourceName}:${asset.path}`),
...
function writeSnapshot(outputDirectory, files) {
  ...
  try {
    for (const name of OUTPUT_FILES) {
      if (!(name in files)) throw new Error(`Missing generated output ${name}`);
      writeFileSync(join(temporary, name), stableJson(files[name]), "utf8");
    }
    ...
    rmSync(temporary, { recursive: true, force: true });
    if (!existsSync(outputDirectory) && existsSync(backup)) renameSync(backup, outputDirectory);
    throw error;
  }
}
...
export function extractRebasedSourceOracle({
  ...
  outputDirectory = join(workspaceRoot, "apps/git-client/parity/rebased/1.1.8/source"),
  write = true,
} = {}) {
  ...
  const extensions = extractExtensions(reader, actionSystem.parsedDescriptors);
  const dynamicProviders = extractDynamicProviders(reader, closure, actionSystem);
  const themes = extractThemes(reader, closure, actionSystem.parsedDescriptors);
  const icons = extractIcons(reader, closure, actionSystem.parsedDescriptors, themes);
  const summary = {
    schemaVersion: 1,
    baseline,
    counts: {
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--workspace-root") options.workspaceRoot = resolve(argv[++index]);
    else if (argument === "--rebased-repository")
      options.rebasedRepository = resolve(argv[++index]);
    else if (argument === "--output") options.outputDirectory = resolve(argv[++index]);
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    const result = extractRebasedSourceOracle(parseArguments(process.argv.slice(2)));
