...
function readText(relativePath: string): string {
  return readFileSync(
    fileURLToPath(new URL(relativePath, new URL("../../", import.meta.url))),
    "utf8",
  );
}

function readReport(name: string): Record<string, unknown> {
  return JSON.parse(readText(`parity/rebased/1.1.8/reports/${name}`)) as Record<string, unknown>;
}
...
function gitBridgeMethods(): readonly string[] {
  ...
  return [
    ...source.slice(interfaceStart, interfaceEnd).matchAll(/^\s{2,}([A-Za-z][A-Za-z0-9]*)\(/gmu),
  ].map((match) => match[1] ?? "");
}
...
describe("Electron bridge support report", () => {
  it("accounts for every GitBridge method in source order as connected", () => {
    ...
    for (const method of methods) {
      expect(method).toMatchObject({
        ...
        packageVerification: packageVerifiedMethods.has(String(method.method))
          ? "passed"
          : "pending",
  ...
  it("fails if the production Electron bridge regains an unsupported, empty-fallback, or no-op path", () => {
    ...
    expect(source).not.toMatch(/UnsupportedElectronGitFeatureError|#unsupported\s*[<(]/u);
    expect(source).not.toMatch(/Promise\.resolve\(\s*\[\s*\]\s*\)/u);
    expect(source).not.toMatch(/return\s+Promise\.resolve\(\s*undefined\s*\)/u);
    for (const method of gitBridgeMethods()) {
      expect(source, method).toMatch(new RegExp(`\\b${method}\\s*\\(`, "u"));
    }
  });
  ...
  it("keeps implementation progress separate from incomplete bidirectional parity coverage", () => {
    ...
    expect(record(coverage.implementationProgress).countsAsParityCoverage).toBe(false);
    expect(record(coverage.packaging)).toMatchObject({
      ...
      artifactSha256: "05f241b616b57351f88e260c382759e7d995112e2f1b0a765de80e0c79b883d5",
  ...
  it("only cites evidence files that exist", () => {
    ...
    for (const evidence of records(report.evidence)) {
      ...
      const evidencePath = `${APP_ROOT}${relativePath}`;
      if (relativePath.startsWith("release-artifacts/") && !existsSync(evidencePath)) {
        expect(report.complete).toBe(false);
        continue;
      }
      expect(statSync(evidencePath).isFile(), relativePath).toBe(true);
    }
  });
});
