...
function safeError(error, fallback) {
  ...
  return new VisualComparisonError("io", error instanceof Error ? error.message : fallback);
}
...
async function pinRoot(untrustedRoot) {
  ...
  if (after.isSymbolicLink() || !after.isDirectory() || !sameIdentity(after, identity(before))) {
...
async function resolveOutput(root, untrustedPath) {
  ...
  const parent = await inspectContainedPath(root, dirname(path), "Output report", "directory");
  let existing = null;
  try {
...
function decodeRows(inflated, width, height, bytesPerPixel, rowBytes) {
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < rowBytes; x += 1) {
      ...
      const upperLeft =
        y > 0 && x >= bytesPerPixel ? decoded[previousOffset + x - bytesPerPixel] : 0;
      let value;
      switch (filter) {
        case 0:
          value = raw;
  ...
  for (let pixel = 0; pixel < width * height; pixel += 1) {
    ...
    switch (colorType) {
      case 0: {
        ...
        rgba[target + 1] = gray;
        rgba[target + 2] = gray;
        const transparentGray = transparency === null ? null : transparency.readUInt16BE(0);
...
export function decodePng(bytes, label = "PNG") {
  ...
  while (offset < bytes.length) {
    ...
    if (type === "IHDR") {
      ...
      if (compression !== 0 || filter !== 0 || interlace !== 0) {
        fail("malformedPng", `${label} uses unsupported compression, filtering, or interlacing`);
      }
      ...
      const validLength =
        ...
        (header.colorType === 3 && palette !== null && data.length <= palette.length / 3);
      if (!validLength) fail("malformedPng", `${label} has an invalid tRNS payload`);
      if (
        (header.colorType === 0 && data.readUInt16BE(0) > 255) ||
        (header.colorType === 2 && [0, 2, 4].some((offset_) => data.readUInt16BE(offset_) > 255))
      ) {
  ...
  const decoded = decodeRows(inflated, header.width, header.height, layout.bytesPerPixel, rowBytes);
...
function validateMaskReview(untrusted) {
  if (
    ...
    (untrusted.reviewedAt !== undefined &&
      (typeof untrusted.reviewedAt !== "string" || untrusted.reviewedAt.length > 128)) ||
  ...
  return Object.freeze({
    ...
    ...(untrusted.reviewedAt === undefined ? {} : { reviewedAt: untrusted.reviewedAt }),
  });
}
...
function structuralSsim(reference, candidate, width, height, mask) {
  ...
  for (let originY = 0; originY < height; originY += SSIM_WINDOW_SIZE) {
    for (let originX = 0; originX < width; originX += SSIM_WINDOW_SIZE) {
      ...
      let productSum = 0;
      for (let y = originY; y < Math.min(height, originY + SSIM_WINDOW_SIZE); y += 1) {
        for (let x = originX; x < Math.min(width, originX + SSIM_WINDOW_SIZE); x += 1) {
      ...
      const referenceVariance = Math.max(
        0,
        (referenceSquaredSum - (referenceSum * referenceSum) / count) / divisor,
      );
      const candidateVariance = Math.max(
        0,
        (candidateSquaredSum - (candidateSum * candidateSum) / count) / divisor,
      );
      const covariance = (productSum - (referenceSum * candidateSum) / count) / divisor;
      const score =
        ((2 * referenceMean * candidateMean + SSIM_C1) * (2 * covariance + SSIM_C2)) /
...
async function writeAtomic(output, bytes, protectedFiles) {
  ...
  if (
    ...
    protectedFiles.some(
      (file) => file.device === output.existing.device && file.inode === output.existing.inode,
    )
  ) {
  ...
  const temporary = join(output.parent.path, `.${basename(output.path)}.${randomUUID()}.tmp`);
  let handle;
...
export async function compareVisuals(options) {
  ...
  if (
    ...
    (referenceFile.device === candidateFile.device && referenceFile.inode === candidateFile.inode)
  ) {
  ...
  if (options.maskReviewPath !== undefined) {
    optionalFiles.review = await resolveInput(root, options.maskReviewPath, "Mask review");
  }
  if (options.metadataPath !== undefined) {
    optionalFiles.metadata = await resolveInput(root, options.metadataPath, "Comparison metadata");
  }
  ...
  if (optionalFiles.mask !== undefined) {
    maskBytes = await readBoundedFile(optionalFiles.mask, MAXIMUM_PNG_FILE_BYTES, "Mask PNG");
    maskImage = decodePng(maskBytes, "Mask PNG");
    if (maskImage.width !== reference.width || maskImage.height !== reference.height) {
  ...
  const rectangles = validateRectangles(rectanglesSource, reference.width, reference.height);
  ...
  if (optionalFiles.review !== undefined) {
    reviewBytes = await readBoundedFile(optionalFiles.review, MAXIMUM_JSON_BYTES, "Mask review");
    try {
  ...
  if (optionalFiles.metadata !== undefined) {
    const loaded = await readJsonInput(root, options.metadataPath, "Comparison metadata");
  ...
  const ssim = structuralSsim(referenceRgb, candidateRgb, reference.width, reference.height, mask);
  const mismatch = mismatchMetrics(referenceRgb, candidateRgb, mask);
  const mismatchPercent = (mismatch.mismatchedPixels / mismatch.comparedPixels) * 100;
  ...
  if (optionalFiles.rectangles !== undefined) {
    inputs.maskRectangles = inputDescriptor(root, optionalFiles.rectangles, rectanglesBytes);
  }
  if (optionalFiles.review !== undefined) {
    inputs.maskReview = inputDescriptor(root, optionalFiles.review, reviewBytes);
  }
  if (optionalFiles.metadata !== undefined) {
    inputs.metadata = inputDescriptor(root, optionalFiles.metadata, metadataBytes);
  }
  ...
  const report = {
    algorithm: {
      ...
      maskPixels: "non-zero alpha on transparent masks; non-black RGB on fully opaque masks",
...
function parseArguments(arguments_) {
  ...
  return {
    ...
    ...(values.has("--mask-rectangles")
      ...
      : {}),
    ...(values.has("--mask-review") ? { maskReviewPath: values.get("--mask-review") } : {}),
    ...(values.has("--metadata") ? { metadataPath: values.get("--metadata") } : {}),
  };
}
...
async function main() {
  try {
    ...
    process.stdout.write(`${JSON.stringify({ pass: report.pass, report: options.outputPath })}\n`);
