...
import {
    buildActionRegistry,
    loadActionRegistryInputs,
} from "./build-action-registry.mjs";
...
describe("Rebased generated action registry", () => {
    it("covers every source obligation and every candidate command", () => {
        ...
        expect(registry.summary).toEqual({
            boundCandidateCommands: 47,
            sourceObligations: 7_260,
            sourceObligationsWithCandidateBindings: 47,
            unresolvedSourceObligations: 7_041,
        });
        expect(new Set(registry.entries.map((entry) => entry.sourceId)).size).toBe(
            7_260,
        );
        const bindings = registry.entries.flatMap(
            (entry) => entry.candidateBindings,
        );
        expect(new Set(bindings.map((binding) => binding.commandId)).size).toBe(
            47,
        );
        expect(
            bindings.every(
