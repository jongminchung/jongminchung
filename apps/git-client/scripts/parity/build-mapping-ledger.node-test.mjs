...
void test("collects deterministic unique obligations without expanding icons", () => {
  ...
  assert.equal(new Set(first.map((entry) => entry.stableId)).size, first.length);
  ...
  assert.equal(
    iconObligations.reduce((total, entry) => total + entry.sourceOccurrenceCount, 0),
    sourceIcons.reduce((total, icon) => total + icon.occurrences.length, 0),
  );

  const bookmarks = first.find((entry) => entry.stableId === "toolwindow:Bookmarks");
...
void test("links captured runtime surfaces and source-classifies internal declarations", () => {
  ...
  const observed = first.sourceToRuntime.entries.filter((entry) => entry.status === "observed");
  assert.deepEqual(
  ...
  assert.equal(first.runtimeToSource.scope.actionableNodeEnumerationComplete, false);
});
...
void test("checked-in ledgers exactly match the deterministic builder", () => {
  ...
  const sourceToRuntime = JSON.parse(
    readFileSync(`${DEFAULT_PARITY_ROOT}/mappings/source-to-runtime.json`, "utf8"),
  );
  const runtimeToSource = JSON.parse(
    readFileSync(`${DEFAULT_PARITY_ROOT}/mappings/runtime-to-source.json`, "utf8"),
  );
