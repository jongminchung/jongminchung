...
<idea-plugin>
  ...
  <actions resource-bundle="messages.Test">
    ...
    assert.equal(
        elements.filter((element) => element.name === "action").length,
        1,
    );
    assert.equal(
        elements.find((element) => element.name === "action")?.attrs.id,
        "Real",
    );
    assert.equal(
        elements.find((element) => element.name === "action")?.line,
        6,
    );
});
...
void test(
    "Rebased 1.1.8 source oracle is complete and deterministic",
    { timeout: 120_000 },
    () => {
        ...
        assert.equal(
            new Set(icons.map((icon) => icon.stableId)).size,
            icons.length,
        );
        assert.equal(
            icons.every(
        ...
        assert.equal(
            first.files["actions.json"].actions.some(
                (action) => action.id === "Git.Fetch",
            ),
            true,
        );
        assert.equal(
            first.files["groups.json"].groups.some(
                (group) => group.id === "Git.Menu",
            ),
            true,
        );
        assert.equal(
            first.files["themes.json"].themes.some(
                (theme) => theme.name === "Islands Light",
            ),
            true,
        );
        assert.equal(
