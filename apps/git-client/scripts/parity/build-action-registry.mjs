...
function parseBinding(value, index) {
    ...
    return {
        checkedWhen:
            value.checkedWhen === null
                ...
                : requireNonEmptyString(
                      value.checkedWhen,
                      `Action binding ${index}.checkedWhen`,
                  ),
        commandId: requireNonEmptyString(
            value.commandId,
            `Action binding ${index}.commandId`,
        ),
        effectKind: requireOneOf(
            value.effectKind,
            EFFECT_KINDS,
            `Action binding ${index}.effectKind`,
        ),
        enabledWhen: requireNonEmptyString(
            value.enabledWhen,
            `Action binding ${index}.enabledWhen`,
        ),
        ...
        sourceId: requireNonEmptyString(
            value.sourceId,
            `Action binding ${index}.sourceId`,
        ),
        testSurface: requireNonEmptyString(
            value.testSurface,
            `Action binding ${index}.testSurface`,
        ),
        uiSurface: requireNonEmptyString(
            value.uiSurface,
            `Action binding ${index}.uiSurface`,
        ),
        visibleWhen: requireNonEmptyString(
            value.visibleWhen,
            `Action binding ${index}.visibleWhen`,
        ),
    };
}
...
function commandById(commandManifest) {
    ...
    return new Map(
        commandManifest.commands.map((command) => [command.id, command]),
    );
}
...
export function buildActionRegistry({
    ...
    const obligationIds = new Set(
        obligations.map((obligation) => obligation.stableId),
    );
    ...
    for (const binding of bindings) {
        if (!obligationIds.has(binding.sourceId)) {
            throw new Error(
                `Action binding references missing source obligation: ${binding.sourceId}`,
            );
        }
        const command = commands.get(binding.commandId);
        if (command === undefined) {
            throw new Error(
                `Action binding references missing command: ${binding.commandId}`,
            );
        }
    ...
    const sourceMapping = new Map(
        sourceToRuntime.entries.map((entry) => [entry.sourceId, entry]),
    );
    ...
    const entries = obligations.map((obligation) => {
        ...
        if (mapping === undefined) {
            throw new Error(
                `Mapping ledger is missing source obligation: ${obligation.stableId}`,
            );
        }
        return {
            candidateBindings: (
                bindingsBySource.get(obligation.stableId) ?? []
            ).sort((left, right) =>
                compareText(left.commandId, right.commandId),
            ),
    ...
    return {
        ...
        summary: {
            ...
            unresolvedSourceObligations: entries.filter(
...
export function loadActionRegistryInputs(
    ...
    return {
        ...mappingInputs,
        bindings: parseActionBindings(
            readJson(join(parityRoot, "manifest", "action-bindings.json")),
        ),
        commandManifest: readJson(join(appRoot, "src", "command-manifest.json")),
        sourceToRuntime: readJson(
            join(parityRoot, "mappings", "source-to-runtime.json"),
        ),
    };
}

export function writeActionRegistry(
    registry,
    parityRoot = DEFAULT_PARITY_ROOT,
) {
