...
function readCatalog(image: any, volumeHeaderOffset: any) {
    ...
    if (allocationBlockSize < 512 || allocationBlockSize % 512 !== 0) {
        throw new Error(
            `Invalid HFS allocation block size: ${allocationBlockSize}`,
        );
    }
    ...
    for (
        let index = 0;
        index < HFS_EXTENT_COUNT && catalogOffset < logicalSize;
        index += 1
    ) {
        const extentOffset =
            forkOffset + HFS_FORK_EXTENTS_OFFSET + index * HFS_EXTENT_BYTES;
    ...
    if (catalogOffset !== logicalSize) {
        throw new Error(
            "HFS catalog fork is fragmented beyond its inline extents",
        );
    }
...
function normalizeCatalog(catalog: any) {
    ...
    if (
        nodeSize < 512 ||
        nodeSize % 512 !== 0 ||
        totalNodes * nodeSize > catalog.length
    ) {
    ...
    for (let nodeIndex = 0; nodeIndex < totalNodes; nodeIndex += 1) {
        ...
        for (let recordIndex = 0; recordIndex < recordCount; recordIndex += 1) {
            ...
            for (const dateOffset of HFS_CATALOG_DATE_OFFSETS) {
                catalog.writeUInt32BE(
                    HFS_FIXED_TIMESTAMP,
                    dataOffset + dateOffset,
                );
            }
...
export async function normalizeUdifImage(
    ...
    const report = normalizeUdifTrailerBuffer(
        image.subarray(image.length - UDIF_TRAILER_BYTES),
    );
    await writeFile(filePath, image);
    return report;
}
...
export async function createReproducibleDmg(
    ...
    if (!appStats.isDirectory() || appStats.isSymbolicLink()) {
        throw new Error(
            `Reproducible DMG input must be an app directory: ${appPath}`,
        );
    }
    ...
    const workingDirectory = await mkdtemp(
        join(temporaryDirectory, "git-client-dmg-"),
    );
    ...
    try {
        ...
        await runCommand("/usr/bin/xattr", ["-cr", sourceDirectory], {
            capture: true,
        });
        await runCommand(
            "/usr/bin/hdiutil",
        ...
        if (!targetStats.isFile()) {
            throw new Error(
                `hdiutil did not create a regular DMG: ${targetPath}`,
            );
        }
        return Object.freeze({
