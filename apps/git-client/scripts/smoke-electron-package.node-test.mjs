...
void describe("packaged Electron smoke outcome", () => {
  ...
  void it("recognizes preload, module resolution, renderer, uncaught, and fatal failures", () => {
    const fixtures = [
      ["preload load failure", "Unable to load preload script: /app/preload.cjs"],
      ["preload load failure", "Preload script startup failed before context bridge exposure"],
      ["preload load failure", "Error loading preload script before navigation"],
      ["module-not-found", "Error: module not found: node:crypto"],
      ["module-not-found", "Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'missing'"],
      ...
      ["uncaught startup error", "Unhandled Promise Rejection at startup"],
      ["uncaught startup error", "UnhandledPromiseRejectionWarning: startup exploded"],
      ...
      ["fatal startup error", "FATAL ERROR: renderer initialization failed"],
    ];
