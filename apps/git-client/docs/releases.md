...
GitHub Actions의 `Git Client` workflow는 `workflow_dispatch`로만 시작한다. 실행자는 stable semantic version을 입력하며 이미 게시된 version과 tag는 교체하지 않는다.
...
publisher는 release와 tag가 아직 존재하지 않을 때만 새 production artifact를 게시한다. 새 draft의 태그, 제목, asset 목록을 검증한 다음 공개 상태로 전환한다. 실패한 현재 실행이 만든 draft와 tag만 정리하며 기존 release identity는 삭제하지 않는다.
...
결과 이름에는 `_adhoc`가 들어간다. 예: `Git-Client_<version>_macos_arm64_adhoc.dmg`. 이 파일은 strict `codesign --verify --deep --strict`, Electron 43.3.0, ARM64, fuse, ASAR integrity, locale, node-pty와 크기 정책을 검증하지만 Apple notarization artifact가 아니므로 게시할 수 없다.
...
pnpm --filter @jongminchung/git-client run release:build -- <version>
```

결과는 `apps/git-client/release-artifacts`에 만들어진다. 스크립트는 다음 순서를 강제한다.

1. 요청한 stable version, macOS ARM64, Developer ID identity와 notarization profile preflight
...
version과 릴리스 노트만 확인할 때는 토큰이 필요 없다.

```sh
pnpm --filter @jongminchung/git-client run release:dry-run -- <version>
```
...
```sh
test -n "${GH_PAT:-}"
pnpm --filter @jongminchung/git-client run release -- <version>
```
