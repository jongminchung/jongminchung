...
export type GitUtilityClientState = "connecting" | "ready" | "disposing" | "disposed" | "crashed";
...
export class GitUtilityProtocolRuntime {
  ...
  setWatchListener(repositoryId: RepositoryId, listener: RepositoryChangedListener): void {
  ...
  clearWatchListener(repositoryId: RepositoryId, listener?: RepositoryChangedListener): void {
    if (listener === undefined || this.#watchListeners.get(repositoryId) === listener) {
    ...
    this.#disposePromise = responsePromise
      .then((response) => {
        ...
        this.#rejectOutstanding(
          new GitUtilityTransportError("disposed", "Git utility client was disposed"),
        );
  ...
  #commandResult(message: GitUtilityToMainMessage): void {
    if (
      ...
      this.#crash(
        new GitUtilityTransportError("protocolViolation", "Unexpected Git utility response"),
      );
      return;
    }
  ...
  #remoteError(
    ...
    if (
      correlationId === null ||
      (correlationId === this.#handshakeCorrelationId && this.#state === "connecting")
    ) {
      this.#crash(error);
      return;
    }
