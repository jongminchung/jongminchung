...
describe("QA hosting safe storage", () => {
  it("round-trips ciphertext without exposing the credential or requiring the macOS Keychain", () => {
    const storage = QaHostingSafeStorage.fromSeed(Buffer.from("isolated-hosting-fixture"));
    ...
    expect(() =>
      QaHostingSafeStorage.fromSeed(Buffer.from("another-fixture")).decryptString(encrypted),
    ).toThrow();
  });
});
