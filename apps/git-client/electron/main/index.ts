...
async function createMainWindow(
  ...
  const stream = new DesktopStreamHub(window, MAIN_WINDOW_VITE_DEV_SERVER_URL);
  ...
  const hosting = ElectronHostingFoundation.of(
    ...
    undefined,
    {
      clients: {
        gitHubClientId:
          process.env.GIT_CLIENT_GITHUB_OAUTH_CLIENT_ID?.trim() || undefined,
        gitLabClientId:
          process.env.GIT_CLIENT_GITLAB_OAUTH_CLIENT_ID?.trim() || undefined,
        gitLabRedirectUri:
          process.env.GIT_CLIENT_GITLAB_OAUTH_REDIRECT_URI?.trim() || undefined,
      },
    },
  );
...
function reportUtilityCrash(
  ...
  void dialog
    .showMessageBox(window, {
      ...
      detail:
        "Pending operations were cancelled and will not be retried automatically.",
