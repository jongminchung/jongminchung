...
export function sanitizeDiagnosticLogField(value: string, maxCharacters: number): string {
  if (!Number.isSafeInteger(maxCharacters) || maxCharacters <= 0) {
    throw new Error("Diagnostic log field limit must be a positive integer.");
  }
...
export async function appendBoundedDiagnosticLog(
  ...
  if (!Number.isSafeInteger(maxBytes) || maxBytes <= 0) {
    throw new Error("Diagnostic log size limit must be a positive integer.");
  }
  ...
  if (lineBytes > maxBytes) {
    throw new Error("Diagnostic log entry exceeds the configured file limit.");
  }
