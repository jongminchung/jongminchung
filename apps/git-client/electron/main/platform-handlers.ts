...
export function registerPlatformHandlers(
    ...
    authorize: (_event, authorization, domain, procedure, input) => {
      ...
      if (authorization.kind === "repositoryCapability") {
        for (const repositoryId of desktopTrpcRepositoryCapabilityIds(
          ...
          assertRepositoryCapability(repositoryId, authorization.capability);
        }
      }
    },
  });
  ...
  if (providedDiagnostics !== undefined) {
    ...
    router.handle(
      ...
      async (_event): Promise<boolean> => {
        ...
        await writeFile(
          target,
          await diagnostics.createLogArchive(settings.createSnapshot()),
          {
            mode: 0o600,
          },
        );
  ...
  router.handle("platform", "analysisOpenOfflineInspection", async (_event) => {
    const selection = await dialog.showOpenDialog(window, {
      title: "Select Path",
    ...
    const candidates = metadata.isDirectory()
      ? (
          ...
          .filter(
            (entry) =>
              entry.isFile() &&
              extname(entry.name).toLocaleLowerCase() === ".xml",
          )
          .slice(0, 100)
