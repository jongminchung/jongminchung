...
import type { JsonValue, RuntimeInfo } from "../../src/shared/contracts/ipc";
import {
...
function parseImportedSettings(bytes: Uint8Array): Readonly<Record<string, JsonValue>> {
    ...
    if (Reflect.get(raw, "format") !== "git-client-settings" || Reflect.get(raw, "schemaVersion") !== 1) {
...
export function registerPlatformHandlers(
    ...
    const createdRepository = (
        terminal: GitCreationTerminalEvent,
    ): RepositoryRecord => {
        ...
        ipcMain.handle(
            ...
            async (event): Promise<boolean> => {
                ...
                const selection = await dialog.showSaveDialog(window, {
                    ...
                    defaultPath: `git-client-diagnostics-${new Date()
                        .toISOString()
                        .slice(0, 10)}.zip`,
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<boolean> => {
            ...
            for (const file of request.files) {
                ...
                await writeFile(
                    target,
                    exportedHtml(file.path, file.content, request.includeLineNumbers),
    ...
    ipcMain.handle(
        ...
        async (event) => {
            ...
            const candidates = metadata.isDirectory()
                ? (await readdir(await realpath(selectedPath), { withFileTypes: true }))
            ...
            for (const path of candidates) {
                ...
                files.push({ name: basename(path), content: await readFile(path, "utf8") });
            }
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<RepositoryRecord> => {
            ...
            const terminal = await gitUtility.initializeRepository(
                request,
                creationListener,
            );
            const repository = RepositoryRecordSchema.parse(
                createdRepository(terminal),
            );
            repositoryPaths.set(repository.id, repository.path);
            return repository;
        },
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<RepositoryRecord> => {
            ...
            const terminal = await gitUtility.cloneRepository(
                request,
                creationListener,
            );
            const repository = RepositoryRecordSchema.parse(
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<boolean> => {
            ...
            const closed = await gitUtility.closeRepository(
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<GitTerminalEvent> => {
            assertTrustedSender(event, window);
            const request = GitExecutionRequestSchema.parse(raw);
            const terminal = await gitUtility.executeQuery(
                request,
                (gitEvent) => {
                    if (
                        window.isDestroyed() ||
                        ...
                        return;
                    window.webContents.send(
                        IPC_CHANNELS.gitQueryEvent,
                        GitRequestEventSchema.parse(gitEvent),
                    );
                },
            );
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<void> => {
            assertTrustedSender(event, window);
            await terminalUtility.resize(
                TerminalResizeRequestSchema.parse(raw),
            );
        },
    );
    ipcMain.handle(
    ...
    ipcMain.handle(
        ...
        async (event, raw: unknown): Promise<void> => {
            assertTrustedSender(event, window);
            await terminalUtility.closeRepository(
                TerminalCloseRepositoryRequestSchema.parse(raw),
            );
        },
    );
    ipcMain.handle(
