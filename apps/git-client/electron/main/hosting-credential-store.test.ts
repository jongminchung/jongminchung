...
function fakeSafeStorage(available = true): SafeStorageLike {
  return {
    ...
    decryptString: (value) => {
      ...
      if (!decoded.startsWith("encrypted:")) throw new Error(`bad ciphertext ${decoded}`);
...
describe("SafeStorageHostingCredentialStore", () => {
  it("persists only encrypted ciphertext and restores through safeStorage", async () => {
    ...
    const store = new SafeStorageHostingCredentialStore(fakeSafeStorage(), settings);
    ...
    expect(persisted).toContain(Buffer.from("encrypted:ghp_super-secret-token").toString("base64"));

    const restored = new SafeStorageHostingCredentialStore(
      fakeSafeStorage(),
      await SettingsStore.of(filePath),
    );
    await expect(restored.get(accountId)).resolves.toBe("ghp_super-secret-token");
  ...
  it("rejects unavailable encryption, invalid ids, and invalid tokens", async () => {
    ...
    const unavailable = new SafeStorageHostingCredentialStore(fakeSafeStorage(false), settings);
