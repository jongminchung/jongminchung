...
describe("DesktopStreamHub", () => {
    it("validates the sender, replaces duplicate ports, and publishes typed envelopes", () => {
        ...
        if (connect === undefined)
            throw new Error("Missing stream connection listener");
        const first = new TestPort();
        connect(
            { sender: webContents, senderFrame: mainFrame, ports: [first] },
            { version: 1 },
        );
        ...
        connect(
            { sender: webContents, senderFrame: mainFrame, ports: [second] },
            { version: 1 },
        );
