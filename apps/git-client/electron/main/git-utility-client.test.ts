...
class FakeGitUtility implements GitUtilityServiceLike {
  ...
  async executeQuery(_request: unknown, listener: GitEventListener): Promise<GitTerminalEvent> {
    listener({
      kind: "started",
      requestId: REQUEST_ID,
      displayCommand: "git status",
...
describe("GitUtilityProtocolServer", () => {
  it("requires a matching handshake and validates/routes utility messages", async () => {
    ...
    await flushServer();
    expect(GitUtilityToMainMessageSchema.parse(port.posted.at(-1))).toMatchObject({
      kind: "handshakeAck",
      correlationId: handshakeId,
    });
    ...
    const creationMessages = port.posted
      ...
      .filter(
        (message) => message.kind === "creationEvent" && message.correlationId === initializeId,
      );
    expect(
    ...
    const queryMessages = port.posted
      .map((message) => GitUtilityToMainMessageSchema.parse(message))
      .filter((message) => message.kind === "queryEvent" && message.correlationId === queryId);
    expect(
      queryMessages.map((message) => (message.kind === "queryEvent" ? message.event.kind : "")),
    ).toEqual(["started", "completed"]);
