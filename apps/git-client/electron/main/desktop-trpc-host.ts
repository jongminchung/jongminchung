...
> = Extract<
    keyof RouterInputs<TRouter>[TDomain] &
        keyof RouterOutputs<TRouter>[TDomain],
    string
>;
...
export class DesktopTrpcHost<TRouter extends AnyTRPCRouter> {
    ...
    constructor({
        ...
        for (const [rawDomain, rawProcedureKeys] of Object.entries(
            procedureKeys,
        )) {
            ...
            contents.ipc.handle(
                ...
                (event, raw: unknown) =>
                    this.#dispatch(event, domain, allowed, raw),
            );
        }
    }
    ...
    >(
        ...
        handler: (
            ...
            HandlerOutput<RouterOutputs<TRouter>[TDomain][TProcedure]>
        >,
    ): void {
        ...
        if (this.#handlers.has(path)) {
            throw new Error(
                `Desktop tRPC procedure ${path} is already registered`,
            );
        }
    ...
    async #dispatch(
        ...
        try {
            ...
            const data: unknown = await Reflect.apply(
                procedureCaller,
                domainCaller,
                [request.input],
            );
