...
function hostingCertificatePath(args: readonly string[]): string | null {
  const values = args
    .filter((argument) => argument.startsWith(QA_HOSTING_CERTIFICATE_ARGUMENT))
    .map((argument) => argument.slice(QA_HOSTING_CERTIFICATE_ARGUMENT.length));
...
export function resolveRuntimeProfile(args: readonly string[]): RuntimeProfile {
  ...
  const enabledProfiles = [qaFixture, qaHosting, qaRuntime, qaSmokeTest].filter(Boolean).length;
  if (enabledProfiles > 1) throw new Error("Electron QA profile arguments are mutually exclusive");

  const certificatePath = hostingCertificatePath(args);
  if (qaHosting !== (certificatePath !== null)) {
    throw new Error("QA hosting profile and certificate path must be provided together");
  }
...
export function trustsQaHostingCertificate(
  ...
  return hostname === "127.0.0.1" && fingerprint.length > 0 && fingerprint === expectedFingerprint;
}
