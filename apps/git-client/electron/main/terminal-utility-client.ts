...
): TerminalUtilityProcessTransport {
    return {
...
export class TerminalUtilityClient {
    ...
    private constructor(
        ...
        const timeoutMs =
            options.handshakeTimeoutMs ?? TERMINAL_UTILITY_HANDSHAKE_TIMEOUT_MS;
    ...
    static async fork(
        ...
        return TerminalUtilityClient.connect(
            createElectronTransport(child),
            options,
        );
    }
    ...
    async closeRepository(untrustedRequest: unknown): Promise<number> {
        const request =
            TerminalCloseRepositoryRequestSchema.parse(untrustedRequest);
        const response = await this.#request(
        ...
        for (const [requestId, session] of this.#sessions) {
            if (session.repositoryId === request.repositoryId)
    ...
    #request(
        ...
        return new Promise((resolve, reject) => {
            this.#pending.set(message.correlationId, {
                expectedKind,
                resolve,
                reject,
            });
            try {
                this.#transport.postMessage(
                    MainToTerminalUtilityMessageSchema.parse(message),
                );
            } catch (error) {
    ...
    #receive(untrustedMessage: unknown): void {
        const parsed =
            TerminalUtilityToMainMessageSchema.safeParse(untrustedMessage);
        if (!parsed.success) {
            this.#crash(
                new TerminalUtilityTransportError(
                    "protocolViolation",
    ...
    #crash(error: Error, kill = true): void {
        ...
        try {
            ...
            console.error(
                "[git-client] Terminal utility crash observer failed",
                callbackError,
            );
        }
    }
