...
function lastMessage(
    transport: FakeTerminalUtilityTransport,
...
describe("터미널유틸리티클라이언트", () => {
    ...
    it("[성공] 터미널 제어를 검증하고 유틸리티를 사용하여 모든 세션을 삭제함", async () => {
        ...
        if (list.kind !== "listLaunchTargets")
            throw new Error("Expected target list request");
        transport.emitMessage({
        ...
        if (resize.kind !== "resize")
            throw new Error("Expected resize request");
        transport.emitMessage({
            kind: "resizeResult",
            correlationId: resize.correlationId,
        });
        ...
        transport.emitMessage({
            kind: "closeResult",
            correlationId: close.correlationId,
        });
        await expect(closing).resolves.toBeUndefined();

        const closingRepository = client.closeRepository({
            repositoryId: REPOSITORY_ID,
        });
        ...
        if (dispose.kind !== "dispose")
            throw new Error("Expected dispose request");
        transport.emitMessage({
            kind: "disposeResult",
            correlationId: dispose.correlationId,
        });
