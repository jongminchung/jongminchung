...
function shortcutDocument(): string {
  ...
  const body = rows
    .map(
      ([action, shortcut]) => `<tr><td>${action}</td><td>${shortcut}</td></tr>`,
    )
    .join("");
...
export class DiagnosticsService {
  ...
  async recordRuntimeFailure(failure: RuntimeFailure): Promise<void> {
    ...
    const details = Object.fromEntries(
      Object.entries(failure.details ?? {}).map(([key, value]) => [
        ...
        typeof value === "string"
          ? sanitizeDiagnosticLogField(value, 2_048)
          : value,
      ]),
    );
  ...
  private async appendLog(message: string): Promise<void> {
    const write = this.logWrite.then(() =>
      appendBoundedDiagnosticLog(this.paths.logs, message),
    );
