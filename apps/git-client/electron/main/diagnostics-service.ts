...
const LEFTOVER_PROFILE_NAME = /^Git Client Electron(?:$|[ -][A-Za-z0-9 ._-]+)$/u;
  ...
  const visit = async (directory: string): Promise<number> => {
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (visited > MAX_LEFTOVER_DIRECTORY_ENTRIES) {
        throw new Error("A leftover profile contains too many entries to inspect safely.");
      }
...
function validateConfiguration(kind: DiagnosticConfigurationKind, content: string): string {
  ...
  if (kind === "vmOptions") {
    for (const line of lines) {
      ...
      if (!Number.isInteger(memory) || memory < 256 || memory > 32_768) {
        throw new Error("Only --max-old-space-size=256..32768 is supported.");
      }
    }
  }
  if (kind === "customProperties") {
  ...
  if (kind === "debugLog") {
    for (const line of lines) {
      ...
      if (!/^[#A-Za-z0-9_.$-]{1,256}(?::(?:TRACE|ALL))?$/u.test(value)) {
        throw new Error("Debug log entries must be logger names with optional :TRACE or :ALL.");
      }
    }
  }
...
function reportSection(report: unknown, key: string): unknown {
  return typeof report === "object" && report !== null ? Reflect.get(report, key) : null;
}
...
export class DiagnosticsService {
  ...
  async preparePath(kind: DiagnosticPathKind): Promise<string> {
    ...
    if (kind === "customProperties" || kind === "vmOptions") {
      const configurationKind = kind === "customProperties" ? "customProperties" : "vmOptions";
      try {
        await lstat(path);
      } catch (error) {
        ...
        await this.writeConfiguration(configurationKind, defaultConfiguration(configurationKind));
      }
      return path;
    }
  ...
  async snapshot(): Promise<DiagnosticSnapshot> {
    const processes = app.getAppMetrics().map((metric) => ({
      ...
      memoryBytes: Math.max(0, Math.round(metric.memory.workingSetSize * 1_024)),
    }));
  ...
  async readConfiguration(kind: DiagnosticConfigurationKind): Promise<string> {
  ...
  async writeConfiguration(kind: DiagnosticConfigurationKind, content: string): Promise<void> {
  ...
  async createLogArchive(settings: Readonly<Record<string, JsonValue>>): Promise<Uint8Array> {
    ...
    const safeSettings = Object.fromEntries(
      Object.entries(settings).filter(([key]) => !key.startsWith(CREDENTIAL_PREFIX)),
    );
    ...
    entries["settings.json"] = strToU8(`${JSON.stringify(safeSettings, null, 2)}\n`);
  ...
  async dumpThreads(): Promise<string> {
    ...
    const safeReport = {
      ...
      uvthreadResourceUsage: reportSection(report, "uvthreadResourceUsage"),
  ...
  async createKeyboardShortcutsPdf(): Promise<string> {
    const target = join(this.paths.userData, "Git Client Keyboard Shortcuts.pdf");
  ...
  async listLeftoverDirectories(): Promise<readonly DiagnosticLeftoverDirectory[]> {
    ...
    return leftovers.sort((left, right) => right.lastModifiedMs - left.lastModifiedMs);
  }

  async deleteLeftoverDirectories(ids: readonly string[]): Promise<readonly string[]> {
    const available = new Map(
      (await this.listLeftoverDirectories()).map((entry) => [entry.id, entry]),
    );
    const parent = await realpath(this.paths.appData);
    const current = await realpath(this.paths.userData);
    ...
    for (const id of ids) {
      if (!available.has(id) || !LEFTOVER_PROFILE_NAME.test(id)) {
        throw new Error("The selected leftover profile is no longer available.");
      }
      const candidate = join(parent, id);
      const metadata = await lstat(candidate);
      if (metadata.isSymbolicLink() || !metadata.isDirectory()) {
        throw new Error("Leftover profile deletion refuses symbolic links and non-directories.");
      }
      const canonical = await realpath(candidate);
