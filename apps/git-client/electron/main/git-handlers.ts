        ...
        async (_event, raw): Promise<RepositoryRecord> => {
            ...
            const nextMode =
                safePaths.has(request.path) || safePaths.has(repository.path)
                    ...
                    : "trusted";
            if (nextMode === "safe") {
                const repositoriesToClose = [
                    ...repositoryPaths.entries(),
                ].flatMap(([id, path]) =>
        ...
        async (_event, raw): Promise<GitCreationTerminalEvent> => {
            ...
            try {
                const terminal = GitCreationEventSchema.parse(
                    await gitUtility.initializeRepository(
                        request,
                        creationListener,
                    ),
                ...
                if (terminal.kind === "completed") {
                    repositoryPaths.set(
                        terminal.repository.id,
                        terminal.repository.path,
                    );
                    repositoryAccessModes.set(
                        terminal.repository.id,
                        "trusted",
                    );
                }
            ...
            await gitUtility.watchRepository(
                ...
                (repositoryEvent) => {
                    ...
                    stream.publish({
                        ...
                        event: RepositoryChangedEventSchema.parse(
                            repositoryEvent,
                        ),
                    });
                },
