...
export function monitorWindowRuntime(
  ...
  const record = (failure: RuntimeFailure): void => {
    void dependencies.diagnostics.recordRuntimeFailure(failure).catch((error: unknown) => {
      console.error("[git-client] failed to record runtime failure", error);
    });
  };
  const prompt = async (failure: RuntimeFailure, options: MessageBoxOptions): Promise<void> => {
    record(failure);
    if (promptOpen || window.isDestroyed()) return;
    promptOpen = true;
    try {
      const { response } = await dependencies.showMessageBox(window, options);
      if (failure.kind === "rendererUnresponsive" && response === 0) return;
  ...
  window.webContents.on("render-process-gone", (_event, details) => {
    void prompt(
      ...
      },
      {
        ...
        buttons: ["Reload Window", "Restart Git Client", "Close Window"],
        defaultId: 0,
        cancelId: 2,
        noLink: true,
      },
