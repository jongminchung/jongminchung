...
export class RepositoryCreateService {
    ...
    static create(): RepositoryCreateService {
        return new RepositoryCreateService(
            RepositoryCreateProcessRunner.create(),
        );
    }

    static of(
        runner: RepositoryCreateProcessRunnerLike,
    ...
    async initialize(
        ...
        try {
            ...
            if (
                request.initialBranch !== null &&
                request.initialBranch !== undefined
            ) {
                validateRevision(request.initialBranch);
            }
            target = await RepositoryCreateTarget.prepare(
                request.path,
                "initialize",
            );
        } catch (error) {
        ...
        const initialBranch =
            request.initialBranch ?? (request.bare ? null : "main");
        const args = [
            "init",
            ...(request.bare ? ["--bare"] : []),
            ...(initialBranch === null
        ...
        const terminal = await this.#finalizeTarget(
            "initialize",
            target,
            outcome,
        );
    ...
    async clone(
        ...
        try {
            ...
            if (request.options.branch !== null)
                validateRevision(request.options.branch);
            target = await RepositoryCreateTarget.prepare(
                request.path,
                "clone",
            );
        ...
        const args = [
            ...
            "--origin",
            "origin",
            ...(request.options.depth === null
                ? []
                : ["--depth", String(request.options.depth)]),
            ...(request.options.branch === null
                ? []
                : ["--branch", request.options.branch]),
            ...(request.singleBranch ? ["--single-branch"] : []),
            ...(request.options.recurseSubmodules
                ? ["--recurse-submodules"]
                : []),
            "--",
            request.url,
    ...
    async #finalizeTarget(
            ...
            try {
                ...
                return {
                    ...
                    message: safeErrorMessage(
                        error instanceof Error
                            ? error.message
                            : "Unable to finalize repository target",
                    ),
                    exitCode: outcome.exitCode,
                    durationMs: outcome.durationMs,
                };
        ...
        try {
            ...
            const cleanup =
                error instanceof Error
                    ? error.message
                    : "Unable to clean utility-owned staging directory";
            return {
                kind: "failed",
                operation,
                code: "commandFailed",
