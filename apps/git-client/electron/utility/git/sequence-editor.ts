...
import type { RebasePlanEntry } from "../../../src/shared/contracts/model";
...
const ACTION_COMMANDS = new Set(["pick", "p", "reword", "edit", "squash", "fixup", "drop"]);
...
function macFor(payload: string, nonce: string): string {
  return createHmac("sha256", Buffer.from(nonce, "hex")).update(payload, "utf8").digest("hex");
}
...
function parseOperation(operation: unknown): HistoryRewriteOperation {
  ...
  if (!result.success) {
    throw invalid(result.error.issues[0]?.message ?? "Invalid interactive rebase plan");
  }
  if (
    ...
    throw invalid("Sequence editor payload must contain a history rewrite operation");
  }
...
async function readBoundedUtf8(
  ...
  try {
    ...
    if (!metadata.isFile()) throw invalid(`${field} must be a regular file`);
    if (
      expectedTarget !== undefined &&
      (metadata.dev !== expectedTarget.device || metadata.ino !== expectedTarget.inode)
    ) {
...
async function readPayload(payloadPath: string, nonce: string): Promise<SequenceEditorPayload> {
  ...
  if (
    ...
    throw new SequenceEditorError("authenticationFailed", "Sequence editor payload is not trusted");
  }
  ...
  try {
    ...
    throw new SequenceEditorError("authenticationFailed", "Sequence editor payload is malformed");
  }
  ...
  if (parsed.data.expiresAtMs < Date.now()) {
    throw new SequenceEditorError("expired", "Sequence editor payload expired");
  }
  if (
...
async function checkedTarget(
  ...
  if (metadata.isSymbolicLink() || !metadata.isFile()) {
    throw invalid("Editor target must be a regular file, not a symbolic link");
  }
  ...
  if (!isInside(gitDirectory, parent)) {
    throw invalid("Editor target must stay inside the registered Git directory");
  }
  return {
    path: targetPath,
    device: metadata.dev,
...
function todoOid(command: string, fields: readonly string[]): string | null {
  ...
  for (let index = 1; index + 1 < fields.length; index += 1) {
    if (fields[index] === "-C" || fields[index] === "-c") return fields[index + 1] ?? null;
  }
  return null;
}
...
function entryForOid(entries: readonly RebasePlanEntry[], oid: string): RebasePlanEntry {
  ...
  if (matches.length === 0) throw invalid(`Commit ${oid} is missing from the rebase plan`);
  if (matches.length > 1) throw invalid(`Commit ${oid} is ambiguous in the rebase plan`);
  const entry = matches[0];
  if (entry === undefined) throw invalid(`Commit ${oid} is missing from the rebase plan`);
  return entry;
}
...
function rewritePlanLine(line: TodoLine, entry: RebasePlanEntry): string {
  if (line.command === "merge") {
    if (!entry.mergeCommit || entry.action !== "pick") {
      throw invalid("Merge commits must remain pick entries when preserving merges");
    }
    return line.line;
  }
  if (entry.mergeCommit) {
    throw invalid("A merge commit is missing its merge-preserving todo command");
  }
...
function replaceTodoCommand(line: TodoLine, command: string): string {
  if (line.command === "merge") {
    throw invalid("Merge commits cannot be rewritten by this history operation");
  }
...
function rewriteTargetedTodo(
  todo: string,
  operation: Exclude<HistoryRewriteOperation, Readonly<{ kind: "interactiveRebase" }>>,
): string {
  const parsed = parseTodo(todo);
  const revisions = operation.kind === "rewordCommit" ? [operation.revision] : operation.revisions;
  const matched = new Set<string>();
  const targeted = parsed.flatMap((line, index) => {
    if (line.oid === null) return [];
    const matches = revisions.filter((revision) => oidMatches(revision, line.oid ?? ""));
    if (matches.length > 1) throw invalid(`Commit ${line.oid} is ambiguous in the rebase todo`);
  ...
  if (revisions.some((revision) => !matched.has(revision.toLowerCase()))) {
    throw invalid("Not every selected commit is present in the rebase todo");
  }
  if (operation.kind === "squashCommits") {
    if (targeted.length < 2) throw invalid("Squash requires at least two commits");
    for (let index = 1; index < targeted.length; index += 1) {
      const previous = targeted[index - 1];
      const current = targeted[index];
      if (previous === undefined || current === undefined || current.index !== previous.index + 1) {
        throw invalid("Squashed commits must be consecutive in the rebase todo");
      }
    }
  }
  const targetIndexes = new Map(targeted.map((target, index) => [target.index, index]));
  const output = parsed.map((line, index) => {
    const targetIndex = targetIndexes.get(index);
    if (targetIndex === undefined) return line.line;
    if (operation.kind === "dropCommits") return replaceTodoCommand(line, "drop");
    if (operation.kind === "rewordCommit") return replaceTodoCommand(line, "reword");
    return targetIndex === 0 ? line.line : replaceTodoCommand(line, "squash");
  });
  return `${output.join("\n")}\n`;
}

function rewriteTodo(todo: string, operation: HistoryRewriteOperation): string {
  if (operation.kind !== "interactiveRebase") return rewriteTargetedTodo(todo, operation);
  const parsed = parseTodo(todo);
  const preserveMerges =
    operation.options.preserveMerges || parsed.some((line) => line.command === "merge");
  const matched = new Set<string>();
  if (preserveMerges) {
    const output = parsed.map((line) => {
      ...
      if (matched.has(normalizedOid)) throw invalid("Rebase todo contains a duplicate commit");
  ...
  const output = operation.entries.map((entry) => {
    ...
    if (matches.length !== 1)
      throw invalid(`Commit ${entry.oid} is missing or ambiguous in the todo`);
    const line = matches[0];
    if (line === undefined) throw invalid(`Commit ${entry.oid} is missing from the todo`);
    return rewritePlanLine(line, entry);
  });
  output.push(...parsed.filter((line) => line.oid === null).map((line) => line.line));
  return `${output.join("\n")}\n`;
}

function rewriteMessage(existing: string, operation: HistoryRewriteOperation): string {
  if (operation.kind === "rewordCommit") return `${operation.message.trim()}\n`;
  ...
  if (matching.length > 1) throw invalid("Commit message matches multiple reword entries");
  const squashMessage = operation.entries.some((entry) => entry.action === "squash")
    ? (operation.entries.find((entry) => entry.action === "reword")?.message ?? null)
    : null;
...
async function atomicWrite(
  ...
  const temporary = join(dirname(target.path), `.git-client-sequence-${randomUUID()}.tmp`);
  try {
...
export class SequenceEditorSession {
  ...
  private constructor(directory: string, payloadPath: string, nonce: string, signal?: AbortSignal) {
    this.directory = directory;
    this.payloadPath = payloadPath;
    this.nonce = nonce;
    this.#signal = signal ?? null;
    this.#abortListener = signal === undefined ? null : () => void this.cleanup();
  ...
  static async create(
    ...
    try {
      ...
      const session = new SequenceEditorSession(directory, payloadPath, nonce, signal);
      if (signal?.aborted === true) {
      ...
      throw new SequenceEditorError(
        "io",
        error instanceof Error ? error.message : "Could not create sequence editor payload",
      );
    }
  }
...
export async function applySequenceEditor(request: ApplySequenceEditorRequest): Promise<void> {
