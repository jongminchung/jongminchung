...
describe("리더 서비스", () => {
  it("[성공] 공간을 통해 에너지원을 갖춘, 작업 공간 및 포인트 단위의 추적이 아닌 레이어링", async () => {
    ...
    expect(await readFile(join(directory, "untracked", untrackedPath))).toEqual(
      untrackedBytes,
    );
  ...
  it("[실패] 고정 Git argv, 마이애미 cwd, 권한 승인 및 호출자의 중단 신호만 사용함", async () => {
    ...
    expect(
      runner.specs
        .slice(0, 2)
        .every((spec) => spec.stdoutLimitBytes === MAX_SHELF_PATCH_BYTES + 1),
    ).toBe(true);
    expect(runner.signals.every((signal) => signal === controller.signal)).toBe(
      true,
    );
    expect(
      await readFile(
        join(storageRoot, "shelves", record.id, entry.id, "index.patch"),
      ),
    ).toEqual(indexPatch);
  });
  ...
  it("[실패] Git 또는 파일 시스템 균열이 발생하기 전에 먼저 잘못된 메시지 및 해석을 수행함", async () => {
    ...
    await expect(access(join(storageRoot, "shelves"))).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
  ...
  it("[실패] 추적하지 않은 응답을 프리플라이트하고 하위 링크 링크 함 이스케이프를 사랑했습니다", async () => {
    ...
    await expect(access(join(outside, "payload.bin"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(service.list(record.id)).resolves.toEqual([entry]);
  });
