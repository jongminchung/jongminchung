...
export class ChangelistService {
    ...
    async save(
        ...
        return this.#mutex.run(validatedRepositoryId, signal, async () => {
            ...
            if (input.id === null) {
                ...
                changelists = [
                    ...manifest.changelists.map(cloneChangelist),
                    saved,
                ];
            } else {
                ...
                if (existing === undefined)
                    throw invalid("Changelist does not exist");
    ...
    async commit(
        ...
        return this.#mutex.run(validatedRepositoryId, signal, async () => {
            ...
            if (changelist === undefined)
                throw invalid("Changelist does not exist");
            if (changelist.paths.length === 0)
                throw invalid("Changelist has no files");

            const originalHead = await captureHead(
                this.#runner,
                repository,
                signal,
            );
            const indexBackup = await backupIndex(
                this.#runner,
                repository,
                signal,
            );
    ...
    async #writeManifest(
        ...
        if (directory === null)
            throw invalid("Unable to create changelist directory");
        ...
        if (
            checksum(payloadBytes(persisted)) !==
            checksum(payloadBytes(payload))
        ) {
