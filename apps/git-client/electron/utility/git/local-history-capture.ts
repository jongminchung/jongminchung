...
import { LocalHistoryStorage, gunzipLocalHistory } from "./local-history-storage";
import {
  commandFailure,
  contained,
  invalid,
...
export class LocalHistoryCapture {
  ...
  async captureState(
    ...
    const [indexText, untrackedText, statusText] = await Promise.all([
      ...
      this.git(repository.path, ["ls-files", "--others", "--exclude-standard", "-z", "--"], signal),
      this.git(
        repository.path,
  ...
  async readContent(
    ...
    if (state.content.kind === "blob") {
      return gunzipLocalHistory(await readFile(this.storage.blobPath(state.content.sha256)));
    }
    const repository = this.registry.get(repositoryId);
    const outcome = await this.runner.run(
      {
