...
export function validateRepositoryPath(path: string): void {
    if (path.length === 0 || path.length > 16_384) {
        throw new GitUtilityError(
            "invalidInput",
            "Repository path must contain 1 to 16384 characters",
        );
    }
    if (path.includes("\0")) {
        throw new GitUtilityError(
            "invalidInput",
            "Repository path must not contain a null byte",
        );
    }
    if (!isAbsolute(path)) {
        throw new GitUtilityError(
...
export function validateRevision(revision: string): void {
    if (revision.length === 0 || revision.length > 512) {
        throw new GitUtilityError(
            "invalidInput",
            "Revision must contain 1 to 512 characters",
        );
    }
    ...
    if (revision.startsWith("-") || containsUnsafeCharacter) {
        throw new GitUtilityError(
            "invalidInput",
            "Revision contains an unsafe character",
        );
    }
}
