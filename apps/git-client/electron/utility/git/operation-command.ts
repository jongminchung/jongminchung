...
export type GitOperationCommand =
    | ProcessOperationCommand
    | SequenceOperationCommand;
...
function validateRevisions(revisions: readonly string[]): void {
    if (revisions.length === 0)
        invalid("revisions must contain at least one revision");
...
function validateUrl(url: string): void {
    ...
    if (url.length === 0 || url.startsWith("-"))
        invalid("url must not be an option");
}

function validateWorktreePath(path: string): void {
    validateText(path, "worktreePath");
    if (!isAbsolute(path))
        invalid("worktreePath must be an absolute path selected by the user");
}
...
function validateRebasePlan(entries: readonly RebasePlanEntry[]): void {
    ...
    for (const entry of entries) {
        ...
        if (seen.has(entry.oid))
            invalid("entries must not contain duplicate commits");
        seen.add(entry.oid);
    ...
    switch (operation.kind) {
        ...
        case "push": {
            ...
            if (operation.mode.kind === "forceWithLease") {
                validateObjectOid(
                    operation.mode.expectedRemoteOid,
                    "expectedRemoteOid",
                );
                args.push(
        ...
        case "deleteBranch":
            ...
            return {
                ...
                args: [
                    "branch",
                    operation.force ? "--delete-force" : "--delete",
                    operation.name,
                ],
            };
        case "setUpstream":
        ...
        case "squashCommits": {
            ...
            if (operation.revisions.length < 2) {
                invalid(
                    "revisions must contain at least two commits for squash",
                );
            }
            const oldest = operation.revisions.at(-1);
            if (oldest === undefined) invalid("revisions must not be empty");
        ...
        case "rewordCommit":
            validateRevision(operation.revision);
            validateText(operation.message, "message");
            if (operation.message.trim().length === 0)
                invalid("message must not be empty");
            return {
                kind: "sequence",
                args: [
                    "rebase",
        ...
        case "setConfig": {
            ...
            if (operation.value === null)
                args.push("--unset-all", operation.key);
            else {
