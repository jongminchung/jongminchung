...
class FakeRepositoryWatchSource implements RepositoryWatchSource {
  ...
  subscribe(
    ...
    const listeners = this.#listeners.get(root) ?? new Set<(path: string) => void>();
    const errorListeners = this.#errorListeners.get(root) ?? new Set<(error: Error) => void>();
    ...
    return {
      close: () => {
        ...
        if (errorListeners.size === 0) this.#errorListeners.delete(root);
      },
    };
  }
...
async function createFixture(): Promise<{
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "git-client-watcher-"));
...
function nextEvent(events: RepositoryChangedEvent[]): Promise<RepositoryChangedEvent> {
  return new Promise((resolve, reject) => {
    ...
    const poll = (): void => {
      ...
      if (Date.now() - startedAt > 3_000) {
        reject(new Error("Timed out waiting for repository watcher event"));
        return;
      }
      setTimeout(poll, 10);
    };
...
afterEach(async () => {
  await Promise.all(watcherServices.splice(0).map((service) => service.dispose()));
  await Promise.all(
...
describe("RepositoryWatcherService", () => {
  it("cleans up an initial subscription failure so a later watch can succeed", async () => {
    ...
    await expect(service.watch(repositoryId, (event) => events.push(event))).rejects.toThrow(
      "Injected repository watcher subscription failure",
    );
    await expect(
  ...
  it("reconnects after a runtime error and retries a failed reconnect with bounded backoff", async () => {
    ...
    await waitUntil(() => source.subscriptionAttempts === 3 && source.roots.length === 1);

    await expect(nextEvent(events)).resolves.toEqual({
      repositoryId,
      invalidations: ["status", "history", "stash", "operation", "management"],
    });
    source.emit(join(root, "tracked.txt"));
    await expect(nextEvent(events)).resolves.toEqual({
      repositoryId,
  ...
  it("invalidates status and history when HEAD changes", async () => {
    const { gitDirectory, repositoryId, service, source } = await createFixture();
    const events: RepositoryChangedEvent[] = [];
    await service.watch(repositoryId, (event) => events.push(event));
  ...
  it("debounces a burst into one canonically ordered invalidation event", async () => {
    const { root, gitDirectory, repositoryId, service, source } = await createFixture();
    const events: RepositoryChangedEvent[] = [];
    await service.watch(repositoryId, (event) => events.push(event));
  ...
  it("invalidates management for another linked worktree metadata change", async () => {
    ...
    source.emit(join(linked.commonDirectory, "worktrees", "another", "HEAD"));
