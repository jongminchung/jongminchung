...
import { MultiRootService, type MultiRootRecoveryRecorder } from "./multi-root-service";
...
function state(root: string): RepositoryState {
  return {
    ...
    branches: git(root, "for-each-ref", "--format=%(refname):%(objectname)", "refs/heads"),
...
class RecordingRecovery implements MultiRootRecoveryRecorder {
  ...
  recordBeforeOperation(
    ...
    if (signal?.aborted === true) return Promise.reject(new Error("recovery cancelled"));
...
class EventRunner implements GitProcessRunnerLike {
  ...
  async run(spec: GitProcessSpec, signal?: AbortSignal): Promise<GitProcessOutcome> {
    if (isMutation(spec)) {
      this.#events.push(
...
class MutationGateRunner implements GitProcessRunnerLike {
  ...
  async run(spec: GitProcessSpec, signal?: AbortSignal): Promise<GitProcessOutcome> {
    ...
    this.maximumActiveMutations = Math.max(this.maximumActiveMutations, this.activeMutations);
    try {
...
describe("MultiRootService", () => {
  it("synchronizes unique repositories in deterministic order and restores exact branch state", async () => {
    ...
    const sorted = [...records].sort((left, right) => left.id.localeCompare(right.id, "en"));
    const before = new Map(fixtures.map((fixture) => [fixture.record.id, state(fixture.root)]));
    const events: string[] = [];
    const recovery = new RecordingRecovery(events);
    const service = MultiRootService.of(registry, recovery, new EventRunner(events, records));
    ...
    expect(result.rollbackPlan.every((step) => step.operations.length === 2)).toBe(true);
    ...
    const rollbackOutcomes = await service.applyMultiRootRollback(result.rollbackPlan);
  ...
  it("stops at the first failing repository and returns rollback only for prior successes", async () => {
    ...
    expect(git(sorted[0]?.root ?? "", "branch", "--show-current")).toBe("feat/sync");
    expect(git(sorted[2]?.root ?? "", "branch", "--show-current")).toBe("main");

    await expect(service.applyMultiRootRollback(result.rollbackPlan)).resolves.toMatchObject([
      { succeeded: true },
    ]);
  ...
  it("prevents mutation when recovery recording fails and redacts credentials", async () => {
    ...
    const recovery = new RecordingRecovery(
      null,
      new Error("https://alice:super-secret@example.invalid token=ghp_12345678901234567890"),
    );
    const service = MultiRootService.of(registry, recovery);

    const result = await service.executeSynchronizedBranchOperation([fixture.record.id], {
      kind: "createBranch",
      name: "feat/blocked",
      startPoint: "HEAD",
      checkout: true,
  ...
  it("honors cancellation and recovery timeout without changing the repository", async () => {
    ...
    const timeoutResult = await timeoutService.executeSynchronizedBranchOperation(
      [fixture.record.id],
      { kind: "checkout", target: "topic", force: false },
    );
  ...
  it("validates rollback path identity and allowed operations before applying the plan", async () => {
    ...
    if (first === undefined || second === undefined) throw new Error("fixtures are missing");
    const service = MultiRootService.of(registry, new RecordingRecovery());
    const result = await service.executeSynchronizedBranchOperation([first.record.id], {
      kind: "createBranch",
      name: "feat/rollback",
      startPoint: "HEAD",
      checkout: true,
    ...
    await expect(service.applyMultiRootRollback([wrongPath])).rejects.toMatchObject({
      code: "invalidInput",
    });
    await expect(
    ...
    await expect(service.applyMultiRootRollback([step, step])).rejects.toMatchObject({
      code: "invalidInput",
    });
  ...
  it("serializes concurrent operations for the same repository without deadlock", async () => {
    ...
    const service = MultiRootService.of(registry, new RecordingRecovery(), runner);

    const first = service.executeSynchronizedBranchOperation([fixture.record.id], {
      kind: "checkout",
      target: "topic-a",
      force: false,
    });
    await firstStarted;
    const second = service.executeSynchronizedBranchOperation([fixture.record.id], {
      kind: "checkout",
      target: "topic-b",
      force: false,
    });
