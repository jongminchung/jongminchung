...
import {
    SubmoduleDiffService,
    parseNestedSubmoduleStatus,
} from "./submodule-diff-service";
...
async function createNestedSubmoduleFixture(): Promise<{
    ...
    const temporaryDirectory = await mkdtemp(
        join(tmpdir(), "git-client-submodule-foundation-"),
    );
    ...
    await writeFile(
        join(nestedCheckout, "nested.txt"),
        "dirty nested\n",
        "utf8",
    );
...
describe("해석NestedSubmoduleStatus", () => {
    it("[실패] 공백이 있는 곳에는 존재하지 않고 잘못된 형식의 데이터를 가지고 있음", () => {
        ...
        expect(
            parseNestedSubmoduleStatus(
                ` ${oid} deps/space module (heads/main)\n`,
            ),
        ).toEqual([
            {
