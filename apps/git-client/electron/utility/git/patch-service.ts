...
export type PatchCancellationReason =
    | "requested"
    | "repositoryClosed"
    | "timeout";
...
function signalCancellationReason(
    signal: AbortSignal,
): PatchCancellationReason {
    return signal.reason === "repositoryClosed"
        ? "repositoryClosed"
        : "requested";
}
...
function redactedStderr(chunks: readonly Buffer[]): Buffer {
    return Buffer.from(
        redactCredentials(joined(chunks).toString("utf8")),
        "utf8",
    );
}
...
export class PatchProcessRunner implements PatchProcessRunnerLike {
    ...
    run(
        ...
        return new Promise((resolve) => {
            ...
            let stdoutBytes = 0;
            let stderrBytes = 0;
            let stopReason: PatchCancellationReason | "outputLimit" | null =
                null;
            let settled = false;
            let forceKillTimer: ReturnType<typeof setTimeout> | null = null;

            const stop = (
                reason: PatchCancellationReason | "outputLimit",
            ): void => {
            ...
            child.once("close", (exitCode) => {
                ...
                if (stopReason === "outputLimit") {
                    finish({
                        ...
                        code: "outputLimit",
                        message:
                            "Git patch output exceeded its configured limit",
                        exitCode,
                ...
                finish({
                    ...
                    code: "commandFailed",
                    message:
                        capturedStderr.toString("utf8") ||
                        `Git exited with status ${exitCode ?? -1}`,
                    exitCode,
                    stdout: capturedStdout,
                    stderr: capturedStderr,
                    durationMs: elapsed(startedAt),
...
function validatedSelectedPath(untrustedPath: unknown): string {
    if (
        ...
        throw invalidPath(
            "Patch path must be an absolute file path without null bytes",
        );
    }
...
async function selectedParent(untrustedPath: unknown): Promise<SelectedParent> {
    ...
    try {
        ...
        if (
            ...
            throw invalidPath(
                "Patch parent must be a real directory, not a symbolic link",
            );
        }
        ...
        if (
            ...
            throw invalidPath(
                "Canonical patch parent must remain a real directory",
            );
        }
        return {
...
async function destinationState(path: string): Promise<DestinationState> {
    try {
        ...
        if (metadata.isSymbolicLink())
            throw invalidPath("Patch target must not be a symbolic link");
        if (!metadata.isFile())
            throw invalidPath("Patch target must be a regular file");
...
async function assertStableParent(parent: SelectedParent): Promise<void> {
    try {
        const [selectedMetadata, canonicalMetadata, canonicalAgain] =
            await Promise.all([
        ...
        if (
            ...
            !sameIdentity(
                identity(selectedMetadata),
                parent.selectedIdentity,
            ) ||
...
async function prepareAtomicTarget(
    untrustedPath: unknown,
): Promise<AtomicTarget> {
    const parent = await selectedParent(untrustedPath);
    return {
        ...parent,
        destination: await destinationState(parent.canonicalTarget),
    };
}
...
async function atomicWritePatch(
    ...
    if (
        !sameDestination(
            target.destination,
            await destinationState(target.canonicalTarget),
        )
    ) {
    ...
    try {
        ...
        if (
            committed.kind !== "file" ||
            !sameIdentity(temporaryIdentity, committed)
        ) {
            throw invalidPath(
                "Patch target changed while it was being committed",
            );
        }
        await assertStableParent(target);
...
async function readSelectedPatch(
    ...
    if (initial.kind !== "file")
        throw invalidPath("Selected patch must be a regular file");
    await assertStableParent(parent);
    try {
        const handle = await open(
            parent.canonicalTarget,
        ...
        try {
            ...
            if (
                !metadata.isFile() ||
                !sameIdentity(initial, identity(metadata))
            ) {
                throw invalidPath(
                    "Selected patch changed before it could be opened",
                );
            }
            await assertStableParent(parent);
            const chunks: Buffer[] = [];
...
function validatedRevisions(untrustedRevisions: unknown): readonly string[] {
    if (
        ...
        throw new GitUtilityError(
            "invalidInput",
            "Patch revisions must contain 1 to 500 entries",
        );
    }
    ...
    for (const revision of untrustedRevisions) {
        if (typeof revision !== "string") {
            throw new GitUtilityError(
                "invalidInput",
                "Every patch revision must be a string",
            );
        }
...
export class PatchService {
    ...
    async createPatchText(
        ...
        if (!isUtf8(patch)) {
            throw new GitUtilityError(
                "invalidInput",
                "Clipboard patch is not valid UTF-8 text",
            );
        }
