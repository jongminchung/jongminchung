  ...
  run(
    ...
    return new Promise((resolve) => {
      ...
      const append = (
        ...
        const retained =
          value.byteLength <= available ? value : value.subarray(0, available);
      ...
      const onAbort = (): void => {
        stop(
          signal === undefined ? "requested" : signalCancellationReason(signal),
        );
      };
...
async function selectedParent(untrustedPath: unknown): Promise<SelectedParent> {
  ...
  try {
    ...
    const detail =
      error instanceof Error ? error.message : "Patch parent is inaccessible";
...
async function atomicWritePatch(
  ...
  try {
    ...
    if (error instanceof GitUtilityError) throw error;
    const detail =
      error instanceof Error ? error.message : "Unable to write patch file";
...
async function readSelectedPatch(
  ...
  try {
    ...
    if (error instanceof GitUtilityError) throw error;
    const detail =
      error instanceof Error ? error.message : "Unable to read selected patch";
    throw invalidPath(safeErrorMessage(detail));
  }
}
