...
export class WorkingTreeFileResolver {
    ...
    constructor(
        ...
        fileSystem: WorkingTreeFileSystem = nodeFileSystem,
    ) {
    ...
    async resolve(repositoryId: RepositoryId, path: string): Promise<string> {
        ...
        if (repository.isBare) {
            throw invalidPath(
                "Bare repositories do not have working-tree files",
            );
        }
        ...
        try {
            ...
            if (!isBelow(canonicalRoot, canonicalFile)) {
                throw invalidPath(
                    "Working-tree path resolves outside the repository",
                );
            }
            ...
            if (
                ...
                throw invalidPath(
                    "Working-tree path changed while it was being resolved",
                );
            }
