...
interface BinaryGitReader {
  capture(cwd: string, args: readonly string[], maximumBytes: number): Promise<Buffer | null>;
}
...
class SpawnBinaryGitReader implements BinaryGitReader {
  capture(cwd: string, args: readonly string[], maximumBytes: number): Promise<Buffer | null> {
    return new Promise((resolve, reject) => {
      ...
      timeout = setTimeout(() => {
        child.kill("SIGKILL");
        finish(() => reject(new GitUtilityError("commandFailed", "Git file read timed out")));
      }, 120_000);
      timeout.unref();
      child.stdout.on("data", (value: Buffer) => {
        ...
        if (sizeBytes > maximumBytes) {
          child.kill("SIGKILL");
          finish(() =>
            reject(new GitUtilityError("outputLimit", `Git output exceeded ${maximumBytes} bytes`)),
          );
          return;
        }
      ...
      child.once("error", (error) => {
        finish(() => reject(new GitUtilityError("gitUnavailable", error.message)));
      });
      child.once("close", (exitCode) => {
        finish(() => resolve(exitCode === 0 ? Buffer.concat(chunks) : null));
      });
    });
  }
}
...
export function classifyFileContent(path: string, bytes: Uint8Array): FileContent {
...
export function classifyFilePreview(path: string, bytes: Uint8Array): FilePreview {
  const buffer = Buffer.from(bytes);
  const sizeBytes = buffer.byteLength;
  if (sizeBytes > MAX_FILE_BYTES) return { kind: "tooLarge", path, sizeBytes };
...
export class GitFileService {
  ...
  static of(registry: RepositoryRegistry): GitFileService {
    return new GitFileService(registry, new SpawnBinaryGitReader(), new GitProcessRunner());
  }
  ...
  async writeWorkingTreeFile(
    ...
    if (repository.isBare) {
      throw new GitUtilityError("invalidInput", "Bare repositories do not have worktree files");
    }
    const bytes = Buffer.from(content, "utf8");
    if (bytes.byteLength > MAX_FILE_BYTES) {
      throw new GitUtilityError("outputLimit", `File content exceeds ${MAX_FILE_BYTES} bytes`);
    }
    ...
    try {
      ...
      if (!metadata.isFile()) {
        throw new GitUtilityError("invalidInput", "Only regular worktree files can be edited");
      }
      ...
      try {
        ...
        if (isMissingFileError(parentError)) {
          throw new GitUtilityError("invalidInput", "The parent directory does not exist");
        }
        throw parentError;
      }
    ...
    const temporary = join(dirname(canonical), `.git-client-edit-${randomUUID()}.tmp`);
  ...
  async loadSubmoduleDiff(
    ...
    validateRelativePath(path);
    const repository = this.#registry.get(repositoryId);
    const beforeOid = await this.#submoduleOid(repository.path, before, path);
    ...
    const beforeSubject =
      directory === null || beforeOid === null
        ? null
        : await this.#captureOptional(directory, ["show", "-s", "--format=%s", beforeOid]);
    const afterSubject =
      directory === null || afterOid === null
        ? null
        : await this.#captureOptional(directory, ["show", "-s", "--format=%s", afterOid]);
    const counts =
  ...
  async #readSource(
    ...
    const object =
      source.kind === "index" ? `:${path}` : this.#revisionObject(source.revision, path);
  ...
  async #readWorkingTree(root: string, path: string): Promise<SourceReadResult> {
    let canonical: string;
    try {
    ...
    if (metadata.size > MAX_FILE_BYTES) return { kind: "tooLarge", sizeBytes: metadata.size };
    return { kind: "bytes", bytes: await readFile(canonical) };
  }

  async #readObject(root: string, object: string, path: string): Promise<SourceReadResult> {
    const rawSize = await this.#git.capture(root, ["cat-file", "-s", object], 1_024);
    if (rawSize === null) return { kind: "missing" };
    const text = rawSize.toString("ascii").trim();
    if (!/^\d+$/u.test(text)) {
      throw new GitUtilityError("commandFailed", `Git returned an invalid size for ${path}`);
    }
    ...
    const bytes = await this.#git.capture(root, ["cat-file", "blob", object], MAX_FILE_BYTES);
    return bytes === null ? { kind: "missing" } : { kind: "bytes", bytes };
  }

  async #submoduleOid(root: string, source: FileSource, path: string): Promise<string | null> {
    ...
    if (source.kind === "workingTree") {
      ...
      output =
        directory === null
          ? null
          : await this.#captureOptional(directory, ["rev-parse", "--verify", "HEAD"]);
    } else if (source.kind === "index") {
      output = await this.#captureOptional(root, ["ls-files", "--stage", "--", path]);
    } else {
      validateRevision(source.revision);
      output = await this.#captureOptional(root, ["ls-tree", source.revision, "--", path]);
    }
    ...
    return fields.map((field) => this.#parseOid(field)).find((oid) => oid !== null) ?? null;
  }
  ...
  async #submoduleDirectory(root: string, path: string): Promise<string | null> {
    ...
    if (relativePath === ".." || relativePath.startsWith(`..${sep}`) || isAbsolute(relativePath)) {
      throw new GitUtilityError("invalidInput", "Submodule path resolves outside the repository");
    }
    return (await stat(canonical)).isDirectory() ? canonical : null;
  }

  async #captureOptional(cwd: string, args: readonly string[]): Promise<string | null> {
    ...
    if (outcome.kind === "failed" && outcome.code === "commandFailed") return null;
