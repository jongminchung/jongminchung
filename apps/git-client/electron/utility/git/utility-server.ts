...
export class GitUtilityProtocolServer {
    ...
    async #receive(untrustedMessage: unknown): Promise<void> {
        ...
        const result =
            MainToGitUtilityMessageSchema.safeParse(untrustedMessage);
        if (!result.success) {
            this.#postError(
                null,
                "invalidMessage",
    ...
    async #route(
        ...
        switch (message.kind) {
            case "openRepository": {
                const repository = await this.#utility.openRepository(
                    message.request,
                );
                this.#post({
            ...
            case "initializeRepository": {
                await this.#utility.initializeRepository(
                    message.request,
                    (event) => {
                        if (this.#disposed) return;
                        this.#post({
            ...
            case "cloneRepository": {
                await this.#utility.cloneRepository(
                    message.request,
                    (event) => {
                        if (this.#disposed) return;
                        this.#post({
            ...
            case "closeRepository": {
                const closed = this.#utility.closeRepository(
            ...
            case "inspectSnapshot": {
                const snapshot = await this.#utility.inspectSnapshot(
                    message.repositoryId,
                );
                this.#post({
            ...
            case "repositoryService": {
                const result = await this.#utility.executeRepositoryService(
                    message.request,
                );
                this.#post({
            ...
            case "watchRepository": {
                await this.#utility.watchRepository(
                    message.repositoryId,
                    (event) => {
                        if (this.#disposed) return;
        ...
        if (error instanceof ZodError) {
            return {
                ...
                message: safeErrorMessage(
                    error.issues[0]?.message ?? "Invalid Git utility request",
                ),
            };
        }
        return {
