...
async function createRepository(): Promise<{
    ...
    readonly repositoryId: Awaited<
        ReturnType<RepositoryRegistry["open"]>
    >["id"];
}> {
...
describe("GitPreviewService push preview", () => {
    test("reports a new branch then an exact fast-forward lease", async () => {
        ...
        const second = await fixture.service.pushPreview(
            fixture.repositoryId,
            null,
            null,
            "HEAD",
    ...
    test("reports divergent local and remote commits after fetch", async () => {
        ...
        await writeFile(
            join(fixture.repository, "local.txt"),
            "local\n",
            "utf8",
        );
        ...
        expect(
            preview.remoteOnlyCommits.map((commit) => commit.subject),
        ).toEqual(["remote"]);
...
describe("GitPreviewService history rewrite preview", () => {
    test("captures published commits and dependent refs without changing the repository", async () => {
        ...
        await writeFile(
            join(fixture.repository, "three.txt"),
            "three\n",
            "utf8",
        );
        ...
        const preview = await fixture.service.historyRewritePreview(
            fixture.repositoryId,
            "HEAD~1",
        );
        expect(preview).toMatchObject({
        ...
        expect(
            preview.entries.map((entry) => [entry.subject, entry.published]),
        ).toEqual([
            ["two", true],
            ["three", false],
        ]);
        expect(preview.dependentRefs.map((entry) => entry.name)).toEqual([
        ...
        expect(
            git(fixture.repository, "status", "--porcelain=v2", "--branch"),
        ).toBe(before);
    });
