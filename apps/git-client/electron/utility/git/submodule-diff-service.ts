...
export function parseNestedSubmoduleStatus(
    output: string,
): readonly GitSubmoduleInfo[] {
    ...
    for (const line of output.split("\n")) {
        ...
        entries.push(
            Object.freeze({
                ...
                oid,
                branch:
                    branchMatch === null || branchMatch[1]?.length === 0
                        ? null
                        : (branchMatch[1] ?? null),
                status:
                    marker === "-"
...
export class SubmoduleDiffService {
    ...
    async loadSubmoduleDiff(
        ...
        const beforeSubject = await this.#subject(
            directory?.path ?? null,
            beforeOid,
            signal,
        );
        const afterSubject = await this.#subject(
            directory?.path ?? null,
            afterOid,
            signal,
        );
        ...
        const nestedSubmodules = await this.#nestedSubmodules(
            directory,
    ...
    async #nestedSubmodules(
        ...
        for (const entry of entries) {
            ...
            if (entry.initialized) {
                const nestedDirectory = await this.#resolveSubmoduleDirectory(
                    directory.path,
                    entry.path,
                );
                if (nestedDirectory === null) {
                    throw invalidInput(
                        "Nested submodule changed while metadata was loaded",
                    );
                }
                const status = await this.#captureRequired(
                ...
                dirty = status.length > 0;
                await this.#verifyDirectoryPin(
                    join(directory.path, entry.path),
                    nestedDirectory,
                );
            }
    ...
    async #resolveSubmoduleDirectory(
        root: string,
        path: string,
    ): Promise<DirectoryPin | null> {
    ...
    async #pinDirectory(
        ...
        try {
            ...
            if (requireChild && canonical === path) {
                throw invalidInput(
                    "Submodule path is not below its repository",
                );
            }
            const metadata = await stat(canonical);
    ...
    async #verifyDirectoryPin(
        ...
        try {
            ...
            if (
                ...
                throw invalidInput(
                    "Repository path changed while submodule metadata was loaded",
                );
            }
        } catch (error) {
            if (error instanceof GitUtilityError) throw error;
