import type { GitRequest } from "../../../src/generated";
...
export type QueryOnlyGitRequest = Exclude<
    GitRequest,
...
function validateRefName(value: string, field: string): void {
    ...
    if (
        ...
        throw new GitUtilityError(
            "invalidInput",
            `${field} is not a safe Git ref name`,
        );
    }
}
...
function validateRefComponent(value: string, field: string): void {
    ...
    if (value.includes("/")) {
        throw new GitUtilityError(
            "invalidInput",
            `${field} must be a single ref component`,
        );
    }
}
...
function validatePage(skip: number, limit: number): void {
    if (!Number.isSafeInteger(skip) || skip < 0) {
        throw new GitUtilityError(
            "invalidInput",
            "Git query skip must be a non-negative integer",
        );
    }
    if (!Number.isSafeInteger(limit)) {
        throw new GitUtilityError(
            "invalidInput",
            "Git query limit must be an integer",
        );
    }
}
...
function logArguments(
    request: Extract<QueryOnlyGitRequest, Readonly<{ kind: "log" }>>,
): string[] {
    ...
    if (request.filters.query !== null) {
        ...
        args.push("--regexp-ignore-case", `--grep=${request.filters.query}`);
    }
...
export function buildRequestArguments(
    request: QueryOnlyGitRequest,
): readonly string[] {
    switch (request.kind) {
        case "status":
            return [
