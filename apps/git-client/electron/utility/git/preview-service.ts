...
function validateRemote(value: string): void {
  ...
  if (value.includes("/")) throw invalid("Remote must be a single ref component");
}
...
function protectedBranch(branch: string): boolean {
  return (
    ["main", "master", "production", "release"].includes(branch) || branch.startsWith("release/")
  );
}
...
export class GitPreviewService {
  ...
  async pushPreview(
    ...
    const sourceBranch = this.#trimmed(
      await this.#captureOptional(repositoryId, ["symbolic-ref", "--quiet", "--short", "HEAD"]),
    );
    ...
    if (remoteOutcome.kind === "completed") {
      remoteOid = outputText(remoteOutcome, "stdout").trim().split(/\s+/u)[0] || null;
    } else if (remoteOutcome.kind === "cancelled" && remoteOutcome.reason === "timeout") {
      remoteStateError = "Remote state could not be verified. Check connectivity and try again.";
    } else {
      remoteStateError =
    ...
    const remoteObjectAvailable =
      remoteOid !== null &&
      (await this.#succeeds(repositoryId, ["cat-file", "-e", `${remoteOid}^{commit}`]));
    const fastForward =
      remoteOid === null
        ? true
        : remoteObjectAvailable
          ? await this.#succeeds(repositoryId, ["merge-base", "--is-ancestor", remoteOid, localOid])
          : null;
    const [ahead, behind] =
      remoteOid !== null && remoteObjectAvailable
        ...
        : ([0, 0] as const);
    const commitsRange =
      remoteOid !== null && remoteObjectAvailable ? `${remoteOid}..${localOid}` : localOid;
    const commits = await this.#previewCommits(repositoryId, commitsRange);
    const remoteOnlyCommits =
      remoteOid !== null && remoteObjectAvailable
        ? await this.#previewCommits(repositoryId, `${localOid}..${remoteOid}`)
        : [];
    ...
    if (remoteOid !== null && !remoteObjectAvailable) {
      warnings.push("Fetch before force-pushing so remote-only commits can be reviewed locally.");
    }
    if (fastForward === false) {
      warnings.push("The destination is not a fast-forward. Normal push is disabled.");
    }
    if (isProtected) {
      warnings.push(
  ...
  async historyRewritePreview(
    ...
    const repository = this.#registry.get(repositoryId);
    if (
      (await detectInProgressOperation(repository.gitDirectory, repository.commonDirectory)) !==
      null
    ) {
      throw invalid("Finish or abort the current Git operation before rewriting history");
    }
    const branch = this.#trimmed(
      await this.#captureOptional(repositoryId, ["symbolic-ref", "--quiet", "--short", "HEAD"]),
    );
    if (branch === null) throw invalid("History rewrite requires an attached branch");
    const headOid = (await this.#capture(repositoryId, ["rev-parse", "--verify", "HEAD"])).trim();
    validateObjectId(headOid, "HEAD");
    if (
      !(await this.#succeeds(repositoryId, ["merge-base", "--is-ancestor", fromRevision, "HEAD"]))
    ) {
      throw invalid("From revision must belong to the current branch history");
    }
    ...
    if (entries.length === 0) throw invalid("From revision did not produce a rewrite range");
    ...
    const dependentRefs: DependentRefImpact[] = refs.split("\n").flatMap((line) => {
      const separator = line.indexOf("\0");
      if (separator < 0) return [];
    ...
    const publishedCommitCount = entries.filter((entry) => entry.published).length;
    ...
    if (dependentRefs.length > 0) {
      warnings.push(`${dependentRefs.length} dependent local ref(s) may need updating.`);
    }
    if (isProtected) {
      warnings.push(
  ...
  async #succeeds(repositoryId: RepositoryId, args: readonly string[]): Promise<boolean> {
    const repository = this.#registry.get(repositoryId);
    return (await this.#runner.run({ cwd: repository.path, args })).kind === "completed";
  }
