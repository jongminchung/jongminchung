...
async function createRepository(): Promise<string> {
    const temporaryDirectory = await mkdtemp(
        join(tmpdir(), "git-client-conflict-"),
    );
...
async function createOperationConflict(
...
describe("힘내 충돌 서비스", () => {
    it("[성공] 세 가지의 충돌 단계를 모두 설명하고, 알아보고, 설명하고, 선택적으로 준비함", async () => {
        ...
        await expect(
            readFile(join(root, "conflict.txt"), "utf8"),
        ).resolves.toBe("reviewed\n");
        expect(git(root, "diff", "--name-only", "--diff-filter=U", "-z")).toBe(
            "conflict.txt\0",
        );
    ...
    it("[실패] 일체형에서 경로를 평가하지 않고 기본적으로 바이너리 플레이어를 해결함", async () => {
        ...
        const { repositoryId, root, service } = await createMergeConflict(
            path,
            {
                ...
                remote: Buffer.from([
                    0x00, 0x72, 0x65, 0x6d, 0x6f, 0x74, 0x65, 0xfd,
                ]),
            },
        );
        ...
        await expect(service.list(repositoryId)).resolves.toMatchObject([
            { path, binary: true },
        ]);
        ...
        await expect(pathExists(join(root, "injected.bin"))).resolves.toBe(
            false,
        );
    ...
    it("[성공] 바이트 또는 라인 경계를 넘어서는 충돌 그룹을 경계로 구분함", async () => {
        ...
        expect(
            Buffer.byteLength(`base\n${oversizedShared}`, "utf8"),
        ).toBeGreaterThan(MAX_CONFLICT_TEXT_BYTES);
        await expect(
            oversized.service.list(oversized.repositoryId),
        ).resolves.toMatchObject([{ path: "oversized.txt", binary: true }]);
        await expect(
        ...
        expect(
            Buffer.byteLength(`base\n${excessiveLinesShared}`, "utf8"),
        ).toBeLessThan(MAX_CONFLICT_TEXT_BYTES);
        await expect(
            excessiveLines.service.list(excessiveLines.repositoryId),
        ).resolves.toMatchObject([
            { path: "too-many-lines.txt", binary: true },
        ]);
        await expect(
            excessiveLines.service.read(
                excessiveLines.repositoryId,
                "too-many-lines.txt",
            ),
        ).resolves.toMatchObject({
            base: null,
    ...
    it("[실패] 외부 파일을 변경하지 않고 순회하고, 키보드 링크 및 키보드 링크가 상위 링크가 되었습니다", async () => {
        ...
        await expect(
            service.write(
                repositoryId,
                "../outside/conflict.txt",
                "escaped\n",
                false,
            ),
        ).rejects.toMatchObject({ code: "invalidInput" });
        await expect(
            service.read(repositoryId, "../outside/conflict.txt"),
        ...
        await expect(
            service.write(repositoryId, path, "escaped\n", false),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
