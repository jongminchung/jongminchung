...
import {
    ElectronGitBridge,
    type ElectronGitApi,
} from "../../../src/bridge/ElectronGitBridge";
...
> {
    const temporaryDirectory = await mkdtemp(
        join(tmpdir(), "git-client-bridge-submodule-"),
    );
    temporaryDirectories.push(temporaryDirectory);
...
async function createQueryRepository(): Promise<string> {
    const temporaryDirectory = await mkdtemp(
        join(tmpdir(), "git-client-bridge-queries-"),
    );
    temporaryDirectories.push(temporaryDirectory);
    ...
    await writeFile(
        join(repository, "stashed-untracked.txt"),
        "stash me\n",
        "utf8",
    );
    git(
    ...
    await writeFile(
        join(repository, "token=private-token.txt"),
    ...
    return {
        ...
        async initializeRepository(path, bare) {
            ...
            if (terminal.kind !== "completed")
                throw new Error(`Initialize ${terminal.kind}`);
        ...
        async cloneRepository(url, path, options) {
            ...
            if (terminal.kind !== "completed")
                throw new Error(`Clone ${terminal.kind}`);
            return terminal.repository;
        },
        inspectSnapshot: (repositoryId) =>
            utility.inspectSnapshot(repositoryId),
        ...
        listMergedBranches: (repositoryId, target) =>
            utility.listMergedBranches(repositoryId, target),
        ...
        readIgnoreRules: (repositoryId) =>
            utility.readIgnoreRules(repositoryId),
        writeIgnoreRules: (repositoryId, rules) =>
            utility.writeIgnoreRules(repositoryId, rules),
        loadPushPreview: (repositoryId, remote, remoteRef, localRevision) =>
            utility.loadPushPreview(
                repositoryId,
                remote,
                remoteRef,
                localRevision,
        ...
        async exportPatch(repositoryId, revisions, targetPath) {
            ...
            if (result.operation !== "exportPatch")
                throw new Error("Unexpected exportPatch result");
            return result.value;
        },
        ...
        async importPatch(repositoryId, path) {
            ...
            if (result.operation !== "importPatch")
                throw new Error("Unexpected importPatch result");
        },
        async createShelf(repositoryId, message, paths) {
            ...
            if (result.operation !== "createShelf")
                throw new Error("Unexpected createShelf result");
            return result.value;
        },
        async listShelves(repositoryId) {
            ...
            if (result.operation !== "listShelves")
                throw new Error("Unexpected listShelves result");
            return result.value;
        },
        async applyShelf(repositoryId, shelfId, dropAfterApply) {
            ...
            if (result.operation !== "applyShelf")
                throw new Error("Unexpected applyShelf result");
        },
        async deleteShelf(repositoryId, shelfId) {
            ...
            if (result.operation !== "deleteShelf")
                throw new Error("Unexpected deleteShelf result");
        },
        ...
        async listConflicts(repositoryId) {
            ...
            if (result.operation !== "listConflicts")
                throw new Error("Unexpected listConflicts result");
            return result.value;
        },
        async readConflict(repositoryId, path) {
            ...
            if (result.operation !== "readConflict")
                throw new Error("Unexpected readConflict result");
            return result.value;
        },
        ...
        readFile: (repositoryId, source, path) =>
            utility.readFile(repositoryId, source, path),
        ...
        async executeSynchronizedBranchOperation(repositoryIds, gitOperation) {
            ...
            if (result.operation !== "executeSynchronizedBranchOperation")
                throw new Error(
                    "Unexpected executeSynchronizedBranchOperation result",
                );
            return result.value;
        },
        ...
        watchRepository: (repositoryId, listener) =>
            utility.watchRepository(repositoryId, listener),
        unwatchRepository: (repositoryId) =>
            utility.unwatchRepository(repositoryId),
        closeRepository: async (repositoryId) =>
            utility.closeRepository(repositoryId),
        executeQuery: (request, listener) =>
            utility.executeQuery(request, listener),
...
describe("ElectronGitBridge 실제 유틸리티 구성", () => {
    it("[성공] 무시하고, 검사하고, 관리하고, 관리하고 데이터를 관리하세요", async () => {
        ...
        await expect(
            bridge.compareBranches(snapshot.id, "main", "HEAD"),
        ).resolves.toEqual({
            ahead: 0,
            behind: 0,
            leftOnly: [],
        ...
        await expect(
            bridge.listMergedBranches(snapshot.id, "HEAD"),
    ...
    it("[실패] 실제 하위 모듈 gitlink diff를 로드하고 안전하지 않은 작업 트리 파일을 반환하고 있음", async () => {
        ...
        await expect(
            bridge.openWorkingTreeFile(snapshot.id, ".gitmodules"),
        ).resolves.toBeUndefined();
        await expect(
            bridge.openWorkingTreeFile(snapshot.id, "../outside.txt"),
    ...
    it("[성공] 두 개의 실제 서버에 하나의 분기를 생성하고 롤백함", async () => {
        ...
        const result = await bridge.executeSynchronizedBranchOperation(
            [second.id, first.id],
            {
                kind: "createBranch",
        ...
        expect(git(firstPath, "branch", "--show-current").trim()).toBe(
            "feature/synchronized-flow",
        );
        expect(git(secondPath, "branch", "--show-current").trim()).toBe(
            "feature/synchronized-flow",
        );

        const rollback = await bridge.applyMultiRootRollback(
            result.rollbackPlan,
        );
        ...
        await Promise.all([
            bridge.unwatchRepository(first.id),
            bridge.unwatchRepository(second.id),
        ]);
    });
    ...
    it("[성공] 렌더러 다리를 통해 생성된 17개의 작업 비 요청을 모두 실행함", async () => {
        ...
        for (const request of requests) {
            const { requestId, events } = await executeToTerminal(
                bridge,
                request,
            );
            ...
            const sequences = events
                ...
                .map((event) =>
                    event.kind === "output" ? event.sequence : -1,
                );
            expect(sequences, request.kind).toEqual(
        ...
        expect(output.get("configList")).not.toContain(
            "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890",
        );
        expect(output.get("pushPreview")).toContain("refs/heads/main");
        await expect(
            bridge.loadPushPreview(
                repositoryId,
                "origin",
                "refs/heads/main",
                "HEAD",
        ...
        await expect(
            bridge.loadHistoryRewritePreview(repositoryId, "HEAD~1"),
        ).resolves.toMatchObject({
            branch: "main",
            descendantCount: 2,
            entries: [
    ...
    it("[성공] 하나의 비상용 부품에 대해 패치, 선반, 변경 목록, 복구 및 충돌 서비스를 전송함", async () => {
        ...
        const exported = await bridge.exportPatch(
            repositoryId,
            ["HEAD"],
            patchPath,
        );
        ...
        expect(await readFile(join(path, "tracked.txt"), "utf8")).toBe(
            "second\n",
        );
        ...
        expect(await readFile(join(path, "untracked.txt"), "utf8")).toBe(
            "untracked\n",
        );
        ...
        const changelist = await bridge.saveChangelist(
            repositoryId,
            null,
            "selected files",
            ["tracked.txt"],
        ...
        const restored = await bridge.restoreRecoveryEntry(
            repositoryId,
            recoveryEntry.id,
        );
        expect(restored.restoredRefs).toEqual(["refs/heads/main"]);
        expect(git(path, "log", "-1", "--format=%s").trim()).toBe(
            "fixture commit",
        );
        git(path, "reset", "--hard", "HEAD");

        const disposable = await bridge.saveChangelist(
            repositoryId,
            null,
            "delete me",
            [],
        ...
        await expect(
            bridge.readConflict(repositoryId, "tracked.txt"),
        ).resolves.toMatchObject({
            path: "tracked.txt",
            local: "main\n",
            remote: "feature\n",
        });
        await bridge.writeConflictResult(
            repositoryId,
            "tracked.txt",
            "resolved\n",
            true,
    ...
    it("[성공] 하나의 다리 생활주기를 통해 대표 색상, 커밋, 참조, 숨김, 구성, 원격 및 작업 트리 변형을 실행함", async () => {
        ...
        expect(git(path, "branch", "--list", "bridge-feature")).toContain(
            "bridge-feature",
        );
        expect(git(path, "tag", "--list", "bridge-v1").trim()).toBe(
            "bridge-v1",
        );
        ...
        expect(git(path, "config", "--local", "gitclient.fixture").trim()).toBe(
            "enabled",
        );
        await executeCompletedOperation(bridge, repositoryId, {
            kind: "remoteAdd",
        ...
        expect(git(path, "remote", "get-url", "backup").trim()).toBe(
            join(path, "..", "backup.git"),
        );
        await executeCompletedOperation(bridge, repositoryId, {
            kind: "remoteRemove",
            name: "backup",
        ...
        expect(git(path, "worktree", "list", "--porcelain")).toContain(
        ...
        expect(git(path, "worktree", "list", "--porcelain")).not.toContain(
            worktreePath,
        );
        await bridge.unwatchRepository(repositoryId);
    });
    ...
    it("[성공] 프로세스 실행기에서 다리까지 교대로 stdout 및 stderr 연대를 유지함", async () => {
        ...
        const queries = new GitQueryService(
            registry,
            new GitProcessRunner(fakeGit),
        );
        const api: ElectronGitApi = {
            ...bridgeApi(new GitUtility()),
            executeQuery: (request, listener) =>
                queries.execute(request, listener),
        };
