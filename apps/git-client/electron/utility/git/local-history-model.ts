...
export const StoredActivitySchema = z
  .object({
    ...
    system: z.boolean(),
    changes: z.array(StoredChangeSchema).max(MAX_LOCAL_HISTORY_ACTIVITY_COUNT),
  })
  .strict();
...
export const ManifestSchema = z
  .object({
    version: z.literal(LOCAL_HISTORY_STORAGE_VERSION),
    activityIds: z.array(z.string().uuid()).max(MAX_LOCAL_HISTORY_ACTIVITY_COUNT),
  })
  .strict();
...
export function isLocalHistoryTextPath(path: string): boolean {
  ...
  return TEXT_EXTENSIONS.has(index < 0 ? "" : name.slice(index + 1).toLowerCase());
}

export function publicLocalHistoryChange(change: StoredChange): GitLocalHistoryChange {
...
export function publicLocalHistoryActivity(activity: StoredActivity): GitLocalHistoryActivity {
