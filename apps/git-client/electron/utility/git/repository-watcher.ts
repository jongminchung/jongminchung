...
function isOperationMetadata(path: string): boolean {
  if (
    ["MERGE_HEAD", "CHERRY_PICK_HEAD", "REVERT_HEAD", "BISECT_LOG", "AUTO_MERGE"].includes(path)
  ) {
    return true;
  }
...
function classifyRepositoryPath(
  ...
  if (metadata !== null) {
    ...
    if (metadataPath === join("refs", "stash")) return ["status", "history", "stash"];
    if (metadataPath === "HEAD") return ["status", "history"];
    if (metadataPath === "refs" || metadataPath.startsWith(`refs${sep}`)) return ["history"];
    if (isOperationMetadata(metadataPath)) return ["status", "operation"];
    if (metadataPath === "config" || metadataPath === "config.worktree") return ["management"];
    if (
      !metadata.currentWorktree &&
      (metadataPath === "worktrees" || metadataPath.startsWith(`worktrees${sep}`))
    ) {
      return ["management"];
    }
    return [];
...
function canonicalWatchRoots(record: RepositoryRecord): readonly string[] {
  const roots: string[] = [];
  for (const candidate of [record.path, record.commonDirectory, record.gitDirectory]) {
...
export class RepositoryWatcherService {
  ...
  static of(
    ...
    if (!Number.isSafeInteger(debounceMs) || debounceMs < 1 || debounceMs > 10_000) {
      throw new Error(
  ...
  async watch(repositoryId: RepositoryId, listener: RepositoryChangedListener): Promise<void> {
    ...
    try {
      ...
      if (this.#sessions.get(repositoryId) === session) this.#sessions.delete(repositoryId);
  ...
  async dispose(): Promise<void> {
    await Promise.all([...this.#sessions.keys()].map((repositoryId) => this.unwatch(repositoryId)));
  }
      ...
      for (const root of canonicalWatchRoots(session.record)) {
        subscriptions.push(
          this.#source.subscribe(
            root,
            (path) => {
              for (const invalidation of classifyRepositoryPath(session.record, path)) {
                this.#invalidate(repositoryId, invalidation);
              }
            },
            () => queueMicrotask(() => this.#handleRuntimeError(repositoryId, session)),
          ),
        );
      }
  ...
  #handleRuntimeError(repositoryId: RepositoryId, session: WatchSession): void {
    if (this.#sessions.get(repositoryId) !== session || session.reconnectTimer !== null) return;
  ...
  #scheduleReconnect(repositoryId: RepositoryId, session: WatchSession): void {
    if (this.#sessions.get(repositoryId) !== session || session.reconnectTimer !== null) return;
    const delay = Math.min(
      this.#reconnectMaxDelayMs,
      this.#reconnectInitialDelayMs * 2 ** Math.min(session.reconnectAttempt, 16),
    );
  ...
  #invalidate(repositoryId: RepositoryId, invalidation: RepositoryInvalidation): void {
    const session = this.#sessions.get(repositoryId);
    ...
    session.timer = setTimeout(() => {
      session.timer = null;
      if (this.#sessions.get(repositoryId) !== session || session.invalidations.size === 0) return;
