...
export class GitUtility {
    ...
    constructor(
        ...
        this.#inspection = new RepositoryInspectionService(
            this.#registry,
            runner,
        );
    ...
    async openRepository(untrustedRequest: unknown): Promise<RepositoryRecord> {
        ...
        return this.#registry.open(request.path);
    }
    ...
    async executeRepositoryService(
        ...
        try {
            const result = await this.#executeRepositoryService(
                request,
                cancellation.signal,
            );
    ...
    watchRepository(
        ...
        const repositoryId = RepositoryIdSchema.parse(untrustedRepositoryId);
        return this.#watchers.watch(repositoryId, listener);
    }
        ...
        switch (request.operation) {
            case "compareBranches":
                return {
                    ...
                    value: await this.compareBranches(
                        request.repositoryId,
                        request.left,
                        request.right,
                    ),
            ...
            case "listMergedBranches":
                return {
                    operation: request.operation,
                    value: await this.listMergedBranches(
                        request.repositoryId,
                        request.target,
                    ),
                };
            case "loadCommitSignature":
                return {
                    operation: request.operation,
                    value: await this.loadCommitSignature(
                        request.repositoryId,
                        request.revision,
                    ),
                };
            ...
            case "historyRewritePreview":
                return {
                    operation: request.operation,
                    value: await this.loadHistoryRewritePreview(
                        request.repositoryId,
            ...
            case "importPatch":
                await this.#patches.importPatch(
                    request.repositoryId,
                    request.path,
                    signal,
                );
            ...
            case "listShelves":
                return {
                    operation: request.operation,
                    value: await this.#stored(this.#shelves).list(
                        request.repositoryId,
                    ),
                };
            case "applyShelf":
                ...
                return { operation: request.operation };
            case "deleteShelf":
                await this.#stored(this.#shelves).delete(
                    request.repositoryId,
                    request.shelfId,
                );
                return { operation: request.operation };
            case "listChangelists":
                return {
                    operation: request.operation,
                    value: await this.#stored(this.#changelists).list(
                        request.repositoryId,
                        signal,
                    ),
                };
            ...
            case "listRecoveryEntries":
                return {
                    operation: request.operation,
                    value: await this.#stored(this.#recovery).list(
                        request.repositoryId,
                        signal,
                    ),
                };
            ...
            case "captureLocalHistory":
                return {
                    operation: request.operation,
                    value: await this.#stored(this.#localHistory).capture(
                        request.repositoryId,
                        request.label,
                        signal,
                    ),
                };
            case "listLocalHistory":
                return {
                    operation: request.operation,
            ...
            case "restoreLocalHistory":
                await this.#stored(this.#localHistory).restore(
                    request.repositoryId,
                    request.entryId,
                    request.path,
                    signal,
                ...
                return { operation: request.operation };
            case "labelLocalHistory":
                return {
                    operation: request.operation,
                    value: await this.#stored(this.#localHistory).label(
                        request.repositoryId,
                        request.entryId,
                        request.label,
                    ),
            ...
            case "listConflicts":
                return {
                    operation: request.operation,
                    value: await this.#conflicts.list(
                        request.repositoryId,
                        signal,
                    ),
                };
            case "readConflict":
                return {
                    operation: request.operation,
                    value: await this.#conflicts.read(
                        request.repositoryId,
                        request.path,
                        signal,
                    ),
            ...
            case "resolveWorkingTreeFile":
                return {
                    operation: request.operation,
                    value: await this.#workingTreeFiles.resolve(
                        request.repositoryId,
                        request.path,
                    ),
                };
