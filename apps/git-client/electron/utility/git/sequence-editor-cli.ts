...
export const SEQUENCE_EDITOR_APPLICATION_ARGUMENT =
    "--git-client-sequence-editor";
...
export function createSequenceEditorCommand(
    request: SequenceEditorCommandRequest,
): string {
    if (
...
function parseMode(value: string | undefined): SequenceEditorMode {
    ...
    throw new SequenceEditorError(
        "invalidInput",
        "Expected --sequence-editor or --message-editor",
    );
}
...
function parseArguments(arguments_: readonly string[]): {
    ...
    if (
        arguments_.length !== 6 ||
        arguments_[1] !== "--payload" ||
        arguments_[3] !== "--nonce"
    ) {
        throw new SequenceEditorError(
            "invalidInput",
            "Invalid sequence editor arguments",
        );
    }
...
function publicError(error: unknown, nonce: string | undefined): string {
    ...
    return message
        ...
        .replaceAll(nonce.toUpperCase(), "[redacted]");
}
...
export async function runSequenceEditorCli(
    ...
    try {
        ...
        writeError(
            `Git Client sequence editor: ${publicError(error, arguments_[4])}\n`,
        );
        return 2;
    }
}
