...
async function canonicalTarget(requestedPath: string): Promise<string> {
    if (!isAbsolute(requestedPath) || requestedPath.includes("\0")) {
        throw new Error(
            "Repository path must be an absolute path without null bytes",
        );
    }
...
export class RepositoryCreateTarget {
    ...
    static async prepare(
        ...
        if (await pathExists(finalPath)) {
            ...
            if (
                operation === "clone" &&
                (await readdir(finalPath)).length > 0
            ) {
                throw new Error("Clone destination must be empty");
            }
            return new RepositoryCreateTarget(
                finalPath,
                finalPath,
                null,
                stagePrefix,
    ...
    display(value: string): string {
        return this.#ownedStage === null
            ? value
            : value.replaceAll(this.#ownedStage, this.finalPath);
    }
    ...
    async commit(): Promise<string> {
        if (this.#ownedStage !== null) {
            if (await pathExists(this.finalPath)) {
                throw new Error(
                    "Repository target was created by another process during the operation",
                );
            }
