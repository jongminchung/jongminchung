...
function appendFastImportCommits(
  repository: string,
  ref: string,
  count: number,
): void {
  ...
  if (result.status !== 0)
    throw new Error(result.stderr || "git fast-import failed");
}
...
describe("전자 Git 유틸리티", () => {
  ...
  it("[성공] 젤리 URL 스페이스 UUID를 사용하여 일반적인 유니코드/백그라운드를 사용하다", async () => {
    ...
    expect(record.gitVersion.major > 2 || record.gitVersion.minor >= 39).toBe(
      true,
    );
  ...
  it("[성공] 500개의 커밋 페이지에 실제 토폴로지 기록을 로드함", async () => {
    ...
    const firstPage = parseLog(
      stdout(
        await execute(
          service,
          query("log", record.id, { skip: 0, limit: 500, filters }),
        ),
      ),
    );
    const secondPage = parseLog(
      stdout(
        await execute(
          service,
          query("log", record.id, { skip: 500, limit: 500, filters }),
        ),
      ),
    );
    ...
    expect(
      new Set([...firstPage, ...secondPage].map((commit) => commit.oid)),
    ).toHaveLength(502);
  }, 30_000);
  ...
  it("[성공] 작업, 용도 서비스 및 직접 파일 표면에 변형을 적용함", async () => {
    ...
    const queuedDirectWrite = utility.writeWorkingTreeFile(
      first.id,
      "queued.txt",
      "queued\n",
    );
    ...
    try {
      await utility.writeWorkingTreeFile(
        second.id,
        "parallel.txt",
        "parallel\n",
      );
      ...
      await expect(
        readFile(join(secondPath, "parallel.txt"), "utf8"),
      ).resolves.toBe("parallel\n");
      await expect(access(join(firstPath, ".gitignore"))).rejects.toMatchObject(
        { code: "ENOENT" },
      );
      await expect(access(join(firstPath, "queued.txt"))).rejects.toMatchObject(
        { code: "ENOENT" },
      );
  ...
  it("[성공] Git 응답을 반환하는 대신 범위를 캡처했습니다", async () => {
    ...
    expect(outcome.output.map((entry) => entry.data).join("")).not.toContain(
      "token=x",
    );
  });
