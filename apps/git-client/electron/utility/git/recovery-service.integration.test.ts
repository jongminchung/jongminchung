...
class DelegatingRunner implements GitProcessRunnerLike {
  ...
  constructor(
    beforeRun: (spec: GitProcessSpec, signal: AbortSignal | undefined) => void,
  ) {
...
describe("복구 서비스", () => {
  ...
  it("[성공] 부분적으로 식별된, 식별된, 삭제된, 추적되지 않은, 유니크하고 바이너리 상태의 유지", async () => {
    ...
    expect(
      git(fixture.repository, "status", "--porcelain=v2", "--branch", "-z"),
    ).toBe(beforeStatus);
    expect(
      git(fixture.repository, "diff", "--cached", "--binary", "--no-ext-diff"),
    ).toBe(beforeIndexDiff);
    expect(git(fixture.repository, "diff", "--binary", "--no-ext-diff")).toBe(
      beforeWorktreeDiff,
    );
    await expect(
  ...
  it("[성공] 복원이 취소되기 전에 역을 유지하고 참조를 그대로 유지함", async () => {
    ...
    expect(
      entries.some((candidate) => candidate.operation === "restore commit"),
    ).toBe(true);
