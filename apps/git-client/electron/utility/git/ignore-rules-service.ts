...
import type { IgnoreRules } from "../../../src/generated";
    ...
    try {
        ...
        if (metadata.isSymbolicLink()) {
            throw new GitUtilityError(
                "invalidInput",
                "Ignore rule files must not be symbolic links",
            );
        }
        if (metadata.size > MAX_IGNORE_RULE_BYTES) {
            throw new GitUtilityError(
                "invalidInput",
                "Ignore rule files must not exceed 1 MiB",
            );
        }
        ...
        if (!Buffer.from(text, "utf8").equals(content)) {
            throw new GitUtilityError(
                "invalidInput",
                "Ignore rule files must contain valid UTF-8",
            );
        }
        return text;
    } catch (error) {
...
function validateRules(rules: IgnoreRules): void {
    for (const content of [rules.gitignore, rules.infoExclude]) {
        if (Buffer.byteLength(content, "utf8") > MAX_IGNORE_RULE_BYTES) {
            throw new GitUtilityError(
                "invalidInput",
                "Ignore rules must not exceed 1 MiB per file",
            );
        }
...
export class IgnoreRulesService {
    ...
    async write(repositoryId: RepositoryId, rules: IgnoreRules): Promise<void> {
        validateRules(rules);
        const repository = this.#registry.get(repositoryId);
        await writeFixedText(
            join(repository.path, ".gitignore"),
            rules.gitignore,
        );
        await writeFixedText(
            join(repository.gitDirectory, "info", "exclude"),
            rules.infoExclude,
        );
    }
}
