...
describe("로컬히스토리서비스", () => {
    it("[성공] 초기 프로젝트 상태를 기준선으로만 사용함", async () => {
        ...
        await expect(
            service.list(
                { kind: "project", repositoryId: repository.id },
                null,
            ),
    ...
    it("[실패] 역방향 텍스트를 기록하고 최선을 다하지 않고 트리를 복원함", async () => {
        ...
        expect(activity).toMatchObject({
            name: "Editing tracked.txt",
            changeCount: 1,
        });
        if (activity === null)
            throw new Error("Expected a Local History activity");
        await expect(
            service.diff(repository.id, activity.id, "tracked.txt"),
        ).resolves.toContain("+three");
    ...
    it("[실패] 바이너리 내용을 저장하지 않고 바이너리 구조 변경 사항을 기록함", async () => {
        ...
        if (activity === null)
            throw new Error("Expected a Local History activity");
        await expect(
            service.detail(repository.id, activity.id),
    ...
    it("[성공] 다중 파일 작업 이름의 그룹을 수신하고 유니코드 이름 변경을 감지함", async () => {
        ...
        await service.initialize(repository.id);
        await writeFile(
            join(repositoryPath, "first.txt"),
            "changed first\n",
            "utf8",
        );
        await service.record(repository.id, "Replace in Files");
        await writeFile(
            join(repositoryPath, "second.txt"),
            "changed second\n",
            "utf8",
        );
        await service.record(repository.id, "Replace in Files");

        const grouped = await service.list(
            { kind: "project", repositoryId: repository.id },
            null,
        );
        expect(grouped.activities).toHaveLength(1);
        expect(grouped.activities[0]).toMatchObject({
        ...
        await rename(
            join(repositoryPath, "first.txt"),
            join(repositoryPath, "한글 file.txt"),
        );
        ...
        await expect(
            service.detail(repository.id, renamed.id),
        ).resolves.toMatchObject({
            changes: [
    ...
    it("[성공] 연속 v1 스냅샷을 마이그레이션하고 원본 보관함을 유지함", async () => {
        ...
        await writeFile(
            ...
            JSON.stringify({
                version: 1,
                entries: [
                    entry(secondId, 2, "Before migration"),
                    entry(firstId, 1, null),
                ],
            }),
        );
        ...
        await writeFile(
            join(legacy, `${firstId}.json`),
            JSON.stringify(snapshot("one\n")),
        );
        await writeFile(
            join(legacy, `${secondId}.json`),
