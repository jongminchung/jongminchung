...
export class LocalHistoryService {
  ...
  async list(
    ...
    return GitLocalHistoryActivitiesPageSchema.parse({
      ...
      nextCursor:
        start + limit < filtered.length ? (page.at(-1)?.id ?? null) : null,
    });
  }
  ...
  async diff(
    ...
    const change = activity.changes.find(
      (candidate) =>
        candidate.path === safePath || candidate.previousPath === safePath,
    );
    ...
    await this.#serialize(id, async () => {
      ...
      const activities = await Promise.all(
        manifest.activityIds
          ...
          .map((value) =>
            this.#storage.readActivity(id, includeLater ? value : activityId),
          ),
      );
