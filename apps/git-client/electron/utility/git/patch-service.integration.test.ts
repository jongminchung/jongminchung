...
): Readonly<{ get(id: RepositoryId): RepositoryRecord }> {
    return {
        get(id) {
            if (id !== record.id)
...
class RecordingPatchRunner implements PatchProcessRunnerLike {
    ...
    readonly #onRun:
        | ((spec: PatchProcessSpec) => void | Promise<void>)
        | undefined;
    ...
    async run(
        ...
        if (outcome === undefined)
            throw new Error("Missing fake patch-process outcome");
        return outcome;
    }
}
...
describe("PatchService contracts", () => {
    it("validates 1..500 revisions and uses canonical repository cwd with fixed argv", async () => {
        ...
        await expect(
            service.createPatchText(record.id, ["HEAD", "feature/한글"]),
        ).resolves.toBe("first patch\nsecond patch\n");
        ...
        for (const revisions of invalid) {
            await expect(
                service.createPatchText(record.id, revisions),
    ...
    it("preserves credential-like stdout bytes and redacts only failure diagnostics", async () => {
        ...
        await expect(
            new PatchService(registry(record), success).createPatchText(
                record.id,
                ["HEAD"],
            ),
        ...
        const secretDiagnostic =
            "https://bob:password@example.invalid token=secret-value";
        const failure = new RecordingPatchRunner([
            failed(
                "commandFailed",
                secretDiagnostic,
                Buffer.from(secretDiagnostic),
            ),
        ]);
    ...
    it("enforces clipboard/export/import byte limits and propagates cancel/timeout/output-limit", async () => {
        ...
        await expect(access(exportTarget)).rejects.toMatchObject({
            code: "ENOENT",
        });

        const largeImport = join(
            await temporaryDirectory(),
            "large-import.patch",
        );
        await writeFile(
            largeImport,
            Buffer.alloc(MAX_IMPORTED_PATCH_BYTES + 1, 0x78),
        );
        const neverRun = new RecordingPatchRunner([]);
        await expect(
            new PatchService(registry(record), neverRun).importPatch(
                record.id,
                largeImport,
            ),
        ...
        await expect(
            new PatchService(registry(record), cancelled).createPatchText(
                record.id,
                ["HEAD"],
            ),
        ).rejects.toMatchObject({
        ...
        await expect(
            new PatchService(registry(record), timedOut).createPatchText(
                record.id,
                ["HEAD"],
            ),
        ).rejects.toMatchObject({
        ...
        await expect(
            new PatchService(registry(record), limited).createPatchText(
                record.id,
                ["HEAD"],
            ),
        ).rejects.toMatchObject({ code: "outputLimit" });
    ...
    it("atomically exports without following target symlinks or a replaced parent", async () => {
        ...
        await expect(
            access(join(outsideDirectory, "escaped.patch")),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
        ...
        await expect(
            new PatchService(registry(record), raceRunner).exportPatch(
                record.id,
                ["HEAD"],
                raceTarget,
            ),
        ).rejects.toMatchObject({ code: "invalidInput" });
        await expect(access(raceTarget)).rejects.toMatchObject({
            code: "ENOENT",
        });
        await expect(
            access(join(movedParent, "result.patch")),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
    });
    ...
    it("imports only a selected regular file and sends its raw bytes through fixed apply argv", async () => {
        ...
        await expect(
            service.importPatch(record.id, linked),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
        await expect(
            service.importPatch(record.id, root),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
        expect(runner.specs).toHaveLength(1);
...
describe("PatchService real Git round trip", () => {
    it("preserves Unicode, shell-meta paths, binary data, and secret-like patch payloads", async () => {
        ...
        expect(exported.includes(Buffer.from("https://alice:secret@"))).toBe(
            true,
        );
        expect(exported.includes(Buffer.from("token=query-secret"))).toBe(true);
        expect(exported.includes(Buffer.from(`ghp_${"a".repeat(32)}`))).toBe(
            true,
        );
        expect(exported.includes(Buffer.from("GIT binary patch"))).toBe(true);
        await expect(
            service.createPatchText(sourceRecord.id, [patchCommit]),
        ).resolves.toContain(
            "https://alice:secret@example.invalid/repository.git",
        );
        ...
        await new PatchService(
            registry(targetRecord),
            new PatchProcessRunner(),
        ).importPatch(targetRecord.id, exportPath);

        expect(await readFile(join(target, shellMetaName), "utf8")).toBe(
            `${secretText}\n`,
        );
        expect(await readFile(join(target, "binary.dat"))).toEqual(binary);
        expect(
            git(target, "diff", "--cached", "--name-only", "-z").split("\0"),
        ).toEqual(expect.arrayContaining([shellMetaName, "binary.dat"]));
