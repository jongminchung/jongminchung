...
describe("패치서비스 계약", () => {
  ...
  it("[성공] 자격이 있다는 것을 증명하고 stdout 바이트를 계속해서 증상을 변경함", async () => {
    ...
    await expect(
      new PatchService(registry(record), success).createPatchText(record.id, [
        "HEAD",
      ]),
  ...
  it("[실패] 클립보드/내보내기/가져오기 바이트 제한을 적용하고 취소/시간 초과/출력 제한을 함", async () => {
    ...
    await expect(
      new PatchService(registry(record), oversizedClipboard).createPatchText(
        record.id,
        ["HEAD"],
      ),
    ...
    await expect(
      new PatchService(registry(record), cancelled).createPatchText(record.id, [
        "HEAD",
      ]),
    ).rejects.toMatchObject({
      code: "commandFailed",
    ...
    await expect(
      new PatchService(registry(record), timedOut).createPatchText(record.id, [
        "HEAD",
      ]),
    ).rejects.toMatchObject({
      code: "commandFailed",
    ...
    await expect(
      new PatchService(registry(record), limited).createPatchText(record.id, [
        "HEAD",
      ]),
    ).rejects.toMatchObject({ code: "outputLimit" });
...
describe("PatchService 실제 Git 상담", () => {
  it("[성공] 유니코드, 공유하다, 바이너리 데이터 및 공유과 패치", async () => {
    ...
    ).resolves.toContain("https://alice:secret@example.invalid/repository.git");
