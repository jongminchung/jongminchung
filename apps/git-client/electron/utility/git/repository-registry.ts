...
export class RepositoryRegistry {
  ...
  async open(path: string): Promise<RepositoryRecord> {
    ...
    try {
      ...
      if (!(await stat(canonicalInput)).isDirectory())
        throw new Error("Path is not a directory");
    } catch (error) {
      const detail =
        error instanceof Error ? error.message : "Path is not accessible";
      throw new GitUtilityError(
        "notRepository",
        `${path} (${safeErrorMessage(detail)})`,
      );
    }
  ...
  async #detectGitVersion(): Promise<GitVersion> {
      ...
      throw new GitUtilityError(
        ...
        safeErrorMessage(
          detail ||
            (outcome.kind === "failed"
              ? outcome.message
              : "Git version check was cancelled"),
        ),
    ...
    if (match === null)
      throw new GitUtilityError(
        "gitUnavailable",
        `Unrecognized Git version: ${display}`,
      );
  ...
  async #canonicalGitPath(
    ...
    try {
      ...
      const detail =
        error instanceof Error ? error.message : "Git path is not accessible";
