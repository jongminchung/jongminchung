...
describe("packaged Git operation flow", () => {
  it("stages, commits, amends, creates a branch, and pushes to the local remote", async () => {
    const fixtureRoot = await mkdtemp(join(tmpdir(), "git-client-operation-flow-"));
    temporaryDirectories.push(fixtureRoot);
    const fixture = await createAuditFixture(fixtureRoot);
    const utility = new GitUtility();
    const repository = await utility.openRepository({ path: fixture.gitClientPath });
    ...
    expect(gitText(fixture.gitClientPath, "branch", "--show-current").trim()).toBe(
      "qa/operation-flow",
    );
    expect(gitText(fixture.gitClientPath, "status", "--porcelain")).toBe("");
    expect(gitText(fixture.gitClientPath, "log", "-2", "--format=%s").trim().split("\n")).toEqual([
