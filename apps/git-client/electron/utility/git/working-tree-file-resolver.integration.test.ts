...
import {
    WorkingTreeFileResolver,
    type WorkingTreeFileSystem,
} from "./working-tree-file-resolver";
...
describe("WorkingTreeFileResolver", () => {
    it("[성공] 내부의 일반 파일에 대해서만 표준을 유지해야 함", async () => {
        ...
        await expect(
            resolver.resolve(record.id, "nested/inside-link"),
    ...
    it("[실패] 이스케이프 링크, 일체형, 힌지된 기하학, 순회, 나선형 바이트 및 인사과 응용 프로그램을 적용함", async () => {
        ...
        for (const path of [
            ...
            await expect(
                resolver.resolve(record.id, path),
    ...
    it("[실패] 정규화와 신원 확인 사이에 대체된 파일을 포함함", async () => {
        ...
        await expect(
            resolver.resolve(record.id, "tracked.txt"),
        ).rejects.toMatchObject({
            code: "invalidInput",
