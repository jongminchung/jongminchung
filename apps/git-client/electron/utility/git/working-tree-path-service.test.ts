...
describe("WorkingTreePathService", () => {
  it("resolves only existing regular files inside an opened repository", async () => {
    ...
    await expect(service.resolveFile(repository.id, "inside.txt")).resolves.toBe(
      await realpath(join(root, "inside.txt")),
    );
    await expect(service.resolveFile(repository.id, "../outside.txt")).rejects.toMatchObject({
      code: "invalidInput",
    });
    await expect(service.resolveFile(repository.id, "link.txt")).rejects.toMatchObject({
      code: "invalidInput",
    });
    await expect(service.resolveFile(repository.id, "missing.txt")).rejects.toMatchObject({
      code: "invalidInput",
    });
  });
});
