...
describe("표면CreateService", () => {
  it("[성공] 메인을 기본 브랜치로 사용하여 어디에서든 유니코드를 찾아보세요", async () => {
    ...
    expect(git(requestedPath, "symbolic-ref", "--short", "HEAD").trim()).toBe(
      "main",
    );
  ...
  it("[실패] 작업용 나무 없이 라이더를 불러함", async () => {
    ...
    expect(git(requestedPath, "rev-parse", "--is-bare-repository").trim()).toBe(
      "true",
    );
    expect(
  ...
  it("[성공] 스트리밍 진행을 통해 분기 분기를 단일 분기로 복제함", async () => {
    ...
    expect(events).toEqual(
      expect.arrayContaining([expect.objectContaining({ kind: "progress" })]),
    );
  });
