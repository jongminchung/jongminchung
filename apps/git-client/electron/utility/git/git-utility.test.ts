...
import { parseLog, parseRefs, parseStashList, parseStatusV2 } from "../../../src/domain/parsers";
import {
...
function appendFastImportCommits(repository: string, ref: string, count: number): void {
  const commands: string[] = [];
  for (let index = 1; index <= count; index += 1) {
    const message = `pagination commit ${index}`;
    commands.push(
      `commit ${ref}`,
      `mark :${index}`,
      `author Git Client Test <git-client@example.invalid> ${1_700_000_000 + index} +0000`,
      `committer Git Client Test <git-client@example.invalid> ${1_700_000_000 + index} +0000`,
      `data ${Buffer.byteLength(message)}`,
      message,
      `from ${index === 1 ? "refs/heads/main" : `:${index - 1}`}`,
      "",
    );
  }
  commands.push("done", "");
...
async function createFixtureRepository(): Promise<string> {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "git-client-electron-"));
...
function query(
  ...
  properties: Partial<Extract<GitQueryRequest, Readonly<{ kind: "log" }>>> = {},
): GitQueryRequest {
  if (kind === "log") {
    return {
      kind,
...
describe("Electron Git utility", () => {
  it("opens a large repository without creating a Local History activity", async () => {
    ...
    for (const index of [1, 2, 3, 4]) {
      await writeFile(join(repositoryPath, `large-${index}.bin`), block);
    }
    const storageRoot = await mkdtemp(join(tmpdir(), "git-client-history-"));
    temporaryDirectories.push(storageRoot);
    const utility = new GitUtility(undefined, undefined, storageRoot);
    ...
    await expect(
      utility.executeRepositoryService({
        ...
        scope: { kind: "project", repositoryId: repository.id },
        cursor: null,
        limit: 100,
        query: "",
      ...
      operation: "listLocalHistoryActivities",
      value: { activities: [], nextCursor: null },
    });
  ...
  it("initializes, registers, and immediately queries a repository", async () => {
    const parent = await mkdtemp(join(tmpdir(), "git-client-electron-create-"));
    ...
    if (terminal.kind !== "completed") throw new Error(`Initialization failed: ${terminal.kind}`);
    expect(utility.listRepositories()).toEqual([terminal.repository]);
    const statusEvents: GitRequestEvent[] = [];
    await utility.executeQuery(query("status", terminal.repository.id), (event) =>
  ...
  it("clones, registers, and immediately queries a repository", async () => {
    ...
    const parent = await mkdtemp(join(tmpdir(), "git-client-electron-clone-"));
    temporaryDirectories.push(parent);
    ...
    if (terminal.kind !== "completed") throw new Error(`Clone failed: ${terminal.kind}`);
    const logEvents: GitRequestEvent[] = [];
    await utility.executeQuery(query("log", terminal.repository.id, { limit: 1 }), (event) =>
  ...
  it("opens a canonical Unicode/space path with the stable URL namespace UUID", async () => {
    ...
    expect(record.gitVersion.major > 2 || record.gitVersion.minor >= 39).toBe(true);
      ...
      value: [
        {
          ...
          pushUrl: "https://[redacted]@example.invalid/repository.git",
        },
      ],
    });
    await expect(
  ...
  it("queries porcelain-v2 status, refs, log, and stashes with ordered terminal events", async () => {
    ...
    const record = await registry.open(repository);
    await writeFile(join(repository, "새 파일 with space.txt"), "untracked\n", "utf8");

    const statusEvents = await execute(service, query("status", record.id));
    expect(statusEvents[0]?.kind).toBe("started");
    expect(statusEvents.at(-1)?.kind).toBe("completed");
    expect(statusEvents.slice(1, -1).every((event) => event.kind === "output")).toBe(true);
    ...
    const stashEvents = await execute(service, query("stashList", record.id));
  ...
  it("loads real topology history in 500-commit pages", async () => {
    ...
    const firstPage = parseLog(
      stdout(await execute(service, query("log", record.id, { skip: 0, limit: 500, filters }))),
    );
    const secondPage = parseLog(
      stdout(await execute(service, query("log", record.id, { skip: 500, limit: 500, filters }))),
    );

    expect(firstPage).toHaveLength(500);
    expect(secondPage).toHaveLength(2);
    expect(firstPage[0]?.subject).toBe("pagination commit 501");
    expect(secondPage.at(-1)?.subject).toBe("첫 commit");
  ...
  it("reads bounded worktree, index, revision, and image content by repository id", async () => {
    ...
    await expect(
      utility.readFile(record.id, { kind: "revision", revision: "HEAD" }, "tracked.txt"),
    ).resolves.toMatchObject({ kind: "text", content: "first\n" });
    await expect(
      utility.readFilePreview(record.id, { kind: "workingTree" }, "image.png"),
    ).resolves.toMatchObject({
      kind: "image",
      preview: {
        mimeType: "image/png",
  ...
  it("owns watcher registration and closes it with the repository", async () => {
    ...
    expect(events).toEqual([{ repositoryId: record.id, invalidations: ["status", "history"] }]);
  ...
  it("bounds captured output instead of returning an unbounded Git response", async () => {
    const repository = await createFixtureRepository();
    await writeFile(join(repository, "large.txt"), "token=x\n".repeat(16 * 1024), "utf8");
    ...
    expect(
      outcome.output.reduce((size, entry) => size + Buffer.byteLength(entry.data), 0),
    ).toBeLessThanOrEqual(1_024);
    expect(outcome.output.map((entry) => entry.data).join("")).not.toContain("token=x");
  });
  ...
  it("cancels an active query when its repository is closed", async () => {
    ...
    const service = new GitQueryService(registry, new GitProcessRunner(blockingGit));
    ...
    const childStartDeadline = Date.now() + 5_000;
    while (Date.now() < childStartDeadline) {
      try {
  ...
  it("makes a request cancellable before delivering its started event", async () => {
    ...
    const terminal = await service.execute(request, (event) => {
      events.push(event);
      if (event.kind === "started") expect(service.cancel(request.requestId)).toBe(true);
    });
...
class FakeRepositoryWatcher {
  ...
  async watch(_repositoryId: RepositoryId, listener: RepositoryChangedListener): Promise<void> {
...
class WaitingRepositoryCreator {
  ...
  #wait(
    ...
    if (signal === undefined) throw new Error("Expected a creation AbortSignal");
    this.signal = signal;
    listener({
      kind: "started",
      operation,
