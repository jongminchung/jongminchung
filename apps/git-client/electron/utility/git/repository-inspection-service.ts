...
export async function parseWorktrees(
  ...
  for (const field of output.split("\0")) {
    ...
    if (field.startsWith("HEAD "))
      current.headOid = field.slice("HEAD ".length);
...
export async function detectInProgressOperation(
  ...
  if (await pathExists(join(gitDirectory, "CHERRY_PICK_HEAD")))
    return "cherryPick";
...
export class RepositoryInspectionService {
  ...
  async inspectSnapshot(
    ...
    if (headOid !== null && upstream !== null) {
      ...
      const [aheadValue = "0", behindValue = "0"] =
        counts?.trim().split(/\s+/u) ?? [];
  ...
  async listSubmodules(
    repositoryId: RepositoryId,
  ): Promise<readonly SubmoduleInfo[]> {
  ...
  async preCommitCheck(repositoryId: RepositoryId): Promise<PreCommitCheck> {
    ...
    for (const path of paths) {
      ...
      const sizeOutput = await this.#captureOptional(repositoryId, [
        "cat-file",
        "-s",
        indexObject,
      ]);
      ...
      if (size > 10 * 1024 * 1024) largeFiles.push(path);
        ...
        const content = await this.#captureOptional(repositoryId, [
          "show",
          indexObject,
        ]);
    ...
    for (const hookName of hookNames) {
      try {
        if (
          (
            await stat(join(repository.commonDirectory, "hooks", hookName))
          ).isFile()
        ) {
          hooks.push(hookName);
        }
  ...
  async listRemotes(
    ...
    const names = (await this.#capture(repositoryId, ["remote"]))
      .split("\n")
      .filter(Boolean);
    return Promise.all(
      names.map(async (name) => ({
        ...
        fetchUrl: redactCredentials(
          (
            await this.#capture(repositoryId, ["remote", "get-url", name])
          ).trim(),
        ),
