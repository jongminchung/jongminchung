...
export async function ensureChildDirectory(
    ...
    if (created === null)
        throw invalid(`${label} disappeared while being created`);
    return created;
}
    ...
    await mkdir(path, { recursive: true, mode: 0o700 }).catch(
        (error: unknown) => {
            throw filesystemError(
                error,
                "Unable to create changelist storage root",
            );
        },
    );
    ...
    try {
        const destinationBefore = await lstat(destination).catch(
            (error: unknown) => {
                ...
                throw filesystemError(
                    error,
                    "Changelist manifest is not accessible",
                );
            },
        );
        if (
        ...
        if (!metadata.isFile())
            throw invalid("Temporary changelist manifest is invalid");
        ...
        if (
            ...
            throw invalid(
                "Temporary changelist manifest changed before commit",
            );
        }
