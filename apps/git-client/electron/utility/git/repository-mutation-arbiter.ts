...
export class RepositoryMutationArbiter {
  ...
  async run<T>(
    ...
    try {
      ...
      if (signal.aborted) throw new RepositoryMutationCancelledError(signal.reason);
  ...
  #acquire(repositoryId: string, signal: AbortSignal): Promise<ReleaseLock> {
    if (signal.aborted) {
      return Promise.reject(new RepositoryMutationCancelledError(signal.reason));
    }
  ...
  #cancelWaiter(repositoryId: string, lock: RepositoryLock, waiter: MutationWaiter): void {
    ...
    if (!lock.held && lock.waiters.length === 0) this.#locks.delete(repositoryId);
  }
