...
export class RecoveryService {
  ...
  async list(
    ...
    return this.#mutex.run(validatedRepositoryId, async () => {
      ...
      return Object.freeze(
        resolved.sort((left, right) => right.createdAtMs - left.createdAtMs),
      );
    });
  }
  ...
  async restore(
    ...
    return this.#mutex.run(validatedRepositoryId, async () => {
      ...
      if (
        !(await this.#refsAreRecoverable(repository.path, entry.refs, signal))
      ) {
      ...
      try {
        ...
        return {
          entryId,
          restoredRefs:
            transaction === null ? [] : [...transaction.restoredRefs],
        };
      } catch (error) {
        try {
          ...
          if (refsApplied) {
            ...
            const rollbackTransaction = createRecoveryRefTransaction(
              currentRefs,
              rollbackCurrent,
            );
