...
const RollbackStepSchema = z
    .object({
        ...
        description: z
            ...
            .refine(
                (value) => !value.includes("\0"),
                "Description must not contain a null byte",
            ),
...
function assertNotAborted(signal: AbortSignal | undefined): void {
    if (signal?.aborted === true)
        throw new MultiRootCancellationError(cancellationReason(signal));
}
...
function createTimedSignal(
    ...
    const onAbort = (): void => {
        if (!controller.signal.aborted)
            controller.abort(parent?.reason ?? "requested");
    };
...
function waitForSignal<T>(
    ...
    if (signal.aborted)
        return Promise.reject(
            new MultiRootCancellationError(cancellationReason(signal)),
        );
...
function validateSynchronizedOperation(
...
function validateRollbackOperation(
    operation: ValidatedGitOperation,
): RollbackOperation {
    if (operation.kind === "checkout" && !operation.force) return operation;
    if (operation.kind === "deleteBranch" && !operation.force) return operation;
    throw invalid(
        "Rollback contains an operation that is not a non-forced checkout or delete",
    );
}

function validateRollbackSteps(
    untrustedSteps: unknown,
): readonly MultiRootRollbackStep[] {
    const parsed = z
        .array(RollbackStepSchema)
        .max(10_000)
        .safeParse(untrustedSteps);
    if (!parsed.success) {
        throw invalid(
            parsed.error.issues[0]?.message ??
                "Invalid multi-root rollback plan",
        );
    }
    ...
    return parsed.data.map((step) => {
        if (seen.has(step.repositoryId)) {
            throw invalid(
                "Rollback plan must not contain duplicate repository ids",
            );
        }
        seen.add(step.repositoryId);
        const operations: RollbackOperation[] = step.operations.map(
            (operation) => validateRollbackOperation(operation),
        );
        return {
...
function normalizeRepositoryIds(
    untrustedIds: readonly RepositoryId[],
): readonly RepositoryId[] {
    if (untrustedIds.length === 0)
        throw invalid("Repository ids must not be empty");
    ...
    try {
        ...
        const detail =
            error instanceof Error
                ? error.message
                : "Repository path is inaccessible";
...
async function assertRepositoryIdentity(
    ...
    if (
        ...
        throw new GitUtilityError(
            "commandFailed",
            "Repository path identity changed during operation",
        );
    }
}
...
async function validateRollbackPath(
    ...
    try {
        ...
        const detail =
            error instanceof Error
                ? error.message
                : "Rollback path is inaccessible";
        throw invalid(safeErrorMessage(detail));
    }
    if (suppliedPath !== record.path) {
        throw invalid(
            "Rollback path does not identify the registered repository",
        );
    }
...
function errorMessage(error: unknown): string {
    if (error instanceof MultiRootCancellationError)
...
function rollbackForOperation(
    ...
    const operations: RollbackOperation[] = [
        { kind: "checkout", target, force: false },
    ];
    ...
    async executeSynchronizedBranchOperation(
        ...
        await Promise.all(
            records.map(async (record, index) => {
                if (record.id !== ids[index])
                    throw invalid("Repository registry identity mismatch");
                await pinRepository(record);
            }),
        );
        ...
        for (const record of records) {
            ...
            if (execution.rollback !== null)
                rollbackPlan.unshift(execution.rollback);
        }
    ...
    async applyMultiRootRollback(
        ...
        const prepared = await Promise.all(
            steps.map(async (step) => {
                ...
                if (record.id !== step.repositoryId)
                    throw invalid("Repository registry identity mismatch");
    ...
    async #executeOne(
        ...
        try {
            ...
            if (command.kind !== "process") {
                throw new Error(
                    "Synchronized operation unexpectedly requires a sequence editor",
                );
            }
            await assertRepositoryIdentity(record, identity);
            assertNotAborted(timed.signal);
            await waitForSignal(
                this.#recovery.recordBeforeOperation(
                    record.id,
                    operation,
                    timed.signal,
                ),
            ...
            if (outcome.kind !== "completed") {
                return {
                    ...
                    outcome: failureOutcome(
                        record,
                        outcomeMessage(outcome, timed.signal),
                    ),
                };
            }
            return {
    ...
    async #rollbackOne(
        ...
        try {
            ...
            for (const untrustedOperation of operations) {
                const parsed = GitOperationSchema.safeParse(untrustedOperation);
                if (!parsed.success)
                    throw invalid("Rollback operation is invalid");
                const operation = validateRollbackOperation(parsed.data);
                const command = buildOperationCommand(operation);
                if (command.kind !== "process") {
                    throw new Error(
                        "Rollback operation unexpectedly requires a sequence editor",
                    );
                }
                await assertRepositoryIdentity(record, identity);
                assertNotAborted(timed.signal);
