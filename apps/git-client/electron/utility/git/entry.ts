...
function storageRootFromArguments(arguments_: readonly string[]): string {
    const positions = arguments_
        .map((argument, index) =>
            argument === GIT_UTILITY_STORAGE_ROOT_ARGUMENT ? index : -1,
        )
        .filter((index) => index >= 0);
    if (positions.length !== 1) {
        throw new Error(
            "Git utility requires exactly one storage root argument",
        );
    }
...
if (parentPort === undefined)
    throw new Error(
        "Git utility entry requires an Electron utility process parent port",
    );
...
const server = new GitUtilityProtocolServer(
    ...
    new GitUtility(
        undefined,
        undefined,
        storageRootFromArguments(process.argv),
    ),
    {
        onDispose: () => {
