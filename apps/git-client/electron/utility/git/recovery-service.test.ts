...
async function createFixture(): Promise<Fixture> {
    ...
    await writeFile(
        join(repository, "binary.bin"),
        Buffer.from([0, 255, 1, 2, 128, 10]),
    );
...
class DeterministicRunner implements GitProcessRunnerLike {
    ...
    run(
        ...
        if (command === "symbolic-ref")
            return Promise.resolve(output("stdout", "main\n"));
        if (command === "rev-parse")
            return Promise.resolve(output("stdout", `${this.oid}\n`));
...
describe("RecoveryService", () => {
    ...
    it("restores refs, index, tracked worktree, and untracked bytes and records an exact inverse", async () => {
        ...
        const initialUnicodeBytes = await readFile(
            join(fixture.repository, "문서.txt"),
        );
        const initialBinaryBytes = await readFile(
            join(fixture.repository, "binary.bin"),
        );
        const entry = await service.recordBeforeOperation(
            fixture.record.id,
            commitOperation(),
        ...
        await writeFile(
            join(fixture.repository, "second.txt"),
            "second\n",
            "utf8",
        );
        ...
        await expect(
            service.restore(fixture.record.id, entry.id),
        ).resolves.toEqual({
            entryId: entry.id,
            restoredRefs: ["refs/heads/main"],
        });
        expect(git(fixture.repository, "rev-parse", "refs/heads/main")).toBe(
            first,
        );
        await expect(
            readFile(join(fixture.repository, "문서.txt")),
        ).resolves.toEqual(initialUnicodeBytes);
        await expect(
            readFile(join(fixture.repository, "binary.bin")),
        ).resolves.toEqual(initialBinaryBytes);
        await expect(
            access(join(fixture.repository, "second.txt")),
        ...
        const inverse = entries.find(
            (candidate) => candidate.operation === "restore commit",
        );
        expect(inverse?.refs).toEqual([
            { name: "refs/heads/main", oid: second },
        ]);
        if (inverse === undefined)
            throw new Error("Expected inverse recovery entry");
        await expect(
            service.restore(fixture.record.id, inverse.id),
        ...
        await expect(
            readFile(join(fixture.repository, "문서.txt")),
        ).resolves.toEqual(unicodeBytes);
        await expect(
            readFile(join(fixture.repository, "binary.bin")),
        ).resolves.toEqual(binaryBytes);
        await expect(
            readFile(join(fixture.repository, "second.txt"), "utf8"),
    ...
    it("round-trips partially staged, staged, deleted, untracked, Unicode, and binary state", async () => {
        ...
        await writeFile(
            join(fixture.repository, "새 파일.bin"),
            untrackedBinary,
        );
        ...
        const beforeIndex = await readFile(
            join(fixture.record.gitDirectory, "index"),
        );
        const entry = await service.recordBeforeOperation(
            fixture.record.id,
            commitOperation(),
        ...
        await writeFile(
            join(fixture.repository, "extra.txt"),
            "extra\n",
            "utf8",
        );
        git(fixture.repository, "add", "--", "문서.txt");
        ...
        ).toBe(beforeIndexDiff);
        expect(
            git(fixture.repository, "diff", "--binary", "--no-ext-diff"),
        ).toBe(beforeWorktreeDiff);
        await expect(
            readFile(join(fixture.record.gitDirectory, "index")),
        ).resolves.toEqual(beforeIndex);
        await expect(
            readFile(join(fixture.repository, "문서.txt")),
        ).resolves.toEqual(worktreeUnicode);
        await expect(
            readFile(join(fixture.repository, "binary.bin")),
        ).resolves.toEqual(worktreeBinary);
        await expect(
            readFile(join(fixture.repository, "새 파일.bin")),
        ).resolves.toEqual(untrackedBinary);
        await expect(
            access(join(fixture.repository, "deleted.txt")),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
        await expect(
            access(join(fixture.repository, "extra.txt")),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
    });
    ...
    it("atomically restores a rename and applies oid-null as a compare-and-swap ref deletion", async () => {
        ...
        expect(optionalRef(fixture.repository, `refs/heads/${oldName}`)).toBe(
            entry.refs[0]?.oid,
        );
        expect(
            optionalRef(fixture.repository, `refs/heads/${newName}`),
        ).toBeNull();
    ...
    it("aborts the whole multi-ref transaction on a CAS race without a partial restore", async () => {
        ...
        expect(
            optionalRef(fixture.repository, `refs/heads/${oldName}`),
        ).toBeNull();
        expect(optionalRef(fixture.repository, `refs/heads/${newName}`)).toBe(
            racedOid,
        );
        expect(git(fixture.repository, "status", "--porcelain=v2", "-z")).toBe(
            beforeStatus,
        ...
        await expect(
            readFile(join(fixture.repository, "문서.txt")),
        ).resolves.toEqual(dirtyBytes);
        const entries = await service.list(fixture.record.id);
        expect(
            entries.some(
                (candidate) => candidate.operation === "restore rename branch",
            ),
        ).toBe(true);
    });
    ...
    it("marks an entry unrecoverable when its recorded object has expired and refuses to mutate refs", async () => {
        ...
        expect(
            optionalRef(fixture.repository, "refs/tags/ephemeral"),
        ).toBeNull();
    ...
    it("rejects tampered, oversized, symbolic-link, and hard-linked manifests without following them", async () => {
        ...
        await writeFile(
            path,
            Buffer.alloc(MAX_RECOVERY_MANIFEST_BYTES + 1, 0x78),
        );
        await expect(service.list(fixture.record.id)).rejects.toMatchObject({
            code: "outputLimit",
        });
    ...
    it("rejects storage and repository directory symlinks without writing outside them", async () => {
        ...
        await expect(
            access(join(outside, `${fixture.record.id}.json`)),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
        ...
        await expect(
            RecoveryService.of(linkedRegistry, fixture.storage).list(
                fixture.record.id,
            ),
        ).rejects.toMatchObject({ code: "invalidInput" });
    });
    ...
    it("fails closed before storage or repository mutation when snapshot limits are exceeded", async () => {
        ...
        await writeFile(
            oversized,
            Buffer.alloc(MAX_RECOVERY_SNAPSHOT_FILE_BYTES + 1, 0x5a),
        );
        const beforeStatus = git(
            fixture.repository,
            "status",
        ...
        await expect(
            access(join(fixture.storage, "recovery")),
        ).rejects.toMatchObject({
            code: "ENOENT",
        });
    });
    ...
    it("detects a worktree race during capture and writes no recovery entry", async () => {
        ...
        const runner = new DelegatingRunner((spec) => {
            ...
            if (raced.status !== 0)
                throw new Error("Unable to inject worktree race");
        });
        const service = RecoveryService.of(
            fixture.registry,
            fixture.storage,
    ...
    it("reads version 1 ref-only entries and preserves their legacy worktree behavior", async () => {
        ...
        await writeFile(
            join(fixture.repository, "later.txt"),
            "later\n",
            "utf8",
        );
        ...
        await expect(
            readFile(join(fixture.repository, "문서.txt"), "utf8"),
    ...
    it("persists the inverse before a cancelled restore and leaves the ref untouched", async () => {
        ...
        await expect(
            service.list(fixture.record.id, preCancelled.signal),
        ).rejects.toMatchObject({
            code: "commandFailed",
        });
    });
    ...
    it("retains only the newest 200 entries and rejects a pre-cancelled record before storage changes", async () => {
        ...
        expect(new Set(entries.map((entry) => entry.id))).toHaveLength(
            MAX_RECOVERY_ENTRIES,
        );
