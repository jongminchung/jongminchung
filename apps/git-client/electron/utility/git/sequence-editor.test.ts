...
function operation(entries: readonly RebasePlanEntry[], preserveMerges = false): GitOperation {
  return {
...
describe("SequenceEditorSession", () => {
  it.each([
  ...
  ])("rewrites a utility-created $name history operation", async (scenario) => {
    ...
    const session = await SequenceEditorSession.create(gitDirectory, scenario.operation);
    ...
    await expect(readFile(todoPath, "utf8")).resolves.toBe(scenario.expected);
    if (scenario.operation.kind === "rewordCommit") {
      ...
      await expect(readFile(messagePath, "utf8")).resolves.toBe("replacement subject\n");
    }
  ...
  it("authenticates a plan and atomically rewrites Git's todo in visual order", async () => {
    ...
    const session = await SequenceEditorSession.create(gitDirectory, rebaseOperation);
  ...
  it("atomically replaces the todo while preserving its file permissions", async () => {
    ...
    expect(
      (await readdir(dirname(todoPath))).some((name) => name.startsWith(".git-client-sequence-")),
    ).toBe(false);
    await session.cleanup();
  });
  ...
  it("rejects a payload whose authenticated plan was modified", async () => {
    ...
    await writeFile(session.payloadPath, envelope.replace("first", "forged"), "utf8");
  ...
  it("rejects duplicate commits in an authenticated plan", async () => {
    ...
    await expect(
      SequenceEditorSession.create(gitDirectory, operation([planEntry("1", "first"), duplicate])),
    ).rejects.toMatchObject({ code: "invalidInput" });
  });
  ...
  it("rejects an abbreviated todo object ID that matches multiple plan entries", async () => {
    ...
    const session = await SequenceEditorSession.create(gitDirectory, operation([first, second]));
  ...
  it("preserves merge structure while applying non-merge plan actions", async () => {
    const { gitDirectory, todoPath } = await fixture();
    await writeFile(todoPath, "label onto\npick aaaaaaa first\nmerge -C bbbbbbb side\n", "utf8");
    const mergeEntry = {
      ...planEntry("b", "side"),
      mergeCommit: true,
    };
    const session = await SequenceEditorSession.create(
      gitDirectory,
      operation([planEntry("a", "first", "reword", "rewritten first"), mergeEntry], true),
    );
  ...
  it("writes the authenticated reword message for the matching subject", async () => {
    ...
    await writeFile(messagePath, "first\n\nold body\n# Please enter the commit message\n", "utf8");
    const session = await SequenceEditorSession.create(
      gitDirectory,
      operation([planEntry("1", "first", "reword", "replacement subject\n\nreplacement body")]),
    );
  ...
  it("rejects a reword message containing a null byte", async () => {
    ...
    await expect(
      SequenceEditorSession.create(
        gitDirectory,
        operation([planEntry("1", "first", "reword", "subject\0forged")]),
      ),
    ).rejects.toMatchObject({ code: "invalidInput" });
  });
  ...
  it("rejects a reword message larger than the bounded file limit", async () => {
    ...
    await expect(
      SequenceEditorSession.create(
        gitDirectory,
        operation([
          planEntry("1", "first", "reword", "a".repeat(MAX_SEQUENCE_EDITOR_FILE_BYTES + 1)),
        ]),
      ),
    ).rejects.toMatchObject({ code: "invalidInput" });
  });
  ...
  it("cancels before replacement and leaves no atomic-write temporary file", async () => {
    ...
    expect(siblings.some((name) => name.startsWith(".git-client-sequence-"))).toBe(false);
    await session.cleanup();
  });
  ...
  it("rejects an authenticated payload copied outside the utility temp root", async () => {
    ...
    await writeFile(forgedPayloadPath, await readFile(session.payloadPath), { mode: 0o600 });
  ...
  it("rejects an oversized payload before parsing it", async () => {
    ...
    await writeFile(session.payloadPath, Buffer.alloc(MAX_SEQUENCE_EDITOR_PAYLOAD_BYTES + 1, 0x61));
