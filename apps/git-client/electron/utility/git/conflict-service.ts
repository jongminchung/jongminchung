...
import type { ConflictContent, ConflictFile } from "../../../src/generated";
        ...
        return new Promise((resolve, reject) => {
            if (isAborted(signal)) {
                reject(
                    new GitUtilityError(
                        "commandFailed",
                        "Git conflict command was cancelled",
                    ),
                );
                return;
            }
            ...
            const timeout = setTimeout(() => {
                ...
                finish(() =>
                    reject(
                        new GitUtilityError(
                            "commandFailed",
                            "Git conflict command timed out",
                        ),
                    ),
                );
            }, GIT_TIMEOUT_MS);
                ...
                if (outputBytes > MAX_GIT_OUTPUT_BYTES) {
                    ...
                    finish(() =>
                        reject(
                            new GitUtilityError(
                                "outputLimit",
                                "Git conflict output exceeded 16 MiB",
                            ),
                        ),
                    );
                    return;
...
function assertNotAborted(signal: AbortSignal | undefined): void {
    ...
    throw new GitUtilityError(
        "commandFailed",
        "Conflict operation was cancelled",
    );
}
...
function textFromBytes(bytes: Buffer): string | null {
    if (
        bytes.byteLength > MAX_CONFLICT_TEXT_BYTES ||
        bytes.includes(0) ||
        !isUtf8(bytes)
    ) {
        return null;
...
export class GitConflictService {
    ...
    async list(
        ...
        for (const [path, conflict] of [...conflicts.entries()].sort(
    ...
    async read(
        ...
        const repository = this.#registry.get(repositoryId);
        const conflict = (await this.list(repositoryId, signal)).find(
    ...
    async write(
        ...
        const repository = this.#registry.get(repositoryId);
        const destination = await this.#checkedWorktreePath(
            repository.path,
            path,
        );
        assertNotAborted(signal);
        const temporary = join(
            dirname(destination),
            `.git-client-conflict-${randomUUID()}.tmp`,
        );
        try {
    ...
    #parseUnmerged(output: Buffer): ReadonlyMap<string, ConflictBuilder> {
        ...
        while (offset < output.byteLength) {
            ...
            if (!isUtf8(pathBytes)) {
                throw new GitUtilityError(
                    "commandFailed",
                    "Non-UTF-8 conflict paths are unsupported",
                );
            }
    ...
    async #isBinary(
        ...
        for (const oid of oids) {
            ...
            if ((await this.#readBlobText(root, oid, false, signal)) === null)
                return true;
        }
        return false;
    }
    ...
    async #readResult(
        ...
        try {
            if ((await stat(destination)).size > MAX_CONFLICT_TEXT_BYTES)
                return null;
    ...
    async #checkedWorktreePath(root: string, path: string): Promise<string> {
        ...
        try {
            if ((await lstat(destination)).isSymbolicLink()) {
                throw new GitUtilityError(
                    "invalidInput",
                    "Symbolic-link conflict results are unsupported",
                );
            }
        } catch (error) {
            if (!isMissing(error)) throw error;
