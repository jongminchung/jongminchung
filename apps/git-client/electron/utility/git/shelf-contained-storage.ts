...
export async function walkExistingShelfParent(
  ...
  for (const component of components) {
    const child = await optionalShelfChildDirectory(current, component, label);
...
export async function readContainedShelfFile(
  ...
  const parent = await walkExistingShelfParent(root, components.slice(0, -1), label);
  ...
  try {
    ...
    if (!opened.isFile() || !sameShelfIdentity(opened, fileIdentity(before))) {
...
export async function writeContainedShelfFile(
  ...
  const components = shelfPathComponents(relativePath);
  const parent = await ensureShelfParent(root, components.slice(0, -1), `${label} parent`);
  const path = join(parent.path, components.at(-1) as string);
  let handle;
  try {
    handle = await open(
...
export async function syncShelfDirectory(directory: PinnedShelfDirectory): Promise<void> {
