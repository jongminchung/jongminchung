...
class RecordingRunner implements PatchProcessRunnerLike {
    ...
    async run(
        ...
        if (outcome === undefined)
            throw new Error("Missing fake shelf-process outcome");
        return outcome;
    }
}
...
describe("ShelfService", () => {
    it("round-trips staged, worktree, and byte-exact untracked layers through a reusable shelf", async () => {
        ...
        expect(await readFile(join(directory, "index.patch"))).toEqual(
            expectedIndexPatch,
        );
        expect(await readFile(join(directory, "worktree.patch"))).toEqual(
            expectedWorktreePatch,
        );
        expect(
            await readFile(join(directory, "untracked", untrackedPath)),
        ).toEqual(untrackedBytes);
        expect(entry.files).toEqual([
            { path: "tracked.txt", checksum: "", untracked: false },
            {
                path: untrackedPath,
                checksum: checksum(untrackedBytes),
                untracked: true,
            },
        ]);
        ...
        expect(await readFile(join(repository, untrackedPath))).toEqual(
            untrackedBytes,
        );
        await expect(
            service.apply(record.id, entry.id, false),
    ...
    it("uses only fixed Git argv, canonical cwd, bounded output, and the caller's abort signal", async () => {
        ...
        expect(runner.specs.every((spec) => spec.cwd === repository)).toBe(
            true,
        );
        expect(
            runner.specs
                ...
                .every(
                    (spec) =>
                        spec.stdoutLimitBytes === MAX_SHELF_PATCH_BYTES + 1,
                ),
        ).toBe(true);
        expect(
            runner.signals.every((signal) => signal === controller.signal),
        ).toBe(true);
        expect(
            await readFile(
                join(
    ...
    it("rejects invalid messages and pathspecs before Git or filesystem side effects", async () => {
        ...
        for (const [message, paths] of invalidCases) {
            await expect(
                service.create(record.id, message, paths),
            ).rejects.toMatchObject({
                code: "invalidInput",
            });
        }
        ...
        await expect(
            access(join(storageRoot, "shelves")),
    ...
    it("rejects storage and repository symlinks without following or deleting their targets", async () => {
        ...
        await expect(
            service.create(record.id, "change", ["tracked.txt"]),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
        ...
        await expect(
            service.create(record.id, "link", ["link.txt"]),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
        expect(await readFile(outsideFile, "utf8")).toBe("outside\n");
        expect(await readFile(join(repository, "link.txt"), "utf8")).toBe(
            "outside\n",
        );
        await expect(service.list(record.id)).resolves.toEqual([]);
        ...
        await symlink(
            outsideStorage,
            join(repositoryShelves, shelfLinkId),
            "dir",
        );
        await expect(
            service.delete(record.id, shelfLinkId),
        ).rejects.toMatchObject({
            code: "invalidInput",
        });
        expect(await readdir(outsideStorage)).toEqual([]);
    ...
    it("preflights untracked restore destinations and blocks parent-symlink escapes", async () => {
        ...
        await writeFile(
            join(repository, "nested", "payload.bin"),
        ...
        const entry = await service.create(record.id, "untracked", [
            "nested/payload.bin",
        ]);
        ...
        await expect(
            access(join(outside, "payload.bin")),
        ).rejects.toMatchObject({ code: "ENOENT" });
        await expect(service.list(record.id)).resolves.toEqual([entry]);
    });
    ...
    it("detects tampering before apply or delete and never removes an unverified shelf", async () => {
        ...
        const entry = await service.create(record.id, "tamper target", [
            "tracked.txt",
        ]);
        const directory = join(storageRoot, "shelves", record.id, entry.id);
        await writeFile(
            join(directory, "worktree.patch"),
            "tampered\n",
            "utf8",
        );
    ...
    it("cleans temporary shelves and propagates cancellation, output limits, and redacted diagnostics", async () => {
        ...
        await expect(
            new ShelfService(
                ...
                storageRoot,
                cancelledRunner,
            ).create(record.id, "cancel", ["tracked.txt"], controller.signal),
        ...
        await expect(
            new ShelfService(
                ...
                storageRoot,
                tooLargeRunner,
            ).create(record.id, "large", ["tracked.txt"]),
        ...
        const error = await new ShelfService(
            registry(record),
            storageRoot,
            failedRunner,
        )
        ...
        const repositoryShelves = join(storageRoot, "shelves", record.id);
        const children = await readdir(repositoryShelves).catch(
            () => [] as string[],
        );
