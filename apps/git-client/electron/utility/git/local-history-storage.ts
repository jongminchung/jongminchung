...
export class LocalHistoryStorage {
    ...
    async append(
        ...
        await this.atomicJson(
            join(directory, "activities", `${activity.id}.json`),
            activity,
        );
        await this.writeManifest(repositoryId, [
            activity.id,
    ...
    async writeCurrent(
        ...
        await this.atomicJson(
            join(this.repositoryDirectory(repositoryId), "current.json"),
            {
                version: LOCAL_HISTORY_STORAGE_VERSION,
                files: [...state.entries()],
            },
    ...
    async writeManifest(
        ...
        await this.atomicJson(
            join(this.repositoryDirectory(repositoryId), "manifest.json"),
            {
                version: LOCAL_HISTORY_STORAGE_VERSION,
                activityIds,
            },
    ...
    async collectUnreferencedBlobs(): Promise<void> {
        const repositoriesDirectory = join(
        ...
        try {
            ...
            for (const repository of repositories) {
                ...
                const repositoryId = RepositoryIdSchema.safeParse(
                    repository.name,
                );
                ...
                for (const state of current.values()) {
                    if (state.content.kind === "blob")
                        referenced.add(state.content.sha256);
                }
            ...
            await Promise.all(
                objects
                    .filter(
                        (object) =>
                            object.isFile() && object.name.endsWith(".gz"),
                    )
                    .filter(
                        (object) => !referenced.has(object.name.slice(0, -3)),
                    )
                    .map((object) =>
    ...
    async archiveLegacy(repositoryId: RepositoryId): Promise<void> {
        ...
        try {
            const raw: unknown = JSON.parse(
                await readFile(join(legacy, "manifest.json"), "utf8"),
            );
            ...
            for (const entry of [...manifest.entries].reverse()) {
                const snapshotRaw: unknown = JSON.parse(
                    await readFile(
                        join(legacy, `${entry.snapshotFile}.json`),
                        "utf8",
                    ),
                );
                ...
                for (const file of snapshot.files) {
                    ...
                    const content: ContentReference =
                        file.kind === "symlink" ||
                        (isLocalHistoryTextPath(file.path) && isUtf8(bytes))
                            ? {
                                  kind: "blob",
                    ...
                    files.set(file.path, {
                        ...
                        mode: file.mode,
                        contentType:
                            content.kind === "unavailable" ? "binary" : "text",
                        content,
                    });
                }
            ...
            for (let index = 1; index < snapshots.length; index += 1) {
                ...
                if (changes.length === 0 && current.entry.label === null)
                    continue;
                migrated.push(
                    StoredActivitySchema.parse({
                        version: LOCAL_HISTORY_STORAGE_VERSION,
            ...
            if (migrated.length > 0) {
                ...
                for (const activity of migrated) {
                    await this.atomicJson(
                        join(activityDirectory, `${activity.id}.json`),
                        activity,
                    );
                }
                await this.writeManifest(
                    repositoryId,
                    migrated.map((activity) => activity.id).reverse(),
                );
            }
            const archive = join(this.storageRoot, "local-history-v1-archive");
            await mkdir(archive, { recursive: true, mode: 0o700 });
            await rename(legacy, join(archive, repositoryId)).catch(
                (error: unknown) => {
                    if (!isErrno(error, "EEXIST")) throw error;
                },
            );
    ...
    async quarantineCorruptRepository(
        repositoryId: RepositoryId,
    ): Promise<void> {
        ...
        await rename(
            source,
            join(archive, `${repositoryId}-${randomUUID()}`),
        ).catch((error: unknown) => {
            if (!isErrno(error, "ENOENT")) throw error;
        });
    }
