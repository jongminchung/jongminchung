...
function defaultSequenceEditorRuntime(): SequenceEditorRuntime {
    return {
        ...
        applicationEntryPath:
            process.defaultApp === true ? join(__dirname, "main.cjs") : null,
    };
}
        ...
        try {
            ...
            if (!startedDelivered) {
                this.#emit(listener, {
                    kind: "started",
                    requestId,
                    displayCommand: displayGitCommand([
            ...
            return await this.#runner.run(
                {
                    ...
                    outputLimitBytes: GIT_OUTPUT_LIMIT_BYTES,
                    editorEnvironment: {
                        sequenceEditor: sequenceEditorCommand(
                            this.#sequenceEditorRuntime,
                            "sequence",
                            session,
                        ),
                        messageEditor: sequenceEditorCommand(
                            this.#sequenceEditorRuntime,
                            "message",
                            session,
                        ),
