...
import type { GitRequest, LogFilters, RepositoryId } from "../../../src/shared/contracts/model";
import { buildRequestArguments, type QueryOnlyGitRequest } from "./request-query";
...
describe("buildRequestArguments", () => {
  it.each<readonly [QueryOnlyGitRequest, string]>([
    ...
    [{ kind: "blame", repositoryId, revision: null, path: "file.txt" }, "blame"],
    ...
    [{ kind: "checkIgnored", repositoryId, paths: ["ignored.txt"] }, "ls-files"],
  ...
  it("rejects path traversal, option-like revisions, invalid refs, NUL text, and invalid pages", () => {
    ...
    for (const request of invalidRequests) {
      expect(() => buildRequestArguments(request as QueryOnlyGitRequest)).toThrow();
    }
  });
});
