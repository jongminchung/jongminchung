...
const REDACTION_RULES: readonly RedactionRule[] = [
    ...
    {
        pattern:
            /\b(authorization[ \t]*:[ \t]*(?:basic|bearer)[ \t]+)[^\s\0\r\n]+/giu,
        replacement: (match) => `${match[1] ?? ""}${REDACTED}`,
    },
    {
        pattern:
            /\b(password|passwd|token|access_token|refresh_token)=([^\s&\0\r\n]+)/giu,
        replacement: (match) => `${match[1] ?? "token"}=${REDACTED}`,
    },
    {
        pattern:
            /\b(?:gh[opurs]_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{20,})\b/gu,
        replacement: () => REDACTED,
    },
];
// oxlint-enable eslint/no-control-regex

function redactChunksWithRule(
...
export function redactCredentialChunks(
    chunks: readonly string[],
): readonly string[] {
