...
export const RecoveryObjectIdSchema = z.string().regex(/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/u);
...
export function validateRecoveryOperation(untrustedOperation: unknown): ValidatedGitOperation {
...
export function assertRecoveryNotAborted(signal: AbortSignal | undefined): void {
...
export function createRecoveryRefTransaction(
  ...
  for (const target of targets) {
    ...
    if (target.oid === null && oldOid !== null) commands.push(`delete ${target.name} ${oldOid}`);
