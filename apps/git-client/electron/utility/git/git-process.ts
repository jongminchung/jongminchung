...
import {
    redactCredentialChunks,
    redactCredentials,
    safeErrorMessage,
} from "./redaction";

export type GitCancellationReason =
    | "requested"
    | "repositoryClosed"
    | "timeout";
...
function sanitizedOutput(
    ...
    for (const [stream, decoder] of decoders) {
        ...
        if (tail.length > 0 && lastIndex !== undefined)
            decoded[lastIndex]!.data += tail;
    }
    ...
    for (const stream of ["stdout", "stderr"] as const) {
        ...
        const redacted = redactCredentialChunks(
            indexes.map((index) => sanitized[index] ?? ""),
        );
...
export class GitProcessRunner implements GitProcessRunnerLike {
    ...
    run(
        ...
        const outputLimitBytes =
            spec.outputLimitBytes ?? GIT_OUTPUT_LIMIT_BYTES;
        const redactStdout = spec.redactStdout ?? true;
        if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
            return Promise.resolve(
                this.#invalidSpec(
                    startedAt,
                    "Git timeout must be a positive integer",
                ),
            );
        }
        if (!Number.isSafeInteger(outputLimitBytes) || outputLimitBytes <= 0) {
            return Promise.resolve(
                this.#invalidSpec(
                    startedAt,
                    "Git output limit must be a positive integer",
                ),
            );
        }
        ...
        return new Promise((resolve) => {
                ...
                child = spawn(this.#gitBinary, [...spec.args], {
                    ...
                    env: {
                        ...
                        ...(spec.editorEnvironment === undefined
                            ...
                            : {
                                  GIT_SEQUENCE_EDITOR:
                                      spec.editorEnvironment.sequenceEditor,
                                  GIT_EDITOR:
                                      spec.editorEnvironment.messageEditor,
                              }),
                    },
                    shell: false,
            ...
            const append = (stream: GitOutputStream, value: Buffer) => {
                ...
                if (retained.byteLength > 0)
                    chunks.push({ stream, data: Buffer.from(retained) });
            ...
            child.once("close", (exitCode) => {
                ...
                finish({
                    ...
                    message: safeErrorMessage(
                        stderr || `Git exited with status ${exitCode ?? -1}`,
                    ),
                    exitCode,
                    durationMs: durationMs(),
                    output,
