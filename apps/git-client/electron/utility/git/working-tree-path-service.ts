...
export class WorkingTreePathService {
  ...
  async resolveFile(repositoryId: RepositoryId, path: string): Promise<string> {
    ...
    try {
      ...
      throw new GitUtilityError("invalidInput", "Working tree file does not exist");
    }
    const relativePath = relative(repository.path, canonicalPath);
    if (relativePath === ".." || relativePath.startsWith(`..${sep}`) || isAbsolute(relativePath)) {
      throw new GitUtilityError("invalidInput", "Working tree file escapes the repository");
    }
    if (!(await stat(canonicalPath)).isFile()) {
      throw new GitUtilityError("invalidInput", "Working tree path is not a file");
    }
