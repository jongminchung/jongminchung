...
import type { Changelist, ChangelistCommitOptions } from "../../../src/shared/contracts/model";
...
export const ObjectIdSchema = z.string().regex(/^[0-9a-f]{40}(?:[0-9a-f]{24})?$/u);
...
export function filesystemError(error: unknown, fallback: string): GitUtilityError {
...
export function sameIdentity(metadata: Stats, identity: PinnedDirectory | FileIdentity): boolean {
  return metadata.dev === identity.device && metadata.ino === identity.inode;
}

export function pinnedDirectory(path: string, metadata: Stats): PinnedDirectory {
...
export function validateCommitOptions(untrusted: unknown): ChangelistCommitOptions {
...
export function validatePayload(untrusted: unknown, repositoryId: RepositoryId): ManifestPayload {
