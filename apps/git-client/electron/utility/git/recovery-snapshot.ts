...
import type { GitProcessCompleted, GitProcessOutcome, GitProcessRunnerLike } from "./git-process";
...
const SnapshotFileSchema = z
  .object({
    ...
    bytesBase64: z.string().max(encodedLength(MAX_RECOVERY_SNAPSHOT_FILE_BYTES)),
...
const RepositorySnapshotPayloadSchema = z
  ...
  .superRefine((snapshot, context) => {
    ...
    const managed = new Set([...snapshot.trackedPaths, ...snapshot.untrackedPaths]);
    ...
    const totalBytes = snapshot.files.reduce(
      (total, file) => total + (decodeCanonicalBase64(file.bytesBase64)?.value.byteLength ?? 0),
      snapshot.index.kind === "file"
        ? (decodeCanonicalBase64(snapshot.index.bytesBase64)?.value.byteLength ?? 0)
        : 0,
    );
...
export const RepositorySnapshotSchema = RepositorySnapshotPayloadSchema.extend({
  ...
  .superRefine((snapshot, context) => {
    ...
    if (sha256(Buffer.from(JSON.stringify(payload), "utf8")) !== snapshot.sha256) {
      context.addIssue({
        code: "custom",
...
function assertNotAborted(signal: AbortSignal | undefined): void {
  ...
  throw new GitUtilityError("commandFailed", `Recovery snapshot was cancelled${suffix}`);
}
...
export function copyRepositorySnapshot(snapshot: RepositorySnapshot): RepositorySnapshot {
...
export function readRepositorySnapshotFile(
  ...
  const file = parsedSnapshot.files.find((candidate) => candidate.path === parsedPath);
...
export function mergeRepositorySnapshotPaths(
  ...
  const sourceFiles = new Map(parsedSource.files.map((file) => [file.path, file]));
  ...
  const totalBytes = nextFiles.reduce(
    ...
    parsedCurrent.index.kind === "file"
      ? (decodeCanonicalBase64(parsedCurrent.index.bytesBase64)?.value.byteLength ?? 0)
      : 0,
  );
...
async function optionalMetadata(path: string, label: string): Promise<ExistingMetadata | null> {
  try {
...
function processFailure(outcome: Exclude<GitProcessOutcome, GitProcessCompleted>): GitUtilityError {
  if (outcome.kind === "cancelled") {
    const suffix = outcome.reason === "timeout" ? " timed out" : " was cancelled";
    return new GitUtilityError("commandFailed", `Recovery snapshot Git command${suffix}`);
  }
...
function parseNulPaths(value: string | null): readonly string[] {
  ...
  if (paths.length > MAX_RECOVERY_SNAPSHOT_FILES) {
    throw outputLimit(`Recovery snapshot exceeds ${MAX_RECOVERY_SNAPSHOT_FILES} files`);
  }
...
async function capturePathLists(
  ...
  const [indexPaths, headPaths, untrackedPaths] = await Promise.all([
    captureGitText(runner, repository, ["ls-files", "--cached", "-z", "--"], signal),
    captureGitText(
      runner,
      repository,
  ...
  const tracked = sortedUnique([...parseNulPaths(indexPaths), ...parseNulPaths(headPaths)]);
...
async function pathParents(
  ...
  for (const part of parts.slice(0, -1)) {
    ...
    if (!containedPath(root.path, current)) throw invalid("Repository path escaped its root");
    let metadata: Stats;
    try {
...
async function assertParents(parents: readonly PinnedDirectory[]): Promise<void> {
  for (const parent of parents) await assertPinnedDirectory(parent, "Repository path parent");
}
...
async function readRegularFile(
  ...
  if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1) {
    throw invalid("Recovery snapshot supports only regular non-hard-linked files and symlinks");
  }
...
async function capturePath(root: PinnedDirectory, path: string): Promise<CapturedPath | null> {
  const parents = await pathParents(root, path);
  if (parents === null) return null;
  await assertParents(parents);
  const absolute = join(root.path, ...path.split("/"));
  if (!containedPath(root.path, absolute)) throw invalid("Repository path escaped its root");
  let metadata: Stats;
  try {
  ...
  if (metadata.isDirectory()) {
    throw invalid("Recovery snapshot cannot safely capture a tracked directory or submodule");
  }
  ...
  const captured = await readRegularFile(absolute, metadata, MAX_RECOVERY_SNAPSHOT_FILE_BYTES);
  await assertParents(parents);
  return {
    file: {
      path,
...
async function assertCapturedPath(root: PinnedDirectory, captured: CapturedPath): Promise<void> {
  ...
  const metadata = await lstat(absolute).catch(() => {
    throw invalid("Repository file changed while the snapshot was being captured");
  });
...
async function assertCapturedIndex(index: CapturedIndex): Promise<void> {
  ...
  if (
    ...
    throw invalid("Repository index changed while the snapshot was being captured");
  }
  try {
    await lstat(join(index.parent.path, "index.lock"));
    throw invalid("Repository index became busy while the snapshot was being captured");
  } catch (error) {
    if (!isErrno(error, "ENOENT")) throw error;
  }
}
...
export async function captureRepositorySnapshot(
  ...
  const managedPaths = sortedUnique([...initialPaths.tracked, ...initialPaths.untracked]);
  ...
  for (const path of managedPaths) {
    ...
    if (captured === null) {
      if (initialPaths.untracked.includes(path)) {
        throw invalid("An untracked file disappeared while its snapshot was being captured");
      }
      continue;
    }
    ...
    if (totalBytes > MAX_RECOVERY_SNAPSHOT_BYTES) {
      throw outputLimit(`Recovery snapshot exceeds ${MAX_RECOVERY_SNAPSHOT_BYTES} bytes`);
    }
  ...
  if (!samePathLists(initialPaths, finalPaths)) {
    throw invalid("Repository paths changed while the snapshot was being captured");
  }
  ...
  if (!validated.success) throw invalid("Captured repository snapshot is invalid");
...
async function assertStoredFile(root: PinnedDirectory, expected: SnapshotFile): Promise<void> {
...
async function inspectConflictDirectory(
  ...
  if (!containedPath(root.path, pinned.path)) throw invalid("Recovery directory escaped its root");
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await inspectConflictDirectory(root, childPath, currentFiles, directories, visited);
      continue;
    }
...
async function preflightTopology(
  ...
  for (const targetFile of target.files) {
    ...
    if (metadata.isDirectory() && !metadata.isSymbolicLink()) {
      await inspectConflictDirectory(root, targetFile.path, currentFiles, conflictDirectories, {
        count: 0,
      });
  ...
  return [...conflictDirectories].sort((left, right) => pathDepth(right) - pathDepth(left));
}

async function safeUnlink(root: PinnedDirectory, expected: SnapshotFile): Promise<void> {
  await assertStoredFile(root, expected);
  const parents = await pathParents(root, expected.path);
  if (parents === null) throw invalid("Repository path changed during recovery");
  await assertParents(parents);
  const absolute = join(root.path, ...expected.path.split("/"));
  await unlink(absolute).catch(() => {
    throw filesystemFailure("Repository file could not be removed during recovery");
  });
...
async function ensureParentDirectories(
  ...
  for (const part of parts) {
    ...
    try {
      ...
      if (!isErrno(error, "ENOENT")) throw filesystemFailure("Repository path is not accessible");
...
async function writeSnapshotFile(
  ...
  const temporary = join(parent.path, `.git-client-recovery-${randomUUID()}.tmp`);
  ...
  try {
    if (target.kind === "file") {
      ...
      await chmod(temporary, target.mode).catch(() => {
        throw filesystemFailure("Recovery temporary file mode could not be set");
      });
    } else {
...
async function acquireIndexLock(git: PinnedDirectory, current: SnapshotIndex): Promise<IndexLock> {
  await assertPinnedDirectory(git, "Repository Git directory");
...
async function captureIndexWithExistingLock(git: PinnedDirectory): Promise<SnapshotIndex> {
  const path = join(git.path, "index");
  let metadata: Stats;
  try {
    metadata = await lstat(path);
...
async function commitIndexLock(lock: IndexLock, target: SnapshotIndex): Promise<void> {
...
export async function restoreRepositorySnapshot(
  ...
  assertNotAborted(signal);
  const indexLock = await acquireIndexLock(directories.git, parsedCurrent.data.index);
  try {
    assertNotAborted(signal);
    const targetFiles = new Map(parsedTarget.data.files.map((file) => [file.path, file]));
    const currentFiles = new Map(parsedCurrent.data.files.map((file) => [file.path, file]));
    ...
    for (const file of parsedTarget.data.files) {
      await writeSnapshotFile(directories.root, file, currentFiles.get(file.path) ?? null);
    }
