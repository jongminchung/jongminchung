...
const RecoveryRefSchema = z
  .object({
    name: z
      ...
      .refine(hasSafeRecoveryRefStructure),
...
function legacyManifestPayload(
  entries: readonly RecoveryEntry[],
): LegacyRecoveryManifestPayload {
  return { version: LEGACY_MANIFEST_VERSION, entries };
}

function payloadBytes(
  payload: RecoveryManifestPayload | LegacyRecoveryManifestPayload,
): Buffer {
...
export function copyStoredRecoveryEntry(
  entry: StoredRecoveryEntry,
): StoredRecoveryEntry {
  return {
    ...copyRecoveryEntry(entry),
    snapshot:
      entry.snapshot === null ? null : copyRepositorySnapshot(entry.snapshot),
  };
}

export function encodeRecoveryManifest(
  entries: readonly StoredRecoveryEntry[],
): Buffer {
...
export function decodeRecoveryManifest(
  ...
  if (!isUtf8(bytes))
    throw invalid("Recovery manifest must contain valid UTF-8");
  let decoded: unknown;
  try {
  ...
  if (parsed.success) {
    ...
    expectedChecksum = checksum(
      payloadBytes(legacyManifestPayload(legacyEntries)),
    );
    storedChecksum = legacy.data.sha256;
  }
  if (expectedChecksum !== storedChecksum)
    throw invalid("Recovery manifest checksum mismatch");
  for (const entry of entries) {
    if (entry.repositoryId !== repositoryId) {
      throw invalid(
        "Recovery manifest contains an entry for another repository",
      );
    }
  }
