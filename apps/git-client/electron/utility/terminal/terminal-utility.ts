...
export class TerminalUtility {
  ...
  static of(
    spawner: PtySpawner,
    options: TerminalUtilityOptions,
  ): TerminalUtility {
  ...
  create(
    ...
    try {
      ...
      const detail =
        error instanceof Error ? error.message : "Unknown PTY error";
      throw new TerminalUtilityError(
        "spawnFailed",
        `Unable to start terminal shell: ${detail}`,
      );
    }
  ...
  resize(untrustedRequest: unknown): void {
    ...
    this.#session(request.terminalId).process.resize(
      request.cols,
      request.rows,
    );
  }
  ...
  #output(terminalId: string, data: string): void {
    ...
    for (
      ...
      offset += TERMINAL_OUTPUT_CHUNK_BYTES
    ) {
      const event = TerminalEventEnvelopeSchema.parse({
        ...
        data: Array.from(
          encoded.subarray(offset, offset + TERMINAL_OUTPUT_CHUNK_BYTES),
        ),
      });
  ...
  #exit(terminalId: string, exit: PtyProcessExit): void {
    ...
    const signal =
      exit.signal === null || exit.signal === 0 ? null : String(exit.signal);
    this.#deliver(
      session.listener,
  ...
  #resolveLaunchTarget(
    target: Parameters<
      NonNullable<TerminalUtilityOptions["launchTargetResolver"]>["resolve"]
    >[0],
  ): ResolvedTerminalLaunchTarget {
    ...
    const resolved =
      this.#options.launchTargetResolver?.resolve(target) ?? null;
