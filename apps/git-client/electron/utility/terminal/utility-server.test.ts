...
describe("터미널유틸리티프로토콜버", () => {
    it("[성공] 핸드 셰이크가 필요하고 모든 것을 통합하고 생성된 이벤트의 이벤트를 조사함", async () => {
        ...
        server.start();
        const ready = TerminalUtilityToMainMessageSchema.parse(
            port.posted.at(-1),
        );
        ...
        const createMessages = port.posted
            ...
            .filter(
                (message) =>
                    ...
                    (message.kind === "createResult" &&
                        message.correlationId === createId),
            );
        ...
        await flushServer();
        expect(
            TerminalUtilityToMainMessageSchema.parse(port.posted.at(-1)),
        ).toEqual({
