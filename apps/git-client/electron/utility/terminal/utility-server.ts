...
export class TerminalUtilityProtocolServer {
    ...
    #receive(untrustedMessage: unknown): void {
        ...
        const parsed =
            MainToTerminalUtilityMessageSchema.safeParse(untrustedMessage);
        if (!parsed.success) {
            this.#postError(
                null,
                "invalidMessage",
                parsed.error.issues[0]?.message ??
                    "Invalid terminal utility message",
            );
            return;
        }
    ...
    #route(
        ...
        switch (message.kind) {
            case "create": {
                const result = this.#utility.create(
                    message.request,
                    (event) => {
    ...
    #post(message: TerminalUtilityToMainMessage): void {
        this.#port.postMessage(
            TerminalUtilityToMainMessageSchema.parse(message),
        );
    }
