...
export class TerminalLaunchTargetResolver implements TerminalLaunchTargetResolverPort {
    ...
    private constructor(options: TerminalLaunchTargetResolverOptions) {
        ...
        this.#includeDefaultAgentDirectories =
            options.includeDefaultAgentDirectories ?? true;
    }

    static of(
        options: TerminalLaunchTargetResolverOptions,
    ...
    resolve(
        untrustedTarget: TerminalLaunchTarget,
    ): ResolvedTerminalLaunchTarget | null {
        const target = TerminalLaunchTargetSchema.parse(untrustedTarget);
        const shells = this.#detectShells();
        if (target.kind === "default") {
            const configured = this.#probeExactExecutable(
                this.#defaultShell,
                SHELL_DEFINITIONS.flatMap(
                    ({ canonicalBasenames }) => canonicalBasenames,
                ),
            );
            const detected =
                shells.find(({ executable }) => executable === configured) ??
                shells[0] ??
                null;
        ...
        const detected = this.#detectAgents().find(
            ({ descriptor }) => descriptor.id === target.id,
        );
        if (detected === undefined) return null;
        return {
    ...
    #detectShells(): readonly DetectedTerminalProgram<TerminalShellDescriptor>[] {
        return SHELL_DEFINITIONS.flatMap((definition) => {
            const executable = this.#firstExecutable(
                definition,
    ...
    #firstExecutable<Id extends string>(
        ...
        for (const directory of directories) {
            ...
            const executable = this.#probeExactExecutable(
                candidate,
                definition.canonicalBasenames,
            );
