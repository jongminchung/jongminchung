...
import type { PtyProcess, PtyProcessExit, PtySpawner, PtySpawnOptions } from "./terminal-utility";
...
interface NodePtyModule {
  spawn(file: string, args: readonly string[], options: IPtyForkOptions): IPty;
}

function loadNodePty(): NodePtyModule {
  const require = createRequire(__filename);
  const resourcesPath: unknown = Reflect.get(process, "resourcesPath");
  const packagedModule = typeof resourcesPath === "string" ? join(resourcesPath, "node-pty") : null;
  const moduleId =
    packagedModule !== null && existsSync(join(packagedModule, "package.json"))
      ? packagedModule
      : "node-pty";
  const moduleRoot =
    moduleId === "node-pty" ? dirname(require.resolve("node-pty/package.json")) : moduleId;
