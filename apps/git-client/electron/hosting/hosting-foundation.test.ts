...
describe("열대 전자 재단", () => {
  ...
  it.each([
    ...
    async (status, statusText, reason) => {
      const { foundation, http } = configuredFoundation("gitHub");
      http.enqueue(jsonResponse({ message: statusText }, status, statusText));
      await expect(
        foundation.execute("account-1", {
          kind: "mergeReadiness",
          project: "acme/repo",
  ...
  it("[성공] 전체 크기의 공급자 페이지에서만 다음 페이지를 장식함", async () => {
    const { foundation, http } = configuredFoundation("gitHub");
    http.enqueue(
      jsonResponse(Array.from({ length: 50 }, () => GITHUB_CHANGE_REQUEST)),
    );
  ...
  it("[실패], 시간 초과, 크기, 잘못된 JSON 및 자격 증명을 보고함", async () => {
    ...
    await expect(timeout.execute("account-1", request)).rejects.toMatchObject({
      code: "timeout",
    });
