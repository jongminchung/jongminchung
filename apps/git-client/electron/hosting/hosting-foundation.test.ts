...
class MockHostingHttpClient implements HostingHttpClient {
    ...
    async send(request: HostingHttpRequest): Promise<HostingHttpResponse> {
        ...
        if (response === undefined)
            throw new Error("Mock HTTP response is missing");
...
describe("Electron hosting contracts", () => {
    it("accepts only credential-free HTTPS origins without query or fragment", () => {
        ...
        ).toBe("https://gitlab.example.test");
...
describe("Electron hosting HTTP policy", () => {
    it("does not follow redirects", async () => {
        ...
        await expect(client.send(HTTP_REQUEST)).rejects.toMatchObject({
            code: "redirect",
        });
    ...
    it("stops reading responses at the configured byte limit", async () => {
        ...
        await expect(
            client.send({ ...HTTP_REQUEST, maxResponseBytes: 4 }),
...
describe("Electron hosting foundation", () => {
    ...
    it("maps every GitHub request kind to the provider contract", async () => {
        ...
        for (const item of cases) {
            ...
            const response = await foundation.execute(
                "account-1",
                item.request,
            );
    ...
    it("maps every GitLab request kind and rejects unsupported fork sync", async () => {
        ...
        const cases: readonly RequestCase[] = [
            ...
            },
            {
                ...
                path: "graphql",
                payload: {
                    query: expect.stringContaining("GitClientShareNamespaces"),
                },
                response: {
                    data: {
                        currentUser: {
                            namespace: { fullName: "Fox", fullPath: "fox" },
                        },
                        groups: {
                            nodes: [
                                {
            ...
            },
            {
                ...
                response: {
                    ...
                    web_url:
                        "https://gitlab.example.test/acme/team/new-repository",
                    http_url_to_repo:
                        "https://gitlab.example.test/acme/team/new-repository.git",
                    ssh_url_to_repo:
                        "git@gitlab.example.test:acme/team/new-repository.git",
                },
                responseKind: "repository",
            },
        ];
    ...
    it("maps every review event for both providers", async () => {
        ...
        for (const item of cases) {
            ...
            expect(github.http.requests.at(-1)?.body).toBe(
                JSON.stringify(item.github),
            );
    ...
    it("derives the next page only from a full provider-sized page", async () => {
        ...
        http.enqueue(
            jsonResponse(
                Array.from({ length: 50 }, () => GITHUB_CHANGE_REQUEST),
            ),
        );
    ...
    it("reports redirect, timeout, size, invalid JSON, and missing credential failures", async () => {
        ...
        await expect(
            redirect.foundation.execute("account-1", request),
        ).rejects.toMatchObject({
            code: "redirect",
        });

        const timeoutObservation: { signal: AbortSignal | null } = {
            signal: null,
        };
        ...
        const timeout = ElectronHostingFoundation.of(
            pendingHttp,
            timeoutCredentials,
            {
                timeoutMs: 5,
        ...
        await expect(
            timeout.execute("account-1", request),
        ).rejects.toMatchObject({ code: "timeout" });
        const observedSignal = timeoutObservation.signal;
        if (observedSignal === null)
            throw new Error("Timeout request signal was not observed");
        expect(observedSignal.aborted).toBe(true);

        const limitedHttp = new MockHostingHttpClient();
        const limited = ElectronHostingFoundation.of(
            limitedHttp,
            timeoutCredentials,
            {
                timeoutMs: 1_000,
        ...
        await expect(
            limited.execute("account-1", request),
        ).rejects.toMatchObject({
            code: "responseTooLarge",
        });
