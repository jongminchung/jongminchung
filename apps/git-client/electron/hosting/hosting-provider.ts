...
function record(
    ...
    if (!isRecord(value)) {
        throw invalidResponse(
            `Hosting ${description} response is not an object`,
        );
    }
    return value;
}
...
function array(value: unknown, description: string): readonly unknown[] {
    if (!Array.isArray(value)) {
        throw invalidResponse(
            `Hosting ${description} response is not an array`,
        );
    }
    return value;
}
...
function gitlabNamespaceRestId(value: string): string {
    ...
    if (!match?.[1]) {
        throw new HostingFoundationError(
            "invalidInput",
            "GitLab namespace ID is invalid",
        );
    }
    return match[1];
}
        ...
        case "review":
            if (provider === "gitHub") {
                return Object.freeze({
                    ...
                    payload: {
                        event: reviewEvent(request.event),
                        body: request.body,
                    },
                });
            }
            ...
            return Object.freeze({
                ...
                payload: {
                    body:
                        request.event === "requestChanges"
                            ? `Request changes: ${request.body}`
                            : request.body,
                },
            });
        case "updateBranch":
        ...
        case "checkShareRepository":
            ...
            return Object.freeze({
                ...
                payload: {
                    ...
                    variables: {
                        fullPath: `${request.namespacePath}/${request.name}`,
                    },
                },
            });
                ...
                payload:
                    provider === "gitHub"
                        ...
                        : {
                              name: request.name,
                              description: request.description,
                              visibility: request.private
                                  ? "private"
                                  : "public",
                              ...(request.namespaceId === null
                                  ? {}
                                  : {
                                        namespace_id: gitlabNamespaceRestId(
                                            request.namespaceId,
                                        ),
                                    }),
                          },
...
export function parseHostingResponse(
    ...
    switch (request.kind) {
        ...
        case "timeline":
            return Object.freeze({
                kind: "timeline",
                items: array(value, "timeline").map((item) =>
                    parseTimelineEntry(provider, item),
                ),
            });
        case "viewedFiles": {
            ...
            const repository = record(
                data.repository,
                "viewed files repository",
            );
            const pullRequest = record(
                repository.pullRequest,
                "viewed files pull request",
            );
            const files = record(pullRequest.files, "viewed files connection");
            const paths = array(files.nodes, "viewed files nodes").flatMap(
                (node) => {
                    if (node === null) return [];
        ...
        case "listNamespaces": {
            if (provider !== "gitLab") {
                throw invalidResponse(
                    "GitHub does not return GitLab namespaces",
                );
            }
            ...
            const groupItems = array(groups.nodes, "namespace groups").flatMap(
                (item) => {
                    if (item === null) return [];
                    ...
                    if (!optionalBoolean(permissions, "createProjects"))
                        return [];
                    return [
                        Object.freeze({
        ...
        case "listShareRepositories": {
            if (provider !== "gitHub") {
                throw invalidResponse(
                    "GitLab does not return GitHub owned repositories",
                );
            }
            ...
            const privateLimit =
                plan === null ? null : optionalNumber(plan, "private_repos");
            return Object.freeze({
                ...
                names: Object.freeze(
                    repositories.map((item) =>
                        stringValue(record(item, "owned repository"), "name"),
                    ),
                ),
            });
        }
        case "checkShareRepository": {
            if (provider !== "gitLab") {
                throw invalidResponse(
                    "GitHub does not return GitLab repository availability",
                );
            }
            const data = graphQlData(value, "share repository availability");
            if (data.project !== null && !isRecord(data.project)) {
                throw invalidResponse(
                    "GitLab project availability response is invalid",
                );
            }
            return Object.freeze({
