...
function readinessState(
  ...
  if (
    reasons.some(
      (reason) =>
        reason === "checks-pending" || reason === "provider-unavailable",
    )
  )
    return "pending";
...
function parseGitHubMergeReadiness(
  ...
  if (item.mergeable_state === "behind") reasons.push("branch-update-required");
...
function parseTimelineEntry(
  ...
  if (provider === "gitHub") {
    ...
    return Object.freeze({
      ...
      author:
        actor.length === 0 ? nestedString(item, ["user", "login"]) : actor,
