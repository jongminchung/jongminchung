...
const HostingBaseUrlInputSchema = z.string().trim().min(1).max(MAX_URL_CHARACTERS);

export const HostingBaseUrlSchema = HostingBaseUrlInputSchema.transform((value, context) => {
  const origin = normalizedOrigin(value);
  if (origin === null) {
    context.addIssue({
      code: "custom",
      message: "base URL must be an HTTPS origin without credentials, query, or fragment",
    });
    return z.NEVER;
  }
  return origin;
...
export const HostingAccountIdSchema = z.string().min(1).max(MAX_IDENTIFIER_CHARACTERS);
...
export const HostingAccountSchema = z
  .object({
    ...
    login: z
      ...
      .refine((value) => value.trim().length > 0, "login must not be empty"),
  })
  .strict()
  .readonly();
export type HostingAccount = Readonly<z.infer<typeof HostingAccountSchema>>;

export const HostingAccountsSchema = z.array(HostingAccountSchema).max(1_000).readonly();
...
export const SaveHostingAccountSchema = z
  .object({
    ...
    token: z
      .string()
      .max(16_384)
      .refine((value) => value.trim().length > 0, "token must not be empty"),
  })
  .strict()
  .readonly();
...
const ProjectSchema = z
  ...
  .refine(safeProject, "project must be a safe owner/repository or group/repository value");
const NumberSchema = z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER);
const PageSchema = z.number().int().min(0).max(4_294_967_295);
const BranchSchema = z.string().max(MAX_BRANCH_CHARACTERS).refine(safeBranch, "branch is invalid");
...
const RequiredBodySchema = BodySchema.refine(requiredText, "body must not be empty");
const TitleSchema = z
  .string()
  .max(MAX_TITLE_CHARACTERS)
  .refine(requiredText, "title must not be empty");

export const HostingReviewEventSchema = z.enum(["approve", "requestChanges", "comment"]);
...
const HostingShareRepositoryRequestSchema = z
  .object({
    ...
    namespaceId: z
      ...
      .min(1)
      .max(MAX_IDENTIFIER_CHARACTERS)
      .regex(/^(?:gid:\/\/gitlab\/[A-Za-z:]+\/)?[0-9]+$/u, "namespace ID is invalid")
      .nullable(),
  })
  .strict()
  .readonly();
...
export const HostingChangeRequestSchema = z
  ...
  .strict()
  .readonly();
export type HostingChangeRequest = Readonly<z.infer<typeof HostingChangeRequestSchema>>;
...
export const HostingChangedFileSchema = z
  ...
  .strict()
  .readonly();
export type HostingChangedFile = Readonly<z.infer<typeof HostingChangedFileSchema>>;
...
export const HostingTimelineEntrySchema = z
  ...
  .strict()
  .readonly();
export type HostingTimelineEntry = Readonly<z.infer<typeof HostingTimelineEntrySchema>>;
      ...
      .object({
        ...
        items: z
          .array(
            z
              .object({
                id: z.string().min(1).max(MAX_IDENTIFIER_CHARACTERS).nullable(),
...
} as const satisfies Readonly<Record<HostingRequest["kind"], HostingResponse["kind"]>>;
