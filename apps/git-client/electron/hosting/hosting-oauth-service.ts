...
export class HostingOAuthService {
    ...
    async complete(sessionId: string): Promise<HostingOAuthGrant> {
        ...
        if (session.awaiting)
            throw oauthError(
                "OAuth sign-in session is already being completed",
            );
    ...
    async #beginGitLab(
        ...
        const redirectUri =
            this.#clients.gitLabRedirectUri?.trim() ||
            DEFAULT_GITLAB_REDIRECT_URI;
    ...
    async #completeGitHub(
        ...
        while (this.#runtime.now() < session.expiresAt) {
            ...
            const providerError = OAuthErrorResponseSchema.safeParse(
                result.value,
            );
    ...
    async #discardExpiredSessions(): Promise<void> {
        ...
        await Promise.all(
            expired.map((session) => this.cancel(session.sessionId)),
        );
    }
}
