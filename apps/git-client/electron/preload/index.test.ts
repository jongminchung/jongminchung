...
describe("Electron preload Git API", () => {
  ...
  it("validates HTTP(S) external URLs before crossing IPC", async () => {
    ...
    await expect(api().shell.openExternal("file:///tmp/private")).rejects.toThrow(
  ...
  it("validates repository lifecycle results", async () => {
    ...
    await expect(api().git.openRepository("/tmp/repository")).resolves.toEqual(REPOSITORY);
    await expect(api().git.closeRepository(REPOSITORY_ID)).resolves.toBe(true);
  ...
  it("validates every repository inspection and ignore-rules response", async () => {
    ...
    await expect(api().git.preCommitCheck(REPOSITORY_ID)).resolves.toMatchObject({
      branch: "main",
      protectedBranch: true,
    });
    ...
    await expect(api().git.listMergedBranches(REPOSITORY_ID, "HEAD")).resolves.toEqual(["main"]);
    await expect(api().git.loadCommitSignature(REPOSITORY_ID, "HEAD")).resolves.toMatchObject({
      status: "N",
    });
  ...
  it("validates direct push and history rewrite preview results", async () => {
    ...
    await expect(
      api().git.loadPushPreview(REPOSITORY_ID, "origin", "refs/heads/main", "HEAD"),
  ...
  it("validates all patch, shelf, changelist, recovery, and conflict calls", async () => {
    ...
    await expect(api().git.createPatchText(REPOSITORY_ID, ["HEAD"])).resolves.toBe("patch text");
    await expect(
      api().git.importPatch(REPOSITORY_ID, "/tmp/import.patch"),
    ).resolves.toBeUndefined();
    await expect(api().git.createShelf(REPOSITORY_ID, "saved", ["tracked.txt"])).resolves.toEqual(
      shelf,
    );
    await expect(api().git.listShelves(REPOSITORY_ID)).resolves.toEqual([shelf]);
    await expect(api().git.applyShelf(REPOSITORY_ID, shelfId, true)).resolves.toBeUndefined();
    await expect(api().git.deleteShelf(REPOSITORY_ID, shelfId)).resolves.toBeUndefined();
    await expect(api().git.listChangelists(REPOSITORY_ID)).resolves.toEqual([changelist]);
    await expect(
      api().git.saveChangelist(REPOSITORY_ID, null, "selected", ["tracked.txt"]),
    ).resolves.toEqual(changelist);
    await expect(api().git.deleteChangelist(REPOSITORY_ID, changelistId)).resolves.toBeUndefined();
    await expect(
      api().git.commitChangelist(REPOSITORY_ID, changelistId, "commit", false, false, false),
    ).resolves.toMatchObject({ changelistId });
    await expect(api().git.listRecoveryEntries(REPOSITORY_ID)).resolves.toEqual([recovery]);
    await expect(api().git.restoreRecoveryEntry(REPOSITORY_ID, recoveryId)).resolves.toMatchObject({
      restoredRefs: ["refs/heads/main"],
    });
    await expect(api().git.listConflicts(REPOSITORY_ID)).resolves.toEqual([conflict]);
    await expect(api().git.readConflict(REPOSITORY_ID, "tracked.txt")).resolves.toEqual(
      conflictContent,
    );
    await expect(
      api().git.writeConflictResult(REPOSITORY_ID, "tracked.txt", "resolved\n", true),
    ).resolves.toBeUndefined();
    await expect(
      api().git.resolveBinaryConflict(REPOSITORY_ID, "tracked.txt", "ours"),
    ).resolves.toBeUndefined();

    expect(electronMock.invoke.mock.calls.map((call) => call[1]?.operation)).toEqual([
      "exportPatch",
      "createPatchText",
      "importPatch",
      "createShelf",
  ...
  it("rejects unsafe special-service input before invoking main", async () => {
    await expect(api().git.readConflict(REPOSITORY_ID, "../secret.txt")).rejects.toThrow();
    await expect(
      api().git.createShelf(REPOSITORY_ID, "bad\0message", ["tracked.txt"]),
    ).rejects.toThrow();
    await expect(
  ...
  it("validates submodule, file-open, and multi-root requests in the sandboxed preload", async () => {
    ...
    const rollbackStep = {
      ...
      operations: [{ kind: "checkout" as const, target: "main", force: false }],
    };
    electronMock.invoke.mockImplementation(
      async (channel: string, request: Readonly<Record<string, unknown>>): Promise<unknown> => {
    ...
    electronMock.invoke.mockClear();
    await expect(api().git.openWorkingTreeFile(REPOSITORY_ID, "../outside.txt")).rejects.toThrow();
    await expect(
  ...
  it("validates and streams repository initialize and clone operations", async () => {
      ...
      async (channel: string, request: Readonly<Record<string, unknown>>): Promise<unknown> => {
        ...
        emit(
          {},
          {
            ...
            operation,
            displayCommand: operation === "initialize" ? "git init" : "git clone",
            startedAtMs: 1,
          },
        );
        emit(
    ...
    await expect(
      api().git.initializeRepository("/tmp/repository", false, (event) => received.push(event)),
    ...
    expect(received.map((event) => `${event.operation}:${event.kind}`)).toEqual([
      "initialize:started",
  ...
  it("subscribes before invoking and delivers one ordered query lifecycle", async () => {
    ...
    expect(received.map((event) => event.kind)).toEqual(["started", "output", "completed"]);
  });
  ...
  it("delivers an operation through the same validated preload lifecycle", async () => {
    ...
    await expect(api().git.executeQuery(request, (event) => received.push(event))).resolves.toEqual(
      terminal,
    );
    expect(received.map((event) => event.kind)).toEqual(["started", "completed"]);
  });
  ...
  it("validates bounded file results and owns repository watcher cleanup", async () => {
    ...
    await expect(
      api().git.readFile(REPOSITORY_ID, { kind: "workingTree" }, "tracked.txt"),
    ).resolves.toEqual(FILE_CONTENT);
    await expect(
      api().git.readFilePreview(REPOSITORY_ID, { kind: "index" }, "tracked.bin"),
    ...
    await expect(api().git.unwatchRepository(REPOSITORY_ID)).resolves.toBeUndefined();
    repositoryChangedEventHandler()({}, REPOSITORY_CHANGED);
    expect(received).toEqual([REPOSITORY_CHANGED]);
  });
  ...
  it("subscribes before creating a terminal and validates every terminal operation", async () => {
    ...
    await expect(
      api().terminal.create(REPOSITORY_ID, 100, 28, { kind: "default" }, (event) =>
        received.push(event),
      ),
    ...
    await expect(api().terminal.write(TERMINAL_ID, "pwd\r")).resolves.toBeUndefined();
    await expect(api().terminal.resize(TERMINAL_ID, 120, 36)).resolves.toBeUndefined();
    await expect(api().terminal.close(TERMINAL_ID)).resolves.toBeUndefined();
    await expect(api().terminal.closeRepository(REPOSITORY_ID)).resolves.toBeUndefined();

    expect(received).toEqual([{ kind: "output", sequence: 0, data: [112, 119, 100, 13, 10] }]);
  });
});
...
describe("Electron preload hosting API", () => {
  ...
  it("rejects malformed input before IPC and mismatched responses after IPC", async () => {
    await expect(
      api().hosting.restoreAccounts([{ ...account, token: "must-not-cross" }] as never),
    ).rejects.toThrow();
    expect(electronMock.invoke).not.toHaveBeenCalled();
  ...
  it("rejects an account identity that does not match the save request", async () => {
    ...
    await expect(
      api().hosting.saveAccount("gitHub", "https://github.com", "ghp_secret"),
  ...
  it("rejects any credential field returned across the main boundary", async () => {
    ...
    await expect(
      api().hosting.saveAccount("gitHub", "https://github.com", "ghp_request-secret"),
    ).rejects.toThrow();
  });
});
