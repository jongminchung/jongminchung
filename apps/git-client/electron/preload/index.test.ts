    ...
    try {
      ...
      if (
        ...
        setTimeout(() => {
          electronMock.streamPort?.postMessage({
            kind: "git.barrier",
            operation:
              envelope.path === TEST_TRPC_PATHS.gitQuery ? "query" : "creation",
      ...
      return {
        ...
        error: {
          ...
          message:
            error instanceof Error ? error.message : "Desktop RPC failed",
        },
      };
    }
  }),
...
describe("전자 사전 로드 Git API", () => {
  ...
  it("[성공] 리포지토리 수명주기 결과를 인증함", async () => {
    electronMock.invoke.mockImplementation(
      async (channel: string): Promise<unknown> => {
        if (channel === TEST_TRPC_PATHS.gitOpenRepository) return REPOSITORY;
    ...
    await expect(api().git.openRepository("/tmp/repository")).resolves.toEqual(
      REPOSITORY,
    );
  ...
  it("[성공] 모든 패치, 쉘프, 변경 목록, 복구 및 호출을 인증함", async () => {
    ...
    await expect(api().git.listRecoveryEntries(REPOSITORY_ID)).resolves.toEqual(
      [recovery],
    );
    await expect(
      ...
      async (
        ...
        emit(
          {},
          {
            ...
            operation,
            displayCommand:
              operation === "initialize" ? "git init" : "git clone",
            startedAtMs: 1,
          },
        );
        const terminal = {
  ...
  it("[성공] 권한 파일 결과를 검증하고 시계자 정리를 소유함", async () => {
    ...
    electronMock.invoke.mockImplementation(
      async (channel: string): Promise<unknown> => {
        if (channel === TEST_TRPC_PATHS.gitReadFile) return FILE_CONTENT;
        if (channel === TEST_TRPC_PATHS.gitReadFilePreview) return FILE_PREVIEW;
        if (channel === TEST_TRPC_PATHS.gitWatchRepository) {
          repositoryChangedEventHandler()({}, REPOSITORY_CHANGED);
          return undefined;
        }
        if (channel === TEST_TRPC_PATHS.gitUnwatchRepository) return undefined;
        throw new Error(`Unexpected channel ${channel}`);
      },
    );
...
describe("전자 사전 로드 거부 API", () => {
  ...
  it("[실패] 저장 요청과 일치하지 않는 계정 ID가 있음", async () => {
    ...
    await expect(
      api().hosting.beginOAuth("gitHub", "https://github.com", "client-id"),
