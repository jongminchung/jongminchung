...
async function invokeRepositoryCreation(
    procedure: "initializeRepository" | "cloneRepository",
    ...
    if (listener !== undefined) {
        if (gitStreamEvents.creationListeners.has(request.requestId)) {
            throw new Error(
                `Git request ${request.requestId} is already running in this renderer`,
            );
        }
    ...
    try {
        const result =
            procedure === "initializeRepository"
                ? await desktopTrpc.git.initializeRepository.mutate(
                      GitInitializeRepositoryRequestSchema.parse(request),
                  )
                : await desktopTrpc.git.cloneRepository.mutate(
                      GitCloneRepositoryRequestSchema.parse(request),
                  );
        ...
        if (listener !== undefined)
            gitStreamEvents.deliverCreationEvent(listener, terminal);
        if (terminal.kind === "completed")
            return RepositoryRecordSchema.parse(terminal.repository);
        if (terminal.kind === "failed") throw new Error(terminal.message);
        throw new Error("Repository creation was cancelled");
    } finally {
        if (
            listener !== undefined &&
            gitStreamEvents.creationListeners.get(request.requestId) ===
                listener
        ) {
...
export function createGitApi(): DesktopApi["git"] {
    return {
        async openRepository(path: string): Promise<RepositoryRecord> {
            ...
            return desktopTrpc.git.openRepository.mutate(request);
        },
        async initializeRepository(
            ...
            return invokeRepositoryCreation(
        ...
        async closeRepository(repositoryId: RepositoryId): Promise<boolean> {
            ...
            try {
                return await desktopTrpc.git.closeRepository.mutate(request);
            } finally {
                gitStreamEvents.repositoryListeners.delete(
        ...
        async inspectSnapshot(repositoryId) {
            ...
            return desktopTrpc.git.inspectSnapshot.query(request);
        },
        ...createGitRepositoryServiceApi(),
        async executeQuery(
            ...
            try {
                const terminal = GitTerminalResultSchema.parse(
                    await desktopTrpc.git.query.mutate(request),
                );
                if (terminal.requestId !== request.requestId) {
                    throw new Error(
                        "Git query result did not match its request",
                    );
                }
                await gitStreamEvents.waitForBarrier(
                    "query",
                    request.requestId,
                );
                if (
                    gitStreamEvents.queryListeners.get(request.requestId) ===
                    listener
                ) {
        ...
        async cancelQuery(requestId: GitRequestId): Promise<boolean> {
            ...
            return desktopTrpc.git.cancelQuery.mutate(request);
        },
        async readFile(repositoryId, source, path) {
            ...
            return desktopTrpc.git.readFile.query(request);
        },
        async readFilePreview(repositoryId, source, path) {
            ...
            return desktopTrpc.git.readFilePreview.query(request);
        },
        async writeWorkingTreeFile(
            ...
            await desktopTrpc.git.writeWorkingTreeFile.mutate(request);
        },
        ...
        async openWorkingTreeFile(repositoryId, path): Promise<void> {
            ...
            await desktopTrpc.git.openWorkingTreeFile.mutate(request);
        },
        ...
        async watchRepository(repositoryId, listener): Promise<void> {
            ...
            if (gitStreamEvents.repositoryListeners.has(request.repositoryId)) {
                gitStreamEvents.repositoryListeners.set(
                    request.repositoryId,
                    listener,
                );
                return;
            ...
            try {
                await desktopTrpc.git.watchRepository.mutate(request);
            } catch (error) {
                if (
                    gitStreamEvents.repositoryListeners.get(
                        request.repositoryId,
                    ) === listener
                ) {
                    gitStreamEvents.repositoryListeners.delete(
        ...
        async unwatchRepository(repositoryId): Promise<void> {
            ...
            try {
                await desktopTrpc.git.unwatchRepository.mutate(request);
            } finally {
                gitStreamEvents.repositoryListeners.delete(
                    request.repositoryId,
                );
