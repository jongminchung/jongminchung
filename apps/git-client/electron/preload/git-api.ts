...
export function createGitApi(): DesktopApi["git"] {
  return {
    ...
    async cloneRepository(
      ...
      return invokeRepositoryCreation("cloneRepository", request, listener);
    },
    ...
    async executeQuery(
      ...
      try {
        ...
        if (
          gitStreamEvents.queryListeners.get(request.requestId) === listener
        ) {
    ...
    async watchRepository(repositoryId, listener): Promise<void> {
      ...
      try {
        ...
        if (
          gitStreamEvents.repositoryListeners.get(request.repositoryId) ===
          listener
        ) {
          gitStreamEvents.repositoryListeners.delete(request.repositoryId);
        }
