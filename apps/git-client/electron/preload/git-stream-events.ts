...
function barrierKey(operation: "query" | "creation", requestId: GitRequestId): string {
...
function waitForBarrier(operation: "query" | "creation", requestId: GitRequestId): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    if (barrierWaiters.has(key)) {
      reject(new Error(`Git ${operation} barrier ${requestId} already has a waiter`));
      return;
    }
...
export const gitStreamEvents = {
  creationListeners,
  deliverCreationEvent(listener: GitCreationEventListener, event: GitCreationEvent): void {
