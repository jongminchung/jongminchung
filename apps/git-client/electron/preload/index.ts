...
const repositoryChangedListeners = new Map<
    RepositoryId,
    RepositoryChangedListener
>();
...
ipcRenderer.on(
    ...
    (_event, raw: unknown): void => {
        ...
        const listener = repositoryChangedListeners.get(
...
async function invokeRepositoryCreation(
    channel:
        | typeof IPC_CHANNELS.gitInitializeRepository
        | typeof IPC_CHANNELS.gitCloneRepository,
    ...
    try {
        ...
        if (
            listener !== undefined &&
            gitCreationListeners.get(request.requestId) === listener
        ) {
...
const api: DesktopApi = {
    ...
    menu: {
        onCommand(listener: (command: NativeCommand) => void): () => void {
            ...
            return () =>
                ipcRenderer.removeListener(IPC_CHANNELS.menuCommand, handler);
        },
    ...
    git: {
        ...
        async captureLocalHistory(repositoryId, label) {
            const result = await invokeRepositoryService({
                operation: "captureLocalHistory",
                repositoryId,
                label,
            });
            if (result.operation !== "captureLocalHistory")
                throw new Error("Unexpected repository result");
            return result.value;
        },
        async listLocalHistory(repositoryId, path) {
            const result = await invokeRepositoryService({
                operation: "listLocalHistory",
                repositoryId,
                path,
            });
            if (result.operation !== "listLocalHistory")
                throw new Error("Unexpected repository result");
            return result.value;
        },
        async readLocalHistoryDiff(repositoryId, entryId, path) {
            const result = await invokeRepositoryService({
        ...
        async restoreLocalHistory(repositoryId, entryId, path) {
            const result = await invokeRepositoryService({
                operation: "restoreLocalHistory",
                repositoryId,
                entryId,
                path,
            });
            if (result.operation !== "restoreLocalHistory")
                throw new Error("Unexpected repository result");
        },
        async labelLocalHistory(repositoryId, entryId, label) {
            const result = await invokeRepositoryService({
                operation: "labelLocalHistory",
                repositoryId,
                entryId,
                label,
            });
            if (result.operation !== "labelLocalHistory")
                throw new Error("Unexpected repository result");
            return result.value;
        },
        ...
        async writeWorkingTreeFile(repositoryId, path, content): Promise<void> {
        ...
        async watchRepository(repositoryId, listener): Promise<void> {
            ...
            if (repositoryChangedListeners.has(request.repositoryId)) {
                throw new Error(
                    "Repository is already watched in this renderer",
                );
            }
            ...
            try {
                ...
                if (
                    repositoryChangedListeners.get(request.repositoryId) ===
                    listener
                ) {
                    repositoryChangedListeners.delete(request.repositoryId);
                }
