...
const contracts = JSON.parse(
  readFileSync(new URL("./contracts/rebased-1.1.8.json", import.meta.url), "utf8"),
) as RebasedContracts;
...
test("matches the Rebased project switcher interaction contract", async ({ page }) => {
...
test("matches the Rebased branch popup structure contract", async ({ page }) => {
  await page
  ...
  await expect(popup.getByRole("combobox", { name: contracts.branchPopup.search })).toBeFocused();
  ...
  await expect(popup.getByRole("button", { name: contracts.branchPopup.settings })).toBeVisible();
});
