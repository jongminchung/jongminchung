...
Git Client는 React renderer와 격리된 preload, main process, utility process로 구성된 Electron 데스크톱 애플리케이션이다. 실제 저장소·stash·파일·Terminal 기능은 Electron 앱에서 확인하고, 레이아웃과 상호작용은 명시적인 브라우저 fixture로 빠르게 확인한다.
...
실제 Git 저장소, 네이티브 디렉터리 선택기와 Electron preload bridge를 확인할 때는 다음 명령 하나를 실행한다.

```sh
pnpm --filter @jongminchung/git-client dev
```

Electron Forge가 Vite 개발 서버를 시작한 뒤 macOS 앱 창을 연다. 앱은 닫을 수 없는 **Manage** 탭으로 시작하며, 여기서 저장소를 열면 저장소별 탭이 추가된다. 탭 순서와 활성 탭은 재시작 후 복원된다.

| 변경 대상                        | 개발 중 반영 방식                        |
| -------------------------------- | ---------------------------------------- |
| React, TypeScript, CSS           | renderer에 HMR로 즉시 반영               |
| Electron main, preload, utility  | Vite 재빌드 후 앱 재시작                 |
| 열어 둔 Git 저장소의 파일과 refs | 250ms debounce 후 상태 조회를 한 번 실행 |

macOS Web Inspector는 앱에서 `⌘⌥I`를 눌러 연다. Electron main과 utility process 로그 및 앱 재시작 상태는 `pnpm dev`를 실행한 터미널에서 확인한다. 개발 세션은 해당 터미널에서 `Ctrl+C`로 종료한다.
...
고정 단축키를 추가하거나 변경할 때는 `src/command-manifest.json`을 먼저 수정한다. Renderer는 외부 JSON을 검증한 뒤 현재 화면의 handler와 disabled reason을 등록하고, Electron main process는 같은 manifest로 native menu와 accelerator를 구성한다. 새 command는 palette, renderer handler, native enabled/check state와 manifest 테스트를 함께 갱신해야 한다.
...
출력된 경로를 Electron 앱의 **Open Repository**로 연다. 테스트가 끝나면 경로를 다시 확인한 뒤 임시 디렉터리를 정리한다.

## Electron IPC 계약

외부 입력은 `unknown`으로 받은 뒤 `src/shared/contracts`의 Zod schema로 검증한다. Renderer, preload, main process와 utility process는 같은 TypeScript 계약을 사용한다.
...
IPC schema, TypeScript 타입과 bridge 호출 형태는 하나의 변경으로 함께 제출한다.
