...
Tauri가 Vite 개발 서버를 `http://localhost:1420`에 시작한 뒤 macOS 앱 창을 연다. 별도 터미널에서 `pnpm dev`를 실행할 필요가 없다. 앱은 닫을 수 없는 **Manage** 탭으로 시작하며, 여기서 저장소를 열면 저장소별 탭이 추가된다. 탭 순서와 활성 탭은 재시작 후 복원된다.
...
macOS Web Inspector는 앱에서 `⌘⌥I`를 눌러 연다. Rust 로그와 앱 재시작 상태는 `tauri:dev`를 실행한 터미널에서 확인한다. 개발 세션은 해당 터미널에서 `Ctrl+C`로 종료한다.
...
QA fixture는 네이티브 디렉터리 선택기, 실제 Git bridge와 PTY를 실행하지 않는다. 실제 저장소 상태나 변경 작업을 확인해야 한다면 `tauri:dev`로 전환한다.

## Git Console과 Terminal

하단의 두 도구창은 역할과 권한이 다르다.

- **Git Console**은 앱이 allowlist를 통해 실행한 Git 요청만 기록한다. request ID, 순서가 있는 stdout/stderr, 종료 코드와 실행 시간을 보존하며 자격 증명을 제거한다.
- **Terminal**은 현재 저장소를 cwd로 하여 사용자의 기본 셸을 실행한다. Git allowlist와 Console 기록·redaction을 적용하지 않으므로 입력한 명령은 사용자 계정의 전체 권한으로 실행된다.
