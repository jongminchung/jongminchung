...
test("[성공] 실제적인 도구 효과를 사용하여 51가지 패키지 작업 종류를 모두 실행함", async () => {
  ...
  try {
    ...
    expect(
      git(repository, "rev-parse", "--abbrev-ref", "main@{upstream}").trim(),
    ).toBe("origin/main");
    ...
    expect(git(rebaseRepository, "merge-base", "main", "feature").trim()).toBe(
      mainOid,
    );
    ...
    expect(git(squashCommitRepository, "log", "-1", "--format=%s")).toContain(
      "squash! baseline",
    );
    ...
    expect(git(stashBranchRepository, "branch", "--show-current").trim()).toBe(
      "from-stash",
    );
    expect(
    ...
    expect(
      git(shallowRepository, "rev-parse", "--is-shallow-repository").trim(),
    ).toBe("true");
