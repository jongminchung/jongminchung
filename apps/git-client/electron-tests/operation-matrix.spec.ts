...
test("[성공] 실제적인 도구 효과를 사용하여 51가지 패키지 작업 종류를 모두 실행함", async () => {
    ...
    async function run(
        ...
        const events = await executeOperation(
            app.page,
            repositoryId,
            operation,
        );
    ...
    try {
        ...
        expect(git(repository, "status", "--short")).toContain(
            "?? unstage.txt",
        );
        git(repository, "clean", "-fd");
        ...
        expect(
            git(repository, "branch", "--list", "operation-renamed"),
        ).toContain("operation-renamed");
        await run(repositoryId, {
            kind: "deleteBranch",
        ...
        expect(
            await readFile(join(cherryRepository, "cherry.txt"), "utf8"),
        ).toBe("picked\n");
        ...
        await writeFile(
            join(skipRepository, "tracked.txt"),
            "source\n",
            "utf8",
        );
        ...
        expect(
            await readFile(join(abortRepository, "tracked.txt"), "utf8"),
        ).toBe("main\n");
        ...
        await writeFile(
            join(stashRepository, "untracked.txt"),
            "new\n",
            "utf8",
        );
        await run(stashId, {
        ...
        expect(
            await readFile(join(stashRepository, "tracked.txt"), "utf8"),
        ).toBe("stashed\n");
        ...
        expect(
            git(shallowRepository, "rev-list", "--count", "HEAD").trim(),
        ).toBe("3");
