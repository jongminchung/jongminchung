...
test("executes all 51 packaged Git operation kinds with real disposable effects", async () => {
  ...
  const parent = await mkdtemp(join(tmpdir(), "git-client-electron-operation-matrix-"));
  const observedKinds = new Set<GitOperation["kind"]>();
  const app = await launchPackaged(["--qa-isolated-profile"]);

  async function run(repositoryId: string, operation: GitOperation): Promise<void> {
  ...
  try {
    ...
    expect(git(repository, "diff", "--cached", "--name-only")).toContain("stage.txt");
    ...
    expect(git(repository, "diff", "--cached", "--name-only")).toContain("stage-all.txt");
    git(repository, "reset", "--hard", "HEAD");
    git(repository, "clean", "-fd");

    await writeFile(join(repository, "tracked.txt"), "tracked update\n", "utf8");
    await run(repositoryId, { kind: "stageTracked" });
    expect(git(repository, "diff", "--cached", "--name-only")).toContain("tracked.txt");
    git(repository, "reset", "--hard", "HEAD");
    ...
    expect(await readFile(join(repository, "tracked.txt"), "utf8")).toBe("baseline\n");
    ...
    expect(await readFile(join(repository, "tracked.txt"), "utf8")).toBe("apply patch\n");
    git(repository, "restore", "--", "tracked.txt");

    await writeFile(join(repository, "tracked.txt"), "partial patch\n", "utf8");
    const partialPatch = git(repository, "diff", "--unified=0", "--", "tracked.txt");
    git(repository, "restore", "--", "tracked.txt");
    await run(repositoryId, {
      kind: "partialPatch",
      patch: partialPatch,
    ...
    expect(await readFile(join(repository, "tracked.txt"), "utf8")).toBe("partial patch\n");
    git(repository, "restore", "--", "tracked.txt");
    ...
    expect(git(repository, "log", "-1", "--format=%s").trim()).toBe("packaged commit operation");
    ...
    expect(git(repository, "log", "-1", "--format=%s").trim()).toBe("packaged advanced commit");
    ...
    expect(git(repository, "branch", "--remotes")).toContain("origin/remote-feature");
    ...
    expect(git(repository, "log", "-1", "--format=%s").trim()).toBe("remote main update");
    ...
    expect(git(repository, "branch", "--list", "operation-renamed")).toContain("operation-renamed");
    await run(repositoryId, {
      kind: "deleteBranch",
    ...
    expect(git(repository, "branch", "--show-current").trim()).toBe("checkout-target");
    ...
    expect(git(repository, "tag", "--list", "operation-v1").trim()).toBe("operation-v1");
    ...
    expect(git(repository, "config", "--local", "gitclient.operationmatrix").trim()).toBe(
      "verified",
    );
    ...
    expect(git(repository, "worktree", "list", "--porcelain")).toContain(worktreePath);
    await run(repositoryId, {
      kind: "worktreeRemove",
      path: worktreePath,
      force: false,
    ...
    expect(git(revertRepository, "log", "-1", "--format=%s")).toContain("Revert");

    const cherryRepository = await createRepository(parent, "cherry-pick");
    git(cherryRepository, "switch", "-c", "source");
    const cherryOid = await commitFile(cherryRepository, "cherry.txt", "picked\n", "pick target");
    ...
    expect(await readFile(join(cherryRepository, "cherry.txt"), "utf8")).toBe("picked\n");
    ...
    expect(
      git(mergeRepository, "rev-list", "--parents", "-n", "1", "HEAD").trim().split(" "),
    ).toHaveLength(3);

    const rebaseRepository = await createRepository(parent, "rebase");
    git(rebaseRepository, "switch", "-c", "feature");
    await commitFile(rebaseRepository, "feature.txt", "feature\n", "feature commit");
    git(rebaseRepository, "switch", "main");
    const mainOid = await commitFile(rebaseRepository, "main.txt", "main\n", "main commit");
    ...
    expect(git(rebaseRepository, "merge-base", "main", "feature").trim()).toBe(mainOid);

    const interactiveRepository = await createRepository(parent, "interactive-rebase");
    const interactiveRoot = git(interactiveRepository, "rev-parse", "HEAD").trim();
    ...
    const interactiveId = await openRepository(app.page, interactiveRepository);
    ...
    const dropOid = await commitFile(dropRepository, "drop.txt", "drop\n", "drop this commit");
    await commitFile(dropRepository, "keep.txt", "keep\n", "keep after drop");
    const dropId = await openRepository(app.page, dropRepository);
    await run(dropId, { kind: "dropCommits", revisions: [dropOid] });
    expect(git(dropRepository, "log", "--format=%s")).not.toContain("drop this commit");

    const squashRepository = await createRepository(parent, "squash-commits");
    ...
    expect(Number.parseInt(git(squashRepository, "rev-list", "--count", "HEAD").trim(), 10)).toBe(
      2,
    );
    ...
    expect(git(rewordRepository, "log", "-1", "--format=%s").trim()).toBe("new reword subject");

    const undoRepository = await createRepository(parent, "undo-commit");
    const undoBase = git(undoRepository, "rev-parse", "HEAD").trim();
    await commitFile(undoRepository, "undo.txt", "undo\n", "undo target");
    ...
    expect(git(undoRepository, "diff", "--cached", "--name-only")).toContain("undo.txt");
    ...
    expect(git(fixupRepository, "log", "-1", "--format=%s")).toContain("fixup! baseline");

    const squashCommitRepository = await createRepository(parent, "squash-commit");
    const squashTarget = git(squashCommitRepository, "rev-parse", "HEAD").trim();
    await writeFile(join(squashCommitRepository, "tracked.txt"), "squash commit\n", "utf8");
    git(squashCommitRepository, "add", "tracked.txt");
    const squashCommitId = await openRepository(app.page, squashCommitRepository);
    await run(squashCommitId, {
      kind: "createSquashCommit",
      revision: squashTarget,
    });
    expect(git(squashCommitRepository, "log", "-1", "--format=%s")).toContain("squash! baseline");
    ...
    await writeFile(join(continueRepository, "tracked.txt"), "topic\n", "utf8");
    git(continueRepository, "commit", "-am", "topic change");
    git(continueRepository, "switch", "main");
    await writeFile(join(continueRepository, "tracked.txt"), "main\n", "utf8");
    ...
    await writeFile(join(continueRepository, "tracked.txt"), "continued\n", "utf8");
    ...
    expect(await readFile(join(skipRepository, "tracked.txt"), "utf8")).toBe("main\n");
    ...
    expect(await readFile(join(abortRepository, "tracked.txt"), "utf8")).toBe("main\n");
    ...
    expect(await readFile(join(stashRepository, "tracked.txt"), "utf8")).toBe("stashed\n");
    ...
    for (const [index, content] of ["first", "second"].entries()) {
      await writeFile(join(stashRepository, "tracked.txt"), `${content}\n`, "utf8");
      git(stashRepository, "stash", "push", "--message", `clear ${index}`);
    }
    await run(stashId, { kind: "stashClear" });
    expect(git(stashRepository, "stash", "list")).toBe("");

    const stashBranchRepository = await createRepository(parent, "stash-branch");
    await writeFile(join(stashBranchRepository, "tracked.txt"), "branch stash\n", "utf8");
    git(stashBranchRepository, "stash", "push", "--message", "branch stash");
    const stashBranchId = await openRepository(app.page, stashBranchRepository);
    await run(stashBranchId, {
      kind: "stashBranch",
      stash: "stash@{0}",
      branch: "from-stash",
    });
    expect(git(stashBranchRepository, "branch", "--show-current").trim()).toBe("from-stash");
    expect(await readFile(join(stashBranchRepository, "tracked.txt"), "utf8")).toBe(
      "branch stash\n",
    );

    const shallowSource = await createRepository(parent, "shallow-source");
    await commitFile(shallowSource, "second.txt", "second\n", "second shallow commit");
    await commitFile(shallowSource, "third.txt", "third\n", "third shallow commit");
    const shallowRepository = join(parent, "shallow-clone");
    git(parent, "clone", "--depth", "1", pathToFileURL(shallowSource).href, shallowRepository);
    const shallowId = await openRepository(app.page, shallowRepository);
    expect(git(shallowRepository, "rev-parse", "--is-shallow-repository").trim()).toBe("true");
    await run(shallowId, { kind: "unshallow" });
    expect(git(shallowRepository, "rev-parse", "--is-shallow-repository").trim()).toBe("false");
    expect(git(shallowRepository, "rev-list", "--count", "HEAD").trim()).toBe("3");

    const submoduleChild = await createRepository(parent, "submodule-child");
    const submoduleRepository = await createRepository(parent, "submodule-root");
    git(
      submoduleRepository,
      "-c",
    ...
    expect(
      await readFile(join(submoduleRepository, "modules", "child", "tracked.txt"), "utf8"),
    ).toBe("baseline\n");

    expect(VALID_GIT_OPERATIONS).toHaveLength(51);
    const expectedKinds = new Set(VALID_GIT_OPERATIONS.map(({ kind }) => kind));
