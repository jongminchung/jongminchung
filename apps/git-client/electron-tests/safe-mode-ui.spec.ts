...
async function initializeSafeRepository(parent: string): Promise<string> {
  ...
  await writeFile(join(repository, "README.md"), "safe mode baseline\n", "utf8");
  git(repository, "add", "README.md");
  git(repository, "commit", "-m", "safe mode baseline");
  await writeFile(join(repository, "README.md"), "safe mode baseline\ndirty change\n", "utf8");
  await writeFile(join(repository, "untracked.txt"), "must remain untracked\n", "utf8");
...
test("keeps a packaged safe-mode project read-only across restart", async () => {
  ...
  try {
    ...
    try {
      ...
      await expect(page.getByRole("status", { name: "Safe Mode" })).toBeVisible();
      await expect(page.getByRole("region", { name: "Commit log" })).toBeVisible();
      await expect(page.getByRole("table", { name: "Git log" }).locator("[data-oid]")).toHaveCount(
        1,
      );

      const mainToolbar = page.getByRole("banner", { name: "Main Toolbar" });
      await expect(mainToolbar.getByRole("button", { name: "Update Project..." })).toBeDisabled();
      await expect(mainToolbar.getByRole("button", { name: "Push…" })).toBeDisabled();
      await expect(mainToolbar.getByRole("button", { name: "main", exact: true })).toBeDisabled();

      const leftToolbar = page.getByRole("navigation", { name: "Left Toolbar" });
      await expect(leftToolbar.getByRole("button", { name: "Commit", exact: true })).toBeDisabled();
      await expect(
        leftToolbar.getByRole("button", { name: "Terminal", exact: true }),
      ).toBeDisabled();

      await mainToolbar.getByRole("button", { name: "Search Everywhere" }).click();
      const palette = page.getByRole("dialog", { name: "Search Everywhere" });
      const search = palette.getByRole("combobox", { name: "Search Everywhere" });
      await search.fill("Manage Accounts");
      const hosting = palette.getByRole("option", { name: /Manage Accounts…/u });
      ...
      const mutation = palette.getByRole("option", { name: /Stage All Tracked/u });
      ...
      const editor = page.getByRole("region", { name: "Editor: README.md" });
      ...
      const directBoundaryResults = await page.evaluate(async (path) => {
        ...
        if (api === undefined) throw new Error("Git Client preload API is unavailable");
        const repositoryRecord = await api.git.openRepository(path);
        const rejected = async (label: string, operation: () => Promise<unknown>) => {
          try {
            await operation();
        ...
        return Promise.all([
          ...
          rejected("pre-commit", () => api.git.preCommitCheck(repositoryRecord.id)),
          rejected("write", () =>
            api.git.writeWorkingTreeFile(repositoryRecord.id, "README.md", "unsafe write\n"),
          ),
          rejected("external", () => api.git.openWorkingTreeFile(repositoryRecord.id, "README.md")),
          rejected("terminal", () => api.terminal.listLaunchTargets()),
      ...
      expect(compareGitStates(before, await captureGitState(repository))).toEqual({
        differences: [],
        equal: true,
      });

      await mainToolbar.getByRole("button", { name: "Search Everywhere" }).click();
      const closePalette = page.getByRole("dialog", { name: "Search Everywhere" });
      await closePalette.getByRole("combobox", { name: "Search Everywhere" }).fill("Close Project");
      const closeProject = closePalette.getByRole("option", { name: /Close Project/u });
      await expect(closeProject).not.toHaveAttribute("aria-disabled", "true");
      ...
      await expect
        .poll(async () =>
          page.evaluate(async () => {
            ...
            if (api === undefined) throw new Error("Git Client preload API is unavailable");
            return {
              openRepositoryPaths: await api.settings.get("openRepositoryPaths"),
              safeRepositoryPaths: await api.settings.get("safeRepositoryPaths"),
            };
          }),
        )
        .toEqual({ openRepositoryPaths: [], safeRepositoryPaths: [repository] });
    } finally {
      await app.close();
    }
    ...
    try {
      ...
      const recentProject = reopened.page
        ...
        .getByRole("option", { name: new RegExp(basename(repository), "u") });
      await recentProject.dblclick();
      await expect(reopened.page.getByRole("status", { name: "Safe Mode" })).toBeVisible();
      await expect(
        reopened.page
