...
export async function launchPackaged(args: readonly string[]): Promise<PackagedApp> {
  const port = await freePort();
  const child = spawn(executablePath, [...args, `--remote-debugging-port=${port}`], {
  ...
  while (browser === null && Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Git Client exited before CDP was ready (${child.exitCode}): ${stderr}`);
    }
    try {
