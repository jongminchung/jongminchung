...
function profilePath(): string {
  return join(homedir(), "Library", "Application Support", HOSTING_PROFILE_NAME);
}
...
export async function inspectHostingProfile(credentials: readonly string[]): Promise<{
  readonly credentialCount: number;
  readonly containsCredential: boolean;
}> {
  const settings = await readFile(join(profilePath(), "settings.json"), "utf8");
  return {
    credentialCount: settings.match(/"hostingCredential:/gu)?.length ?? 0,
    containsCredential: credentials.some((credential) => settings.includes(credential)),
  };
}
...
export async function startLoopbackHostingServer(
  ...
  const server = createHttpsServer({ cert: certificate, key }, (request, response) => {
    handleHostingRequest(request, response, gitHubCredential, gitLabCredential, captured);
  });
  server.listen(0, "127.0.0.1");
  await once(server, "listening");
  const address = server.address();
...
function handleHostingRequest(
  ...
  if (provider === "gitHub" && url.pathname === "/api/v3/repos/owner/error/pulls") {
    respond(response, 401, {
      message: `token=${gitHubCredential} Authorization: Bearer ${gitHubCredential}`,
    });
    return;
  }
  if (provider === "gitHub" && url.pathname === "/api/v3/repos/owner/repo/pulls") {
    respond(response, 200, [gitHubChangeRequest()]);
    return;
  }
  if (provider === "gitLab" && url.pathname === "/api/v4/projects/group%2Frepo/merge_requests") {
...
export async function launchPackagedHosting(certificatePath: string): Promise<PackagedHostingApp> {
  ...
  while (browser === null && Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Git Client exited before its CDP endpoint was ready (${child.exitCode})`);
    }
    try {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          typeof api === "object" && api !== null && typeof Reflect.get(api, "hosting") === "object"
        );
      },
      undefined,
      { timeout: 12_000 },
    ...
    await stopChild(child);
    throw new Error("Git Client hosting preload API did not become available");
  }
  let closed = false;
  return {
    page,
