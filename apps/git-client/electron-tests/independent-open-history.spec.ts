...
async function initializeRepository(repository: string, label: string): Promise<void> {
...
async function persistedValues(profileName: string): Promise<Readonly<Record<string, unknown>>> {
...
test("opens a recent project through the packaged UI and restores its history review", async () => {
  ...
  const parent = await mkdtemp(join(tmpdir(), "git-client-independent-open-history-"));
  ...
  await writeFile(join(repository, "README.md"), "foundation\naudit guide linked\n", "utf8");
  ...
  await writeFile(join(repository, "src", "head.txt"), "current head\n", "utf8");
  ...
  try {
    ...
    try {
      ...
      await expect(projectDialog).toContainText(basename(canonicalOtherRepository));
      ...
      await expect(review.getByText(auditOid, { exact: true })).toBeVisible();
      await expect(review).toContainText("Git Client QA");
      const changedFiles = review.getByRole("navigation", {
        name: "Changed files",
      });
      const auditFile = changedFiles.getByRole("button").filter({ hasText: "audit-guide.md" });
      ...
      await expect(logTable.locator("[data-oid]")).toHaveCount(3);
      await expect(logTable.locator(`[data-oid="${auditOid}"]`)).toHaveAttribute(
        "aria-selected",
        "true",
      );
      ...
      await expect
        .poll(async () => {
          ...
          const repositoryState = Object.entries(values).find(([key]) =>
            key.startsWith("repositoryUiState:"),
          )?.[1] as
            | {
          ...
          return {
            ...
            historySelectedPath: repositoryState?.historySelectedPath,
    ...
    try {
      ...
      await expect(restoredCommit).toHaveAttribute("aria-selected", "true");
      const restoredReview = page.getByRole("complementary", {
        name: "Revision review",
      });
      await expect(restoredReview.getByText(auditOid, { exact: true })).toBeVisible();
      ...
      await restoredReview.getByRole("button", { name: "Show Diff" }).click();
      await expect(
        page.getByRole("region", {
          name: "Diff content for docs/audit-guide.md",
        }),
      ...
      await expect(restoredProjects).toContainText(basename(canonicalRepository));
      await expect(restoredProjects).toContainText(basename(canonicalOtherRepository));
    } finally {
