            ...
            const result = await app.page.evaluate(
                async ({ initializedPath, sourceUrl, clonedPath }) => {
                    ...
                    const initializedRepository =
                        await api.git.initializeRepository(
                            initializedPath,
                            false,
                        );
...
test("executes packaged index, commit, ref, stash, config, remote, and worktree mutations", async () => {
    ...
    try {
        ...
        ).toContain(await realpath(linkedWorktree));
        await executePackagedOperation(page, repositoryId, {
            kind: "worktreeRemove",
            path: linkedWorktree,
            force: false,
    ...
    try {
        const result = await app.page.evaluate(
            async ({ path, exportedPatchPath }) => {
                ...
                const merged = await api.git.listMergedBranches(
                    snapshot.id,
                    "HEAD",
                );
                const withIdentity = <Request extends object>(
                    request: Request,
                ) => ({
                    ...request,
                ...
                const patchText = await api.git.createPatchText(snapshot.id, [
                    "HEAD",
                ]);
        ...
        expect(
            result.queryEvidence.find(({ kind }) => kind === "stashList")
                ?.output,
...
test("executes packaged shelf, changelist, recovery, and conflict boundaries", async () => {
    ...
    try {
        ...
        const conflict = await app.page.evaluate(async (id) => {
            ...
            await api.git.writeConflictResult(
                id,
                "tracked.txt",
                "resolved\n",
                true,
    ...
    try {
        ...
        await expect(
            app.page.evaluate(async (repositoryId) => {
                ...
                if (api === undefined)
                    throw new Error("Electron preload API is unavailable");
                await api.git.openWorkingTreeFile(
                    repositoryId,
                    "../outside.txt",
                );
            }, result.repositoryId),
...
test("executes and rolls back a packaged synchronized multi-root branch operation", async () => {
    ...
    try {
        ...
        expect(
            operation.result.outcomes.every(({ succeeded }) => succeeded),
        ).toBe(true);
        for (const repository of repositories) {
