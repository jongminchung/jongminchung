      ...
      if (terminal.kind !== "completed") {
        throw new Error(
          terminal.kind === "failed"
            ? terminal.message
            : `Git operation ended as ${terminal.kind}`,
        );
      }
      return eventKinds;
    },
...
test("[성공] 부스에 실제 패키지화된 Electron PTY를 요청다", async () => {
  ...
  const repositoryPath = await mkdtemp(
    join(tmpdir(), "git-client-electron-terminal-"),
  );
  try {
    ...
    try {
      ...
      await expect(surface.locator("textarea")).toHaveAttribute(
        "aria-label",
        "Editor",
      );
      await expect(
...
test("[성공]패키지화된 전자 조종사를 통해 리포지토리를 방수하고 복제함", async () => {
  ...
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-create-e2e-"),
  );
    ...
    try {
      const result = await app.page.evaluate(
        async ({ initializedPath, sourceUrl, clonedPath }) => {
          ...
          if (api === undefined)
            throw new Error("Electron preload API is unavailable");
          const initializedRepository = await api.git.initializeRepository(
            initializedPath,
            false,
          );
          const clonedRepository = await api.git.cloneRepository(
            sourceUrl,
            clonedPath,
            {
              depth: null,
      ...
      expect(
        gitText(initialized, "symbolic-ref", "--short", "HEAD").trim(),
      ).toBe("main");
      expect(gitText(cloned, "log", "-1", "--format=%s").trim()).toBe(
        "clone fixture",
      );
    } finally {
      await app.close();
...
test("[성공] 실제로 인증된 Git 기록 및 커밋 세부 정보를 표시함", async () => {
  ...
  await writeFile(
    join(watchedRepository, "committed.txt"),
    "committed fixture\n",
    "utf8",
  );
  await writeFile(
  ...
  await writeFile(
    join(watchedRepository, "staged.txt"),
    "staged addition\n",
    "utf8",
  );
  ...
  await writeFile(
    join(replacementRepository, "README.md"),
    "replacement fixture\n",
    "utf8",
  );
...
test("[성공]패시브 형태, 커밋, 참조, 숨김, 구성, 원격 및 작업 트리 변형을 실행함", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-mutation-e2e-"),
  );
  ...
  try {
    ...
    expect(gitText(repository, "status", "--short")).toContain(
      'A  "new file.txt"',
    );
    ...
    expect(
      gitText(repository, "branch", "--list", "packaged-feature"),
    ).toContain("packaged-feature");
    expect(gitText(repository, "tag", "--list", "packaged-v1").trim()).toBe(
      "packaged-v1",
    );
    ...
    expect(gitText(repository, "status", "--porcelain")).toContain(
      "tracked.txt",
    );
    await executePackagedOperation(page, repositoryId, {
      kind: "reset",
    ...
    expect(
      gitText(repository, "config", "--local", "gitclient.packaged").trim(),
    ).toBe("verified");
    await executePackagedOperation(page, repositoryId, {
      kind: "remoteAdd",
      name: "backup",
    ...
    expect(
      gitText(repository, "worktree", "list", "--porcelain"),
...
test("[성공] 클러스터 경계선 감지기, 무시, 미리보기 및 패치 경계를 실행함", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-inspection-e2e-"),
  );
  const repository = join(parent, "repository");
  ...
  try {
    const result = await app.page.evaluate(
      async ({ path, exportedPatchPath }) => {
        ...
        const signature = await api.git.loadCommitSignature(
          snapshot.id,
          "HEAD",
        );
        ...
        for (const request of requests) {
          ...
          const terminal = await api.git.executeQuery(request, (event) =>
            events.push(event),
          );
          queryEvidence.push({
            kind: request.kind,
            ...
            sequences: events.flatMap((event) =>
              event.kind === "output" && event.sequence !== undefined
                ? [event.sequence]
                : [],
            ),
        ...
        const exported = await api.git.exportPatch(
          snapshot.id,
          ["HEAD"],
          exportedPatchPath,
        );
    ...
    expect(result.historyRewrite.entries.map(({ subject }) => subject)).toEqual(
      ["first packaged inspection commit", "second packaged inspection commit"],
    );
    ...
    expect(result.patchText).toContain(
      "Subject: [PATCH] second packaged inspection commit",
    );
    ...
    expect(
      result.queryEvidence.find(({ kind }) => kind === "diff")?.output,
    ).toContain("+working");
    expect(
      result.queryEvidence.find(({ kind }) => kind === "stashList")?.output,
    ).toContain("packaged query stash");
...
test("[성공]패키지인 베어링, 변경 목록, 복구 및 충돌 경계를 실행함", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-special-git-e2e-"),
  );
  const repository = join(parent, "repository");
  await mkdir(repository);
  git(repository, "init", "--initial-branch=main");
  ...
  try {
    ...
    expect(await readFile(join(repository, "untracked.txt"), "utf8")).toBe(
      "untracked\n",
    );
    ...
    const committed = await app.page.evaluate(async (id) => {
      ...
      if (api === undefined)
        throw new Error("Electron preload API is unavailable");
      const changelist = await api.git.saveChangelist(
        id,
        null,
        "packaged selected files",
        ["tracked.txt"],
      );
    ...
    expect(committed.commit.commitOid).toBe(
      gitText(repository, "rev-parse", "HEAD").trim(),
    );
    expect(gitText(repository, "log", "-1", "--format=%s").trim()).toBe(
      "packaged changelist commit",
    );
    ...
    const restored = await app.page.evaluate(async (id) => {
      ...
      const entry = entries.find(
        (candidate) => candidate.operation === "commit",
      );
      if (entry === undefined)
        throw new Error("Packaged commit recovery entry is missing");
      const result = await api.git.restoreRecoveryEntry(id, entry.id);
      const disposable = await api.git.saveChangelist(
        id,
        null,
        "delete me",
        [],
...
test("[성공] [실패패]패키지 모듈 하위 검사를 실행하고 안전하지 않은 파일을 가지고 있음", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-submodule-e2e-"),
  );
...
test("[성공] 세트가 되는 2개의 분기 작업을 실행하고 롤백함", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-multi-root-e2e-"),
  );
  ...
  try {
    ...
    expect(operation.result.outcomes.every(({ succeeded }) => succeeded)).toBe(
      true,
    );
    for (const repository of repositories) {
      expect(gitText(repository, "branch", "--show-current").trim()).toBe(
        "feature/packaged-flow",
      );
    }
    ...
    for (const repository of repositories) {
      ...
      expect(
        gitText(repository, "branch", "--list", "feature/packaged-flow").trim(),
      ).toBe("");
    }
  } finally {
    await app.close();
...
test("[실패] 터미널 이벤트 시간을 사용하여 패키지화된 내 Git 쿼리 취소함", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(
    join(tmpdir(), "git-client-electron-cancel-e2e-"),
  );
  const repository = join(parent, "repository");
  await mkdir(repository);
  git(repository, "init", "--initial-branch=main");
