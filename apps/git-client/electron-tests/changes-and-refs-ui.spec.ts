...
function evidence(repository: string): GitEvidence {
    ...
    const revision = git(repository, "rev-list", "--parents", "-n", "1", "HEAD")
        .trim()
        .split(/\s+/);
    return {
        head,
...
async function expectRepositoryLoaded(
    page: Page,
    repository: string,
): Promise<void> {
    await expect(
...
async function openBranchActions(
    page: Page,
    current: string,
    target: string,
): Promise<Locator> {
...
test("[성공] 보이는 것을 바꿀 수 있는 부분, 작업 트리, 커밋 및 수정 상태를 변경함", async () => {
    ...
    try {
        ...
        try {
            ...
            await worktreeGroup
                .getByRole("button", { name: /partial\.txt/ })
                .dblclick();
            await page
            ...
            expect(partiallyStaged.refs).toContain(
                `refs/heads/feature/ui\u0000${baseHead}`,
            );
            expect(partiallyStaged.index).not.toContain(
                "line 100 changed for discarded hunk",
            );
            expect(partiallyStaged.worktree).toContain(
                "line 100 changed for discarded hunk",
            );
            expect(partiallyStaged.stash).toBe("");

            await page
                .getByRole("button", { name: "Exit focused diff" })
                .click();
            await worktreeGroup
                .getByRole("button", { name: /partial\.txt/ })
                .click();
            await changedFiles(page)
            ...
            await expect(
                worktreeGroup.getByRole("button", { name: /partial\.txt/ }),
            ).toBeVisible();
            await worktreeGroup
                .getByRole("button", { name: /partial\.txt/ })
                .click();
            ...
            expect(amended.refs).toContain(
                `refs/heads/feature/ui\u0000${amended.head}`,
            );
...
test("[성공] 브랜치 팝업을 통해 브랜치 생성, 출력, 이름 변경, 알림, 삭제", async () => {
    ...
    try {
        ...
        try {
            ...
            expect(created.refs).toContain(
                `refs/heads/ui-created\u0000${mainHead}`,
            );
            expect(created.index).toBe("");
            expect(created.worktree).toBe("");

            const renameActions = await openBranchActions(
                page,
                "ui-created",
                "ui-created",
            );
            ...
            expect(renamed.refs).toContain(
                `refs/heads/ui-renamed\u0000${mainHead}`,
            );
            ...
            const mergeActions = await openBranchActions(
                page,
                "main",
                "merge-source",
            );
            await mergeActions
                .getByRole("button", { name: "Merge into main…" })
                .click();
            ...
            await mergeDialog
                ...
                .click();
            await expect
                .poll(() => evidence(repository).parents)
                .toHaveLength(2);
            const merged = evidence(repository);
            expect(merged.parents).toEqual([mainHead, sourceHead]);
            expect(merged.refs).toContain(
                `refs/heads/main\u0000${merged.head}`,
            );
            expect(merged.index).toBe("");
            expect(merged.worktree).toBe("");
            expect(merged.stash).toBe("");
            expect(git(repository, "show", "HEAD:source.txt")).toBe(
                "source branch\n",
            );

            const deleteActions = await openBranchActions(
                page,
                "main",
                "ui-renamed",
            );
            ...
            await deleteDialog
                ...
                .click();
            await expect
                .poll(() => evidence(repository).refs)
                .not.toContain("refs/heads/ui-renamed");
...
test("[성공] 보이는 VCS 작업을 통해 숨김을 생성, 적용 및 삭제함", async () => {
    ...
    try {
        ...
        try {
            ...
            await page
                .getByRole("option", { name: /VCS Operations Popup…/ })
                .click();
            const operations = page.getByRole("dialog", {
                name: "VCS Operations",
            });
            await operations
                .getByRole("option", { name: /Stash Changes…/ })
                .click();
            ...
            await createDialog
                ...
                .fill("UI stash");
            await createDialog
                .getByRole("button", { name: "Stash", exact: true })
                .click();

            await expect
                .poll(() => evidence(repository).stash)
                .toContain("UI stash");
            const stashed = evidence(repository);
            expect(stashed.head).toBe(initial.head);
            expect(stashed.parents).toEqual(initial.parents);
            expect(stashed.refs).toContain(
                `refs/heads/feature/stash\u0000${initial.head}`,
            );
            ...
            await applyDialog
                .getByRole("button", { name: "Apply stash" })
                .click();
            await expect
                .poll(() => evidence(repository).status)
                .toContain("README.md");
