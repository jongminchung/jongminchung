...
async function waitForRepository(page: Page, repository: string): Promise<void> {
  await expect(
...
async function openBranches(page: Page, branch: string): Promise<void> {
  await page
    ...
    .click();
  await expect(page.getByRole("dialog", { name: "Git Branches" })).toBeVisible();
}
...
async function openPush(page: Page): Promise<Locator> {
  ...
  await expect(dialog.getByLabel("Destination branch")).toHaveValue("refs/heads/main");
  await expect(dialog.getByRole("button", { name: "Review destination" })).toBeEnabled();
  return dialog;
}
...
test("uses visible Fetch, Pull, Push, and exact force-with-lease controls against a real remote", async () => {
  ...
  const parent = await mkdtemp(join(tmpdir(), "git-client-electron-remote-ui-"));
  ...
  try {
    ...
    await Promise.all([configureRepository(repository), configureRepository(peer)]);

    git(peer, "switch", "-c", "fetch-only");
    await commitFile(peer, "fetch-only.txt", "fetch fixture\n", "fetch-only remote commit");
    ...
    try {
      ...
      await expect
        .poll(() => gitRefExists(repository, "refs/remotes/origin/fetch-only"))
        .toBe(true);
      await expect
        .poll(() => git(repository, "rev-parse", "refs/remotes/origin/fetch-only").trim())
      ...
      await page
        ...
        .click();
      await expect.poll(() => git(repository, "rev-parse", "HEAD").trim()).toBe(pulledOid);
      ...
      await expect(normalPushDialog.getByText("Fast-forward", { exact: true })).toBeVisible();
      await normalPushDialog.getByRole("button", { name: "Push", exact: true }).click();
      ...
      await expect(staleDialog.getByText("Diverged / rewritten", { exact: true })).toBeVisible();
      await staleDialog.getByRole("radio", { name: /Force push with lease/ }).check();
      await staleDialog.getByLabel(/confirm force push with lease/).fill("main");
      ...
      await staleDialog
        ...
        .click();
      await expect(
        staleDialog.getByText(/reviewed remote state is no longer reusable/i),
      ).toBeVisible();
      expect(git(remote, "rev-parse", "refs/heads/main").trim()).toBe(racedRemoteOid);
      await staleDialog.getByRole("button", { name: "Cancel", exact: true }).click();

      await fetchThroughUi(page, "main");
      await expect
        .poll(() => git(repository, "rev-parse", "refs/remotes/origin/main").trim())
        .toBe(racedRemoteOid);
      await page.keyboard.press("Escape");
      ...
      await reviewedDialog.getByRole("radio", { name: /Force push with lease/ }).check();
      await reviewedDialog.getByLabel(/confirm force push with lease/).fill("main");
...
test("aborts a conflicting rebase from visible recovery controls and restores the Git oracle", async () => {
  ...
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(join(tmpdir(), "git-client-electron-rebase-abort-ui-"));
  const repository = join(parent, "repository");
  ...
  try {
    ...
    await writeFile(join(repository, "worktree.txt"), "worktree baseline\n", "utf8");
    git(repository, "add", ".");
    git(repository, "commit", "--no-gpg-sign", "-m", "rebase baseline");

    await writeFile(join(repository, "worktree.txt"), "saved stash\n", "utf8");
    git(repository, "stash", "push", "--message", "pre-existing stash");
    git(repository, "switch", "-c", "target");
    await commitFile(repository, "conflict.txt", "target change\n", "target conflicting change");
    git(repository, "switch", "main");
    git(repository, "switch", "-c", "topic");
    await commitFile(repository, "conflict.txt", "topic change\n", "topic conflicting change");
    await writeFile(join(repository, "staged.txt"), "staged local edit\n", "utf8");
    git(repository, "add", "staged.txt");
    await writeFile(join(repository, "worktree.txt"), "unstaged local edit\n", "utf8");
    await writeFile(join(repository, "untracked.txt"), "untracked local edit\n", "utf8");
    ...
    try {
      ...
      await confirmation.getByRole("button", { name: "Start rebase", exact: true }).click();
      await expect.poll(() => gitRefExists(repository, "REBASE_HEAD")).toBe(true);
      ...
      await branchesDialog
        ...
        .click();
      await expect(page.getByText("rebase in progress", { exact: true })).toBeVisible();
      await page.getByRole("button", { name: "Abort", exact: true }).click();
      const abortDialog = page.getByRole("dialog", {
        name: "Abort rebase?",
      });
      await abortDialog.getByRole("button", { name: "Abort operation", exact: true }).click();

      await expect
        .poll(async () => compareGitStates(before, await captureGitState(repository)), {
          timeout: 20_000,
        })
...
test("resolves and continues a conflicting cherry-pick through visible history and recovery UI", async () => {
  test.setTimeout(90_000);
  await resetQaProfile(runtimeProfileName);
  const parent = await mkdtemp(join(tmpdir(), "git-client-electron-cherry-continue-ui-"));
  const repository = join(parent, "repository");
  ...
  try {
    ...
    git(repository, "commit", "--no-gpg-sign", "-m", "cherry-pick baseline");
    ...
    try {
      const { page } = app;
      await waitForRepository(page, repository);
      const sourceRow = page.getByRole("row").filter({ hasText: "source conflicting change" });
      await expect(sourceRow).toHaveCount(1);
      await sourceRow.click({ button: "right" });
      await page.getByRole("menuitem", { name: "Cherry-Pick", exact: true }).click();
      await expect.poll(() => gitRefExists(repository, "CHERRY_PICK_HEAD")).toBe(true);
      await expect(page.getByText("cherryPick in progress", { exact: true })).toBeVisible();

      await page.getByRole("button", { name: "Commit", exact: true }).click();
      const changedFiles = page.locator('aside[aria-label="Changed files"]');
      const conflictFile = changedFiles.getByText("conflict.txt", { exact: true }).first();
      ...
      await conflictDialog.getByRole("button", { name: "Remote", exact: true }).click();
      await conflictDialog.getByRole("button", { name: "Save and stage", exact: true }).click();
      await expect(conflictDialog).toHaveCount(0);
      await page.getByRole("button", { name: "Continue", exact: true }).click();

      await expect.poll(() => gitRefExists(repository, "CHERRY_PICK_HEAD")).toBe(false);
      expect(git(repository, "rev-parse", "HEAD").trim()).not.toBe(mainOid);
      expect(git(repository, "rev-parse", "HEAD").trim()).not.toBe(sourceOid);
      expect(git(repository, "log", "-1", "--format=%s").trim()).toBe("source conflicting change");
      expect(git(repository, "show", "HEAD:conflict.txt")).toBe("source change\n");
