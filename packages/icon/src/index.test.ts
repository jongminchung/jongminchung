...
describe("app icon source", () => {
    it("defines unique variants, preview sizes, and owned output paths", () => {
        ...
        expect(
            new Set(iconAssetTargets.map((target) => target.path)).size,
        ).toBe(iconAssetTargets.length);
        expect(
            iconAssetTargets.every((target) =>
                target.path.startsWith(`apps/${target.app}/`),
            ),
        ).toBe(true);
        expect(iconAssetTargets.every((target) => target.kind === "svg")).toBe(
            true,
        );
    });
    ...
    it.each(iconVariants)("renders safe standalone SVG for %s", (variant) => {
        ...
        expect(createIconDataUrl(variant)).toBe(
            `data:image/svg+xml;base64,${btoa(svg)}`,
        );
    });

    it.each(iconVariants)(
        "keeps the open center visible for %s at 16px",
        async (variant) => {
