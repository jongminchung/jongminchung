...
const excludedFile =
    /\.(?:e2e|integration|spec|test)\.[^.]+$|\.stories\.[^.]+$/;
...
const literalColorBoundaries = new Map<string, string>([
    ...
    [
        ...
        "browser theme-color metadata requires an sRGB value",
    ],
    [
    ...
    [
        ...
        "canonical SVG and PNG assets require an sRGB palette",
    ],
]);
...
function productPolicyFiles(directory: string): readonly string[] {
    return sourceFiles(directory).filter((file) => {
        ...
        return productPolicyExclusions.every(
...
function violations(
    ...
    return files.flatMap((file) => {
        const path = relativePath(file);
        return matchingLines(readFileSync(file, "utf8"), pattern).map(
            (line) => `${path}:${line}`,
        );
    });
}
            ...
            .flatMap((line, index) => {
                ...
                const values = patterns.flatMap((pattern) =>
                    Array.from(line.matchAll(pattern), (match) =>
                        capture(match, 1).toLowerCase(),
                    ),
                );
                return values.some((value) => !allowedValues.has(value))
                    ? [`${path}:${index + 1}`]
                    : [];
            });
    });
...
describe("@jongminchung/ui theme contract", () => {
    ...
    test("provides complete low-specificity defaults and allows app-owned overrides", () => {
        ...
        const defaultRoot = defaultScopes.find(
        ...
        for (const scope of [defaultRoot, defaultDark]) {
            ...
            const missingTokens = coreColorTokens.filter(
        ...
        for (const themePath of appThemes) {
            ...
            expect(
                rootScope,
                `${themePath} must declare app overrides at :root`,
            ).toBeDefined();
    ...
    test("requires every app-local Tailwind color mapping to have a provider", () => {
        ...
        for (const { mappingPath, themePath } of localMappings) {
            ...
            const scopes = themeScopes(
                readFileSync(join(workspaceRoot, themePath), "utf8"),
            );
            const rootScope = scopes.find(
                ({ selector }) => selector === ":root",
            );
            ...
            expect(
                [...mappedProviders].filter((provider) =>
                    coreColorTokenSet.has(provider),
                ),
            ...
            for (const scope of scopes) {
                ...
                expect(
                    [...mappedProviders].filter(
                        (provider) => !definedTokens.has(provider),
                    ),
    ...
    test("requires every production CSS variable use to have a provider", () => {
        ...
        for (const app of [
            ...
            expect(
                missingProviders,
                `apps/${app} CSS variable providers`,
            ).toEqual([]);
        }
    });
    ...
    test("keeps production UI on semantic colors and perceptual mixing", () => {
        ...
        const oklchViolations = files.flatMap((file) => {
            ...
            return matchingLines(readFileSync(file, "utf8"), /oklch\(/i).map(
                (line) => `${path}:${line}`,
            );
        });
        ...
        expect(
            literalViolations,
            "literal colors outside documented runtime boundaries",
        ).toEqual([]);
        expect(namedColorViolations(files), "CSS named colors").toEqual([]);
        expect(
            oklchViolations,
            "OKLCH literals outside app themes and public CSS fallbacks",
        ).toEqual([]);
        expect(
            violations(/color-mix\(\s*in[_\s-]+srgb\b/i, files),
            "sRGB color mixing",
        ).toEqual([]);
        expect(
            violations(paletteUtility, files),
        ...
        for (const [path, reason] of literalColorBoundaries) {
            expect(
                reason.length,
                `${path} must document why it is exempt`,
            ).toBeGreaterThan(0);
            expect(
                matchingLines(
            ...
            expect(
                sourceFiles(join(workspaceRoot, path, "..")).some(
    ...
    test("rejects legacy token names and one-off radii in production UI", () => {
        ...
        const sharedUiFiles = sourceFiles(
            join(workspaceRoot, "packages", "ui"),
        );
        const legacyToken =
        ...
        expect(
            violations(legacyToken, files),
            "legacy hue or material token names",
        ).toEqual([]);
        expect(
    ...
    test("routes Button usage through the shared shadcn variant contract", () => {
        ...
        for (const file of files) {
            ...
            for (const tag of jsxOpeningTags(contents, "Button")) {
                ...
                if (
                    !/\bvariant\s*=/.test(tag.text) ||
                    !/\bsize\s*=/.test(tag.text)
                ) {
        ...
        expect(
            importViolations,
            "Button imports outside @jongminchung/ui",
        ).toEqual([]);
        expect(
            exportViolations,
            "local Button declarations or re-exports",
        ).toEqual([]);
        ...
        expect(
            explicitVariantViolations,
            "Button calls without explicit variant and size",
        ).toEqual([]);
    });
});
