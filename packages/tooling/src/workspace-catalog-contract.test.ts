...
interface PackageManifest {
    readonly dependencyRanges: Readonly<
        Record<DependencyGroup, Readonly<Record<string, string>>>
    >;
...
function parseDependencyRanges(
    ...
    if (!isRecord(value))
        throw new Error(
            `${relative(rootDir, filePath)} ${group} must be an object`,
        );
    ...
    for (const [name, range] of Object.entries(value)) {
        if (typeof range !== "string") {
            throw new Error(
                `${relative(rootDir, filePath)} ${group}.${name} must be a string`,
            );
        }
        ranges[name] = range;
    }
...
async function readPackageManifest(filePath: string): Promise<PackageManifest> {
    ...
    if (!isRecord(parsed) || typeof parsed.name !== "string") {
        throw new Error(
            `${relative(rootDir, filePath)} must declare a package name`,
        );
    }

    return Object.freeze({
        dependencyRanges: Object.freeze({
            dependencies: parseDependencyRanges(
                parsed.dependencies,
                filePath,
                "dependencies",
            ),
            devDependencies: parseDependencyRanges(
...
async function listSourceModulePaths(
    directory: string,
...
function conditionTargets(
    ...
    return Object.entries(value).flatMap(([key, nested]) =>
        key === condition
            ? stringTargets(nested)
            : conditionTargets(nested, condition),
    );
}
...
async function listWorkspacePackageJsonPaths(
    ...
    const entries = await readdir(join(rootDir, directory), {
        withFileTypes: true,
    });
    return entries
    ...
    const nestedPackageJsonPaths = (
        await Promise.all(
            workspaceDirectories.map((directory) =>
                listWorkspacePackageJsonPaths(directory),
            ),
        )
    ).flat();
    const packageJsonPaths = [
        join(rootDir, "package.json"),
        ...nestedPackageJsonPaths,
    ];
    return Promise.all(
        packageJsonPaths.map((filePath) => readPackageManifest(filePath)),
    );
}
...
describe("workspace catalog contract", () => {
    ...
    it("keeps package source APIs on named exports", async () => {
        ...
        for (const manifest of packageManifests) {
            ...
            const sourceFiles = await listSourceModulePaths(
                join(dirname(manifest.filePath), "src"),
            );
            for (const file of sourceFiles) {
                expect(
                    await readFile(file, "utf8"),
                    relative(rootDir, file),
                ).not.toMatch(/\bexport\s+default\b/u);
            }
        }
    });
    ...
    it("publishes Node 24 ESM-only packages", async () => {
        ...
        for (const manifest of publishedPackages) {
            ...
            expect(
                conditionTargets(exportsField, "require"),
                `${manifest.name} require exports`,
            ).toEqual([]);
            expect(
                conditionTargets(exportsField, "default").filter((target) =>
                    target.endsWith(".js"),
                ),
                `${manifest.name} JavaScript default exports`,
            ).toEqual([]);
            expect(
                conditionTargets(exportsField, "import").filter((target) =>
                    target.endsWith(".js"),
                ).length,
