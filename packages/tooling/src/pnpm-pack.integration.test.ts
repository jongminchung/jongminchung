...
async function resolveTarballPath(
    ...
    const tarballFiles = (await readdir(tempDir)).filter((file) =>
        file.endsWith(".tgz"),
    );
...
describe("pnpm package tarball contracts", () => {
    ...
    it("packs @jongminchung/ui with ESM components, declarations, and theme assets", async () => {
        ...
        try {
            ...
            expect(packed.files.some((file) => file.endsWith(".test.ts"))).toBe(
                false,
            );
            expect(
                packed.files.some((file) => file.endsWith(".test.tsx")),
            ).toBe(false);
            ...
            for (const [sourceDirectory, sourceExtension, outputDirectory] of [
                ...
                const sourceFiles = packed.files.filter(
                    (file) =>
                        file.startsWith(sourceDirectory) &&
