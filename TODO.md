...
> 검토일: 2026-07-30  
> 범위: `apps/`, `packages/`, 관련 루트 설정·계약 테스트  
> 기준: React 19.2, Next.js 16 App Router, Tailwind CSS 4.3, shadcn/Base UI
>
> 결정 사항: GitHub 앱/패키지 CI는 의도적으로 제거했으며 재도입을 TODO에 포함하지 않는다.

## 총평

- 전체 방향은 시니어 수준에 가깝다.
  - 앱별 UI 소유권, Server Component 우선, 의미 토큰, OKLCH, 접근성·비주얼 테스트가 좋다.
...
| shadcn/Base UI | 방향 양호, 정책 재검토 필요 | 앱이 소스를 소유하는 원칙은 좋다. Button 예외 정책과 손수 만든 list/form 계약은 shadcn의 재사용·접근성 이점을 약화한다.   |

## 유지할 것

- [ ] 현재 프로젝트 결정대로 `packages/ui`를 만들지 않고 앱이 컴포넌트와 시각 언어를 소유한다.
  - 이는 shadcn 필수 원칙이 아니라 이 저장소의 의도적 소유권 결정이다.
...
- 근거
  ...
  - `lib/documents.ts:26-45`의 수동 `docLoaders`에는 두 항목이 없다.
  - 실제 `/en/handbook/app-icons`, `/ko/handbook/app-icons` 응답은 404다.
  - `content/ko/handbook/ddd copy.mdx`는 metadata와 경로가 달라 build를 막는다.
  - `scripts/content-contract.test.ts:18,31`은 문서 수 `22`를 하드코딩하지만 manifest는 20개, loader는 18개다.
- 변경
  - 중복 `ddd copy.mdx`를 삭제하거나 올바른 별도 문서로 정식 등록한다.
...
- 근거
  ...
  - `apps/git-client/src/domain/designSystem.test.ts:171-177`은 존재하지 않는 `apps/readme/components/ui`를 무조건 순회한다.
...
- 변경
  ...
  - “UI primitive가 0개인 앱”을 정상 상태로 모델링한다.
...
- 변경
  - ADR에서 현재 정책과 앱 로컬 variant 정책의 중복·일관성·명시성 trade-off를 비교한다.
  - 정책을 전환한다면 Button을 쓰는 Engineering Docs/Git Client에 Base UI + CVA 기반 `components/ui/button.tsx`를 둔다.
  - `variant`, `size`, `iconOnly`, loading/disabled 계약을 타입으로 제한한다.
  - `theme-contract`는 토큰/provider만 검사하고 Button 동작은 앱별 테스트로 이동한다.
  - 공용 `packages/ui`는 만들지 않는다.
- 완료 조건
  - 선택 기준과 결과를 ADR 및 `DESIGN_SYSTEM.md`에 반영한다.
  - 전환한다면 호출부는 의미 variant와 배치 class만 지정한다.
  - 유지한다면 반복 recipe의 일관성 검사와 허용 가능한 중복 기준을 둔다.
  - 어느 선택이든 Base UI 상태·focus·disabled 계약과 시각 회귀가 유지된다.

### [ ] 검색 palette를 실제 combobox 계약으로 구현한다
...
- 변경
  - Base UI Combobox 또는 완전한 ARIA combobox/listbox pattern을 사용한다.
  - active option, 결과 수, empty/loading/error 상태를 연결한다.
  - Arrow Up/Down, Enter, Escape, pointer 이동의 단일 focus 전략을 정한다.
- 완료 조건
  - 스크린 리더가 현재 선택과 결과 상태를 읽는다.
  - 열린 상태 Axe와 키보드 회귀 테스트가 통과한다.

### [ ] 검색 index 적재를 lazy·취소 가능·실패 명시 상태로 바꾼다
...
- 변경
  - 최초 open/intent 시 locale별 한 번만 적재한다.
  - Effect 내부 async 함수와 동기 cleanup의 `AbortController.abort()`로 요청을 취소한다.
  - `idle | loading | ready | error` discriminated union과 retry UI를 사용한다.
- 완료 조건
  - 느린 locale 전환에서 이전 결과가 덮어쓰지 않는다.
  - 404·잘못된 JSON·취소 테스트에서 unhandled error가 없다.

### [ ] form/list 복합 위젯의 접근성 primitive를 좁힌다
...
- 변경
  - static item, listbox option, menu item, button/link를 별도 typed component로 분리한다.
  - 가능한 경우 Base UI/shadcn의 전용 `Item`, Menu, Select, Checkbox를 사용한다.
  - field description/error ID, `aria-describedby`, `aria-invalid`, mixed checkbox 상태를 공통 처리한다.
- 완료 조건
  - Tab/Arrow/Enter/Space, selection, focus restoration, error announcement 테스트가 통과한다.
...
### [ ] Engineering Docs의 생성되지 않는 animation utility를 정리한다

- 근거
  - dialog/tooltip이 `animate-in`, `fade-in`, `zoom-in`, `slide-in-*`을 사용한다.
  - 앱에는 `tw-animate-css` import/dependency가 없어 해당 CSS가 생성되지 않는다.
- 변경
  - plugin을 명시적으로 추가하거나 Base UI state 기반 CSS를 직접 작성하거나 죽은 class를 제거한다.
- 완료 조건
  - production CSS에 의도한 animation rule이 존재하고 reduced-motion/visual test가 통과한다.

### [ ] 불필요한 Client Component 경계를 줄인다

- 근거
  - `EditPageLink.tsx`는 정적 anchor뿐인데 `"use client"`다.
  - `DocumentOutline.tsx`는 scroll button 하나 때문에 outline 전체가 client다.
- 변경
  - `EditPageLink`를 Server Component로 되돌린다.
  - 정적 outline과 작은 `BackToTopButton` Client Component를 분리한다.
- 완료 조건
  - hydration 대상이 상호작용 component로만 제한되고 기능이 동일하다.

### [ ] README의 빈 shadcn scaffold와 미사용 의존성을 정리한다
...
- 변경
  - 미사용 의존성을 제거한다.
  - 향후 CLI 사용을 위해 `components.json`을 남길지는 명시적으로 결정한다.
  - 계약 테스트는 빈 UI root를 정상으로 처리한다.
- 완료 조건
  - fake directory 없이 install/test/build가 통과한다.
...
- 통과
  - `pnpm run typecheck`
  - `pnpm exec oxfmt --config .oxfmtrc.mjs --check apps packages`
  - `pnpm exec oxlint apps packages` — 경고 1개
  - 두 Next 앱의 직접 `next build`
  - Git Client production build
- 실패
  - Engineering Docs Vitest: 15 통과, 2 실패
  - Git Client Vitest: 788 통과, 4 실패
  - packages Vitest: 35 통과, 3 실패
  - `pnpm run icon:check`
  - 표준 README/Engineering Docs build — icon/content precheck에서 차단
- 참고
  - root `pnpm run check`의 첫 실패는 범위 밖 `draft/platform.md` formatting이다.
...
pnpm --filter @jongminchung/readme run build
