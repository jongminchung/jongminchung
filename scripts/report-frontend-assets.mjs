    ...
    if (options["update-baseline"]) {
        ...
        report.comparison = Object.fromEntries(
            ["javascript", "css"].map((type) => {
                const difference = {
                    bytes:
                        report.totals[type].bytes - baseline.totals[type].bytes,
                    gzipBytes:
                        report.totals[type].gzipBytes -
                        baseline.totals[type].gzipBytes,
                };
                const largestChanges = assets
                    ...
                    .map((asset) => ({
                        ...
                        bytes:
                            asset.bytes -
                            (baselineAssets.get(asset.logicalName)?.bytes ?? 0),
                        gzipBytes:
                            asset.gzipBytes -
                            (baselineAssets.get(asset.logicalName)?.gzipBytes ??
                                0),
                    }))
                    .filter(
                        (asset) => asset.bytes !== 0 || asset.gzipBytes !== 0,
                    )
                    .sort(
                        (left, right) =>
                            Math.abs(right.gzipBytes) -
                            Math.abs(left.gzipBytes),
                    )
                    .slice(0, 10);
                return [
                    type,
